"use strict";

exports.__esModule = true;
exports.default = void 0;

var _lazyResult = _interopRequireDefault(require("./lazy-result"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj ***REMOVED***; ***REMOVED***

function _createForOfIteratorHelperLoose(o, allowArrayLike) { var it; if (typeof Symbol === "undefined" || o[Symbol.iterator] == null) { if (Array.isArray(o) || (it = _unsupportedIterableToArray(o)) || allowArrayLike && o && typeof o.length === "number") { if (it) o = it; var i = 0; return function () { if (i >= o.length) return { done: true ***REMOVED***; return { done: false, value: o[i++] ***REMOVED***; ***REMOVED***; ***REMOVED*** throw new TypeError("Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); ***REMOVED*** it = o[Symbol.iterator](); return it.next.bind(it); ***REMOVED***

function _unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return _arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return _arrayLikeToArray(o, minLen); ***REMOVED***

function _arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) { arr2[i] = arr[i]; ***REMOVED*** return arr2; ***REMOVED***

/**
 * Contains plugins to process CSS. Create one `Processor` instance,
 * initialize its plugins, and then use that instance on numerous CSS files.
 *
 * @example
 * const processor = postcss([autoprefixer, precss])
 * processor.process(css1).then(result => console.log(result.css))
 * processor.process(css2).then(result => console.log(result.css))
 */
var Processor = /*#__PURE__*/function () {
  /**
   * @param {Array.<Plugin|pluginFunction>|Processor***REMOVED*** plugins PostCSS plugins.
   *        See {@link Processor#use***REMOVED*** for plugin format.
   */
  function Processor(plugins) {
    if (plugins === void 0) {
      plugins = [];
    ***REMOVED***

    /**
     * Current PostCSS version.
     *
     * @type {string***REMOVED***
     *
     * @example
     * if (result.processor.version.split('.')[0] !== '6') {
     *   throw new Error('This plugin works only with PostCSS 6')
     * ***REMOVED***
     */
    this.version = '7.0.39';
    /**
     * Plugins added to this processor.
     *
     * @type {pluginFunction[]***REMOVED***
     *
     * @example
     * const processor = postcss([autoprefixer, precss])
     * processor.plugins.length //=> 2
     */

    this.plugins = this.normalize(plugins);
  ***REMOVED***
  /**
   * Adds a plugin to be used as a CSS processor.
   *
   * PostCSS plugin can be in 4 formats:
   * * A plugin created by {@link postcss.plugin***REMOVED*** method.
   * * A function. PostCSS will pass the function a @{link Root***REMOVED***
   *   as the first argument and current {@link Result***REMOVED*** instance
   *   as the second.
   * * An object with a `postcss` method. PostCSS will use that method
   *   as described in #2.
   * * Another {@link Processor***REMOVED*** instance. PostCSS will copy plugins
   *   from that instance into this one.
   *
   * Plugins can also be added by passing them as arguments when creating
   * a `postcss` instance (see [`postcss(plugins)`]).
   *
   * Asynchronous plugins should return a `Promise` instance.
   *
   * @param {Plugin|pluginFunction|Processor***REMOVED*** plugin PostCSS plugin
   *                                                 or {@link Processor***REMOVED***
   *                                                 with plugins.
   *
   * @example
   * const processor = postcss()
   *   .use(autoprefixer)
   *   .use(precss)
   *
   * @return {Processes***REMOVED*** Current processor to make methods chain.
   */


  var _proto = Processor.prototype;

  _proto.use = function use(plugin) {
    this.plugins = this.plugins.concat(this.normalize([plugin]));
    return this;
  ***REMOVED***
  /**
   * Parses source CSS and returns a {@link LazyResult***REMOVED*** Promise proxy.
   * Because some plugins can be asynchronous it doesn’t make
   * any transformations. Transformations will be applied
   * in the {@link LazyResult***REMOVED*** methods.
   *
   * @param {string|toString|Result***REMOVED*** css String with input CSS or any object
   *                                     with a `toString()` method,
   *                                     like a Buffer. Optionally, send
   *                                     a {@link Result***REMOVED*** instance
   *                                     and the processor will take
   *                                     the {@link Root***REMOVED*** from it.
   * @param {processOptions***REMOVED*** [opts]      Options.
   *
   * @return {LazyResult***REMOVED*** Promise proxy.
   *
   * @example
   * processor.process(css, { from: 'a.css', to: 'a.out.css' ***REMOVED***)
   *   .then(result => {
   *      console.log(result.css)
   *   ***REMOVED***)
   */
  ;

  _proto.process = function (_process) {
    function process(_x) {
      return _process.apply(this, arguments);
    ***REMOVED***

    process.toString = function () {
      return _process.toString();
    ***REMOVED***;

    return process;
  ***REMOVED***(function (css, opts) {
    if (opts === void 0) {
      opts = {***REMOVED***;
    ***REMOVED***

    if (this.plugins.length === 0 && opts.parser === opts.stringifier) {
      if (process.env.NODE_ENV !== 'production') {
        if (typeof console !== 'undefined' && console.warn) {
          console.warn('You did not set any plugins, parser, or stringifier. ' + 'Right now, PostCSS does nothing. Pick plugins for your case ' + 'on https://www.postcss.parts/ and use them in postcss.config.js.');
        ***REMOVED***
      ***REMOVED***
    ***REMOVED***

    return new _lazyResult.default(this, css, opts);
  ***REMOVED***);

  _proto.normalize = function normalize(plugins) {
    var normalized = [];

    for (var _iterator = _createForOfIteratorHelperLoose(plugins), _step; !(_step = _iterator()).done;) {
      var i = _step.value;

      if (i.postcss === true) {
        var plugin = i();
        throw new Error('PostCSS plugin ' + plugin.postcssPlugin + ' requires PostCSS 8.\n' + 'Migration guide for end-users:\n' + 'https://github.com/postcss/postcss/wiki/PostCSS-8-for-end-users');
      ***REMOVED***

      if (i.postcss) i = i.postcss;

      if (typeof i === 'object' && Array.isArray(i.plugins)) {
        normalized = normalized.concat(i.plugins);
      ***REMOVED*** else if (typeof i === 'function') {
        normalized.push(i);
      ***REMOVED*** else if (typeof i === 'object' && (i.parse || i.stringify)) {
        if (process.env.NODE_ENV !== 'production') {
          throw new Error('PostCSS syntaxes cannot be used as plugins. Instead, please use ' + 'one of the syntax/parser/stringifier options as outlined ' + 'in your PostCSS runner documentation.');
        ***REMOVED***
      ***REMOVED*** else if (typeof i === 'object' && i.postcssPlugin) {
        throw new Error('PostCSS plugin ' + i.postcssPlugin + ' requires PostCSS 8.\n' + 'Migration guide for end-users:\n' + 'https://github.com/postcss/postcss/wiki/PostCSS-8-for-end-users');
      ***REMOVED*** else {
        throw new Error(i + ' is not a PostCSS plugin');
      ***REMOVED***
    ***REMOVED***

    return normalized;
  ***REMOVED***;

  return Processor;
***REMOVED***();

var _default = Processor;
/**
 * @callback builder
 * @param {string***REMOVED*** part          Part of generated CSS connected to this node.
 * @param {Node***REMOVED***   node          AST node.
 * @param {"start"|"end"***REMOVED*** [type] Node’s part type.
 */

/**
 * @callback parser
 *
 * @param {string|toString***REMOVED*** css   String with input CSS or any object
 *                                with toString() method, like a Buffer.
 * @param {processOptions***REMOVED*** [opts] Options with only `from` and `map` keys.
 *
 * @return {Root***REMOVED*** PostCSS AST
 */

/**
 * @callback stringifier
 *
 * @param {Node***REMOVED*** node       Start node for stringifing. Usually {@link Root***REMOVED***.
 * @param {builder***REMOVED*** builder Function to concatenate CSS from node’s parts
 *                          or generate string and source map.
 *
 * @return {void***REMOVED***
 */

/**
 * @typedef {object***REMOVED*** syntax
 * @property {parser***REMOVED*** parse          Function to generate AST by string.
 * @property {stringifier***REMOVED*** stringify Function to generate string by AST.
 */

/**
 * @typedef {object***REMOVED*** toString
 * @property {function***REMOVED*** toString
 */

/**
 * @callback pluginFunction
 * @param {Root***REMOVED*** root     Parsed input CSS.
 * @param {Result***REMOVED*** result Result to set warnings or check other plugins.
 */

/**
 * @typedef {object***REMOVED*** Plugin
 * @property {function***REMOVED*** postcss PostCSS plugin function.
 */

/**
 * @typedef {object***REMOVED*** processOptions
 * @property {string***REMOVED*** from             The path of the CSS source file.
 *                                     You should always set `from`,
 *                                     because it is used in source map
 *                                     generation and syntax error messages.
 * @property {string***REMOVED*** to               The path where you’ll put the output
 *                                     CSS file. You should always set `to`
 *                                     to generate correct source maps.
 * @property {parser***REMOVED*** parser           Function to generate AST by string.
 * @property {stringifier***REMOVED*** stringifier Class to generate string by AST.
 * @property {syntax***REMOVED*** syntax           Object with `parse` and `stringify`.
 * @property {object***REMOVED*** map              Source map options.
 * @property {boolean***REMOVED*** map.inline                    Does source map should
 *                                                   be embedded in the output
 *                                                   CSS as a base64-encoded
 *                                                   comment.
 * @property {string|object|false|function***REMOVED*** map.prev Source map content
 *                                                   from a previous
 *                                                   processing step
 *                                                   (for example, Sass).
 *                                                   PostCSS will try to find
 *                                                   previous map automatically,
 *                                                   so you could disable it by
 *                                                   `false` value.
 * @property {boolean***REMOVED*** map.sourcesContent            Does PostCSS should set
 *                                                   the origin content to map.
 * @property {string|false***REMOVED*** map.annotation           Does PostCSS should set
 *                                                   annotation comment to map.
 * @property {string***REMOVED*** map.from                       Override `from` in map’s
 *                                                   sources`.
 */

exports.default = _default;
module.exports = exports.default;
//# sourceMappingURL=data:application/json;charset=utf8;base64,
