'use strict';Object.defineProperty(exports, "__esModule", { value: true ***REMOVED***);exports.availableDocStyleParsers = undefined;exports.




captureDoc = captureDoc;var _doctrine = require('doctrine');var _doctrine2 = _interopRequireDefault(_doctrine);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { 'default': obj ***REMOVED***;***REMOVED*** /**
                                                                                                                                                                                                                * parse docs from the first node that has leading comments
                                                                                                                                                                                                                */function captureDoc(source, docStyleParsers) {var metadata = {***REMOVED***;
  // 'some' short-circuits on first 'true'
  for (var _len = arguments.length, nodes = Array(_len > 2 ? _len - 2 : 0), _key = 2; _key < _len; _key++) {nodes[_key - 2] = arguments[_key];***REMOVED***nodes.some(function (n) {
    try {

      var leadingComments = void 0;

      // n.leadingComments is legacy `attachComments` behavior
      if ('leadingComments' in n) {
        leadingComments = n.leadingComments;
      ***REMOVED*** else if (n.range) {
        leadingComments = source.getCommentsBefore(n);
      ***REMOVED***

      if (!leadingComments || leadingComments.length === 0) {return false;***REMOVED***

      for (var name in docStyleParsers) {
        var doc = docStyleParsers[name](leadingComments);
        if (doc) {
          metadata.doc = doc;
        ***REMOVED***
      ***REMOVED***

      return true;
    ***REMOVED*** catch (err) {
      return false;
    ***REMOVED***
  ***REMOVED***);

  return metadata;
***REMOVED***

/**
   * parse JSDoc from leading comments
   * @param {object[]***REMOVED*** comments
   * @return {{ doc: object ***REMOVED******REMOVED***
   */
function captureJsDoc(comments) {
  var doc = void 0;

  // capture XSDoc
  comments.forEach(function (comment) {
    // skip non-block comments
    if (comment.type !== 'Block') {return;***REMOVED***
    try {
      doc = _doctrine2['default'].parse(comment.value, { unwrap: true ***REMOVED***);
    ***REMOVED*** catch (err) {
      /* don't care, for now? maybe add to `errors?` */
    ***REMOVED***
  ***REMOVED***);

  return doc;
***REMOVED***

/**
    * parse TomDoc section from comments
    */
function captureTomDoc(comments) {
  // collect lines up to first paragraph break
  var lines = [];
  for (var i = 0; i < comments.length; i++) {
    var comment = comments[i];
    if (comment.value.match(/^\s*$/)) {break;***REMOVED***
    lines.push(comment.value.trim());
  ***REMOVED***

  // return doctrine-like object
  var statusMatch = lines.join(' ').match(/^(Public|Internal|Deprecated):\s*(.+)/);
  if (statusMatch) {
    return {
      description: statusMatch[2],
      tags: [{
        title: statusMatch[1].toLowerCase(),
        description: statusMatch[2] ***REMOVED***] ***REMOVED***;


  ***REMOVED***
***REMOVED***

var availableDocStyleParsers = exports.availableDocStyleParsers = {
  jsdoc: captureJsDoc,
  tomdoc: captureTomDoc ***REMOVED***;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,