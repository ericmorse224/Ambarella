...
```js
const { program ***REMOVED*** = require('commander');
...
```js
const { Command ***REMOVED*** = require('commander');
...
```js
// index.mjs
import { Command ***REMOVED*** from 'commander/esm.mjs';
const program = new Command();
```
...
```ts
// index.ts
import { Command ***REMOVED*** from 'commander';
const program = new Command();
```
...
if (options.pizzaType) console.log(`- ${options.pizzaType***REMOVED***`);
```
...
{ debug: true, small: true, pizzaType: 'vegetarian' ***REMOVED***
pizza details:
- small pizza size
- vegetarian
...
console.log(`cheese: ${program.opts().cheese***REMOVED***`);
```
...
const cheeseStr = (options.cheese === false) ? 'no cheese' : `${options.cheese***REMOVED*** cheese`;
console.log(`You ordered a pizza with ${sauceStr***REMOVED*** and ${cheeseStr***REMOVED***`);
```
...
else console.log(`add cheese type ${options.cheese***REMOVED***`);
```
...
Options:  { number: [ '1', '2', '3' ], letter: [ 'a', 'b', 'c' ] ***REMOVED***
Remaining arguments:  []
$ collect --letter=A -n80 operand
Options:  { number: [ '80' ], letter: [ 'A' ] ***REMOVED***
Remaining arguments:  [ 'operand' ]
$ collect --letter -n 1 -n 2 3 -- operand
Options:  { number: [ '1', '2', '3' ], letter: true ***REMOVED***
Remaining arguments:  [ 'operand' ]
```
...
$ PORT=80 extra 
Options:  { timeout: 60, port: '80' ***REMOVED***
```
...
const options = program.opts();
if (options.float !== undefined) console.log(`float: ${options.float***REMOVED***`);
if (options.integer !== undefined) console.log(`integer: ${options.integer***REMOVED***`);
if (options.verbose > 0) console.log(`verbosity: ${options.verbose***REMOVED***`);
...
program
  ...
  .action((first, second) => {
    console.log(`${first***REMOVED*** + ${second***REMOVED*** = ${first + second***REMOVED***`);
  ***REMOVED***)
;
```
...
program
  ...
  .action((name, options, command) => {
    ...
    ***REMOVED***
    const title = options.title ? `${options.title***REMOVED*** ` : '';
    console.log(`Thank-you ${title***REMOVED***${name***REMOVED***`);
  ***REMOVED***);
```
...
```js
async function run() { /* code goes here */ ***REMOVED***
...
program
  ...
  .command('update', 'update installed packages', { executableFile: 'myUpdateSubCommand' ***REMOVED***)
  .command('list', 'list packages installed', { isDefault: true ***REMOVED***);
...
program
  ...
  .hook('preAction', (thisCommand, actionCommand) => {
    if (thisCommand.opts().trace) {
      console.log(`About to call action handler for subcommand: ${actionCommand.name()***REMOVED***`);
...
`.help()`: display help information and exit immediately. You can optionally pass `{ error: true ***REMOVED***` to display on stderr and exit with an error status.

`.outputHelp()`: output help information without exiting. You can optionally pass `{ error: true ***REMOVED***` to display on stderr.
...
program.parse(['-f', 'filename'], { from: 'user' ***REMOVED***);
```
...
program
  ...
  .action((commandAndOptions) => {
    if (commandAndOptions.debug) {
      console.error(`Called ${commandAndOptions.name()***REMOVED***`);
    ***REMOVED***
  ***REMOVED***);
```
...
```js
const { createCommand ***REMOVED*** = require('commander');
...
function errorColor(str) {
  ...
  return `\x1b[31m${str***REMOVED***\x1b[0m`;
***REMOVED***

program
  .configureOutput({
    // Visibly override write routines as example!
    writeOut: (str) => process.stdout.write(`[OUT] ${str***REMOVED***`),
    writeErr: (str) => process.stdout.write(`[ERR] ${str***REMOVED***`),
