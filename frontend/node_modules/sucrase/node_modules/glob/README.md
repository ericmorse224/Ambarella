...
```js
// load using import
import { glob, globSync, globStream, globStreamSync, Glob ***REMOVED*** from 'glob'
...
const {
  ...
  globStreamSync,
  Glob,
***REMOVED*** = require('glob')

// the main glob() and globSync() resolve/return array of filenames

// all js files, but don't look in node_modules
const jsfiles = await glob('**/*.js', { ignore: 'node_modules/**' ***REMOVED***)
...
const images = await glob(['css/*.{png,jpeg***REMOVED***', 'public/*.{png,jpeg***REMOVED***'])

// but of course you can do that with the glob pattern also
// the sync function is the same, just returns a string[] instead
// of Promise<string[]>
const imagesAlt = globSync('{css,public***REMOVED***/*.{png,jpeg***REMOVED***')
...
const g3 = new Glob('**/baz/**', { withFileTypes: true ***REMOVED***)
...
// slower, though.
// For example:
const results = await glob('**', { stat: true, withFileTypes: true ***REMOVED***)
...
const folderNamedModules = await glob('**/*.{ts,js***REMOVED***', {
  ignore: {
    ignored: p => {
      const pp = p.parent
...
paths found.
  ...
  --nobrace              Do not expand {...***REMOVED*** patterns
...
string into an array of strings. So a pattern like `'x{a,b***REMOVED***y'`
...
- `root` A string path resolved against the `cwd` option, which
  ...
  pattern `/../*` with `{root:'/some/path'***REMOVED***` will return all
  files in `/some`, not all files in `/some/path`. The pattern
  `*` with `{root:'/some/path'***REMOVED***` will return all the entries in
  the cwd, not the entries in `/some/path`.

  To start absolute and non-absolute patterns in the same
  path, you can use `{root:''***REMOVED***`. However, be aware that on
...
- `magicalBraces` Treat brace expansion like `{a,b***REMOVED***` as a "magic"
  pattern. Has no effect if {@link nobrace***REMOVED*** is set.

  Only has effect on the {@link hasMagic***REMOVED*** function, no effect on
...
- `nobrace` Do not expand `{a,b***REMOVED***` and `{1..3***REMOVED***` brace sets.
...
- `includeChildMatches` boolean, default `true`. Do not match any
  ...
  **Caveat** It _only_ ignores matches that would be a descendant
  ...
  const results = await glob(
    ...
    { includeChildMatches: false ***REMOVED***,
  )
  ```
...
`***REMOVED***`, with 2 or more comma-delimited sections within. Braced
sections may contain slash characters, so `a{/b/c,bcd***REMOVED***` would
...
- `**` If a "globstar" is alone in a path portion, then it
  ...
  `{follow:true***REMOVED***` is passed in the options object. A pattern
...
setting `{follow:true***REMOVED***` in the options.
...
like `+(a|{b),c)***REMOVED***`, which would not be valid in bash or zsh, is
...
For example, `glob('/tmp', { cwd: 'c:/any/thing' ***REMOVED***)` will return
`['c:/tmp']` as the result.
...
--- pattern: '{**/*.txt,**/?/**/*.txt,**/?/**/?/**/*.txt,**/?/**/?/**/?/**/*.txt,**/?/**/?/**/?/**/?/**/*.txt***REMOVED***' ---
~~ sync ~~
...
--- pattern: './{**/?{/**/?{/**/?{/**/?,,,,***REMOVED***,,,,***REMOVED***,,,,***REMOVED***,,,***REMOVED***/**/*.txt' ---
~~ sync ~~
...
--- pattern: './{*/**/../{*/**/../{*/**/../{*/**/../{*/**,,,,***REMOVED***,,,,***REMOVED***,,,,***REMOVED***,,,,***REMOVED***,,,,***REMOVED***/*.txt' ---
~~ sync ~~
