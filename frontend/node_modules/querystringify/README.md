...
```js
qs.parse('?foo=bar');         // { foo: 'bar' ***REMOVED***
qs.parse('#foo=bar');         // { foo: 'bar' ***REMOVED***
qs.parse('foo=bar');          // { foo: 'bar' ***REMOVED***
qs.parse('foo=bar&bar=foo');  // { foo: 'bar', bar: 'foo' ***REMOVED***
qs.parse('foo&bar=foo');      // { foo: '', bar: 'foo' ***REMOVED***
```
...
```js
qs.stringify({ foo: bar ***REMOVED***);       // foo=bar
qs.stringify({ foo: bar ***REMOVED***, true); // ?foo=bar
qs.stringify({ foo: bar ***REMOVED***, '#');  // #foo=bar
qs.stringify({ foo: '' ***REMOVED***, '&');   // &foo=
```
