...
/**
 * @param {string***REMOVED*** a key
 * @param {string***REMOVED*** b key
 * @returns {number***REMOVED*** the similarity as number
 */
 ...
 * @param {string***REMOVED*** a key
 * @param {string***REMOVED*** b key
 * @param {Set<string>***REMOVED*** usedNames set of already used names
 * @returns {string***REMOVED*** the common part and a single char for the difference
 */
...
/**
 * @param {Record<string, number>***REMOVED*** total total size
 * @param {Record<string, number>***REMOVED*** size single size
...
/**
 * @template T
 * @param {Iterable<Node<T>>***REMOVED*** nodes some nodes
 * @returns {Record<string, number>***REMOVED*** total size
 */
const sumSize = nodes => {
...
/**
 * @param {Record<string, number>***REMOVED*** size size
 * @param {Record<string, number>***REMOVED*** maxSize minimum size
 * @returns {boolean***REMOVED*** true, when size is too big
 */
...
/**
 * @param {Record<string, number>***REMOVED*** size size
 * @param {Record<string, number>***REMOVED*** minSize minimum size
 * @returns {boolean***REMOVED*** true, when size is too small
 */
 ...
 * @param {Record<string, number>***REMOVED*** size size
 * @param {Record<string, number>***REMOVED*** minSize minimum size
 * @returns {Set<string>***REMOVED*** set of types that are too small
 */
 ...
 * @param {T***REMOVED*** size size
 * @param {Set<string>***REMOVED*** types types
 * @returns {number***REMOVED*** number of matching size types
 */
const getNumberOfMatchingSizeTypes = (size, types) => {
	let i = 0;
	for (const key of Object.keys(size)) {
		if (size[/** @type {keyof T***REMOVED*** */ (key)] !== 0 && types.has(key)) i++;
	***REMOVED***
	return i;
***REMOVED***;
 ...
 * @param {Record<string, number>***REMOVED*** size size
 * @param {Set<string>***REMOVED*** types types
 * @returns {number***REMOVED*** selective size sum
 */
...
class Group {
	/**
	 * @param {Node<T>[]***REMOVED*** nodes nodes
	 * @param {number[] | null***REMOVED*** similarities similarities between the nodes (length = nodes.length - 1)
	 * @param {Record<string, number>=***REMOVED*** size size of the group
	 */
	...
	***REMOVED***
	...
	/**
	 * @param {(node: Node<T>) => boolean***REMOVED*** filter filter function
	 * @returns {Node<T>[] | undefined***REMOVED*** removed nodes
	 */
	popNodes(filter) {
		const newNodes = [];
				...
				if (newNodes.length > 0) {
					newSimilarities.push(
						lastNode === this.nodes[i - 1]
							? /** @type {number[]***REMOVED*** */ (this.similarities)[i - 1]
							: similarity(/** @type {Node<T>***REMOVED*** */ (lastNode).key, node.key)
					);
				***REMOVED***
				newNodes.push(node);
				lastNode = node;
...
/**
 * @template T
 * @param {Iterable<Node<T>>***REMOVED*** nodes nodes
 * @returns {number[]***REMOVED*** similarities
 */
...
/**
 * @template T
 * @typedef {object***REMOVED*** GroupedItems<T>
 * @property {string***REMOVED*** key
 * @property {T[]***REMOVED*** items
 * @property {Record<string, number>***REMOVED*** size
 */

/**
 * @template T
 * @typedef {object***REMOVED*** Options
 * @property {Record<string, number>***REMOVED*** maxSize maximum size of a group
 * @property {Record<string, number>***REMOVED*** minSize minimum size of a group (preferred over maximum size)
 * @property {Iterable<T>***REMOVED*** items a list of items
 * @property {(item: T) => Record<string, number>***REMOVED*** getSize function to get size of an item
 * @property {(item: T) => string***REMOVED*** getKey function to get the key of an item
 */

/**
 * @template T
 * @param {Options<T>***REMOVED*** options options object
 * @returns {GroupedItems<T>[]***REMOVED*** grouped items
 */
module.exports = ({ maxSize, minSize, items, getSize, getKey ***REMOVED***) => {
	/** @type {Group<T>[]***REMOVED*** */
	const result = [];
	...
	***REMOVED***
	...
	if (initialNodes.length > 0) {
		...
		/**
		 * @param {Group<T>***REMOVED*** group group
		 * @param {Record<string, number>***REMOVED*** consideredSize size of the group to consider
		 * @returns {boolean***REMOVED*** true, if the group was modified
		 */
		...
		***REMOVED***;
		...
		if (initialGroup.nodes.length > 0) {
			...
			while (queue.length) {
				const group = /** @type {Group<T>***REMOVED*** */ (queue.pop());
				...
				***REMOVED***
				if (left <= right) {
					...
					while (pos <= right + 1) {
						const similarity = /** @type {number[]***REMOVED*** */ (group.similarities)[
							pos - 1
						];
						if (
				...
				***REMOVED***
				...
				for (let i = right + 2; i < group.nodes.length; i++) {
					rightSimilarities.push(
						/** @type {number[]***REMOVED*** */ (group.similarities)[i - 1]
					);
