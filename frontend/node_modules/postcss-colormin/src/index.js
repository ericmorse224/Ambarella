'use strict';
const browserslist = require('browserslist');
const { isSupported ***REMOVED*** = require('caniuse-api');
const valueParser = require('postcss-value-parser');
const minifyColor = require('./minifyColor');

/**
 * @param {{nodes: valueParser.Node[]***REMOVED******REMOVED*** parent
 * @param {(node: valueParser.Node, index: number, parent: {nodes: valueParser.Node[]***REMOVED***) => false | undefined***REMOVED*** callback
 * @return {void***REMOVED***
 */
function walk(parent, callback) {
  parent.nodes.forEach((node, index) => {
    const bubble = callback(node, index, parent);

    if (node.type === 'function' && bubble !== false) {
      walk(node, callback);
    ***REMOVED***
  ***REMOVED***);
***REMOVED***

/*
 * IE 8 & 9 do not properly handle clicks on elements
 * with a `transparent` `background-color`.
 *
 * https://developer.mozilla.org/en-US/docs/Web/Events/click#Internet_Explorer
 */
const browsersWithTransparentBug = new Set(['ie 8', 'ie 9']);
const mathFunctions = new Set(['calc', 'min', 'max', 'clamp']);

/**
 * @param {valueParser.Node***REMOVED*** node
 * @return {boolean***REMOVED***
 */
function isMathFunctionNode(node) {
  if (node.type !== 'function') {
    return false;
  ***REMOVED***
  return mathFunctions.has(node.value.toLowerCase());
***REMOVED***

/**
 * @param {string***REMOVED*** value
 * @param {Record<string, boolean>***REMOVED*** options
 * @return {string***REMOVED***
 */
function transform(value, options) {
  const parsed = valueParser(value);

  walk(parsed, (node, index, parent) => {
    if (node.type === 'function') {
      if (/^(rgb|hsl)a?$/i.test(node.value)) {
        const { value: originalValue ***REMOVED*** = node;

        node.value = minifyColor(valueParser.stringify(node), options);
        /** @type {string***REMOVED*** */ (node.type) = 'word';

        const next = parent.nodes[index + 1];

        if (
          node.value !== originalValue &&
          next &&
          (next.type === 'word' || next.type === 'function')
        ) {
          parent.nodes.splice(
            index + 1,
            0,
            /** @type {valueParser.SpaceNode***REMOVED*** */ ({
              type: 'space',
              value: ' ',
            ***REMOVED***)
          );
        ***REMOVED***
      ***REMOVED*** else if (isMathFunctionNode(node)) {
        return false;
      ***REMOVED***
    ***REMOVED*** else if (node.type === 'word') {
      node.value = minifyColor(node.value, options);
    ***REMOVED***
  ***REMOVED***);

  return parsed.toString();
***REMOVED***

/**
 * @param {Record<string, boolean>***REMOVED*** options
 * @param {string[]***REMOVED*** browsers
 * @return {Record<string, boolean>***REMOVED***
 */
function addPluginDefaults(options, browsers) {
  const defaults = {
    // Does the browser support 4 & 8 character hex notation
    transparent:
      browsers.some((b) => browsersWithTransparentBug.has(b)) === false,
    // Does the browser support "transparent" value properly
    alphaHex: isSupported('css-rrggbbaa', browsers),
    name: true,
  ***REMOVED***;
  return { ...defaults, ...options ***REMOVED***;
***REMOVED***
/**
 * @type {import('postcss').PluginCreator<Record<string, boolean>>***REMOVED***
 * @param {Record<string, boolean>***REMOVED*** config
 * @return {import('postcss').Plugin***REMOVED***
 */
function pluginCreator(config = {***REMOVED***) {
  return {
    postcssPlugin: 'postcss-colormin',

    prepare(result) {
      /** @type {typeof result.opts & browserslist.Options***REMOVED*** */
      const resultOptions = result.opts || {***REMOVED***;
      const browsers = browserslist(null, {
        stats: resultOptions.stats,
        path: __dirname,
        env: resultOptions.env,
      ***REMOVED***);

      const cache = new Map();
      const options = addPluginDefaults(config, browsers);

      return {
        OnceExit(css) {
          css.walkDecls((decl) => {
            if (
              /^(composes|font|src$|filter|-webkit-tap-highlight-color)/i.test(
                decl.prop
              )
            ) {
              return;
            ***REMOVED***

            const value = decl.value;

            if (!value) {
              return;
            ***REMOVED***

            const cacheKey = JSON.stringify({ value, options, browsers ***REMOVED***);

            if (cache.has(cacheKey)) {
              decl.value = cache.get(cacheKey);

              return;
            ***REMOVED***

            const newValue = transform(value, options);

            decl.value = newValue;
            cache.set(cacheKey, newValue);
          ***REMOVED***);
        ***REMOVED***,
      ***REMOVED***;
    ***REMOVED***,
  ***REMOVED***;
***REMOVED***

pluginCreator.postcss = true;
module.exports = pluginCreator;
