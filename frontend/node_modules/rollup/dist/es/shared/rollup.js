...
*/
import require$$0, { resolve, basename, extname, dirname, relative as relative$1, win32, posix, isAbsolute as isAbsolute$1 ***REMOVED*** from 'path';
import process$1 from 'process';
import { performance ***REMOVED*** from 'perf_hooks';
import { createHash as createHash$1 ***REMOVED*** from 'crypto';
import { promises ***REMOVED*** from 'fs';
import { EventEmitter ***REMOVED*** from 'events';

var version$1 = "2.79.2";

var charToInteger = {***REMOVED***;
...
function guessIndent(code) {
	...
	const spaced = lines.filter((line) => /^ {2,***REMOVED***/.test(line));
...
class MagicString {
	constructor(string, options = {***REMOVED***) {
	...
	***REMOVED***
	...
	clone() {
		const cloned = new MagicString(this.original, { filename: this.filename ***REMOVED***);
	...
	indent(indentStr, options) {
		...
		***REMOVED***
		...
		const replacer = (match) => {
			if (shouldIndentNextCharacter) return `${indentStr***REMOVED***${match***REMOVED***`;
	...
	slice(start = 0, end = this.original.length) {
		...
		***REMOVED***
		...
		if (chunk && chunk.edited && chunk.start !== start)
			throw new Error(`Cannot use replaced character ${start***REMOVED*** as slice start anchor.`);
		...
		while (chunk) {
			...
			***REMOVED***
			...
			if (containsEnd && chunk.edited && chunk.end !== end)
				throw new Error(`Cannot use replaced character ${end***REMOVED*** as slice end anchor.`);
	...
	***REMOVED***
	...
	_splitChunk(chunk, index) {
		if (chunk.edited && chunk.content.length) {
			...
			throw new Error(
				`Cannot split a chunk that has already been edited (${loc.line***REMOVED***:${loc.column***REMOVED*** – "${chunk.original***REMOVED***")`
			);
		***REMOVED***
...
class Bundle$1 {
		...
		if (source.filename) {
			...
			***REMOVED*** else {
				...
				if (source.content.original !== uniqueSource.content) {
					throw new Error(`Illegal source: same filename (${source.filename***REMOVED***), different contents`);
				***REMOVED***
			***REMOVED***
		***REMOVED***
	...
	***REMOVED***
	...
	getIndentString() {
		const indentStringCounts = {***REMOVED***;
...
class PathTracker {
    constructor() {
        this.entityPaths = Object.create(null, {
            [EntitiesKey]: { value: new Set() ***REMOVED***
        ***REMOVED***);
    ***REMOVED***
        ...
        for (const pathSegment of path) {
            currentPaths = currentPaths[pathSegment] =
                currentPaths[pathSegment] ||
                    Object.create(null, { [EntitiesKey]: { value: new Set() ***REMOVED*** ***REMOVED***);
        ***REMOVED***
...
class DiscriminatedPathTracker {
    constructor() {
        this.entityPaths = Object.create(null, {
            [EntitiesKey]: { value: new Map() ***REMOVED***
        ***REMOVED***);
    ***REMOVED***
    trackEntityAtPathAndGetIfTracked(path, discriminator, entity) {
        ...
        for (const pathSegment of path) {
            currentPaths = currentPaths[pathSegment] =
                currentPaths[pathSegment] ||
                    Object.create(null, { [EntitiesKey]: { value: new Map() ***REMOVED*** ***REMOVED***);
        ***REMOVED***
...
class ExpressionEntity {
    constructor() {
        this.included = false;
    ***REMOVED***
    deoptimizePath(_path) { ***REMOVED***
    deoptimizeThisOnInteractionAtPath({ thisArg ***REMOVED***, _path, _recursionTracker) {
...
class Variable extends ExpressionEntity {
    ...
    ***REMOVED***
     ...
     */
    addReference(_identifier) { ***REMOVED***
    getBaseVariableName() {
        return this.renderBaseName || this.renderName || this.name;
    ***REMOVED***
    getName(getPropertyAccess) {
        const name = this.renderName || this.name;
        return this.renderBaseName ? `${this.renderBaseName***REMOVED***${getPropertyAccess(name)***REMOVED***` : name;
    ***REMOVED***
    hasEffectsOnInteractionAtPath(path, { type ***REMOVED***, _context) {
    ...
    ***REMOVED***
    ...
    include() {
        this.included = true;
    ***REMOVED***
    markCalledFromTryStatement() { ***REMOVED***
...
class ExternalVariable extends Variable {
    ...
    ***REMOVED***
    addReference(identifier) {
        ...
        ***REMOVED***
    ***REMOVED***
    hasEffectsOnInteractionAtPath(path, { type ***REMOVED***) {
...
const EMPTY_OBJECT = Object.freeze({***REMOVED***);
    ...
    ***REMOVED***
    ...
    return lines
        ...
        while (lineNum.length < digits)
            lineNum = ` ${lineNum***REMOVED***`;
        if (isErrorLine) {
            const indicator = spaces(digits + 2 + tabsToSpaces(str.slice(0, column)).length) + '^';
            return `${lineNum***REMOVED***: ${tabsToSpaces(str)***REMOVED***\n${indicator***REMOVED***`;
        ***REMOVED***
        return `${lineNum***REMOVED***: ${tabsToSpaces(str)***REMOVED***`;
    ***REMOVED***)
        .join('\n');
...
function printQuotedStringList(list, verbs) {
    ...
    const quotedList = list.map(item => `"${item***REMOVED***"`);
    let output = isSingleItem
        ? quotedList[0]
        : `${quotedList.slice(0, -1).join(', ')***REMOVED*** and ${quotedList.slice(-1)[0]***REMOVED***`;
    if (verbs) {
        output += ` ${isSingleItem ? verbs[0] : verbs[1]***REMOVED***`;
    ***REMOVED***
    return output;
***REMOVED***
...
function augmentCodeLocation(props, pos, source, id) {
    if (typeof pos === 'object') {
        const { line, column ***REMOVED*** = pos;
        props.loc = { column, file: id, line ***REMOVED***;
    ***REMOVED***
    else {
        props.pos = pos;
        const { line, column ***REMOVED*** = locate(source, pos, { offsetLine: 1 ***REMOVED***);
        props.loc = { column, file: id, line ***REMOVED***;
    ***REMOVED***
    if (props.frame === undefined) {
        const { line, column ***REMOVED*** = props.loc;
...
***REMOVED***)(Errors || (Errors = {***REMOVED***));
function errAssetNotFinalisedForFileName(name) {
    return {
        code: Errors.ASSET_NOT_FINALISED,
        message: `Plugin error - Unable to get file name for asset "${name***REMOVED***". Ensure that the source is set and that generate is called first.`
    ***REMOVED***;
***REMOVED***
...
function errChunkNotGeneratedForFileName(name) {
    return {
        ...
        message: `Plugin error - Unable to get file name for chunk "${name***REMOVED***". Ensure that generate is called first.`
    ***REMOVED***;
***REMOVED***
function errChunkInvalid({ fileName, code ***REMOVED***, exception) {
    const errorProps = {
        code: Errors.CHUNK_INVALID,
        message: `Chunk "${fileName***REMOVED***" is not valid JavaScript: ${exception.message***REMOVED***.`
    ***REMOVED***;
...
function errCircularReexport(exportName, importedModule) {
    return {
        ...
        id: importedModule,
        message: `"${exportName***REMOVED***" cannot be exported from ${relativeId(importedModule)***REMOVED*** as it is a reexport that references itself.`
    ***REMOVED***;
***REMOVED***
function errCyclicCrossChunkReexport(exportName, exporter, reexporter, importer) {
    return {
        ...
        exporter,
        importer,
        message: `Export "${exportName***REMOVED***" of module ${relativeId(exporter)***REMOVED*** was reexported through module ${relativeId(reexporter)***REMOVED*** while both modules are dependencies of each other and will end up in different chunks by current Rollup settings. This scenario is not well supported at the moment as it will produce a circular dependency between chunks and will likely lead to broken execution order.\nEither change the import in ${relativeId(importer)***REMOVED*** to point directly to the exporting module or do not use "preserveModules" to ensure these modules end up in the same chunk.`,
        reexporter
    ***REMOVED***;
***REMOVED***
function errAssetReferenceIdNotFoundForSetSource(assetReferenceId) {
    return {
        ...
        message: `Plugin error - Unable to set the source for unknown asset "${assetReferenceId***REMOVED***".`
    ***REMOVED***;
***REMOVED***
function errAssetSourceAlreadySet(name) {
    return {
        code: Errors.ASSET_SOURCE_ALREADY_SET,
        message: `Unable to set the source for asset "${name***REMOVED***", source already set.`
    ***REMOVED***;
***REMOVED***
function errNoAssetSourceSet(assetName) {
    return {
        code: Errors.ASSET_SOURCE_MISSING,
        message: `Plugin error creating asset "${assetName***REMOVED***" - no asset source set.`
    ***REMOVED***;
***REMOVED***
function errBadLoader(id) {
    return {
        code: Errors.BAD_LOADER,
        message: `Error loading ${relativeId(id)***REMOVED***: plugin load hook should return a string, a { code, map ***REMOVED*** object, or nothing/null`
    ***REMOVED***;
***REMOVED***
function errDeprecation(deprecation) {
    return {
        code: Errors.DEPRECATED_FEATURE,
        ...(typeof deprecation === 'string' ? { message: deprecation ***REMOVED*** : deprecation)
    ***REMOVED***;
***REMOVED***
function errFileReferenceIdNotFoundForFilename(assetReferenceId) {
    return {
        code: Errors.FILE_NOT_FOUND,
        message: `Plugin error - Unable to get file name for unknown file "${assetReferenceId***REMOVED***".`
    ***REMOVED***;
***REMOVED***
function errFileNameConflict(fileName) {
    return {
        code: Errors.FILE_NAME_CONFLICT,
        message: `The emitted file "${fileName***REMOVED***" overwrites a previously emitted file of the same name.`
    ***REMOVED***;
***REMOVED***
function errInputHookInOutputPlugin(pluginName, hookName) {
    return {
        code: Errors.INPUT_HOOK_IN_OUTPUT_PLUGIN,
        message: `The "${hookName***REMOVED***" hook used by the output plugin ${pluginName***REMOVED*** is a build time hook and will not be run for that plugin. Either this plugin cannot be used as an output plugin, or it should have an option to configure it as an output plugin.`
    ***REMOVED***;
***REMOVED***
function errCannotAssignModuleToChunk(moduleId, assignToAlias, currentAlias) {
    return {
        code: Errors.INVALID_CHUNK,
        message: `Cannot assign ${relativeId(moduleId)***REMOVED*** to the "${assignToAlias***REMOVED***" chunk as it is already in the "${currentAlias***REMOVED***" chunk.`
    ***REMOVED***;
***REMOVED***
function errInvalidExportOptionValue(optionValue) {
    return {
        code: Errors.INVALID_EXPORT_OPTION,
        message: `"output.exports" must be "default", "named", "none", "auto", or left unspecified (defaults to "auto"), received "${optionValue***REMOVED***"`,
...
function errIncompatibleExportOptionValue(optionValue, keys, entryModule) {
    return {
        ...
        message: `"${optionValue***REMOVED***" was specified for "output.exports", but entry module "${relativeId(entryModule)***REMOVED***" has the following exports: ${keys.join(', ')***REMOVED***`
    ***REMOVED***;
***REMOVED***
function errInternalIdCannotBeExternal(source, importer) {
    return {
        code: Errors.INVALID_EXTERNAL_ID,
        message: `'${source***REMOVED***' is imported as an external by ${relativeId(importer)***REMOVED***, but is already an existing non-external module id.`
    ***REMOVED***;
***REMOVED***
function errInvalidOption(option, urlHash, explanation, value) {
    return {
        code: Errors.INVALID_OPTION,
        message: `Invalid value ${value !== undefined ? `${JSON.stringify(value)***REMOVED*** ` : ''***REMOVED***for option "${option***REMOVED***" - ${explanation***REMOVED***.`,
        url: `https://rollupjs.org/guide/en/#${urlHash***REMOVED***`
    ***REMOVED***;
***REMOVED***
function errInvalidAddonPluginHook(hook, plugin) {
    return {
        ...
        hook,
        message: `Error running plugin hook ${hook***REMOVED*** for plugin ${plugin***REMOVED***, expected a string, a function hook or an object with a "handler" string or function.`,
        plugin
    ***REMOVED***;
***REMOVED***
function errInvalidFunctionPluginHook(hook, plugin) {
    return {
        code: Errors.INVALID_PLUGIN_HOOK,
        hook,
        message: `Error running plugin hook ${hook***REMOVED*** for plugin ${plugin***REMOVED***, expected a function hook or an object with a "handler" function.`,
        plugin
    ***REMOVED***;
***REMOVED***
...
function errMissingExport(exportName, importingModule, importedModule) {
    return {
        ...
        message: `'${exportName***REMOVED***' is not exported by ${relativeId(importedModule)***REMOVED***, imported by ${relativeId(importingModule)***REMOVED***`,
...
function errImplicitDependantCannotBeExternal(unresolvedId, implicitlyLoadedBefore) {
    return {
        ...
        message: `Module "${relativeId(unresolvedId)***REMOVED***" that should be implicitly loaded before "${relativeId(implicitlyLoadedBefore)***REMOVED***" cannot be external.`
    ***REMOVED***;
***REMOVED***
function errUnresolvedImplicitDependant(unresolvedId, implicitlyLoadedBefore) {
    return {
        code: Errors.MISSING_IMPLICIT_DEPENDANT,
        message: `Module "${relativeId(unresolvedId)***REMOVED***" that should be implicitly loaded before "${relativeId(implicitlyLoadedBefore)***REMOVED***" could not be resolved.`
    ***REMOVED***;
***REMOVED***
function errImplicitDependantIsNotIncluded(module) {
    ...
    return {
        code: Errors.MISSING_IMPLICIT_DEPENDANT,
        message: `Module "${relativeId(module.id)***REMOVED***" that should be implicitly loaded before ${printQuotedStringList(implicitDependencies)***REMOVED*** is not included in the module graph. Either it was not imported by an included module or only via a tree-shaken dynamic import, or no imported bindings were used and it had otherwise no side-effects.`
    ***REMOVED***;
***REMOVED***
function errMixedExport(facadeModuleId, name) {
    return {
        ...
        id: facadeModuleId,
        message: `Entry module "${relativeId(facadeModuleId)***REMOVED***" is using named and default exports together. Consumers of your bundle will have to use \`${name || 'chunk'***REMOVED***["default"]\` to access the default export, which may not be what you want. Use \`output.exports: "named"\` to disable this warning`,
        url: `https://rollupjs.org/guide/en/#outputexports`
    ***REMOVED***;
***REMOVED***
function errNamespaceConflict(name, reexportingModuleId, sources) {
    return {
        ...
        message: `Conflicting namespaces: "${relativeId(reexportingModuleId)***REMOVED***" re-exports "${name***REMOVED***" from one of the modules ${printQuotedStringList(sources.map(moduleId => relativeId(moduleId)))***REMOVED*** (will be ignored)`,
        name,
...
function errAmbiguousExternalNamespaces(name, reexportingModule, usedModule, sources) {
    return {
        ...
        message: `Ambiguous external namespace resolution: "${relativeId(reexportingModule)***REMOVED***" re-exports "${name***REMOVED***" from one of the external modules ${printQuotedStringList(sources.map(module => relativeId(module)))***REMOVED***, guessing "${relativeId(usedModule)***REMOVED***".`,
        name,
...
function errNoTransformMapOrAstWithoutCode(pluginName) {
    return {
        ...
        message: `The plugin "${pluginName***REMOVED***" returned a "map" or "ast" without returning ` +
...
function errPreferNamedExports(facadeModuleId) {
    ...
    return {
        ...
        id: facadeModuleId,
        message: `Entry module "${file***REMOVED***" is implicitly using "default" export mode, which means for CommonJS output that its default export is assigned to "module.exports". For many tools, such CommonJS output will not be interchangeable with the original ES module. If this is intended, explicitly set "output.exports" to either "auto" or "default", otherwise you might want to consider changing the signature of "${file***REMOVED***" to use named exports only.`,
        url: `https://rollupjs.org/guide/en/#outputexports`
    ***REMOVED***;
***REMOVED***
function errSyntheticNamedExportsNeedNamespaceExport(id, syntheticNamedExportsOption) {
    return {
        ...
        id,
        message: `Module "${relativeId(id)***REMOVED***" that is marked with 'syntheticNamedExports: ${JSON.stringify(syntheticNamedExportsOption)***REMOVED***' needs ${typeof syntheticNamedExportsOption === 'string' && syntheticNamedExportsOption !== 'default'
            ? `an explicit export named "${syntheticNamedExportsOption***REMOVED***"`
            : 'a default export'***REMOVED*** that does not reexport an unresolved named export of the same module.`
    ***REMOVED***;
***REMOVED***
function errUnexpectedNamedImport(id, imported, isReexport) {
    ...
    return {
        ...
        id,
        message: `The named export "${imported***REMOVED***" was ${importType***REMOVED***ed from the external module ${relativeId(id)***REMOVED*** even though its interop type is "defaultOnly". Either remove or change this ${importType***REMOVED*** or change the value of the "output.interop" option.`,
...
function errUnexpectedNamespaceReexport(id) {
    return {
        code: Errors.UNEXPECTED_NAMED_IMPORT,
        id,
        message: `There was a namespace "*" reexport from the external module ${relativeId(id)***REMOVED*** even though its interop type is "defaultOnly". This will be ignored as namespace reexports only reexport named exports. If this is not intended, either remove or change this reexport or change the value of the "output.interop" option.`,
        url: 'https://rollupjs.org/guide/en/#outputinterop'
    ***REMOVED***;
***REMOVED***
function errEntryCannotBeExternal(unresolvedId) {
    return {
        ...
        message: `Entry module cannot be external (${relativeId(unresolvedId)***REMOVED***).`
    ***REMOVED***;
***REMOVED***
function errUnresolvedEntry(unresolvedId) {
    return {
        code: Errors.UNRESOLVED_ENTRY,
        message: `Could not resolve entry module (${relativeId(unresolvedId)***REMOVED***).`
    ***REMOVED***;
***REMOVED***
function errUnresolvedImport(source, importer) {
    return {
        code: Errors.UNRESOLVED_IMPORT,
        message: `Could not resolve '${source***REMOVED***' from ${relativeId(importer)***REMOVED***`
    ***REMOVED***;
***REMOVED***
function errUnresolvedImportTreatedAsExternal(source, importer) {
    return {
        ...
        message: `'${source***REMOVED***' is imported by ${relativeId(importer)***REMOVED***, but could not be resolved – treating it as an external dependency`,
        source,
...
function errExternalSyntheticExports(source, importer) {
    return {
        ...
        importer: relativeId(importer),
        message: `External '${source***REMOVED***' can not have 'syntheticNamedExports' enabled.`,
        source
    ***REMOVED***;
***REMOVED***
...
class ExternalModule {
    constructor(options, id, moduleSideEffects, meta, renormalizeRenderPath) {
        ...
        const { importers, dynamicImporters ***REMOVED*** = this;
    ...
    warnUnusedImports() {
        ...
        ***REMOVED***
        ...
        this.options.onwarn({
            ...
            message: `${printQuotedStringList(unused, ['is', 'are'])***REMOVED*** imported from external module "${this.id***REMOVED***" but never used in ${printQuotedStringList(importersArray.map(importer => relativeId(importer)))***REMOVED***.`,
            names: unused,
            source: this.id,
...
function getAugmentedNamespace(n) {
  ...
  ***REMOVED*** else a = {***REMOVED***;
  Object.defineProperty(a, '__esModule', {value: true***REMOVED***);
...
var picomatch$1 = {exports: {***REMOVED******REMOVED***;

var utils$3 = {***REMOVED***;

const path$1 = require$$0;
const WIN_SLASH = '\\\\/';
const WIN_NO_SLASH = `[^${WIN_SLASH***REMOVED***]`;
...
const ONE_CHAR = '(?=.)';
const QMARK = '[^/]';
const END_ANCHOR = `(?:${SLASH_LITERAL***REMOVED***|$)`;
const START_ANCHOR = `(?:^|${SLASH_LITERAL***REMOVED***)`;
const DOTS_SLASH = `${DOT_LITERAL***REMOVED***{1,2***REMOVED***${END_ANCHOR***REMOVED***`;
const NO_DOT = `(?!${DOT_LITERAL***REMOVED***)`;
const NO_DOTS = `(?!${START_ANCHOR***REMOVED***${DOTS_SLASH***REMOVED***)`;
const NO_DOT_SLASH = `(?!${DOT_LITERAL***REMOVED***{0,1***REMOVED***${END_ANCHOR***REMOVED***)`;
const NO_DOTS_SLASH = `(?!${DOTS_SLASH***REMOVED***)`;
const QMARK_NO_DOT = `[^.${SLASH_LITERAL***REMOVED***]`;
const STAR = `${QMARK***REMOVED****?`;
...
const WINDOWS_CHARS = {
  ...
  SLASH_LITERAL: `[${WIN_SLASH***REMOVED***]`,
  QMARK: WIN_NO_SLASH,
  STAR: `${WIN_NO_SLASH***REMOVED****?`,
  DOTS_SLASH: `${DOT_LITERAL***REMOVED***{1,2***REMOVED***(?:[${WIN_SLASH***REMOVED***]|$)`,
  NO_DOT: `(?!${DOT_LITERAL***REMOVED***)`,
  NO_DOTS: `(?!(?:^|[${WIN_SLASH***REMOVED***])${DOT_LITERAL***REMOVED***{1,2***REMOVED***(?:[${WIN_SLASH***REMOVED***]|$))`,
  NO_DOT_SLASH: `(?!${DOT_LITERAL***REMOVED***{0,1***REMOVED***(?:[${WIN_SLASH***REMOVED***]|$))`,
  NO_DOTS_SLASH: `(?!${DOT_LITERAL***REMOVED***{1,2***REMOVED***(?:[${WIN_SLASH***REMOVED***]|$))`,
  QMARK_NO_DOT: `[^.${WIN_SLASH***REMOVED***]`,
  START_ANCHOR: `(?:^|[${WIN_SLASH***REMOVED***])`,
  END_ANCHOR: `(?:[${WIN_SLASH***REMOVED***]|$)`
***REMOVED***;
...
var constants$2 = {
  ...
  // regular expressions
  REGEX_BACKSLASH: /\\(?![*+?^${***REMOVED***(|)[\]])/g,
  REGEX_NON_SPECIAL_CHARS: /^[^@![\].,$*+?^{***REMOVED***()|\\/]+/,
  REGEX_SPECIAL_CHARS: /[-*+?.^${***REMOVED***(|)[\]]/,
  REGEX_SPECIAL_CHARS_BACKREF: /(\\?)((\W)(\3*))/g,
  REGEX_SPECIAL_CHARS_GLOBAL: /([-*+?.^${***REMOVED***(|)[\]])/g,
  ...
  ***REMOVED***,
  ...
  extglobChars(chars) {
    return {
      '!': { type: 'negate', open: '(?:(?!(?:', close: `))${chars.STAR***REMOVED***)` ***REMOVED***,
      '?': { type: 'qmark', open: '(?:', close: ')?' ***REMOVED***,
      '+': { type: 'plus', open: '(?:', close: ')+' ***REMOVED***,
      '*': { type: 'star', open: '(?:', close: ')*' ***REMOVED***,
      '@': { type: 'at', open: '(?:', close: ')' ***REMOVED***
    ***REMOVED***;
  ***REMOVED***,
...
(function (exports) {
	...
	***REMOVED***;
	...
	exports.escapeLast = (input, char, lastIdx) => {
	  ...
	  return `${input.slice(0, idx)***REMOVED***\\${input.slice(idx)***REMOVED***`;
	***REMOVED***;

	exports.removePrefix = (input, state = {***REMOVED***) => {
	  let output = input;
	...
	***REMOVED***;
	...
	exports.wrapOutput = (input, state = {***REMOVED***, options = {***REMOVED***) => {
	  const prepend = options.contains ? '' : '^';
	  const append = options.contains ? '' : '$';

	  let output = `${prepend***REMOVED***(?:${input***REMOVED***)${append***REMOVED***`;
 ...
 * { isGlob: true, input: 'foo/bar/*.js', base: 'foo/bar', glob: '*.js' ***REMOVED***
 * ```
 * @param {String***REMOVED*** `str`
 * @param {Object***REMOVED*** `options`
 * @return {Object***REMOVED*** Returns an object with tokens and regex source string.
 * @api public
 */

const scan$1 = (input, options) => {
  const opts = options || {***REMOVED***;
  ...
  let prev;
  let code;
  let token = { value: '', depth: 0, isGlob: false ***REMOVED***;
  ...
  while (index < length) {
    ...
    ***REMOVED***
    ...
    if (code === CHAR_FORWARD_SLASH) {
      slashes.push(index);
      tokens.push(token);
      token = { value: '', depth: 0, isGlob: false ***REMOVED***;
...
const syntaxError = (type, char) => {
  return `Missing ${type***REMOVED***: "${char***REMOVED***" - use "\\\\${char***REMOVED***" to match literal characters`;
***REMOVED***;
...
const parse$1 = (input, options) => {
  ...
  ***REMOVED***
  ...
  if (len > max) {
    throw new SyntaxError(`Input length: ${len***REMOVED***, exceeds maximum allowed length: ${max***REMOVED***`);
  ***REMOVED***

  const bos = { type: 'bos', value: '', output: opts.prepend || '' ***REMOVED***;
  const tokens = [bos];
  ...
  ***REMOVED*** = PLATFORM_CHARS;
  ...
  const globstar = opts => {
    return `(${capture***REMOVED***(?:(?!${START_ANCHOR***REMOVED***${opts.dot ? DOTS_SLASH : DOT_LITERAL***REMOVED***).)*?)`;
  ***REMOVED***;
  ...
  ***REMOVED***;
  ...
  const extglobOpen = (type, value) => {
    const token = { ...EXTGLOB_CHARS[value], conditions: 1, inner: '' ***REMOVED***;
    ...
    increment('parens');
    push({ type, value, output: state.output ? '' : ONE_CHAR ***REMOVED***);
    push({ type: 'paren', extglob: true, value: advance(), output ***REMOVED***);
    extglobs.push(token);
  ***REMOVED***;
  ...
  const extglobClose = token => {
    ...
    if (token.type === 'negate') {
      ...
      ***REMOVED***
      ...
      if (extglobStar !== star || eos() || /^\)+$/.test(remaining())) {
        output = token.close = `)$))${extglobStar***REMOVED***`;
      ***REMOVED***

      if (token.inner.includes('*') && (rest = remaining()) && /^\.[^\\/.]+$/.test(rest)) {
        // Any non-magical string (`.ts`) or even nested expression (`.{ts,tsx***REMOVED***`) can follow after the closing parenthesis.
        // In this case, we need to parse the string and use it in the output of the original pattern.
        // Suitable patterns: `/!(*.d).ts`, `/!(*.d).{ts,tsx***REMOVED***`, `**/!(*-dbg).@(js)`.
        //
        // Disabling the `fastpaths` option due to a problem with parsing strings as `.ts` in the pattern like `**/!(*.d).ts`.
        const expression = parse$1(rest, { ...options, fastpaths: false ***REMOVED***).output;

        output = token.close = `)${expression***REMOVED***)${extglobStar***REMOVED***)`;
      ***REMOVED***
    ...
    ***REMOVED***
    ...
    push({ type: 'paren', extglob: true, value, output ***REMOVED***);
    decrement('parens');
  ***REMOVED***;
  ...
  if (opts.fastpaths !== false && !/(^[*!]|[/()[\]{***REMOVED***"])/.test(input)) {
    let backslashes = false;
  ...
  ***REMOVED***
  ...
  while (!eos()) {
    ...
    ***REMOVED***
    ...
    if (value === ')') {
      ...
      ***REMOVED***
      ...
      push({ type: 'paren', value, output: state.parens ? ')' : '\\)' ***REMOVED***);
      decrement('parens');
      continue;
    ***REMOVED***
    ...
    if (value === '[') {
      ...
      ***REMOVED***
      ...
      push({ type: 'bracket', value ***REMOVED***);
      continue;
    ***REMOVED***

    if (value === ']') {
      if (opts.nobracket === true || (prev && prev.type === 'bracket' && prev.value.length === 1)) {
        push({ type: 'text', value, output: `\\${value***REMOVED***` ***REMOVED***);
        continue;
      ***REMOVED***
      ...
      ***REMOVED***
      ...
      prev.value = `(${capture***REMOVED***${escaped***REMOVED***|${prev.value***REMOVED***)`;
    ...
    ***REMOVED***
    ...
    if (value === '***REMOVED***') {
      ...
      if (opts.nobrace === true || !brace) {
        push({ type: 'text', value, output: value ***REMOVED***);
        continue;
      ***REMOVED***
      ...
      ***REMOVED***
      ...
      push({ type: 'brace', value, output ***REMOVED***);
      decrement('braces');
      braces.pop();
      continue;
    ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (value === ',') {
      ...
      ***REMOVED***
      ...
      push({ type: 'comma', value, output ***REMOVED***);
      continue;
    ***REMOVED***
    ...
    if (value === '/') {
      ...
      ***REMOVED***
      ...
      push({ type: 'slash', value, output: SLASH_LITERAL ***REMOVED***);
      continue;
    ***REMOVED***
    ...
    if (value === '.') {
      ...
      ***REMOVED***
      ...
      if ((state.braces + state.parens) === 0 && prev.type !== 'bos' && prev.type !== 'slash') {
        push({ type: 'text', value, output: DOT_LITERAL ***REMOVED***);
        continue;
      ***REMOVED***

      push({ type: 'dot', value, output: DOT_LITERAL ***REMOVED***);
      continue;
    ***REMOVED***
      ...
      ***REMOVED***
      ...
      if (prev && prev.type === 'paren') {
        ...
        ***REMOVED***
        ...
        push({ type: 'text', value, output ***REMOVED***);
        continue;
      ***REMOVED***

      if (opts.dot !== true && (prev.type === 'slash' || prev.type === 'bos')) {
        push({ type: 'qmark', value, output: QMARK_NO_DOT ***REMOVED***);
        continue;
      ***REMOVED***

      push({ type: 'qmark', value, output: QMARK ***REMOVED***);
      continue;
    ***REMOVED***
    ...
    if (value === '+') {
      ...
      ***REMOVED***
      ...
      if ((prev && prev.value === '(') || opts.regex === false) {
        push({ type: 'plus', value, output: PLUS_LITERAL ***REMOVED***);
        continue;
      ***REMOVED***

      if ((prev && (prev.type === 'bracket' || prev.type === 'paren' || prev.type === 'brace')) || state.parens > 0) {
        push({ type: 'plus', value ***REMOVED***);
        continue;
      ***REMOVED***

      push({ type: 'plus', value: PLUS_LITERAL ***REMOVED***);
      continue;
    ***REMOVED***
    ...
    if (value === '@') {
      if (opts.noextglob !== true && peek() === '(' && peek(2) !== '?') {
        push({ type: 'at', extglob: true, value, output: '' ***REMOVED***);
        continue;
      ***REMOVED***
    ...
    if (prev.type === 'star') {
      ...
      ***REMOVED***
      ...
      if (opts.bash === true && (!isStart || (rest[0] && rest[0] !== '/'))) {
        push({ type: 'star', value, output: '' ***REMOVED***);
        continue;
      ***REMOVED***
      ...
      ***REMOVED***
      ...
      if (prior.type === 'slash' && prior.prev.type !== 'bos' && !afterStar && eos()) {
        ...
        prior.output = `(?:${prior.output***REMOVED***`;
      ...
      ***REMOVED***
      ...
      if (prior.type === 'slash' && prior.prev.type !== 'bos' && rest[0] === '/') {
        ...
        prev.type = 'globstar';
        prev.output = `${globstar(opts)***REMOVED***${SLASH_LITERAL***REMOVED***|${SLASH_LITERAL***REMOVED***${end***REMOVED***)`;
        prev.value += value;
        ...
        push({ type: 'slash', value: '/', output: '' ***REMOVED***);
        continue;
      ***REMOVED***
      ...
      if (prior.type === 'bos' && rest[0] === '/') {
        prev.type = 'globstar';
        prev.value += value;
        prev.output = `(?:^|${SLASH_LITERAL***REMOVED***|${globstar(opts)***REMOVED***${SLASH_LITERAL***REMOVED***)`;
        state.output = prev.output;
        state.globstar = true;
        consume(value + advance());
        push({ type: 'slash', value: '/', output: '' ***REMOVED***);
    ...
    ***REMOVED***
    ...
    const token = { type: 'star', value, output: star ***REMOVED***;
  ...
  ***REMOVED***
  ...
  if (opts.strictSlashes !== true && (prev.type === 'star' || prev.type === 'bracket')) {
    push({ type: 'maybe_slash', value: '', output: `${SLASH_LITERAL***REMOVED***?` ***REMOVED***);
  ***REMOVED***
...
parse$1.fastpaths = (input, options) => {
  ...
  ***REMOVED***
  ...
  const {
    ...
    STAR,
    START_ANCHOR
  ***REMOVED*** = constants$1.globChars(win32);

  const nodot = opts.dot ? NO_DOTS : NO_DOT;
  const slashDot = opts.dot ? NO_DOTS_SLASH : NO_DOT;
  const capture = opts.capture ? '' : '?:';
  const state = { negated: false, prefix: '' ***REMOVED***;
  ...
  ***REMOVED***;
  ...
  const create = str => {
    switch (str) {
      case '*':
        return `${nodot***REMOVED***${ONE_CHAR***REMOVED***${star***REMOVED***`;

      case '.*':
        return `${DOT_LITERAL***REMOVED***${ONE_CHAR***REMOVED***${star***REMOVED***`;

      case '*.*':
        return `${nodot***REMOVED***${star***REMOVED***${DOT_LITERAL***REMOVED***${ONE_CHAR***REMOVED***${star***REMOVED***`;

      case '*/*':
        return `${nodot***REMOVED***${star***REMOVED***${SLASH_LITERAL***REMOVED***${ONE_CHAR***REMOVED***${slashDot***REMOVED***${star***REMOVED***`;

      case '**':
        return nodot + globstar(opts);

      case '**/*':
        return `(?:${nodot***REMOVED***${globstar(opts)***REMOVED***${SLASH_LITERAL***REMOVED***)?${slashDot***REMOVED***${ONE_CHAR***REMOVED***${star***REMOVED***`;

      case '**/*.*':
        return `(?:${nodot***REMOVED***${globstar(opts)***REMOVED***${SLASH_LITERAL***REMOVED***)?${slashDot***REMOVED***${star***REMOVED***${DOT_LITERAL***REMOVED***${ONE_CHAR***REMOVED***${star***REMOVED***`;

      case '**/.*':
        return `(?:${nodot***REMOVED***${globstar(opts)***REMOVED***${SLASH_LITERAL***REMOVED***)?${DOT_LITERAL***REMOVED***${ONE_CHAR***REMOVED***${star***REMOVED***`;
 ...
 * ```
 * @name picomatch
 * @param {String|Array***REMOVED*** `globs` One or more glob patterns.
 * @param {Object=***REMOVED*** `options`
 * @return {Function=***REMOVED*** Returns a matcher function.
 * @api public
 */
...
const picomatch = (glob, options, returnState = false) => {
  ...
  ***REMOVED***
  ...
  let isIgnored = () => false;
  if (opts.ignore) {
    const ignoreOpts = { ...options, ignore: null, onMatch: null, onResult: null ***REMOVED***;
    isIgnored = picomatch(opts.ignore, ignoreOpts, returnState);
  ***REMOVED***

  const matcher = (input, returnObject = false) => {
    const { isMatch, match, output ***REMOVED*** = picomatch.test(input, regex, options, { glob, posix ***REMOVED***);
    const result = { glob, state, regex, posix, input, output, match, isMatch ***REMOVED***;
 ...
 * // { isMatch: true, match: [ 'foo/', 'foo', 'bar' ], output: 'foo/bar' ***REMOVED***
 * ```
 * @param {String***REMOVED*** `input` String to test.
 * @param {RegExp***REMOVED*** `regex`
 * @return {Object***REMOVED*** Returns an object with matching info.
 * @api public
 */

picomatch.test = (input, regex, options, { glob, posix ***REMOVED*** = {***REMOVED***) => {
  if (typeof input !== 'string') {
    throw new TypeError('Expected input to be a string');
  ***REMOVED***

  if (input === '') {
    return { isMatch: false, output: '' ***REMOVED***;
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  return { isMatch: Boolean(match), match, output ***REMOVED***;
***REMOVED***;
 ...
 * ```
 * @param {String***REMOVED*** `input` String to test.
 * @param {RegExp|String***REMOVED*** `glob` Glob pattern or regex created by [.makeRe](#makeRe).
 ...
 * ```
 * @param {String|Array***REMOVED*** str The string to test.
 * @param {String|Array***REMOVED*** patterns One or more glob patterns to use for matching.
 * @param {Object***REMOVED*** [options] See available [options](#options).
 * @return {Boolean***REMOVED*** Returns true if any patterns match `str`
 * @api public
 */
 ...
 * @param {Object***REMOVED*** `options`
 * @return {Object***REMOVED*** Returns an object with useful properties and output to be used as a regex source string.
 * @api public
 */

picomatch.parse = (pattern, options) => {
  if (Array.isArray(pattern)) return pattern.map(p => picomatch.parse(p, options));
  return parse(pattern, { ...options, fastpaths: false ***REMOVED***);
***REMOVED***;
 ...
 * ```
 * @param {String***REMOVED*** `input` Glob pattern to scan.
 * @param {Object***REMOVED*** `options`
 * @return {Object***REMOVED*** Returns an object with
 * @api public
 */
 ...
 * @param {Object***REMOVED*** `options`
 * @param {Boolean***REMOVED*** `returnOutput` Intended for implementors, this argument allows you to return the raw output from the parser.
 * @param {Boolean***REMOVED*** `returnState` Adds the state to a `state` property on the returned regex. Useful for implementors and debugging.
...
picomatch.compileRe = (state, options, returnOutput = false, returnState = false) => {
  ...
  ***REMOVED***
  ...
  let source = `${prepend***REMOVED***(?:${state.output***REMOVED***)${append***REMOVED***`;
 ...
 * //=> /^(?:(?!\.)(?=.)[^/]*?\.js)$/
 * ```
 * @param {String***REMOVED*** `state` The object returned from the `.parse` method.
 * @param {Object***REMOVED*** `options`
 * @param {Boolean***REMOVED*** `returnOutput` Implementors may use this argument to return the compiled output, instead of a regular expression. This is not exposed on the options to prevent end-users from mutating the result.
 * @param {Boolean***REMOVED*** `returnState` Implementors may use this argument to return the state from the parsed glob with the returned regular expression.
 * @return {RegExp***REMOVED*** Returns a regex created from the given pattern.
 * @api public
 */

picomatch.makeRe = (input, options = {***REMOVED***, returnOutput = false, returnState = false) => {
  if (!input || typeof input !== 'string') {
    throw new TypeError('Expected a non-empty string');
  ***REMOVED***

  let parsed = { negated: false, fastpaths: true ***REMOVED***;
 ...
 *
 * const { output ***REMOVED*** = picomatch.parse('*.js');
 * console.log(picomatch.toRegex(output));
 * //=> /^(?:(?!\.)(?=.)[^/]*?\.js)$/
 * ```
 * @param {String***REMOVED*** `source` Regular expression source string.
 * @param {Object***REMOVED*** `options`
 * @return {RegExp***REMOVED***
 * @api public
 */
    ...
    const getMatcher = (id) => id instanceof RegExp
        ...
        : {
            test: (what) => {
                ...
                const fn = pm(pattern, { dot: true ***REMOVED***);
...
const forbiddenIdentifiers = new Set(`${reservedWords$1***REMOVED*** ${builtins$1***REMOVED***`.split(' '));
...
function skipThrough(node, st, c) { c(node, st); ***REMOVED***
function ignore(_node, _st, _c) {***REMOVED***
...
base$1.ExpressionStatement = base$1.ParenthesizedExpression = base$1.ChainExpression =
  function (node, st, c) { return c(node.expression, st, "Expression"); ***REMOVED***;
base$1.IfStatement = function (node, st, c) {
  c(node.test, st, "Expression");
  c(node.consequent, st, "Statement");
  if (node.alternate) { c(node.alternate, st, "Statement"); ***REMOVED***
***REMOVED***;
base$1.LabeledStatement = function (node, st, c) { return c(node.body, st, "Statement"); ***REMOVED***;
...
base$1.SwitchStatement = function (node, st, c) {
  ...
  for (var i$1 = 0, list$1 = node.cases; i$1 < list$1.length; i$1 += 1) {
    ...
    if (cs.test) { c(cs.test, st, "Expression"); ***REMOVED***
...
base$1.SwitchCase = function (node, st, c) {
  if (node.test) { c(node.test, st, "Expression"); ***REMOVED***
...
base$1.ReturnStatement = base$1.YieldExpression = base$1.AwaitExpression = function (node, st, c) {
  if (node.argument) { c(node.argument, st, "Expression"); ***REMOVED***
***REMOVED***;
base$1.ThrowStatement = base$1.SpreadElement =
  function (node, st, c) { return c(node.argument, st, "Expression"); ***REMOVED***;
base$1.TryStatement = function (node, st, c) {
  c(node.block, st, "Statement");
  if (node.handler) { c(node.handler, st); ***REMOVED***
  if (node.finalizer) { c(node.finalizer, st, "Statement"); ***REMOVED***
***REMOVED***;
base$1.CatchClause = function (node, st, c) {
  if (node.param) { c(node.param, st, "Pattern"); ***REMOVED***
  c(node.body, st, "Statement");
***REMOVED***;
...
base$1.ForStatement = function (node, st, c) {
  if (node.init) { c(node.init, st, "ForInit"); ***REMOVED***
  if (node.test) { c(node.test, st, "Expression"); ***REMOVED***
  if (node.update) { c(node.update, st, "Expression"); ***REMOVED***
  c(node.body, st, "Statement");
***REMOVED***;
...
base$1.ForInit = function (node, st, c) {
  if (node.type === "VariableDeclaration") { c(node, st); ***REMOVED***
  else { c(node, st, "Expression"); ***REMOVED***
***REMOVED***;
base$1.DebuggerStatement = ignore;

base$1.FunctionDeclaration = function (node, st, c) { return c(node, st, "Function"); ***REMOVED***;
...
base$1.VariableDeclarator = function (node, st, c) {
  ...
  if (node.init) { c(node.init, st, "Expression"); ***REMOVED***
***REMOVED***;

base$1.Function = function (node, st, c) {
  if (node.id) { c(node.id, st, "Pattern"); ***REMOVED***
...
base$1.Pattern = function (node, st, c) {
  if (node.type === "Identifier")
    { c(node, st, "VariablePattern"); ***REMOVED***
  else if (node.type === "MemberExpression")
    { c(node, st, "MemberPattern"); ***REMOVED***
  else
    { c(node, st); ***REMOVED***
***REMOVED***;
base$1.VariablePattern = ignore;
base$1.MemberPattern = skipThrough;
base$1.RestElement = function (node, st, c) { return c(node.argument, st, "Pattern"); ***REMOVED***;
base$1.ArrayPattern = function (node, st, c) {
  for (var i = 0, list = node.elements; i < list.length; i += 1) {
    var elt = list[i];

    if (elt) { c(elt, st, "Pattern"); ***REMOVED***
  ***REMOVED***
***REMOVED***;
base$1.ObjectPattern = function (node, st, c) {
  for (var i = 0, list = node.properties; i < list.length; i += 1) {
    ...
    if (prop.type === "Property") {
      if (prop.computed) { c(prop.key, st, "Expression"); ***REMOVED***
      c(prop.value, st, "Pattern");
    ***REMOVED*** else if (prop.type === "RestElement") {
...
base$1.ArrayExpression = function (node, st, c) {
  for (var i = 0, list = node.elements; i < list.length; i += 1) {
    ...
    if (elt) { c(elt, st, "Expression"); ***REMOVED***
  ***REMOVED***
***REMOVED***;
...
base$1.MemberExpression = function (node, st, c) {
  c(node.object, st, "Expression");
  if (node.computed) { c(node.property, st, "Expression"); ***REMOVED***
***REMOVED***;
base$1.ExportNamedDeclaration = base$1.ExportDefaultDeclaration = function (node, st, c) {
  if (node.declaration)
    { c(node.declaration, st, node.type === "ExportNamedDeclaration" || node.declaration.id ? "Statement" : "Expression"); ***REMOVED***
  if (node.source) { c(node.source, st, "Expression"); ***REMOVED***
***REMOVED***;
base$1.ExportAllDeclaration = function (node, st, c) {
  if (node.exported)
    { c(node.exported, st); ***REMOVED***
...
***REMOVED***;
base$1.ClassDeclaration = base$1.ClassExpression = function (node, st, c) { return c(node, st, "Class"); ***REMOVED***;
base$1.Class = function (node, st, c) {
  if (node.id) { c(node.id, st, "Pattern"); ***REMOVED***
  if (node.superClass) { c(node.superClass, st, "Expression"); ***REMOVED***
  c(node.body, st);
***REMOVED***;
...
base$1.MethodDefinition = base$1.PropertyDefinition = base$1.Property = function (node, st, c) {
  if (node.computed) { c(node.key, st, "Expression"); ***REMOVED***
  if (node.value) { c(node.value, st, "Expression"); ***REMOVED***
***REMOVED***;
...
const SOURCEMAPPING_URL_RE = new RegExp(`^#${whiteSpaceNoNewline***REMOVED***+${SOURCEMAPPING_URL***REMOVED***=.+`);
...
function handlePureAnnotationsOfNode(node, state, type = node.type) {
    const { annotations ***REMOVED*** = state;
...
class NodeBase extends ExpressionEntity {
    ...
    ***REMOVED***
    addExportedVariables(_variables, _exportNamesByVariable) { ***REMOVED***
    /**
        ...
        for (const [key, value] of Object.entries(esTreeNode)) {
            ...
            if (key.charCodeAt(0) === 95 /* _ */) {
                ...
                ***REMOVED***
                    ...
                    for (const { start, end ***REMOVED*** of value)
    ...
    ***REMOVED***
    setAssignedValue(value) {
        this.assignmentInteraction = { args: [value], thisArg: null, type: INTERACTION_ASSIGNED ***REMOVED***;
    ***REMOVED***
...
class SpreadElement extends NodeBase {
    ...
    ***REMOVED***
    hasEffects(context) {
        if (!this.deoptimized)
            this.applyDeoptimizations();
        const { propertyReadSideEffects ***REMOVED*** = this.context.options
            .treeshake;
...
class Method extends ExpressionEntity {
    ...
    ***REMOVED***
    hasEffectsOnInteractionAtPath(path, interaction, context) {
        var _a, _b;
        const { type ***REMOVED*** = interaction;
...
class ObjectEntity extends ExpressionEntity {
    ...
    ***REMOVED***
    buildPropertyMaps(properties) {
        const { allProperties, propertiesAndGettersByKey, propertiesAndSettersByKey, settersByKey, gettersByKey, unknownIntegerProps, unmatchablePropertiesAndGetters, unmatchableGetters, unmatchableSetters ***REMOVED*** = this;
        const unmatchablePropertiesAndSetters = [];
        for (let index = properties.length - 1; index >= 0; index--) {
            const { key, kind, property ***REMOVED*** = properties[index];
...
***REMOVED***, OBJECT_PROTOTYPE_FALLBACK, true);

const NEW_ARRAY_PROPERTIES = [
    { key: UnknownInteger, kind: 'init', property: UNKNOWN_EXPRESSION ***REMOVED***,
    { key: 'length', kind: 'init', property: UNKNOWN_LITERAL_NUMBER ***REMOVED***
];
        ...
        ***REMOVED***
        ...
        for (let index = 0; index < this.elements.length; index++) {
            ...
            if (hasSpread || element instanceof SpreadElement) {
                if (element) {
                    hasSpread = true;
                    properties.unshift({ key: UnknownInteger, kind: 'init', property: element ***REMOVED***);
                ***REMOVED***
            ***REMOVED***
            else if (!element) {
                properties.push({ key: String(index), kind: 'init', property: UNDEFINED_EXPRESSION ***REMOVED***);
            ***REMOVED***
            else {
                properties.push({ key: String(index), kind: 'init', property: element ***REMOVED***);
            ***REMOVED***
        ***REMOVED***
...
function getSafeName(baseName, usedNames) {
    ...
    while (usedNames.has(safeName) || RESERVED_NAMES$1.has(safeName)) {
        safeName = `${baseName***REMOVED***$${toBase64(count++)***REMOVED***`;
    ***REMOVED***
 ...
 *   computed: boolean;
 *   value: Node
 * ***REMOVED******REMOVED*** NodeWithPropertyDefinition */

/**
 *
 * @param {NodeWithPropertyDefinition***REMOVED*** node
 * @param {NodeWithPropertyDefinition***REMOVED*** parent
 * @returns boolean
 */
	...
	***REMOVED***
	...
	if (node.type === 'Identifier') {
		...
		switch (parent.type) {
			...
			// disregard the `foo` in `class {foo(){***REMOVED******REMOVED***` but keep it in `class {[foo](){***REMOVED******REMOVED***`
			case 'MethodDefinition': return parent.computed;

			// disregard the `foo` in `class {foo=bar***REMOVED***` but keep it in `class {[foo]=bar***REMOVED***` and `class {bar=foo***REMOVED***`
			case 'PropertyDefinition': return parent.computed || node === parent.value;

			// disregard the `bar` in `{ bar: foo ***REMOVED***`, but keep it in `{ [bar]: foo ***REMOVED***`
			case 'Property': return parent.computed || node === parent.value;

			// disregard the `bar` in `export { foo as bar ***REMOVED***` or
			// the foo in `import { foo as bar ***REMOVED***`
			case 'ExportSpecifier':
			case 'ImportSpecifier': return node === parent.local;

			// disregard the `foo` in `foo: while (...) { ... break foo; ... continue foo;***REMOVED***`
...
class Identifier extends NodeBase {
    ...
    ***REMOVED***
    declare(kind, init) {
        let variable;
        const { treeshake ***REMOVED*** = this.context.options;
        switch (kind) {
            case 'var':
            ...
            default:
                /* istanbul ignore next */
                throw new Error(`Internal Error: Unexpected identifier kind ${kind***REMOVED***.`);
        ***REMOVED***
        variable.kind = kind;
    ...
    ***REMOVED***
    render(code, { snippets: { getPropertyAccess ***REMOVED*** ***REMOVED***, { renderedParentType, isCalleeOfRenderedParent, isShorthandProperty ***REMOVED*** = BLANK) {
        if (this.variable) {
            ...
            if (name !== this.name) {
                ...
                ***REMOVED***);
                if (isShorthandProperty) {
                    code.prependRight(this.start, `${this.name***REMOVED***: `);
                ***REMOVED***
            ***REMOVED***
    ...
    ***REMOVED***
    disallowImportReassignment() {
        return this.context.error({
            ...
            message: `Illegal reassignment to import '${this.name***REMOVED***'`
        ***REMOVED***, this.start);
    ***REMOVED***
...
const NO_SEMICOLON = { isNoStatement: true ***REMOVED***;
    ...
    initialise() {
        if (this.directive &&
            ...
            // This is necessary, because either way (deleting or not) can lead to errors.
            {
                ...
                message: `Module level directives cause errors when bundled, '${this.directive***REMOVED***' was ignored.`
            ***REMOVED***, this.start);
        ***REMOVED***
    ***REMOVED***
    render(code, options) {
    ...
    ***REMOVED***
    include(context, includeChildrenRecursively) {
        if (!this.deoptimized)
            this.applyDeoptimizations();
        this.included = true;
        const { brokenFlow ***REMOVED*** = context;
    ...
    ***REMOVED***
    hasEffectsOnInteractionAtPath(path, interaction, context) {
        ...
        if (interaction.type === INTERACTION_CALLED) {
            const { ignore, brokenFlow ***REMOVED*** = context;
            context.ignore = {
                breaks: false,
                continues: false,
                labels: new Set(),
...
function getSystemExportStatement(exportedVariables, { exportNamesByVariable, snippets: { _, getObject, getPropertyAccess ***REMOVED*** ***REMOVED***, modifier = '') {
    if (exportedVariables.length === 1 &&
        exportNamesByVariable.get(exportedVariables[0]).length === 1) {
        const variable = exportedVariables[0];
        return `exports('${exportNamesByVariable.get(variable)***REMOVED***',${_***REMOVED***${variable.getName(getPropertyAccess)***REMOVED***${modifier***REMOVED***)`;
    ***REMOVED***
    else {
        const fields = [];
        for (const variable of exportedVariables) {
            ...
            ***REMOVED***
        ***REMOVED***
        return `exports(${getObject(fields, { lineBreakIndent: null ***REMOVED***)***REMOVED***)`;
    ***REMOVED***
***REMOVED***
function renderSystemExportExpression(exportedVariable, expressionStart, expressionEnd, code, { exportNamesByVariable, snippets: { _ ***REMOVED*** ***REMOVED***) {
    code.prependRight(expressionStart, `exports('${exportNamesByVariable.get(exportedVariable)***REMOVED***',${_***REMOVED***`);
    code.appendLeft(expressionEnd, ')');
***REMOVED***
function renderSystemExportFunction(exportedVariables, expressionStart, expressionEnd, needsParens, code, options) {
    const { _, getDirectReturnIifeLeft ***REMOVED*** = options.snippets;
    code.prependRight(expressionStart, getDirectReturnIifeLeft(['v'], `${getSystemExportStatement(exportedVariables, options)***REMOVED***,${_***REMOVED***v`, { needsArrowReturnParens: true, needsWrappedFunction: needsParens ***REMOVED***));
    code.appendLeft(expressionEnd, ')');
***REMOVED***
function renderSystemExportSequenceAfterExpression(exportedVariable, expressionStart, expressionEnd, needsParens, code, options) {
    const { _, getPropertyAccess ***REMOVED*** = options.snippets;
    code.appendLeft(expressionEnd, `,${_***REMOVED***${getSystemExportStatement([exportedVariable], options)***REMOVED***,${_***REMOVED***${exportedVariable.getName(getPropertyAccess)***REMOVED***`);
    if (needsParens) {
...
function renderSystemExportSequenceBeforeExpression(exportedVariable, expressionStart, expressionEnd, needsParens, code, options, modifier) {
    const { _ ***REMOVED*** = options.snippets;
    code.prependRight(expressionStart, `${getSystemExportStatement([exportedVariable], options, modifier)***REMOVED***,${_***REMOVED***`);
    if (needsParens) {
        code.prependRight(expressionStart, '(');
        code.appendLeft(expressionEnd, ')');
    ***REMOVED***
...
class AssignmentExpression extends NodeBase {
    hasEffects(context) {
        const { deoptimized, left, right ***REMOVED*** = this;
        if (!deoptimized)
            this.applyDeoptimizations();
    ...
    ***REMOVED***
    include(context, includeChildrenRecursively) {
        const { deoptimized, left, right, operator ***REMOVED*** = this;
        if (!deoptimized)
            this.applyDeoptimizations();
        this.included = true;
    ...
    ***REMOVED***
    render(code, options, { preventASI, renderedParentType, renderedSurroundingElement ***REMOVED*** = BLANK) {
        const { left, right, start, end, parent ***REMOVED*** = this;
        if (left.included) {
...
class AssignmentPattern extends NodeBase {
    ...
    ***REMOVED***
    render(code, options, { isShorthandProperty ***REMOVED*** = BLANK) {
        this.left.render(code, options, { isShorthandProperty ***REMOVED***);
...
class ThisVariable extends LocalVariable {
    ...
    ***REMOVED***
    addEntityToBeDeoptimized(entity) {
        ...
        ***REMOVED***
        for (const { interaction, path ***REMOVED*** of this.thisDeoptimizationList) {
    ...
    hasEffectsOnInteractionAtPath(path, interaction, context) {
        ...
        if (interaction.type === INTERACTION_CALLED) {
            ...
            context.replacedVariableInits.set(this.scope.thisVariable, interaction.withNew
                ...
                : UNKNOWN_EXPRESSION);
            const { brokenFlow, ignore ***REMOVED*** = context;
            context.ignore = {
                breaks: false,
                continues: false,
                labels: new Set(),
    ...
    include(context, includeChildrenRecursively) {
        ...
        if (!this.included) {
            ...
            checkTopLevelAwait: if (!this.context.usesTopLevelAwait) {
                ...
                ***REMOVED*** while ((parent = parent.parent));
...
class BinaryExpression extends NodeBase {
    ...
    ***REMOVED***
    hasEffectsOnInteractionAtPath(path, { type ***REMOVED***) {
        return type !== INTERACTION_ACCESSED || path.length > 1;
    ***REMOVED***
    render(code, options, { renderedSurroundingElement ***REMOVED*** = BLANK) {
        this.left.render(code, options, { renderedSurroundingElement ***REMOVED***);
        this.right.render(code, options);
    ***REMOVED***
***REMOVED***
...
class Literal extends NodeBase {
    deoptimizeThisOnInteractionAtPath() { ***REMOVED***
    getLiteralValueAtPath(path) {
        if (path.length > 0 ||
...
function getPathIfNotComputed(memberExpression) {
    ...
    if (typeof nextPathKey === 'string') {
        if (object instanceof Identifier) {
            return [
                { key: object.name, pos: object.start ***REMOVED***,
                { key: nextPathKey, pos: memberExpression.property.start ***REMOVED***
            ];
        ***REMOVED***
        if (object instanceof MemberExpression) {
            const parentPath = getPathIfNotComputed(object);
            return (parentPath && [...parentPath, { key: nextPathKey, pos: memberExpression.property.start ***REMOVED***]);
        ***REMOVED***
    ***REMOVED***
    return null;
***REMOVED***
...
class MemberExpression extends NodeBase {
    ...
    ***REMOVED***
    initialise() {
        ...
        this.accessInteraction = { thisArg: this.object, type: INTERACTION_ACCESSED ***REMOVED***;
    ***REMOVED***
    render(code, options, { renderedParentType, isCalleeOfRenderedParent, renderedSurroundingElement ***REMOVED*** = BLANK) {
        if (this.variable || this.replacement) {
            const { snippets: { getPropertyAccess ***REMOVED*** ***REMOVED*** = options;
        ...
        ***REMOVED***
            ...
            if (renderedParentType && isCalleeOfRenderedParent) {
                code.appendRight(this.start, '0, ');
            ***REMOVED***
            this.object.render(code, options, { renderedSurroundingElement ***REMOVED***);
            ...
            if (variable.isNamespace) {
                ...
                ***REMOVED***
                this.context.warn({
                    ...
                    message: `Illegal reassignment to import '${this.object.name***REMOVED***'`
                ***REMOVED***, this.start);
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***
...
function resolveNamespaceVariables(baseVariable, path, astContext) {
    ...
    if (!variable) {
        ...
        astContext.warn({
            ...
            message: `'${exportName***REMOVED***' is not exported by '${relativeId(fileName)***REMOVED***'`,
            missing: exportName,
            url: `https://rollupjs.org/guide/en/#error-name-is-not-exported-by-module`
        ***REMOVED***, path[0].pos);
        return 'undefined';
        ...
        if (this.callee instanceof Identifier) {
            ...
            if (variable.isNamespace) {
                this.context.warn({
                    ...
                    message: `Cannot call a namespace ('${this.callee.name***REMOVED***')`
                ***REMOVED***, this.start);
            ***REMOVED***
...
class CatchClause extends NodeBase {
    ...
    ***REMOVED***
    parseNode(esTreeNode) {
        ...
        const { param ***REMOVED*** = esTreeNode;
        if (param) {
    ...
    deoptimizeThisOnInteractionAtPath(interaction, path, recursionTracker) {
        if (interaction.type === INTERACTION_ACCESSED && this.kind === 'get' && path.length === 0) {
            return this.value.deoptimizeThisOnInteractionAtPath({
                ...
                type: INTERACTION_CALLED,
                withNew: false
            ***REMOVED***, EMPTY_PATH, recursionTracker);
        ***REMOVED***
        ...
        for (const definition of this.body.body) {
            ...
            if (definition.computed) {
                ...
                if (typeof keyValue === 'symbol') {
                    properties.push({ key: UnknownKey, kind, property: definition ***REMOVED***);
                    continue;
                ***REMOVED***
                else {
                    key = String(keyValue);
            ...
            ***REMOVED***
            properties.push({ key, kind, property: definition ***REMOVED***);
        ***REMOVED***
...
class ClassDeclaration extends ClassNode {
    ...
    ***REMOVED***
    render(code, options) {
        const { exportNamesByVariable, format, snippets: { _ ***REMOVED*** ***REMOVED*** = options;
        if (format === 'system' && this.id && exportNamesByVariable.has(this.id.variable)) {
            code.appendLeft(this.end, `${_***REMOVED***${getSystemExportStatement([this.id.variable], options)***REMOVED***;`);
        ***REMOVED***
        super.render(code, options);
    ***REMOVED***
***REMOVED***
...
class ConditionalExpression extends NodeBase {
    ...
    ***REMOVED***
    includeCallArguments(context, args) {
        ...
        ***REMOVED***
    ***REMOVED***
    render(code, options, { isCalleeOfRenderedParent, preventASI, renderedParentType, renderedSurroundingElement ***REMOVED*** = BLANK) {
        const usedBranch = this.getUsedBranch();
        ...
        ***REMOVED***
            ...
            this.test.render(code, options, { renderedSurroundingElement ***REMOVED***);
...
class DoWhileStatement extends NodeBase {
    hasEffects(context) {
        if (this.test.hasEffects(context))
            return true;
        const { brokenFlow, ignore: { breaks, continues ***REMOVED*** ***REMOVED*** = context;
    ...
    ***REMOVED***
    include(context, includeChildrenRecursively) {
        ...
        this.test.include(context, includeChildrenRecursively);
        const { brokenFlow ***REMOVED*** = context;
        this.body.include(context, includeChildrenRecursively, { asSingleStatement: true ***REMOVED***);
        context.brokenFlow = brokenFlow;
    ***REMOVED***
***REMOVED***
...
class ExportDefaultDeclaration extends NodeBase {
    ...
    ***REMOVED***
    render(code, options, nodeRenderOptions) {
        const { start, end ***REMOVED*** = nodeRenderOptions;
    ...
    ***REMOVED***
    ...
    renderNamedDeclaration(code, declarationStart, declarationKeyword, endMarker, needsId, options) {
        const { exportNamesByVariable, format, snippets: { getPropertyAccess ***REMOVED*** ***REMOVED*** = options;
        const name = this.variable.getName(getPropertyAccess);
        // Remove `export default`
        code.remove(this.start, declarationStart);
        if (needsId) {
            code.appendLeft(getIdInsertPosition(code.original, declarationKeyword, endMarker, declarationStart), ` ${name***REMOVED***`);
        ***REMOVED***
        if (format === 'system' &&
            this.declaration instanceof ClassDeclaration &&
            exportNamesByVariable.has(this.variable)) {
            code.appendLeft(this.end, ` ${getSystemExportStatement([this.variable], options)***REMOVED***;`);
        ***REMOVED***
    ***REMOVED***
    renderVariableDeclaration(code, declarationStart, { format, exportNamesByVariable, snippets: { cnst, getPropertyAccess ***REMOVED*** ***REMOVED***) {
        const hasTrailingSemicolon = code.original.charCodeAt(this.end - 1) === 59; /*";"*/
        const systemExportNames = format === 'system' && exportNamesByVariable.get(this.variable);
        if (systemExportNames) {
            code.overwrite(this.start, declarationStart, `${cnst***REMOVED*** ${this.variable.getName(getPropertyAccess)***REMOVED*** = exports('${systemExportNames[0]***REMOVED***', `);
            code.appendRight(hasTrailingSemicolon ? this.end - 1 : this.end, ')' + (hasTrailingSemicolon ? '' : ';'));
        ***REMOVED***
        else {
            code.overwrite(this.start, declarationStart, `${cnst***REMOVED*** ${this.variable.getName(getPropertyAccess)***REMOVED*** = `);
    ...
    ***REMOVED***
    render(code, options, nodeRenderOptions) {
        ...
        ***REMOVED***
            ...
            this.declaration.render(code, options, { end, start ***REMOVED***);
        ***REMOVED***
    ***REMOVED***
    applyDeoptimizations() { ***REMOVED***
***REMOVED***
...
class ForInStatement extends NodeBase {
    ...
    ***REMOVED***
    include(context, includeChildrenRecursively) {
        const { body, deoptimized, left, right ***REMOVED*** = this;
        if (!deoptimized)
            this.applyDeoptimizations();
        this.included = true;
        ...
        right.include(context, includeChildrenRecursively);
        const { brokenFlow ***REMOVED*** = context;
        body.include(context, includeChildrenRecursively, { asSingleStatement: true ***REMOVED***);
        context.brokenFlow = brokenFlow;
    ***REMOVED***
    initialise() {
...
class ForStatement extends NodeBase {
    ...
    ***REMOVED***
    include(context, includeChildrenRecursively) {
        var _a, _b, _c;
        this.included = true;
        (_a = this.init) === null || _a === void 0 ? void 0 : _a.include(context, includeChildrenRecursively, { asSingleStatement: true ***REMOVED***);
    ...
    includeKnownTest(context, testValue) {
        ...
        ***REMOVED***
        if (testValue && this.consequent.shouldBeIncluded(context)) {
            this.consequent.include(context, false, { asSingleStatement: true ***REMOVED***);
        ***REMOVED***
        if (!testValue && ((_a = this.alternate) === null || _a === void 0 ? void 0 : _a.shouldBeIncluded(context))) {
            this.alternate.include(context, false, { asSingleStatement: true ***REMOVED***);
        ***REMOVED***
    ***REMOVED***
    ...
    ***REMOVED***
    renderHoistedDeclarations(hoistedDeclarations, code, getPropertyAccess) {
        ...
        if (hoistedVars) {
            ...
            code.prependRight(this.start, `${needsBraces ? '{ ' : ''***REMOVED***var ${hoistedVars***REMOVED***; `);
            if (needsBraces) {
                code.appendLeft(this.end, ` ***REMOVED***`);
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***
...
const HELPER_GENERATORS = {
    [INTEROP_DEFAULT_LEGACY_VARIABLE](_t, snippets, liveBindings) {
        const { _, getDirectReturnFunction, n ***REMOVED*** = snippets;
        const [left, right] = getDirectReturnFunction(['e'], {
            functionReturn: true,
            lineBreakIndent: null,
            name: INTEROP_DEFAULT_LEGACY_VARIABLE
        ***REMOVED***);
        return (`${left***REMOVED***e${_***REMOVED***&&${_***REMOVED***typeof e${_***REMOVED***===${_***REMOVED***'object'${_***REMOVED***&&${_***REMOVED***'default'${_***REMOVED***in e${_***REMOVED***?${_***REMOVED***` +
            `${liveBindings ? getDefaultLiveBinding(snippets) : getDefaultStatic(snippets)***REMOVED***${right***REMOVED***${n***REMOVED***${n***REMOVED***`);
    ***REMOVED***,
    [INTEROP_DEFAULT_VARIABLE](_t, snippets, liveBindings) {
        ...
        ***REMOVED***);
        return (`${left***REMOVED***e${_***REMOVED***&&${_***REMOVED***e.__esModule${_***REMOVED***?${_***REMOVED***` +
            `${liveBindings ? getDefaultLiveBinding(snippets) : getDefaultStatic(snippets)***REMOVED***${right***REMOVED***${n***REMOVED***${n***REMOVED***`);
    ***REMOVED***,
    [INTEROP_NAMESPACE_DEFAULT_ONLY_VARIABLE](_t, snippets, _liveBindings, freeze, namespaceToStringTag) {
        const { getDirectReturnFunction, getObject, n ***REMOVED*** = snippets;
        const [left, right] = getDirectReturnFunction(['e'], {
            functionReturn: true,
            lineBreakIndent: null,
            name: INTEROP_NAMESPACE_DEFAULT_ONLY_VARIABLE
        ***REMOVED***);
        return `${left***REMOVED***${getFrozen(freeze, getWithToStringTag(namespaceToStringTag, getObject([
            ['__proto__', 'null'],
            ['default', 'e']
        ], { lineBreakIndent: null ***REMOVED***), snippets))***REMOVED***${right***REMOVED***${n***REMOVED***${n***REMOVED***`;
    ***REMOVED***,
    [INTEROP_NAMESPACE_DEFAULT_VARIABLE](t, snippets, liveBindings, freeze, namespaceToStringTag) {
        const { _, n ***REMOVED*** = snippets;
        return (`function ${INTEROP_NAMESPACE_DEFAULT_VARIABLE***REMOVED***(e)${_***REMOVED***{${n***REMOVED***` +
            createNamespaceObject(t, t, snippets, liveBindings, freeze, namespaceToStringTag) +
            `***REMOVED***${n***REMOVED***${n***REMOVED***`);
    ***REMOVED***,
    [INTEROP_NAMESPACE_VARIABLE](t, snippets, liveBindings, freeze, namespaceToStringTag, usedHelpers) {
        ...
        if (usedHelpers.has(INTEROP_NAMESPACE_DEFAULT_VARIABLE)) {
            ...
            ***REMOVED***);
            return `${left***REMOVED***e${_***REMOVED***&&${_***REMOVED***e.__esModule${_***REMOVED***?${_***REMOVED***e${_***REMOVED***:${_***REMOVED***${INTEROP_NAMESPACE_DEFAULT_VARIABLE***REMOVED***(e)${right***REMOVED***${n***REMOVED***${n***REMOVED***`;
        ***REMOVED***
        return (`function ${INTEROP_NAMESPACE_VARIABLE***REMOVED***(e)${_***REMOVED***{${n***REMOVED***` +
            `${t***REMOVED***if${_***REMOVED***(e${_***REMOVED***&&${_***REMOVED***e.__esModule)${_***REMOVED***return e;${n***REMOVED***` +
            createNamespaceObject(t, t, snippets, liveBindings, freeze, namespaceToStringTag) +
            `***REMOVED***${n***REMOVED***${n***REMOVED***`);
    ***REMOVED***,
    [MERGE_NAMESPACES_VARIABLE](t, snippets, liveBindings, freeze, namespaceToStringTag) {
        const { _, cnst, n ***REMOVED*** = snippets;
        const useForEach = cnst === 'var' && liveBindings;
        return (`function ${MERGE_NAMESPACES_VARIABLE***REMOVED***(n, m)${_***REMOVED***{${n***REMOVED***` +
            `${t***REMOVED***${loopOverNamespaces(`{${n***REMOVED***` +
                `${t***REMOVED***${t***REMOVED***${t***REMOVED***if${_***REMOVED***(k${_***REMOVED***!==${_***REMOVED***'default'${_***REMOVED***&&${_***REMOVED***!(k in n))${_***REMOVED***{${n***REMOVED***` +
                (liveBindings
                    ? useForEach
                        ? copyOwnPropertyLiveBinding
                        : copyPropertyLiveBinding
                    : copyPropertyStatic)(t, t + t + t + t, snippets) +
                `${t***REMOVED***${t***REMOVED***${t***REMOVED******REMOVED***${n***REMOVED***` +
                `${t***REMOVED***${t***REMOVED******REMOVED***`, useForEach, t, snippets)***REMOVED***${n***REMOVED***` +
            `${t***REMOVED***return ${getFrozen(freeze, getWithToStringTag(namespaceToStringTag, 'n', snippets))***REMOVED***;${n***REMOVED***` +
            `***REMOVED***${n***REMOVED***${n***REMOVED***`);
    ***REMOVED***
***REMOVED***;
const getDefaultLiveBinding = ({ _, getObject ***REMOVED***) => `e${_***REMOVED***:${_***REMOVED***${getObject([['default', 'e']], { lineBreakIndent: null ***REMOVED***)***REMOVED***`;
const getDefaultStatic = ({ _, getPropertyAccess ***REMOVED***) => `e${getPropertyAccess('default')***REMOVED***${_***REMOVED***:${_***REMOVED***e`;
const createNamespaceObject = (t, i, snippets, liveBindings, freeze, namespaceToStringTag) => {
    const { _, cnst, getObject, getPropertyAccess, n, s ***REMOVED*** = snippets;
    const copyProperty = `{${n***REMOVED***` +
        (liveBindings ? copyNonDefaultOwnPropertyLiveBinding : copyPropertyStatic)(t, i + t + t, snippets) +
        `${i***REMOVED***${t***REMOVED******REMOVED***`;
    return (`${i***REMOVED***${cnst***REMOVED*** n${_***REMOVED***=${_***REMOVED***Object.create(null${namespaceToStringTag
        ? `,${_***REMOVED***{${_***REMOVED***[Symbol.toStringTag]:${_***REMOVED***${getToStringTagValue(getObject)***REMOVED***${_***REMOVED******REMOVED***`
        : ''***REMOVED***);${n***REMOVED***` +
        `${i***REMOVED***if${_***REMOVED***(e)${_***REMOVED***{${n***REMOVED***` +
        `${i***REMOVED***${t***REMOVED***${loopOverKeys(copyProperty, !liveBindings, snippets)***REMOVED***${n***REMOVED***` +
        `${i***REMOVED******REMOVED***${n***REMOVED***` +
        `${i***REMOVED***n${getPropertyAccess('default')***REMOVED***${_***REMOVED***=${_***REMOVED***e;${n***REMOVED***` +
        `${i***REMOVED***return ${getFrozen(freeze, 'n')***REMOVED***${s***REMOVED***${n***REMOVED***`);
***REMOVED***;
const loopOverKeys = (body, allowVarLoopVariable, { _, cnst, getFunctionIntro, s ***REMOVED***) => cnst !== 'var' || allowVarLoopVariable
    ? `for${_***REMOVED***(${cnst***REMOVED*** k in e)${_***REMOVED***${body***REMOVED***`
    : `Object.keys(e).forEach(${getFunctionIntro(['k'], {
        isAsync: false,
        name: null
    ***REMOVED***)***REMOVED***${body***REMOVED***)${s***REMOVED***`;
const loopOverNamespaces = (body, useForEach, t, { _, cnst, getDirectReturnFunction, getFunctionIntro, n ***REMOVED***) => {
    if (useForEach) {
        const [left, right] = getDirectReturnFunction(['e'], {
            functionReturn: false,
            lineBreakIndent: { base: t, t ***REMOVED***,
            name: null
        ***REMOVED***);
        return (`m.forEach(${left***REMOVED***` +
            `e${_***REMOVED***&&${_***REMOVED***typeof e${_***REMOVED***!==${_***REMOVED***'string'${_***REMOVED***&&${_***REMOVED***!Array.isArray(e)${_***REMOVED***&&${_***REMOVED***Object.keys(e).forEach(${getFunctionIntro(['k'], {
                isAsync: false,
                name: null
            ***REMOVED***)***REMOVED***${body***REMOVED***)${right***REMOVED***);`);
    ***REMOVED***
    return (`for${_***REMOVED***(var i${_***REMOVED***=${_***REMOVED***0;${_***REMOVED***i${_***REMOVED***<${_***REMOVED***m.length;${_***REMOVED***i++)${_***REMOVED***{${n***REMOVED***` +
        `${t***REMOVED***${t***REMOVED***${cnst***REMOVED*** e${_***REMOVED***=${_***REMOVED***m[i];${n***REMOVED***` +
        `${t***REMOVED***${t***REMOVED***if${_***REMOVED***(typeof e${_***REMOVED***!==${_***REMOVED***'string'${_***REMOVED***&&${_***REMOVED***!Array.isArray(e))${_***REMOVED***{${_***REMOVED***for${_***REMOVED***(${cnst***REMOVED*** k in e)${_***REMOVED***${body***REMOVED***${_***REMOVED******REMOVED***${n***REMOVED***${t***REMOVED******REMOVED***`);
***REMOVED***;
const copyNonDefaultOwnPropertyLiveBinding = (t, i, snippets) => {
    const { _, n ***REMOVED*** = snippets;
    return (`${i***REMOVED***if${_***REMOVED***(k${_***REMOVED***!==${_***REMOVED***'default')${_***REMOVED***{${n***REMOVED***` +
        copyOwnPropertyLiveBinding(t, i + t, snippets) +
        `${i***REMOVED******REMOVED***${n***REMOVED***`);
***REMOVED***;
const copyOwnPropertyLiveBinding = (t, i, { _, cnst, getDirectReturnFunction, n ***REMOVED***) => {
    const [left, right] = getDirectReturnFunction([], {
        functionReturn: true,
        lineBreakIndent: null,
        name: null
    ***REMOVED***);
    return (`${i***REMOVED***${cnst***REMOVED*** d${_***REMOVED***=${_***REMOVED***Object.getOwnPropertyDescriptor(e,${_***REMOVED***k);${n***REMOVED***` +
        `${i***REMOVED***Object.defineProperty(n,${_***REMOVED***k,${_***REMOVED***d.get${_***REMOVED***?${_***REMOVED***d${_***REMOVED***:${_***REMOVED***{${n***REMOVED***` +
        `${i***REMOVED***${t***REMOVED***enumerable:${_***REMOVED***true,${n***REMOVED***` +
        `${i***REMOVED***${t***REMOVED***get:${_***REMOVED***${left***REMOVED***e[k]${right***REMOVED***${n***REMOVED***` +
        `${i***REMOVED******REMOVED***);${n***REMOVED***`);
***REMOVED***;
const copyPropertyLiveBinding = (t, i, { _, cnst, getDirectReturnFunction, n ***REMOVED***) => {
    const [left, right] = getDirectReturnFunction([], {
        functionReturn: true,
        lineBreakIndent: null,
        name: null
    ***REMOVED***);
    return (`${i***REMOVED***${cnst***REMOVED*** d${_***REMOVED***=${_***REMOVED***Object.getOwnPropertyDescriptor(e,${_***REMOVED***k);${n***REMOVED***` +
        `${i***REMOVED***if${_***REMOVED***(d)${_***REMOVED***{${n***REMOVED***` +
        `${i***REMOVED***${t***REMOVED***Object.defineProperty(n,${_***REMOVED***k,${_***REMOVED***d.get${_***REMOVED***?${_***REMOVED***d${_***REMOVED***:${_***REMOVED***{${n***REMOVED***` +
        `${i***REMOVED***${t***REMOVED***${t***REMOVED***enumerable:${_***REMOVED***true,${n***REMOVED***` +
        `${i***REMOVED***${t***REMOVED***${t***REMOVED***get:${_***REMOVED***${left***REMOVED***e[k]${right***REMOVED***${n***REMOVED***` +
        `${i***REMOVED***${t***REMOVED******REMOVED***);${n***REMOVED***` +
        `${i***REMOVED******REMOVED***${n***REMOVED***`);
***REMOVED***;
const copyPropertyStatic = (_t, i, { _, n ***REMOVED***) => `${i***REMOVED***n[k]${_***REMOVED***=${_***REMOVED***e[k];${n***REMOVED***`;
const getFrozen = (freeze, fragment) => freeze ? `Object.freeze(${fragment***REMOVED***)` : fragment;
const getWithToStringTag = (namespaceToStringTag, fragment, { _, getObject ***REMOVED***) => namespaceToStringTag
    ? `Object.defineProperty(${fragment***REMOVED***,${_***REMOVED***Symbol.toStringTag,${_***REMOVED***${getToStringTagValue(getObject)***REMOVED***)`
    : fragment;
...
class ImportExpression extends NodeBase {
    ...
    ***REMOVED***
    render(code, options) {
        if (this.inlineNamespace) {
            const { snippets: { getDirectReturnFunction, getPropertyAccess ***REMOVED*** ***REMOVED*** = options;
            const [left, right] = getDirectReturnFunction([], {
                functionReturn: true,
                lineBreakIndent: null,
                name: null
            ***REMOVED***);
            code.overwrite(this.start, this.end, `Promise.resolve().then(${left***REMOVED***${this.inlineNamespace.getName(getPropertyAccess)***REMOVED***${right***REMOVED***)`, { contentOnly: true ***REMOVED***);
            return;
        ***REMOVED***
        if (this.mechanism) {
            code.overwrite(this.start, findFirstOccurrenceOutsideComment(code.original, '(', this.start + 6) + 1, this.mechanism.left, { contentOnly: true ***REMOVED***);
            code.overwrite(this.end - 1, this.end, this.mechanism.right, { contentOnly: true ***REMOVED***);
        ***REMOVED***
        this.source.render(code, options);
    ***REMOVED***
    renderFinalResolution(code, resolution, namespaceExportName, { getDirectReturnFunction ***REMOVED***) {
        ...
        if (namespaceExportName) {
            const [left, right] = getDirectReturnFunction(['n'], {
                ...
                name: null
            ***REMOVED***);
            code.prependLeft(this.end, `.then(${left***REMOVED***n.${namespaceExportName***REMOVED***${right***REMOVED***)`);
        ***REMOVED***
    ***REMOVED***
    setExternalResolution(exportMode, resolution, options, snippets, pluginDriver, accessedGlobalsByScope) {
        const { format ***REMOVED*** = options;
        this.inlineNamespace = null;
        this.resolution = resolution;
        const accessedGlobals = [...(accessedImportGlobals[format] || [])];
        let helper;
        ({ helper, mechanism: this.mechanism ***REMOVED*** = this.getDynamicImportMechanismAndHelper(resolution, exportMode, options, snippets, pluginDriver));
        if (helper) {
    ...
    ***REMOVED***
    applyDeoptimizations() { ***REMOVED***
    getDynamicImportMechanismAndHelper(resolution, exportMode, { compact, dynamicImportFunction, format, generatedCode: { arrowFunctions ***REMOVED***, interop ***REMOVED***, { _, getDirectReturnFunction, getDirectReturnIifeLeft ***REMOVED***, pluginDriver) {
        ...
        if (mechanism) {
            return { helper: null, mechanism ***REMOVED***;
        ***REMOVED***
        ...
        switch (format) {
            case 'cjs': {
                ...
                let right = `)`;
                if (helper) {
                    left = `/*#__PURE__*/${helper***REMOVED***(${left***REMOVED***`;
                    right += ')';
                ***REMOVED***
                const [functionLeft, functionRight] = getDirectReturnFunction([], {
                    ...
                    name: null
                ***REMOVED***);
                left = `Promise.resolve().then(${functionLeft***REMOVED***${left***REMOVED***`;
                right += `${functionRight***REMOVED***)`;
                if (!arrowFunctions && hasDynamicTarget) {
                    left = getDirectReturnIifeLeft(['t'], `${left***REMOVED***t${right***REMOVED***`, {
            ...
            case 'amd': {
                ...
                ***REMOVED***);
                const resolveNamespace = helper
                    ? `${resolveLeft***REMOVED***${resolve***REMOVED***(/*#__PURE__*/${helper***REMOVED***(m))${resolveRight***REMOVED***`
                    : resolve;
                const [handlerLeft, handlerRight] = getDirectReturnFunction([resolve, reject], {
                    ...
                    name: null
                ***REMOVED***);
                let left = `new Promise(${handlerLeft***REMOVED***require([`;
                let right = `],${_***REMOVED***${resolveNamespace***REMOVED***,${_***REMOVED***${reject***REMOVED***)${handlerRight***REMOVED***)`;
                if (!arrowFunctions && hasDynamicTarget) {
                    left = getDirectReturnIifeLeft(['t'], `${left***REMOVED***t${right***REMOVED***`, {
                        needsArrowReturnParens: false,
                        needsWrappedFunction: true
            ...
            case 'es':
                if (dynamicImportFunction) {
                    return {
                        helper: null,
                        mechanism: {
                            left: `${dynamicImportFunction***REMOVED***(`,
                            right: ')'
                        ***REMOVED***
                    ***REMOVED***;
                ***REMOVED***
        ***REMOVED***
        return { helper: null, mechanism: null ***REMOVED***;
    ***REMOVED***
***REMOVED***
    ...
    renderFinalMechanism(code, chunkId, format, snippets, outputPluginDriver) {
        ...
        if (metaProperty &&
            ...
            ***REMOVED***
                ...
                warnDeprecation(`Using the "${ASSET_PREFIX***REMOVED***" prefix to reference files is deprecated. Use the "${FILE_PREFIX***REMOVED***" prefix instead.`, true, this.context.options);
                assetReferenceId = metaProperty.substring(ASSET_PREFIX.length);
                fileName = outputPluginDriver.getFileName(assetReferenceId);
            ***REMOVED***
            else {
                warnDeprecation(`Using the "${CHUNK_PREFIX***REMOVED***" prefix to reference files is deprecated. Use the "${FILE_PREFIX***REMOVED***" prefix instead.`, true, this.context.options);
            ...
            ***REMOVED***
            code.overwrite(parent.start, parent.end, replacement, { contentOnly: true ***REMOVED***);
            return;
        ***REMOVED***
        const replacement = outputPluginDriver.hookFirstSync('resolveImportMeta', [
            metaProperty,
            {
                ...
                moduleId: this.context.module.id
            ***REMOVED***
        ]) || ((_a = importMetaMechanisms[format]) === null || _a === void 0 ? void 0 : _a.call(importMetaMechanisms, metaProperty, { chunkId, snippets ***REMOVED***));
        if (typeof replacement === 'string') {
            if (parent instanceof MemberExpression) {
                code.overwrite(parent.start, parent.end, replacement, { contentOnly: true ***REMOVED***);
            ***REMOVED***
            else {
                code.overwrite(this.start, this.end, replacement, { contentOnly: true ***REMOVED***);
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***
***REMOVED***
...
const accessedFileUrlGlobals = {
    ...
    umd: ['document', 'require', 'URL']
***REMOVED***;
const getResolveUrl = (path, URL = 'URL') => `new ${URL***REMOVED***(${path***REMOVED***).href`;
const getRelativeUrlFromDocument = (relativePath, umd = false) => getResolveUrl(`'${relativePath***REMOVED***', ${umd ? `typeof document === 'undefined' ? location.href : ` : ''***REMOVED***document.currentScript && document.currentScript.tagName.toUpperCase() === 'SCRIPT' && document.currentScript.src || document.baseURI`);
const getGenericImportMetaMechanism = (getUrl) => (prop, { chunkId ***REMOVED***) => {
    const urlMechanism = getUrl(chunkId);
    return prop === null
        ? `({ url: ${urlMechanism***REMOVED*** ***REMOVED***)`
        : prop === 'url'
            ? urlMechanism
            : 'undefined';
***REMOVED***;
const getUrlFromDocument = (chunkId, umd = false) => `${umd ? `typeof document === 'undefined' ? location.href : ` : ''***REMOVED***(document.currentScript && document.currentScript.tagName.toUpperCase() === 'SCRIPT' && document.currentScript.src || new URL('${chunkId***REMOVED***', document.baseURI).href)`;
const relativeUrlMechanisms = {
    amd: relativePath => {
        if (relativePath[0] !== '.')
            relativePath = './' + relativePath;
        return getResolveUrl(`require.toUrl('${relativePath***REMOVED***'), document.baseURI`);
    ***REMOVED***,
    cjs: relativePath => `(typeof document === 'undefined' ? ${getResolveUrl(`'file:' + __dirname + '/${relativePath***REMOVED***'`, `(require('u' + 'rl').URL)`)***REMOVED*** : ${getRelativeUrlFromDocument(relativePath)***REMOVED***)`,
    es: relativePath => getResolveUrl(`'${relativePath***REMOVED***', import.meta.url`),
    iife: relativePath => getRelativeUrlFromDocument(relativePath),
    system: relativePath => getResolveUrl(`'${relativePath***REMOVED***', module.meta.url`),
    umd: relativePath => `(typeof document === 'undefined' && typeof location === 'undefined' ? ${getResolveUrl(`'file:' + __dirname + '/${relativePath***REMOVED***'`, `(require('u' + 'rl').URL)`)***REMOVED*** : ${getRelativeUrlFromDocument(relativePath, true)***REMOVED***)`
***REMOVED***;
const importMetaMechanisms = {
    amd: getGenericImportMetaMechanism(() => getResolveUrl(`module.uri, document.baseURI`)),
    cjs: getGenericImportMetaMechanism(chunkId => `(typeof document === 'undefined' ? ${getResolveUrl(`'file:' + __filename`, `(require('u' + 'rl').URL)`)***REMOVED*** : ${getUrlFromDocument(chunkId)***REMOVED***)`),
    iife: getGenericImportMetaMechanism(chunkId => getUrlFromDocument(chunkId)),
    system: (prop, { snippets: { getPropertyAccess ***REMOVED*** ***REMOVED***) => prop === null ? `module.meta` : `module.meta${getPropertyAccess(prop)***REMOVED***`,
    umd: getGenericImportMetaMechanism(chunkId => `(typeof document === 'undefined' && typeof location === 'undefined' ? ${getResolveUrl(`'file:' + __filename`, `(require('u' + 'rl').URL)`)***REMOVED*** : ${getUrlFromDocument(chunkId, true)***REMOVED***)`)
***REMOVED***;
        ...
        ***REMOVED***
        ...
        for (const property of this.properties) {
            if (property instanceof SpreadElement) {
                properties.push({ key: UnknownKey, kind: 'init', property ***REMOVED***);
                continue;
            ***REMOVED***
            let key;
            if (property.computed) {
                ...
                if (typeof keyValue === 'symbol') {
                    properties.push({ key: UnknownKey, kind: property.kind, property ***REMOVED***);
                    continue;
                ***REMOVED***
                else {
                    key = String(keyValue);
            ...
            ***REMOVED***
                ...
                ***REMOVED***
            ***REMOVED***
            properties.push({ key, kind: property.kind, property ***REMOVED***);
        ***REMOVED***
...
class Property extends MethodBase {
    ...
    ***REMOVED***
    render(code, options) {
        ...
        ***REMOVED***
        this.value.render(code, options, { isShorthandProperty: this.shorthand ***REMOVED***);
    ***REMOVED***
    applyDeoptimizations() {
        this.deoptimized = true;
        if (this.declarationInit !== null) {
...
class ReturnStatement extends NodeBase {
    ...
    ***REMOVED***
    render(code, options) {
        if (this.argument) {
            this.argument.render(code, options, { preventASI: true ***REMOVED***);
...
class SequenceExpression extends NodeBase {
    ...
    ***REMOVED***
    include(context, includeChildrenRecursively) {
        ...
        ***REMOVED***
    ***REMOVED***
    render(code, options, { renderedParentType, isCalleeOfRenderedParent, preventASI ***REMOVED*** = BLANK) {
        let includedNodes = 0;
        let lastSeparatorPos = null;
        const lastNode = this.expressions[this.expressions.length - 1];
        for (const { node, separator, start, end ***REMOVED*** of getCommaSeparatedNodesWithBoundaries(this.expressions, code, this.start, this.end)) {
            if (!node.included) {
    ...
    ***REMOVED***
    hasEffects(context) {
        if (this.discriminant.hasEffects(context))
            return true;
        const { brokenFlow, ignore: { breaks ***REMOVED*** ***REMOVED*** = context;
...
class TaggedTemplateExpression extends CallExpressionBase {
    bind() {
        ...
        if (this.tag.type === Identifier$1) {
            ...
            if (variable.isNamespace) {
                this.context.warn({
                    code: 'CANNOT_CALL_NAMESPACE',
                    message: `Cannot call a namespace ('${name***REMOVED***')`
                ***REMOVED***, this.start);
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***
    ...
    ***REMOVED***
    render(code, options) {
        this.tag.render(code, options, { isCalleeOfRenderedParent: true ***REMOVED***);
...
class ExportDefaultVariable extends LocalVariable {
    ...
    ***REMOVED***
    getOriginalVariable() {
        ...
        ***REMOVED*** while (original instanceof ExportDefaultVariable && !checkedVariables.has(original));
...
class ModuleScope extends ChildScope {
    ...
    ***REMOVED***
    addExportDefaultDeclaration(name, exportDefaultDeclaration, context) {
        ...
        return variable;
    ***REMOVED***
    addNamespaceMemberAccess() { ***REMOVED***
    deconflict(format, exportNamesByVariable, accessedGlobalsByScope) {
...
class ThisExpression extends NodeBase {
    ...
    ***REMOVED***
    deoptimizeThisOnInteractionAtPath(interaction, path, recursionTracker) {
        ...
        this.variable.deoptimizeThisOnInteractionAtPath(interaction.thisArg === this ? { ...interaction, thisArg: this.variable ***REMOVED*** : interaction, path, recursionTracker);
    ***REMOVED***
    hasEffectsOnInteractionAtPath(path, interaction, context) {
        if (path.length === 0) {
        ...
        if (format === 'system') {
            ...
            if (exportNames) {
                ...
                ***REMOVED***
                    ...
                    renderSystemExportSequenceBeforeExpression(variable, this.start, this.end, this.parent.type !== ExpressionStatement$1, code, options, `${_***REMOVED***${operator***REMOVED***${_***REMOVED***1`);
                ***REMOVED***
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***
...
class VariableDeclaration extends NodeBase {
    ...
    ***REMOVED***
    hasEffectsOnInteractionAtPath() {
        return false;
    ***REMOVED***
    include(context, includeChildrenRecursively, { asSingleStatement ***REMOVED*** = BLANK) {
        this.included = true;
        for (const declarator of this.declarations) {
    ...
    ***REMOVED***
    ...
    renderDeclarationEnd(code, separatorString, lastSeparatorPos, actualContentEnd, renderedContentEnd, systemPatternExports, options) {
        ...
        ***REMOVED***
        if (systemPatternExports.length > 0) {
            code.appendLeft(renderedContentEnd, ` ${getSystemExportStatement(systemPatternExports, options)***REMOVED***;`);
        ***REMOVED***
    ***REMOVED***
    renderReplacedDeclarations(code, options) {
        ...
        for (const { node, start, separator, contentEnd, end ***REMOVED*** of separatedNodes) {
            if (!node.included) {
                code.remove(start, end);
                continue;
            ***REMOVED***
            ...
            ***REMOVED***
                ...
                ***REMOVED***
                    ...
                    if (hasRenderedContent) {
                        separatorString += ';';
                    ***REMOVED***
                    leadingString += `${this.kind***REMOVED*** `;
...
function gatherSystemExportsAndGetSingleExport(separatedNodes, options, aggregatedSystemExports) {
    ...
    if (options.format === 'system') {
        for (const { node ***REMOVED*** of separatedNodes) {
...
class VariableDeclarator extends NodeBase {
    ...
    ***REMOVED***
    render(code, options) {
        const { exportNamesByVariable, snippets: { _ ***REMOVED*** ***REMOVED*** = options;
        ...
        ***REMOVED***
        if (this.init) {
            this.init.render(code, options, renderId ? BLANK : { renderedSurroundingElement: ExpressionStatement$1 ***REMOVED***);
        ***REMOVED***
        else if (this.id instanceof Identifier &&
            isReassignedExportsMember(this.id.variable, exportNamesByVariable)) {
            code.appendLeft(this.end, `${_***REMOVED***=${_***REMOVED***void 0`);
        ***REMOVED***
    ***REMOVED***
    applyDeoptimizations() { ***REMOVED***
***REMOVED***
...
class NamespaceVariable extends Variable {
    ...
    ***REMOVED***
    renderBlock(options) {
        const { exportNamesByVariable, format, freeze, indent: t, namespaceToStringTag, snippets: { _, cnst, getObject, getPropertyAccess, n, s ***REMOVED*** ***REMOVED*** = options;
        const memberVariables = this.getMemberVariables();
        const members = Object.entries(memberVariables).map(([name, original]) => {
            if (this.referencedEarly || original.isReassigned) {
                return [
                    null,
                    `get ${name***REMOVED***${_***REMOVED***()${_***REMOVED***{${_***REMOVED***return ${original.getName(getPropertyAccess)***REMOVED***${s***REMOVED***${_***REMOVED******REMOVED***`
                ];
            ***REMOVED***
            return [name, original.getName(getPropertyAccess)];
        ***REMOVED***);
        members.unshift([null, `__proto__:${_***REMOVED***null`]);
        let output = getObject(members, { lineBreakIndent: { base: '', t ***REMOVED*** ***REMOVED***);
        if (this.mergedNamespaces.length > 0) {
            const assignmentArgs = this.mergedNamespaces.map(variable => variable.getName(getPropertyAccess));
            output = `/*#__PURE__*/${MERGE_NAMESPACES_VARIABLE***REMOVED***(${output***REMOVED***,${_***REMOVED***[${assignmentArgs.join(`,${_***REMOVED***`)***REMOVED***])`;
        ***REMOVED***
        else {
            // The helper to merge namespaces will also take care of freezing and toStringTag
            if (namespaceToStringTag) {
                output = `/*#__PURE__*/Object.defineProperty(${output***REMOVED***,${_***REMOVED***Symbol.toStringTag,${_***REMOVED***${getToStringTagValue(getObject)***REMOVED***)`;
            ***REMOVED***
            if (freeze) {
                output = `/*#__PURE__*/Object.freeze(${output***REMOVED***)`;
            ***REMOVED***
        ***REMOVED***
        const name = this.getName(getPropertyAccess);
        output = `${cnst***REMOVED*** ${name***REMOVED***${_***REMOVED***=${_***REMOVED***${output***REMOVED***;`;
        if (format === 'system' && exportNamesByVariable.has(this)) {
            output += `${n***REMOVED***${getSystemExportStatement([this], options)***REMOVED***;`;
        ***REMOVED***
        return output;
    ***REMOVED***
    renderFirst() {
...
class SyntheticNamedExportVariable extends Variable {
    ...
    ***REMOVED***
    getName(getPropertyAccess) {
        return `${this.syntheticNamespace.getName(getPropertyAccess)***REMOVED***${getPropertyAccess(this.name)***REMOVED***`;
    ***REMOVED***
    include() {
        this.included = true;
...
***REMOVED***)(BuildPhase || (BuildPhase = {***REMOVED***));
...
function getPersistedLabel(label, level) {
    switch (level) {
        ...
        case 3:
            return label;
        default:
            return `${'  '.repeat(level - 4)***REMOVED***- ${label***REMOVED***`;
    ***REMOVED***
***REMOVED***
...
***REMOVED***
function getTimings() {
    const newTimings = {***REMOVED***;
    for (const [label, { memory, time, totalMemory ***REMOVED***] of timers) {
...
function getPluginWithTimers(plugin, index) {
    for (const hook of TIMED_PLUGIN_HOOKS) {
        if (hook in plugin) {
            let timerLabel = `plugin ${index***REMOVED***`;
            if (plugin.name) {
                timerLabel += ` (${plugin.name***REMOVED***)`;
            ***REMOVED***
            timerLabel += ` - ${hook***REMOVED***`;
            ...
            plugin[hook] = function (...args) {
                ...
                if (result && typeof result.then === 'function') {
                    timeStart(`${timerLabel***REMOVED*** (async)`, 4);
                    return result.then((hookResult) => {
                        timeEnd(`${timerLabel***REMOVED*** (async)`, 4);
                        return hookResult;
                    ***REMOVED***);
                ***REMOVED***
                return result;
...
class Module {
    constructor(graph, id, options, isEntry, moduleSideEffects, syntheticNamedExports, meta) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-this-alias
        const module = this;
        const { dynamicImports, dynamicImporters, implicitlyLoadedAfter, implicitlyLoadedBefore, importers, reexportDescriptions, sources ***REMOVED*** = this;
        this.info = {
            ast: null,
            code: null,
            get dynamicallyImportedIdResolutions() {
                return dynamicImports
                    .map(({ argument ***REMOVED***) => typeof argument === 'string' && module.resolvedIds[argument])
                    .filter(Boolean);
            ***REMOVED***,
            get dynamicallyImportedIds() {
                // We cannot use this.dynamicDependencies because this is needed before
                // dynamicDependencies are populated
                return dynamicImports.map(({ id ***REMOVED***) => id).filter((id) => id != null);
            ***REMOVED***,
            get dynamicImporters() {
                return dynamicImporters.sort();
            ***REMOVED***,
            ...
            ***REMOVED***,
            get importedIds() {
                ...
                return Array.from(sources, source => { var _a; return (_a = module.resolvedIds[source]) === null || _a === void 0 ? void 0 : _a.id; ***REMOVED***).filter(Boolean);
            ***REMOVED***,
            get importers() {
                return importers.sort();
            ***REMOVED***,
        ...
        ***REMOVED***
        ...
        for (const module of this.exportAllModules) {
            if (module instanceof ExternalModule) {
                this.allExportNames.add(`*${module.id***REMOVED***`);
                continue;
            ***REMOVED***
    ...
    ***REMOVED***
    getReexports() {
        ...
        ***REMOVED***
        ...
        for (const module of this.exportAllModules) {
            if (module instanceof ExternalModule) {
                reexports.add(`*${module.id***REMOVED***`);
            ***REMOVED***
            else {
    ...
    ***REMOVED***
    getRenderedExports() {
        ...
        ***REMOVED***
        return { removedExports, renderedExports ***REMOVED***;
    ***REMOVED***
    getSyntheticNamespace() {
        if (this.syntheticNamespace === null) {
            ...
            [this.syntheticNamespace] = this.getVariableForExportName(typeof this.info.syntheticNamedExports === 'string'
                ...
                : 'default', { onlyExplicit: true ***REMOVED***);
        ***REMOVED***
    ...
    ***REMOVED***
    getVariableForExportName(name, { importerForSideEffects, isExportAllSearch, onlyExplicit, searchedNamesAndModules ***REMOVED*** = EMPTY_OBJECT) {
        var _a;
        if (name[0] === '*') {
            if (name.length === 1) {
                // export * from './other'
    ...
    ***REMOVED***
    render(options) {
        ...
        return magicString;
    ***REMOVED***
    setSource({ ast, code, customTransformCache, originalCode, originalSourcemap, resolvedIds, sourcemapChain, transformDependencies, transformFiles, ...moduleOptions ***REMOVED***) {
        this.info.code = code;
        ...
        ***REMOVED***;
        ...
        this.ast = new Program(ast, { context: this.astContext, type: 'Module' ***REMOVED***, this.scope);
        this.info.ast = ast;
    ...
    ***REMOVED***
    toJSON() {
        ...
        ***REMOVED***;
    ***REMOVED***
    traceVariable(name, { importerForSideEffects, isExportAllSearch, searchedNamesAndModules ***REMOVED*** = EMPTY_OBJECT) {
    ...
    ***REMOVED***
    tryParse() {
        ...
        ***REMOVED***
    ***REMOVED***
    updateOptions({ meta, moduleSideEffects, syntheticNamedExports ***REMOVED***) {
    ...
    ***REMOVED***
    addDynamicImport(node) {
        ...
        ***REMOVED***
        this.dynamicImports.push({ argument, id: null, node, resolution: null ***REMOVED***);
    ***REMOVED***
    addExport(node) {
        ...
        ***REMOVED***
            ...
            // export { name ***REMOVED*** from './other'
            const source = node.source.value;
            this.sources.add(source);
        ...
        ***REMOVED***
            ...
            if (declaration instanceof VariableDeclaration) {
                // export var { foo, bar ***REMOVED*** = ...
                // export var foo = 1, bar = 2;
                for (const declarator of declaration.declarations) {
                    for (const localName of extractAssignedNames(declarator.id)) {
                        this.exports.set(localName, { identifier: null, localName ***REMOVED***);
                    ***REMOVED***
                ***REMOVED***
            ***REMOVED***
            else {
                // export function foo () {***REMOVED***
                const localName = declaration.id.name;
                this.exports.set(localName, { identifier: null, localName ***REMOVED***);
            ***REMOVED***
        ***REMOVED***
        else {
            // export { foo, bar, baz ***REMOVED***
            for (const specifier of node.specifiers) {
                const localName = specifier.local.name;
                const exportedName = specifier.exported.name;
                this.exports.set(exportedName, { identifier: null, localName ***REMOVED***);
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***
    addImport(node) {
    ...
    ***REMOVED***
    addLocationToLogProps(props, pos) {
        ...
        const location = locate(code, pos, { offsetLine: 1 ***REMOVED***);
        if (location) {
            let { column, line ***REMOVED*** = location;
            try {
                ({ column, line ***REMOVED*** = getOriginalLocation(this.sourcemapChain, { column, line ***REMOVED***));
            ...
            ***REMOVED***
                ...
                this.options.onwarn({
                    ...
                    loc: {
                        ...
                        line
                    ***REMOVED***,
                    message: `Error when using sourcemap for reporting an error: ${err.message***REMOVED***`,
                    pos
                ***REMOVED***);
            ***REMOVED***
            augmentCodeLocation(props, { column, line ***REMOVED***, code, this.id);
        ***REMOVED***
    ***REMOVED***
    addModulesToImportDescriptions(importDescription) {
        for (const specifier of importDescription.values()) {
            const { id ***REMOVED*** = this.resolvedIds[specifier.source];
    ...
    ***REMOVED***
    shimMissingExport(name) {
        this.options.onwarn({
            ...
            exportName: name,
            message: `Missing export "${name***REMOVED***" has been shimmed in module ${relativeId(this.id)***REMOVED***.`
        ***REMOVED***);
...
function getCompleteAmdId(options, chunkId) {
    if (options.autoId) {
        return `${options.basePath ? options.basePath + '/' : ''***REMOVED***${removeJsExtension(chunkId)***REMOVED***`;
    ***REMOVED***
    return options.id || '';
***REMOVED***

function getExportBlock$1(exports, dependencies, namedExportsMode, interop, snippets, t, externalLiveBindings, mechanism = 'return ') {
    const { _, cnst, getDirectReturnFunction, getFunctionIntro, getPropertyAccess, n, s ***REMOVED*** = snippets;
    if (!namedExportsMode) {
        return `${n***REMOVED***${n***REMOVED***${mechanism***REMOVED***${getSingleDefaultExport(exports, dependencies, interop, externalLiveBindings, getPropertyAccess)***REMOVED***;`;
    ***REMOVED***
    let exportBlock = '';
    for (const { defaultVariableName, id, isChunk, name, namedExportsMode: depNamedExportsMode, namespaceVariableName, reexports ***REMOVED*** of dependencies) {
            ...
            for (const specifier of reexports) {
                if (specifier.reexported !== '*') {
                    ...
                    if (specifier.imported !== '*' && specifier.needsLiveBinding) {
                        ...
                        ***REMOVED***);
                        exportBlock +=
                            `Object.defineProperty(exports,${_***REMOVED***'${specifier.reexported***REMOVED***',${_***REMOVED***{${n***REMOVED***` +
                                `${t***REMOVED***enumerable:${_***REMOVED***true,${n***REMOVED***` +
                                `${t***REMOVED***get:${_***REMOVED***${left***REMOVED***${importName***REMOVED***${right***REMOVED***${n***REMOVED******REMOVED***);`;
                    ***REMOVED***
                    else {
                        exportBlock += `exports${getPropertyAccess(specifier.reexported)***REMOVED***${_***REMOVED***=${_***REMOVED***${importName***REMOVED***;`;
                    ***REMOVED***
                ***REMOVED***
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***
    for (const { exported, local ***REMOVED*** of exports) {
        const lhs = `exports${getPropertyAccess(exported)***REMOVED***`;
        const rhs = local;
        if (lhs !== rhs) {
            if (exportBlock)
                exportBlock += n;
            exportBlock += `${lhs***REMOVED***${_***REMOVED***=${_***REMOVED***${rhs***REMOVED***;`;
        ***REMOVED***
    ***REMOVED***
    for (const { name, reexports ***REMOVED*** of dependencies) {
        if (reexports && namedExportsMode) {
            for (const specifier of reexports) {
                if (specifier.reexported === '*') {
                    if (exportBlock)
                        exportBlock += n;
                    const copyPropertyIfNecessary = `{${n***REMOVED***${t***REMOVED***if${_***REMOVED***(k${_***REMOVED***!==${_***REMOVED***'default'${_***REMOVED***&&${_***REMOVED***!exports.hasOwnProperty(k))${_***REMOVED***${getDefineProperty(name, specifier.needsLiveBinding, t, snippets)***REMOVED***${s***REMOVED***${n***REMOVED******REMOVED***`;
                    exportBlock +=
                        cnst === 'var' && specifier.needsLiveBinding
                            ? `Object.keys(${name***REMOVED***).forEach(${getFunctionIntro(['k'], {
                                isAsync: false,
                                name: null
                            ***REMOVED***)***REMOVED***${copyPropertyIfNecessary***REMOVED***);`
                            : `for${_***REMOVED***(${cnst***REMOVED*** k in ${name***REMOVED***)${_***REMOVED***${copyPropertyIfNecessary***REMOVED***`;
                ***REMOVED***
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***
    if (exportBlock) {
        return `${n***REMOVED***${n***REMOVED***${exportBlock***REMOVED***`;
    ***REMOVED***
    return '';
***REMOVED***
    ...
    if (imported === 'default') {
        if (!isChunk) {
            ...
            return isDefaultAProperty(moduleInterop, externalLiveBindings)
                ? `${variableName***REMOVED***${getPropertyAccess('default')***REMOVED***`
                : variableName;
        ***REMOVED***
        return depNamedExportsMode
            ? `${moduleVariableName***REMOVED***${getPropertyAccess('default')***REMOVED***`
            : moduleVariableName;
    ***REMOVED***
    if (imported === '*') {
        return (isChunk
            ...
            : moduleVariableName;
    ***REMOVED***
    return `${moduleVariableName***REMOVED***${getPropertyAccess(imported)***REMOVED***`;
***REMOVED***
function getEsModuleValue(getObject) {
    ...
    ***REMOVED***);
***REMOVED***
function getNamespaceMarkers(hasNamedExports, addEsModule, addNamespaceToStringTag, { _, getObject ***REMOVED***) {
    if (hasNamedExports) {
        if (addEsModule) {
            if (addNamespaceToStringTag) {
                return `Object.defineProperties(exports,${_***REMOVED***${getObject([
                    ['__esModule', getEsModuleValue(getObject)],
                    [null, `[Symbol.toStringTag]:${_***REMOVED***${getToStringTagValue(getObject)***REMOVED***`]
                ], {
                    lineBreakIndent: null
                ***REMOVED***)***REMOVED***);`;
            ***REMOVED***
            return `Object.defineProperty(exports,${_***REMOVED***'__esModule',${_***REMOVED***${getEsModuleValue(getObject)***REMOVED***);`;
        ***REMOVED***
        if (addNamespaceToStringTag) {
            return `Object.defineProperty(exports,${_***REMOVED***Symbol.toStringTag,${_***REMOVED***${getToStringTagValue(getObject)***REMOVED***);`;
        ***REMOVED***
    ***REMOVED***
    return '';
***REMOVED***
const getDefineProperty = (name, needsLiveBinding, t, { _, getDirectReturnFunction, n ***REMOVED***) => {
    if (needsLiveBinding) {
        const [left, right] = getDirectReturnFunction([], {
            functionReturn: true,
            lineBreakIndent: null,
            name: null
        ***REMOVED***);
        return (`Object.defineProperty(exports,${_***REMOVED***k,${_***REMOVED***{${n***REMOVED***` +
            `${t***REMOVED***${t***REMOVED***enumerable:${_***REMOVED***true,${n***REMOVED***` +
            `${t***REMOVED***${t***REMOVED***get:${_***REMOVED***${left***REMOVED***${name***REMOVED***[k]${right***REMOVED***${n***REMOVED***${t***REMOVED******REMOVED***)`);
    ***REMOVED***
    return `exports[k]${_***REMOVED***=${_***REMOVED***${name***REMOVED***[k]`;
***REMOVED***;
...
function getInteropBlock(dependencies, interop, externalLiveBindings, freeze, namespaceToStringTag, accessedGlobals, indent, snippets) {
    ...
    const addInteropStatement = (helperVariableName, helper, dependencyVariableName) => {
        ...
        interopStatements.push(`${cnst***REMOVED*** ${helperVariableName***REMOVED***${_***REMOVED***=${_***REMOVED***/*#__PURE__*/${helper***REMOVED***(${dependencyVariableName***REMOVED***);`);
    ***REMOVED***;
    for (const { defaultVariableName, imports, id, isChunk, name, namedExportsMode, namespaceVariableName, reexports ***REMOVED*** of dependencies) {
        if (isChunk) {
            for (const { imported, reexported ***REMOVED*** of [
                ...(imports || []),
                ...(reexports || [])
            ]) {
        ...
        ***REMOVED***
    ***REMOVED***
    return `${getHelpersBlock(neededInteropHelpers, accessedGlobals, indent, snippets, externalLiveBindings, freeze, namespaceToStringTag)***REMOVED***${interopStatements.length > 0 ? `${interopStatements.join(n)***REMOVED***${n***REMOVED***${n***REMOVED***` : ''***REMOVED***`;
***REMOVED***
...
function warnOnBuiltins(warn, dependencies) {
    const externalBuiltins = dependencies.map(({ id ***REMOVED***) => id).filter(id => id in builtins);
    if (!externalBuiltins.length)
        return;
    warn({
        code: 'MISSING_NODE_BUILTINS',
        message: `Creating a browser bundle that depends on Node.js built-in modules (${printQuotedStringList(externalBuiltins)***REMOVED***). You might need to include https://github.com/FredKSchott/rollup-plugin-polyfill-node`,
        modules: externalBuiltins
    ***REMOVED***);
***REMOVED***

function amd(magicString, { accessedGlobals, dependencies, exports, hasExports, id, indent: t, intro, isEntryFacade, isModuleFacade, namedExportsMode, outro, snippets, warn ***REMOVED***, { amd, esModule, externalLiveBindings, freeze, interop, namespaceToStringTag, strict ***REMOVED***) {
    warnOnBuiltins(warn, dependencies);
    const deps = dependencies.map(m => `'${updateExtensionForRelativeAmdId(m.id, amd.forceJsExtensionForImports)***REMOVED***'`);
    const args = dependencies.map(m => m.name);
    const { n, getNonArrowFunctionIntro, _ ***REMOVED*** = snippets;
    ...
    ***REMOVED***
    ...
    const params = (completeAmdId ? `'${completeAmdId***REMOVED***',${_***REMOVED***` : ``) +
        (deps.length ? `[${deps.join(`,${_***REMOVED***`)***REMOVED***],${_***REMOVED***` : ``);
    const useStrict = strict ? `${_***REMOVED***'use strict';` : '';
    magicString.prepend(`${intro***REMOVED***${getInteropBlock(dependencies, interop, externalLiveBindings, freeze, namespaceToStringTag, accessedGlobals, t, snippets)***REMOVED***`);
    ...
    ***REMOVED***
    magicString.append(`${exportBlock***REMOVED***${namespaceMarkers***REMOVED***${outro***REMOVED***`);
    return (magicString
        .indent(t)
        // factory function should be wrapped by parentheses to avoid lazy parsing,
        // cf. https://v8.dev/blog/preparser#pife
        .prepend(`${amd.define***REMOVED***(${params***REMOVED***(${getNonArrowFunctionIntro(args, {
        isAsync: false,
        name: null
    ***REMOVED***)***REMOVED***{${useStrict***REMOVED***${n***REMOVED***${n***REMOVED***`)
        .append(`${n***REMOVED***${n***REMOVED******REMOVED***));`));
***REMOVED***

function cjs(magicString, { accessedGlobals, dependencies, exports, hasExports, indent: t, intro, isEntryFacade, isModuleFacade, namedExportsMode, outro, snippets ***REMOVED***, { compact, esModule, externalLiveBindings, freeze, interop, namespaceToStringTag, strict ***REMOVED***) {
    const { _, n ***REMOVED*** = snippets;
    const useStrict = strict ? `'use strict';${n***REMOVED***${n***REMOVED***` : '';
    let namespaceMarkers = getNamespaceMarkers(namedExportsMode && hasExports, isEntryFacade && esModule, isModuleFacade && namespaceToStringTag, snippets);
    if (namespaceMarkers) {
    ...
    ***REMOVED***
    ...
    magicString.prepend(`${useStrict***REMOVED***${intro***REMOVED***${namespaceMarkers***REMOVED***${importBlock***REMOVED***${interopBlock***REMOVED***`);
    const exportBlock = getExportBlock$1(exports, dependencies, namedExportsMode, interop, snippets, t, externalLiveBindings, `module.exports${_***REMOVED***=${_***REMOVED***`);
    return magicString.append(`${exportBlock***REMOVED***${outro***REMOVED***`);
***REMOVED***
function getImportBlock$1(dependencies, { _, cnst, n ***REMOVED***, compact) {
    let importBlock = '';
    let definingVariable = false;
    for (const { id, name, reexports, imports ***REMOVED*** of dependencies) {
        if (!reexports && !imports) {
            if (importBlock) {
                importBlock += compact && !definingVariable ? ',' : `;${n***REMOVED***`;
            ***REMOVED***
            definingVariable = false;
            importBlock += `require('${id***REMOVED***')`;
        ***REMOVED***
        else {
            importBlock += compact && definingVariable ? ',' : `${importBlock ? `;${n***REMOVED***` : ''***REMOVED***${cnst***REMOVED*** `;
            definingVariable = true;
            importBlock += `${name***REMOVED***${_***REMOVED***=${_***REMOVED***require('${id***REMOVED***')`;
        ***REMOVED***
    ***REMOVED***
    if (importBlock) {
        return `${importBlock***REMOVED***;${n***REMOVED***${n***REMOVED***`;
    ***REMOVED***
    return '';
***REMOVED***

function es(magicString, { accessedGlobals, indent: t, intro, outro, dependencies, exports, snippets ***REMOVED***, { externalLiveBindings, freeze, namespaceToStringTag ***REMOVED***) {
    const { _, n ***REMOVED*** = snippets;
...
function getImportBlock(dependencies, _) {
    const importBlock = [];
    for (const { id, reexports, imports, name ***REMOVED*** of dependencies) {
        if (!reexports && !imports) {
            importBlock.push(`import${_***REMOVED***'${id***REMOVED***';`);
            continue;
        ***REMOVED***
        if (imports) {
            ...
            ***REMOVED***
            if (starImport) {
                importBlock.push(`import${_***REMOVED****${_***REMOVED***as ${starImport.local***REMOVED*** from${_***REMOVED***'${id***REMOVED***';`);
            ***REMOVED***
            if (defaultImport && importedNames.length === 0) {
                importBlock.push(`import ${defaultImport.local***REMOVED*** from${_***REMOVED***'${id***REMOVED***';`);
            ***REMOVED***
            else if (importedNames.length > 0) {
                importBlock.push(`import ${defaultImport ? `${defaultImport.local***REMOVED***,${_***REMOVED***` : ''***REMOVED***{${_***REMOVED***${importedNames
                    .map(specifier => {
                    if (specifier.imported === specifier.local) {
                        return specifier.imported;
                    ***REMOVED***
                    else {
                        return `${specifier.imported***REMOVED*** as ${specifier.local***REMOVED***`;
                    ***REMOVED***
                ***REMOVED***)
                    .join(`,${_***REMOVED***`)***REMOVED***${_***REMOVED******REMOVED***${_***REMOVED***from${_***REMOVED***'${id***REMOVED***';`);
            ***REMOVED***
        ***REMOVED***
        if (reexports) {
            let starExport = null;
            ...
            ***REMOVED***
            if (starExport) {
                importBlock.push(`export${_***REMOVED****${_***REMOVED***from${_***REMOVED***'${id***REMOVED***';`);
            ***REMOVED***
            if (namespaceReexports.length > 0) {
                if (!imports ||
                    !imports.some(specifier => specifier.imported === '*' && specifier.local === name)) {
                    importBlock.push(`import${_***REMOVED****${_***REMOVED***as ${name***REMOVED*** from${_***REMOVED***'${id***REMOVED***';`);
                ***REMOVED***
                for (const specifier of namespaceReexports) {
                    importBlock.push(`export${_***REMOVED***{${_***REMOVED***${name === specifier.reexported ? name : `${name***REMOVED*** as ${specifier.reexported***REMOVED***`***REMOVED*** ***REMOVED***;`);
                ***REMOVED***
            ***REMOVED***
            if (namedReexports.length > 0) {
                importBlock.push(`export${_***REMOVED***{${_***REMOVED***${namedReexports
                    .map(specifier => {
                    if (specifier.imported === specifier.reexported) {
                        return specifier.imported;
                    ***REMOVED***
                    else {
                        return `${specifier.imported***REMOVED*** as ${specifier.reexported***REMOVED***`;
                    ***REMOVED***
                ***REMOVED***)
                    .join(`,${_***REMOVED***`)***REMOVED***${_***REMOVED******REMOVED***${_***REMOVED***from${_***REMOVED***'${id***REMOVED***';`);
            ***REMOVED***
    ...
    ***REMOVED***
    return importBlock;
***REMOVED***
function getExportBlock(exports, { _, cnst ***REMOVED***) {
    const exportBlock = [];
    const exportDeclaration = [];
    for (const specifier of exports) {
        if (specifier.expression) {
            exportBlock.push(`${cnst***REMOVED*** ${specifier.local***REMOVED***${_***REMOVED***=${_***REMOVED***${specifier.expression***REMOVED***;`);
        ***REMOVED***
        exportDeclaration.push(specifier.exported === specifier.local
            ? specifier.local
            : `${specifier.local***REMOVED*** as ${specifier.exported***REMOVED***`);
    ***REMOVED***
    if (exportDeclaration.length) {
        exportBlock.push(`export${_***REMOVED***{${_***REMOVED***${exportDeclaration.join(`,${_***REMOVED***`)***REMOVED***${_***REMOVED******REMOVED***;`);
    ***REMOVED***
    return exportBlock;
***REMOVED***
...
function setupNamespace(name, root, globals, { _, getPropertyAccess, s ***REMOVED***, compact) {
    ...
    return (parts
        ...
        return `${propertyPath***REMOVED***${_***REMOVED***=${_***REMOVED***${propertyPath***REMOVED***${_***REMOVED***||${_***REMOVED***{***REMOVED***${s***REMOVED***`;
    ***REMOVED***)
        .join(compact ? ',' : '\n') + (compact && parts.length ? ';' : '\n'));
***REMOVED***
function assignToDeepVariable(deepName, root, globals, assignment, { _, getPropertyAccess ***REMOVED***) {
    ...
    let deepAssignment = parts
        .map(part => {
        propertyPath += getPropertyAccess(part);
        return `${propertyPath***REMOVED***${_***REMOVED***=${_***REMOVED***${propertyPath***REMOVED***${_***REMOVED***||${_***REMOVED***{***REMOVED***`;
    ***REMOVED***)
        .concat(`${propertyPath***REMOVED***${getPropertyAccess(last)***REMOVED***`)
        .join(`,${_***REMOVED***`) + `${_***REMOVED***=${_***REMOVED***${assignment***REMOVED***`;
    if (parts.length > 0) {
        deepAssignment = `(${deepAssignment***REMOVED***)`;
    ***REMOVED***
...
function trimEmptyImports(dependencies) {
    ...
    while (i--) {
        const { imports, reexports ***REMOVED*** = dependencies[i];
...
function iife(magicString, { accessedGlobals, dependencies, exports, hasExports, indent: t, intro, namedExportsMode, outro, snippets, warn ***REMOVED***, { compact, esModule, extend, freeze, externalLiveBindings, globals, interop, name, namespaceToStringTag, strict ***REMOVED***) {
    const { _, getNonArrowFunctionIntro, getPropertyAccess, n ***REMOVED*** = snippets;
    const isNamespaced = name && name.includes('.');
    const useVariableAssignment = !extend && !isNamespaced;
    if (name && useVariableAssignment && !isLegal(name)) {
        return error({
            code: 'ILLEGAL_IDENTIFIER_AS_NAME',
            message: `Given name "${name***REMOVED***" is not a legal JS identifier. If you need this, you can try "output.extend: true".`
        ***REMOVED***);
    ***REMOVED***
    warnOnBuiltins(warn, dependencies);
    ...
    ***REMOVED***
    if (namedExportsMode && hasExports) {
        if (extend) {
            deps.unshift(`this${keypath(name, getPropertyAccess)***REMOVED***${_***REMOVED***=${_***REMOVED***this${keypath(name, getPropertyAccess)***REMOVED***${_***REMOVED***||${_***REMOVED***{***REMOVED***`);
            args.unshift('exports');
        ***REMOVED***
        else {
            deps.unshift('{***REMOVED***');
            args.unshift('exports');
        ***REMOVED***
    ***REMOVED***
    const useStrict = strict ? `${t***REMOVED***'use strict';${n***REMOVED***` : '';
    const interopBlock = getInteropBlock(dependencies, interop, externalLiveBindings, freeze, namespaceToStringTag, accessedGlobals, t, snippets);
    magicString.prepend(`${intro***REMOVED***${interopBlock***REMOVED***`);
    let wrapperIntro = `(${getNonArrowFunctionIntro(args, {
        isAsync: false,
        name: null
    ***REMOVED***)***REMOVED***{${n***REMOVED***${useStrict***REMOVED***${n***REMOVED***`;
    if (hasExports) {
        if (name && !(extend && namedExportsMode)) {
            wrapperIntro =
                (useVariableAssignment ? `var ${name***REMOVED***` : `this${keypath(name, getPropertyAccess)***REMOVED***`) +
                    `${_***REMOVED***=${_***REMOVED***${wrapperIntro***REMOVED***`;
        ***REMOVED***
        if (isNamespaced) {
            wrapperIntro = setupNamespace(name, 'this', globals, snippets, compact) + wrapperIntro;
        ***REMOVED***
    ***REMOVED***
    let wrapperOutro = `${n***REMOVED***${n***REMOVED******REMOVED***)(${deps.join(`,${_***REMOVED***`)***REMOVED***);`;
    if (hasExports && !extend && namedExportsMode) {
        wrapperOutro = `${n***REMOVED***${n***REMOVED***${t***REMOVED***return exports;${wrapperOutro***REMOVED***`;
    ***REMOVED***
    const exportBlock = getExportBlock$1(exports, dependencies, namedExportsMode, interop, snippets, t, externalLiveBindings);
...
function system(magicString, { accessedGlobals, dependencies, exports, hasExports, indent: t, intro, snippets, outro, usesTopLevelAwait ***REMOVED***, { externalLiveBindings, freeze, name, namespaceToStringTag, strict, systemNullSetters ***REMOVED***) {
    const { _, getFunctionIntro, getNonArrowFunctionIntro, n, s ***REMOVED*** = snippets;
    const { importBindings, setters, starExcludes ***REMOVED*** = analyzeDependencies(dependencies, exports, t, snippets);
    const registeredName = name ? `'${name***REMOVED***',${_***REMOVED***` : '';
    ...
    // factory function should be wrapped by parentheses to avoid lazy parsing,
    // cf. https://v8.dev/blog/preparser#pife
    let wrapperStart = `System.register(${registeredName***REMOVED***[` +
        dependencies.map(({ id ***REMOVED***) => `'${id***REMOVED***'`).join(`,${_***REMOVED***`) +
        `],${_***REMOVED***(${getNonArrowFunctionIntro(wrapperParams, { isAsync: false, name: null ***REMOVED***)***REMOVED***{${n***REMOVED***${t***REMOVED***${strict ? "'use strict';" : ''***REMOVED***` +
        getStarExcludesBlock(starExcludes, t, snippets) +
        getImportBindingsBlock(importBindings, t, snippets) +
        `${n***REMOVED***${t***REMOVED***return${_***REMOVED***{${setters.length
            ? `${n***REMOVED***${t***REMOVED***${t***REMOVED***setters:${_***REMOVED***[${setters
                .map(setter => setter
                ? `${getFunctionIntro(['module'], {
                    isAsync: false,
                    name: null
                ***REMOVED***)***REMOVED***{${n***REMOVED***${t***REMOVED***${t***REMOVED***${t***REMOVED***${setter***REMOVED***${n***REMOVED***${t***REMOVED***${t***REMOVED******REMOVED***`
                : systemNullSetters
                    ? `null`
                    : `${getFunctionIntro([], { isAsync: false, name: null ***REMOVED***)***REMOVED***{***REMOVED***`)
                .join(`,${_***REMOVED***`)***REMOVED***],`
            : ''***REMOVED***${n***REMOVED***`;
    wrapperStart += `${t***REMOVED***${t***REMOVED***execute:${_***REMOVED***(${getNonArrowFunctionIntro([], {
        isAsync: usesTopLevelAwait,
        name: null
    ***REMOVED***)***REMOVED***{${n***REMOVED***${n***REMOVED***`;
    const wrapperEnd = `${t***REMOVED***${t***REMOVED******REMOVED***)${n***REMOVED***${t***REMOVED******REMOVED***${s***REMOVED***${n***REMOVED******REMOVED***));`;
    magicString.prepend(intro +
    ...
    magicString.append(`${outro***REMOVED***${n***REMOVED***${n***REMOVED***` +
        getSyntheticExportsBlock(exports, t, snippets) +
        getMissingExportsBlock(exports, t, snippets));
    return magicString.indent(`${t***REMOVED***${t***REMOVED***${t***REMOVED***`).append(wrapperEnd).prepend(wrapperStart);
***REMOVED***
function analyzeDependencies(dependencies, exports, t, { _, cnst, getObject, getPropertyAccess, n ***REMOVED***) {
    const importBindings = [];
    const setters = [];
    let starExcludes = null;
    for (const { imports, reexports ***REMOVED*** of dependencies) {
        const setter = [];
        if (imports) {
            for (const specifier of imports) {
                importBindings.push(specifier.local);
                if (specifier.imported === '*') {
                    setter.push(`${specifier.local***REMOVED***${_***REMOVED***=${_***REMOVED***module;`);
                ***REMOVED***
                else {
                    setter.push(`${specifier.local***REMOVED***${_***REMOVED***=${_***REMOVED***module${getPropertyAccess(specifier.imported)***REMOVED***;`);
                ***REMOVED***
            ***REMOVED***
        ***REMOVED***
        if (reexports) {
            ...
            for (const { imported, reexported ***REMOVED*** of reexports) {
                if (reexported === '*') {
                ...
                ***REMOVED***
                    ...
                    reexportedNames.push([
                        reexported,
                        imported === '*' ? 'module' : `module${getPropertyAccess(imported)***REMOVED***`
                    ]);
                ***REMOVED***
            ***REMOVED***
            if (reexportedNames.length > 1 || hasStarReexport) {
                const exportMapping = getObject(reexportedNames, { lineBreakIndent: null ***REMOVED***);
                if (hasStarReexport) {
                    if (!starExcludes) {
                        starExcludes = getStarExcludes({ dependencies, exports ***REMOVED***);
                    ***REMOVED***
                    setter.push(`${cnst***REMOVED*** setter${_***REMOVED***=${_***REMOVED***${exportMapping***REMOVED***;`, `for${_***REMOVED***(${cnst***REMOVED*** name in module)${_***REMOVED***{`, `${t***REMOVED***if${_***REMOVED***(!_starExcludes[name])${_***REMOVED***setter[name]${_***REMOVED***=${_***REMOVED***module[name];`, '***REMOVED***', 'exports(setter);');
                ***REMOVED***
                else {
                    setter.push(`exports(${exportMapping***REMOVED***);`);
                ***REMOVED***
            ***REMOVED***
            else {
                const [key, value] = reexportedNames[0];
                setter.push(`exports('${key***REMOVED***',${_***REMOVED***${value***REMOVED***);`);
            ***REMOVED***
        ***REMOVED***
        setters.push(setter.join(`${n***REMOVED***${t***REMOVED***${t***REMOVED***${t***REMOVED***`));
    ***REMOVED***
    return { importBindings, setters, starExcludes ***REMOVED***;
***REMOVED***
const getStarExcludes = ({ dependencies, exports ***REMOVED***) => {
    const starExcludes = new Set(exports.map(expt => expt.exported));
    starExcludes.add('default');
    for (const { reexports ***REMOVED*** of dependencies) {
        if (reexports) {
    ...
    ***REMOVED***
    return starExcludes;
***REMOVED***;
const getStarExcludesBlock = (starExcludes, t, { _, cnst, getObject, n ***REMOVED***) => starExcludes
    ? `${n***REMOVED***${t***REMOVED***${cnst***REMOVED*** _starExcludes${_***REMOVED***=${_***REMOVED***${getObject([...starExcludes].map(prop => [prop, '1']), { lineBreakIndent: { base: t, t ***REMOVED*** ***REMOVED***)***REMOVED***;`
    : '';
const getImportBindingsBlock = (importBindings, t, { _, n ***REMOVED***) => (importBindings.length ? `${n***REMOVED***${t***REMOVED***var ${importBindings.join(`,${_***REMOVED***`)***REMOVED***;` : '');
const getHoistedExportsBlock = (exports, t, snippets) => getExportsBlock(exports.filter(expt => expt.hoisted).map(expt => ({ name: expt.exported, value: expt.local ***REMOVED***)), t, snippets);
function getExportsBlock(exports, t, { _, n ***REMOVED***) {
    if (exports.length === 0) {
        return '';
    ***REMOVED***
    if (exports.length === 1) {
        return `exports('${exports[0].name***REMOVED***',${_***REMOVED***${exports[0].value***REMOVED***);${n***REMOVED***${n***REMOVED***`;
    ***REMOVED***
    return (`exports({${n***REMOVED***` +
        exports.map(({ name, value ***REMOVED***) => `${t***REMOVED***${name***REMOVED***:${_***REMOVED***${value***REMOVED***`).join(`,${n***REMOVED***`) +
        `${n***REMOVED******REMOVED***);${n***REMOVED***${n***REMOVED***`);
***REMOVED***
const getSyntheticExportsBlock = (exports, t, snippets) => getExportsBlock(exports
    .filter(expt => expt.expression)
    .map(expt => ({ name: expt.exported, value: expt.local ***REMOVED***)), t, snippets);
const getMissingExportsBlock = (exports, t, snippets) => getExportsBlock(exports
    .filter(expt => expt.local === MISSING_EXPORT_SHIM_VARIABLE)
    .map(expt => ({ name: expt.exported, value: MISSING_EXPORT_SHIM_VARIABLE ***REMOVED***)), t, snippets);

function globalProp(name, globalVar, getPropertyAccess) {
    if (!name)
        return 'null';
    return `${globalVar***REMOVED***${keypath(name, getPropertyAccess)***REMOVED***`;
***REMOVED***
function safeAccess(name, globalVar, { _, getPropertyAccess ***REMOVED***) {
    let propertyPath = globalVar;
    return name
        .split('.')
        .map(part => (propertyPath += getPropertyAccess(part)))
        .join(`${_***REMOVED***&&${_***REMOVED***`);
***REMOVED***
function umd(magicString, { accessedGlobals, dependencies, exports, hasExports, id, indent: t, intro, namedExportsMode, outro, snippets, warn ***REMOVED***, { amd, compact, esModule, extend, externalLiveBindings, freeze, interop, name, namespaceToStringTag, globals, noConflict, strict ***REMOVED***) {
    const { _, cnst, getFunctionIntro, getNonArrowFunctionIntro, getPropertyAccess, n, s ***REMOVED*** = snippets;
    ...
    ***REMOVED***
    warnOnBuiltins(warn, dependencies);
    const amdDeps = dependencies.map(m => `'${updateExtensionForRelativeAmdId(m.id, amd.forceJsExtensionForImports)***REMOVED***'`);
    const cjsDeps = dependencies.map(m => `require('${m.id***REMOVED***')`);
    ...
    if (namedExportsMode && (hasExports || noConflict)) {
        ...
        globalDeps.unshift(assignToDeepVariable(name, globalVar, globals, `${extend ? `${globalProp(name, globalVar, getPropertyAccess)***REMOVED***${_***REMOVED***||${_***REMOVED***` : ''***REMOVED***{***REMOVED***`, snippets));
        factoryParams.unshift('exports');
    ***REMOVED***
    const completeAmdId = getCompleteAmdId(amd, id);
    const amdParams = (completeAmdId ? `'${completeAmdId***REMOVED***',${_***REMOVED***` : ``) +
        (amdDeps.length ? `[${amdDeps.join(`,${_***REMOVED***`)***REMOVED***],${_***REMOVED***` : ``);
    const define = amd.define;
    const cjsExport = !namedExportsMode && hasExports ? `module.exports${_***REMOVED***=${_***REMOVED***` : ``;
    const useStrict = strict ? `${_***REMOVED***'use strict';${n***REMOVED***` : ``;
    let iifeExport;
    if (noConflict) {
        const noConflictExportsVar = compact ? 'e' : 'exports';
        let factory;
        if (!namedExportsMode && hasExports) {
            factory = `${cnst***REMOVED*** ${noConflictExportsVar***REMOVED***${_***REMOVED***=${_***REMOVED***${assignToDeepVariable(name, globalVar, globals, `${factoryVar***REMOVED***(${globalDeps.join(`,${_***REMOVED***`)***REMOVED***)`, snippets)***REMOVED***;`;
        ***REMOVED***
        else {
            const module = globalDeps.shift();
            factory =
                `${cnst***REMOVED*** ${noConflictExportsVar***REMOVED***${_***REMOVED***=${_***REMOVED***${module***REMOVED***;${n***REMOVED***` +
                    `${t***REMOVED***${t***REMOVED***${factoryVar***REMOVED***(${[noConflictExportsVar].concat(globalDeps).join(`,${_***REMOVED***`)***REMOVED***);`;
        ***REMOVED***
        iifeExport =
            `(${getFunctionIntro([], { isAsync: false, name: null ***REMOVED***)***REMOVED***{${n***REMOVED***` +
                `${t***REMOVED***${t***REMOVED***${cnst***REMOVED*** current${_***REMOVED***=${_***REMOVED***${safeAccess(name, globalVar, snippets)***REMOVED***;${n***REMOVED***` +
                `${t***REMOVED***${t***REMOVED***${factory***REMOVED***${n***REMOVED***` +
                `${t***REMOVED***${t***REMOVED***${noConflictExportsVar***REMOVED***.noConflict${_***REMOVED***=${_***REMOVED***${getFunctionIntro([], {
                    isAsync: false,
                    name: null
                ***REMOVED***)***REMOVED***{${_***REMOVED***` +
                `${globalProp(name, globalVar, getPropertyAccess)***REMOVED***${_***REMOVED***=${_***REMOVED***current;${_***REMOVED***return ${noConflictExportsVar***REMOVED***${s***REMOVED***${_***REMOVED******REMOVED***;${n***REMOVED***` +
                `${t***REMOVED******REMOVED***)()`;
    ***REMOVED***
    else {
        iifeExport = `${factoryVar***REMOVED***(${globalDeps.join(`,${_***REMOVED***`)***REMOVED***)`;
        if (!namedExportsMode && hasExports) {
    ...
    ***REMOVED***
    const globalArg = iifeNeedsGlobal ? `this,${_***REMOVED***` : '';
    const iifeStart = iifeNeedsGlobal
        ? `(${globalVar***REMOVED***${_***REMOVED***=${_***REMOVED***typeof globalThis${_***REMOVED***!==${_***REMOVED***'undefined'${_***REMOVED***?${_***REMOVED***globalThis${_***REMOVED***:${_***REMOVED***${globalVar***REMOVED***${_***REMOVED***||${_***REMOVED***self,${_***REMOVED***`
        : '';
    const iifeEnd = iifeNeedsGlobal ? ')' : '';
    const cjsIntro = iifeNeedsGlobal
        ? `${t***REMOVED***typeof exports${_***REMOVED***===${_***REMOVED***'object'${_***REMOVED***&&${_***REMOVED***typeof module${_***REMOVED***!==${_***REMOVED***'undefined'${_***REMOVED***?` +
            `${_***REMOVED***${cjsExport***REMOVED***${factoryVar***REMOVED***(${cjsDeps.join(`,${_***REMOVED***`)***REMOVED***)${_***REMOVED***:${n***REMOVED***`
        : '';
    const wrapperIntro = `(${getNonArrowFunctionIntro(wrapperParams, { isAsync: false, name: null ***REMOVED***)***REMOVED***{${n***REMOVED***` +
        cjsIntro +
        `${t***REMOVED***typeof ${define***REMOVED***${_***REMOVED***===${_***REMOVED***'function'${_***REMOVED***&&${_***REMOVED***${define***REMOVED***.amd${_***REMOVED***?${_***REMOVED***${define***REMOVED***(${amdParams***REMOVED***${factoryVar***REMOVED***)${_***REMOVED***:${n***REMOVED***` +
        `${t***REMOVED***${iifeStart***REMOVED***${iifeExport***REMOVED***${iifeEnd***REMOVED***;${n***REMOVED***` +
        // factory function should be wrapped by parentheses to avoid lazy parsing,
        // cf. https://v8.dev/blog/preparser#pife
        `***REMOVED***)(${globalArg***REMOVED***(${getNonArrowFunctionIntro(factoryParams, {
            isAsync: false,
            name: null
        ***REMOVED***)***REMOVED***{${useStrict***REMOVED***${n***REMOVED***`;
    const wrapperOutro = n + n + '***REMOVED***));';
    magicString.prepend(`${intro***REMOVED***${getInteropBlock(dependencies, interop, externalLiveBindings, freeze, namespaceToStringTag, accessedGlobals, t, snippets)***REMOVED***`);
    const exportBlock = getExportBlock$1(exports, dependencies, namedExportsMode, interop, snippets, t, externalLiveBindings);
    let namespaceMarkers = getNamespaceMarkers(namedExportsMode && hasExports, esModule, namespaceToStringTag, snippets);
    if (namespaceMarkers) {
...
const finalisers = { amd, cjs, es, iife, system, umd ***REMOVED***;
...
class Source {
    ...
    ***REMOVED***
    traceSegment(line, column, name) {
        return { column, line, name, source: this ***REMOVED***;
    ***REMOVED***
***REMOVED***
class Link {
    ...
    traceMappings() {
        ...
        for (const line of this.mappings) {
            ...
            for (const segment of line) {
                ...
                if (traced) {
                    const { column, line, name, source: { content, filename ***REMOVED*** ***REMOVED*** = traced;
                    ...
                    ***REMOVED***
                        ...
                        return error({
                            message: `Multiple conflicting contents for sourcemap source ${filename***REMOVED***`
                        ***REMOVED***);
                    ***REMOVED***
        ...
        ***REMOVED***
        return { mappings, names, sources, sourcesContent ***REMOVED***;
    ***REMOVED***
    traceSegment(line, column, name) {
    ...
    return function linkMap(source, map) {
        ...
        ***REMOVED***
        warn({
            ...
            message: `Sourcemap is likely to be incorrect: a plugin (${map.plugin***REMOVED***) was used to transform ` +
...
function collapseSourcemaps(file, map, modules, bundleSourcemapChain, excludeContent, warn) {
    ...
    let { sources, sourcesContent, names, mappings ***REMOVED*** = source.traceMappings();
    if (file) {
    ...
    ***REMOVED***
    ...
    return new SourceMap({ file, mappings, names, sources, sourcesContent ***REMOVED***);
***REMOVED***
...
function deconflictImportsEsmOrSystem(usedNames, imports, dependenciesToBeDeconflicted, _interop, preserveModules, _externalLiveBindings, chunkByModule, syntheticExports) {
    ...
    ***REMOVED***
***REMOVED***
function deconflictImportsOther(usedNames, imports, { deconflictedDefault, deconflictedNamespace, dependencies ***REMOVED***, interop, preserveModules, externalLiveBindings, chunkByModule) {
    ...
    ***REMOVED***
    for (const externalModuleOrChunk of deconflictedNamespace) {
        externalModuleOrChunk.namespaceVariableName = getSafeName(`${externalModuleOrChunk.suggestedVariableName***REMOVED***__namespace`, usedNames);
    ***REMOVED***
    for (const externalModule of deconflictedDefault) {
        ...
        ***REMOVED***
            ...
            externalModule.defaultVariableName = getSafeName(`${externalModule.suggestedVariableName***REMOVED***__default`, usedNames);
        ***REMOVED***
    ***REMOVED***
    for (const variable of imports) {
        const module = variable.module;
        ...
        if (exportsByName.has(exportName)) {
            do {
                ...
                if (exportName.charCodeAt(0) === 49 /* '1' */) {
                    ...
                    exportName = toBase64(nameIndex);
                ***REMOVED***
            ***REMOVED*** while (RESERVED_NAMES$1.has(exportName) || exportsByName.has(exportName));
        ***REMOVED***
...
function getExportMode(chunk, { exports: exportMode, name, format ***REMOVED***, unsetOptions, facadeModuleId, warn) {
...
function decodedSourcemap(map) {
    ...
    ***REMOVED***
    ...
    return { ...map, mappings ***REMOVED***;
***REMOVED***

function renderChunk({ code, options, outputPluginDriver, renderChunk, sourcemapChain ***REMOVED***) {
    const renderChunkReducer = (code, result, plugin) => {
        ...
        if (result.map !== null) {
            ...
            sourcemapChain.push(map || { missing: true, plugin: plugin.name ***REMOVED***);
        ***REMOVED***
        return result.code;
    ***REMOVED***;
...
function renderNamePattern(pattern, patternName, replacements) {
    if (isPathFragment(pattern))
        return error(errFailedValidation(`Invalid pattern "${pattern***REMOVED***" for "${patternName***REMOVED***", patterns can be neither absolute nor relative paths. If you want your files to be stored in a subdirectory, write its name without a leading slash like this: subdirectory/pattern.`));
    return pattern.replace(/\[(\w+)\]/g, (_match, type) => {
        if (!replacements.hasOwnProperty(type)) {
            return error(errFailedValidation(`"[${type***REMOVED***]" is not a valid placeholder in "${patternName***REMOVED***" pattern.`));
        ***REMOVED***
        const replacement = replacements[type]();
        if (isPathFragment(replacement))
            return error(errFailedValidation(`Invalid substitution "${replacement***REMOVED***" for placeholder "[${type***REMOVED***]" in "${patternName***REMOVED***" pattern, can be neither absolute nor relative path.`));
        return replacement;
    ***REMOVED***);
***REMOVED***
function makeUnique(name, { [lowercaseBundleKeys]: reservedLowercaseBundleKeys ***REMOVED***) {
    ...
    ***REMOVED***
    if (hasExports) {
        warn({
            ...
            message: `No name was provided for external module '${module.id***REMOVED***' in output.globals – guessing '${module.variableName***REMOVED***'`,
            source: module.id
        ***REMOVED***);
...
class Chunk {
            ...
            if (!moduleExportNamesByVariable.has(exposedVariable)) {
                if (moduleExportNamesByVariable.size === 0 &&
                    ...
                    this.inputOptions.onwarn({
                        code: 'EMPTY_FACADE',
                        id: module.id,
                        message: `To preserve the export signature of the entry module "${relativeId(module.id)***REMOVED***", an empty facade chunk was created. This often happens when creating a bundle for a web app where chunks are placed in script tags and exports are ignored. In this case it is recommended to set "preserveEntrySignatures: false" to avoid this and reduce the number of chunks. Otherwise if this is intentional, set "preserveEntrySignatures: 'strict'" explicitly to silence this warning.`,
    ...
    ***REMOVED***
    generateFacades() {
        ...
        const exposedVariables = new Set(this.dynamicEntryModules.map(({ namespace ***REMOVED***) => namespace));
        ...
        ***REMOVED***
        for (const module of entryModules) {
            const requiredFacades = Array.from(new Set(module.chunkNames.filter(({ isUserDefined ***REMOVED***) => isUserDefined).map(({ name ***REMOVED***) => name)), 
            // mapping must run after Set 'name' dedupe
            name => ({
                name
            ***REMOVED***));
            if (requiredFacades.length === 0 && module.isUserDefinedEntryPoint) {
                requiredFacades.push({***REMOVED***);
            ***REMOVED***
            requiredFacades.push(...Array.from(module.chunkFileNames, fileName => ({ fileName ***REMOVED***)));
    ...
    ***REMOVED***
    generateIdPreserveModules(preserveModulesRelativeDir, options, bundle, unsetOptions) {
        const [{ id ***REMOVED***] = this.orderedModules;
        ...
        if (isAbsolute(sanitizedId)) {
            ...
            const fileName = renderNamePattern(pattern, 'output.entryFileNames', {
                ...
                name: () => this.getChunkName()
            ***REMOVED***);
            const currentPath = `${currentDir***REMOVED***/${fileName***REMOVED***`;
            const { preserveModulesRoot ***REMOVED*** = options;
        ...
        ***REMOVED***
            ...
            ***REMOVED***);
            path = `_virtual/${fileName***REMOVED***`;
        ***REMOVED***
    ...
    getRenderedHash() {
        ...
        ***REMOVED***);
        ...
        hash.update(this.getExportNames()
            ...
            return `${relativeId(variable.module.id).replace(/\\/g, '/')***REMOVED***:${variable.name***REMOVED***:${exportName***REMOVED***`;
        ***REMOVED***)
            .join(','));
    ...
    ***REMOVED***
    ...
    preRender(options, inputBase, snippets) {
        const { _, getPropertyAccess, n ***REMOVED*** = snippets;
        const magicString = new Bundle$1({ separator: `${n***REMOVED***${n***REMOVED***` ***REMOVED***);
        this.usedModules = [];
        ...
        ***REMOVED***
        ...
        for (const module of this.orderedModules) {
            ...
            if (module.isIncluded() || this.includedNamespaces.has(module)) {
                ...
                ***REMOVED***
            ***REMOVED***
            const { renderedExports, removedExports ***REMOVED*** = module.getRenderedExports();
            const { renderedModuleSources ***REMOVED*** = this;
        ...
        ***REMOVED***
        ...
        if (this.needsExportsShim) {
            magicString.prepend(`${n***REMOVED***${snippets.cnst***REMOVED*** ${MISSING_EXPORT_SHIM_VARIABLE***REMOVED***${_***REMOVED***=${_***REMOVED***void 0;${n***REMOVED***${n***REMOVED***`);
        ***REMOVED***
        if (options.compact) {
        ...
        ***REMOVED***
        ...
        if (this.isEmpty && this.getExportNames().length === 0 && this.dependencies.size === 0) {
            ...
            this.inputOptions.onwarn({
                chunkName,
                code: 'EMPTY_BUNDLE',
                message: `Generated an empty chunk: "${chunkName***REMOVED***"`
            ***REMOVED***);
        ***REMOVED***
    ...
    async render(options, addons, outputChunk, snippets) {
        ...
        ***REMOVED***
        if (topLevelAwaitModule !== null && format !== 'es' && format !== 'system') {
            return error({
                ...
                message: `Module format ${format***REMOVED*** does not support top-level await. Use the "es" or "system" output formats rather.`
            ***REMOVED***);
        ***REMOVED***
        /* istanbul ignore next */
        if (!this.id) {
        ...
        ***REMOVED***);
        if (options.sourcemap) {
            ...
            const decodedMap = magicString.generateDecodedMap({***REMOVED***);
            ...
            map.sources = map.sources
                .map(sourcePath => {
                const { sourcemapPathTransform ***REMOVED*** = options;
                if (sourcemapPathTransform) {
                    const newSourcePath = sourcemapPathTransform(sourcePath, `${file***REMOVED***.map`);
    ...
    ***REMOVED***
    addNecessaryImportsForFacades() {
        ...
        ***REMOVED***
    ***REMOVED***
    assignFacadeName({ fileName, name ***REMOVED***, facadedModule) {
        if (fileName) {
            this.fileName = fileName;
        ***REMOVED***
        else {
    ...
    checkCircularDependencyImport(variable, importingModule) {
        ...
        if (variableModule instanceof Module) {
            ...
            do {
                ...
                ***REMOVED***
            ***REMOVED*** while (alternativeReexportModule);
        ***REMOVED***
    ***REMOVED***
    computeContentHashWithDependencies(addons, options, bundle) {
        ...
        for (const current of dependenciesForHashing) {
            if (current instanceof ExternalModule) {
                hash.update(`:${current.renderPath***REMOVED***`);
            ***REMOVED***
            else {
    ...
    ***REMOVED***
    finaliseDynamicImports(options, snippets) {
        ...
        for (const [module, code] of this.renderedModuleSources) {
            for (const { node, resolution ***REMOVED*** of module.dynamicImports) {
    ...
    getDependenciesToBeDeconflicted(addNonNamespacesAndInteropHelpers, addDependenciesWithoutBindings, interop) {
        ...
        ***REMOVED***
        if (addDependenciesWithoutBindings) {
            ...
            ***REMOVED***
        ***REMOVED***
        return { deconflictedDefault, deconflictedNamespace, dependencies ***REMOVED***;
    ***REMOVED***
    ...
    ***REMOVED***
    getImportSpecifiers(getPropertyAccess) {
        const { interop ***REMOVED*** = this.outputOptions;
    ...
    ***REMOVED***
    getImportedBindingsPerDependency() {
        const importSpecifiers = {***REMOVED***;
        for (const [dependency, declaration] of this.renderedDependencies) {
            const specifiers = new Set();
            if (declaration.imports) {
                for (const { imported ***REMOVED*** of declaration.imports) {
                    specifiers.add(imported);
                ***REMOVED***
            ***REMOVED***
            if (declaration.reexports) {
                for (const { imported ***REMOVED*** of declaration.reexports) {
                    specifiers.add(imported);
                ***REMOVED***
            ***REMOVED***
    ...
    ***REMOVED***
    getReexportSpecifiers() {
        const { externalLiveBindings, interop ***REMOVED*** = this.outputOptions;
    ...
    ***REMOVED***
    setExternalRenderPaths(options, inputBase) {
        ...
        ***REMOVED***
    ***REMOVED***
    setIdentifierRenderResolutions({ format, interop, namespaceToStringTag ***REMOVED***) {
...
function getChunkNameFromModule(module) {
    var _a, _b, _c, _d;
    return ((_d = (_b = (_a = module.chunkNames.find(({ isUserDefined ***REMOVED***) => isUserDefined)) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !== void 0 ? _b : (_c = module.chunkNames[0]) === null || _c === void 0 ? void 0 : _c.name) !== null && _d !== void 0 ? _d : getAliasName(module.id));
***REMOVED***
const QUERY_HASH_REGEX = /[?#]/;

const concatSep = (out, next) => (next ? `${out***REMOVED***\n${next***REMOVED***` : out);
const concatDblSep = (out, next) => (next ? `${out***REMOVED***\n\n${next***REMOVED***` : out);
async function createAddons(options, outputPluginDriver) {
    try {
        ...
        if (footer.length)
            footer = '\n' + footer;
        return { banner, footer, intro, outro ***REMOVED***;
    ***REMOVED***
    catch (err) {
        return error({
            code: 'ADDON_ERROR',
            message: `Could not retrieve ${err.hook***REMOVED***. Check configuration of plugin ${err.plugin***REMOVED***.
\tError Message: ${err.message***REMOVED***`
        ***REMOVED***);
    ***REMOVED***
***REMOVED***
...
function getChunkAssignments(entryModules, manualChunkAliasByEntry) {
    ...
    ***REMOVED***
    for (const [alias, modules] of Object.entries(manualChunkModulesByAlias)) {
        chunkDefinitions.push({ alias, modules ***REMOVED***);
    ***REMOVED***
    const assignedEntryPointsByModule = new Map();
    const { dependentEntryPointsByModule, dynamicEntryModules ***REMOVED*** = analyzeModuleGraph(entryModules);
    ...
    for (const currentEntry of entriesToHandle) {
        ...
        for (const module of modulesToHandle) {
            ...
            for (const dependency of module.getDependenciesToBeIncluded()) {
                ...
                ***REMOVED***
            ***REMOVED***
            for (const { resolution ***REMOVED*** of module.dynamicImports) {
        ...
        ***REMOVED***
    ***REMOVED***
    return { dependentEntryPointsByModule, dynamicEntryModules ***REMOVED***;
***REMOVED***
...
function analyseModuleExecution(entryModules) {
    ...
    ***REMOVED***
    for (const curEntry of dynamicImports) {
        ...
        ***REMOVED***
    ***REMOVED***
    return { cyclePaths, orderedModules ***REMOVED***;
***REMOVED***
...
function getGenerateCodeSnippets({ compact, generatedCode: { arrowFunctions, constBindings, objectShorthand, reservedNamesAsProps ***REMOVED*** ***REMOVED***) {
    const { _, n, s ***REMOVED*** = compact ? { _: '', n: '', s: '' ***REMOVED*** : { _: ' ', n: '\n', s: ';' ***REMOVED***;
    const cnst = constBindings ? 'const' : 'var';
    const getNonArrowFunctionIntro = (params, { isAsync, name ***REMOVED***) => `${isAsync ? `async ` : ''***REMOVED***function${name ? ` ${name***REMOVED***` : ''***REMOVED***${_***REMOVED***(${params.join(`,${_***REMOVED***`)***REMOVED***)${_***REMOVED***`;
    const getFunctionIntro = arrowFunctions
        ? (params, { isAsync, name ***REMOVED***) => {
            const singleParam = params.length === 1;
            const asyncString = isAsync ? `async${singleParam ? ' ' : _***REMOVED***` : '';
            return `${name ? `${cnst***REMOVED*** ${name***REMOVED***${_***REMOVED***=${_***REMOVED***` : ''***REMOVED***${asyncString***REMOVED***${singleParam ? params[0] : `(${params.join(`,${_***REMOVED***`)***REMOVED***)`***REMOVED***${_***REMOVED***=>${_***REMOVED***`;
        ***REMOVED***
        : getNonArrowFunctionIntro;
    const getDirectReturnFunction = (params, { functionReturn, lineBreakIndent, name ***REMOVED***) => [
        `${getFunctionIntro(params, {
            isAsync: false,
            name
        ***REMOVED***)***REMOVED***${arrowFunctions
            ? lineBreakIndent
                ? `${n***REMOVED***${lineBreakIndent.base***REMOVED***${lineBreakIndent.t***REMOVED***`
                : ''
            : `{${lineBreakIndent ? `${n***REMOVED***${lineBreakIndent.base***REMOVED***${lineBreakIndent.t***REMOVED***` : _***REMOVED***${functionReturn ? 'return ' : ''***REMOVED***`***REMOVED***`,
        arrowFunctions
            ? `${name ? ';' : ''***REMOVED***${lineBreakIndent ? `${n***REMOVED***${lineBreakIndent.base***REMOVED***` : ''***REMOVED***`
            : `${s***REMOVED***${lineBreakIndent ? `${n***REMOVED***${lineBreakIndent.base***REMOVED***` : _***REMOVED******REMOVED***`
    ];
    ...
    return {
        ...
        getDirectReturnIifeLeft: (params, returned, { needsArrowReturnParens, needsWrappedFunction ***REMOVED***) => {
            const [left, right] = getDirectReturnFunction(params, {
                functionReturn: true,
                lineBreakIndent: null,
                name: null
            ***REMOVED***);
            return `${wrapIfNeeded(`${left***REMOVED***${wrapIfNeeded(returned, arrowFunctions && needsArrowReturnParens)***REMOVED***${right***REMOVED***`, arrowFunctions || needsWrappedFunction)***REMOVED***(`;
        ***REMOVED***,
        getFunctionIntro,
        getNonArrowFunctionIntro,
        getObject(fields, { lineBreakIndent ***REMOVED***) {
            const prefix = lineBreakIndent ? `${n***REMOVED***${lineBreakIndent.base***REMOVED***${lineBreakIndent.t***REMOVED***` : _;
            return `{${fields
                .map(([key, value]) => {
                if (key === null)
                    return `${prefix***REMOVED***${value***REMOVED***`;
                const needsQuotes = !isValidPropName(key);
                return key === value && objectShorthand && !needsQuotes
                    ? prefix + key
                    : `${prefix***REMOVED***${needsQuotes ? `'${key***REMOVED***'` : key***REMOVED***:${_***REMOVED***${value***REMOVED***`;
            ***REMOVED***)
                .join(`,`)***REMOVED***${fields.length === 0 ? '' : lineBreakIndent ? `${n***REMOVED***${lineBreakIndent.base***REMOVED***` : _***REMOVED******REMOVED***`;
        ***REMOVED***,
        getPropertyAccess: (name) => isValidPropName(name) ? `.${name***REMOVED***` : `[${JSON.stringify(name)***REMOVED***]`,
        n,
        s
    ***REMOVED***;
***REMOVED***
const wrapIfNeeded = (code, needsParens) => needsParens ? `(${code***REMOVED***)` : code;
...
class Bundle {
    ...
    ***REMOVED***
    async addManualChunks(manualChunks) {
        ...
        ***REMOVED***)));
        for (const { alias, entries ***REMOVED*** of chunkEntries) {
    ...
    ***REMOVED***
    async generateChunks() {
        const { manualChunks ***REMOVED*** = this.outputOptions;
        const manualChunkAliasByEntry = typeof manualChunks === 'object'
            ? await this.addManualChunks(manualChunks)
            : this.assignManualChunks(manualChunks);
        const chunks = [];
        const chunkByModule = new Map();
        for (const { alias, modules ***REMOVED*** of this.outputOptions.inlineDynamicImports
            ? [{ alias: null, modules: getIncludedModules(this.graph.modulesById) ***REMOVED***]
            : this.outputOptions.preserveModules
...
function isInAstralSet(code, set) {
  ...
  for (var i = 0; i < set.length; i += 2) {
    pos += set[i];
    if (pos > code) { return false ***REMOVED***
    pos += set[i + 1];
    if (pos >= code) { return true ***REMOVED***
  ***REMOVED***
***REMOVED***
...
function isIdentifierStart(code, astral) {
  if (code < 65) { return code === 36 ***REMOVED***
  if (code < 91) { return true ***REMOVED***
  if (code < 97) { return code === 95 ***REMOVED***
  if (code < 123) { return true ***REMOVED***
  if (code <= 0xffff) { return code >= 0xaa && nonASCIIidentifierStart.test(String.fromCharCode(code)) ***REMOVED***
  if (astral === false) { return false ***REMOVED***
...
function isIdentifierChar(code, astral) {
  if (code < 48) { return code === 36 ***REMOVED***
  if (code < 58) { return true ***REMOVED***
  if (code < 65) { return false ***REMOVED***
  if (code < 91) { return true ***REMOVED***
  if (code < 97) { return code === 95 ***REMOVED***
  if (code < 123) { return true ***REMOVED***
  if (code <= 0xffff) { return code >= 0xaa && nonASCIIidentifier.test(String.fromCharCode(code)) ***REMOVED***
  if (astral === false) { return false ***REMOVED***
...
function binop(name, prec) {
  return new TokenType(name, {beforeExpr: true, binop: prec***REMOVED***)
***REMOVED***
var beforeExpr = {beforeExpr: true***REMOVED***, startsExpr = {startsExpr: true***REMOVED***;
...
var types$1 = {
  ...
  bracketL: new TokenType("[", {beforeExpr: true, startsExpr: true***REMOVED***),
  bracketR: new TokenType("]"),
  braceL: new TokenType("{", {beforeExpr: true, startsExpr: true***REMOVED***),
  braceR: new TokenType("***REMOVED***"),
  parenL: new TokenType("(", {beforeExpr: true, startsExpr: true***REMOVED***),
  ...
  dollarBraceL: new TokenType("${", {beforeExpr: true, startsExpr: true***REMOVED***),
  ...
  eq: new TokenType("=", {beforeExpr: true, isAssign: true***REMOVED***),
  assign: new TokenType("_=", {beforeExpr: true, isAssign: true***REMOVED***),
  incDec: new TokenType("++/--", {prefix: true, postfix: true, startsExpr: true***REMOVED***),
  prefix: new TokenType("!/~", {beforeExpr: true, prefix: true, startsExpr: true***REMOVED***),
  logicalOR: binop("||", 1),
  logicalAND: binop("&&", 2),
  bitwiseOR: binop("|", 3),
  bitwiseXOR: binop("^", 4),
  ...
  relational: binop("</>/<=/>=", 7),
  bitShift: binop("<</>>/>>>", 8),
  plusMin: new TokenType("+/-", {beforeExpr: true, binop: 9, prefix: true, startsExpr: true***REMOVED***),
  modulo: binop("%", 10),
  star: binop("*", 10),
  slash: binop("/", 10),
  starstar: new TokenType("**", {beforeExpr: true***REMOVED***),
  coalesce: binop("??", 1),
  ...
  _do: kw("do", {isLoop: true, beforeExpr: true***REMOVED***),
  _else: kw("else", beforeExpr),
  _finally: kw("finally"),
  _for: kw("for", {isLoop: true***REMOVED***),
  ...
  _var: kw("var"),
  _const: kw("const"),
  _while: kw("while", {isLoop: true***REMOVED***),
  _with: kw("with"),
  _new: kw("new", {beforeExpr: true, startsExpr: true***REMOVED***),
  ...
  _in: kw("in", {beforeExpr: true, binop: 7***REMOVED***),
  _instanceof: kw("instanceof", {beforeExpr: true, binop: 7***REMOVED***),
  _typeof: kw("typeof", {beforeExpr: true, prefix: true, startsExpr: true***REMOVED***),
  _void: kw("void", {beforeExpr: true, prefix: true, startsExpr: true***REMOVED***),
  _delete: kw("delete", {beforeExpr: true, prefix: true, startsExpr: true***REMOVED***)
***REMOVED***;
...
function nextLineBreak(code, from, end) {
  ...
  for (var i = from; i < end; i++) {
    ...
    if (isNewLine(next))
      { return i < end - 1 && next === 13 && code.charCodeAt(i + 1) === 10 ? i + 2 : i + 1 ***REMOVED***
  ***REMOVED***
  return -1
***REMOVED***
...
function codePointToString(code) {
  // UTF-16 Decoding
  if (code <= 0xFFFF) { return String.fromCharCode(code) ***REMOVED***
  code -= 0x10000;
...
var SourceLocation = function SourceLocation(p, start, end) {
  this.start = start;
  this.end = end;
  if (p.sourceFile !== null) { this.source = p.sourceFile; ***REMOVED***
***REMOVED***;
...
function getLineInfo(input, offset) {
  for (var line = 1, cur = 0;;) {
    ...
    if (nextBreak < 0) { return new Position(line, offset - cur) ***REMOVED***
    ++line;
    cur = nextBreak;
  ***REMOVED***
***REMOVED***
...
var defaultOptions = {
  ...
  // enabled, it is given the location as a `{line, column***REMOVED***` object
  // as second argument.
  ...
  // `start` and `end` properties in `{line, column***REMOVED***` form (with
  ...
  // passed, the full `{line, column***REMOVED***` locations of the start and
...
function getOptions(opts) {
  ...
  for (var opt in defaultOptions)
    { options[opt] = opts && hasOwn(opts, opt) ? opts[opt] : defaultOptions[opt]; ***REMOVED***

  if (options.ecmaVersion === "latest") {
    options.ecmaVersion = 1e8;
  ***REMOVED*** else if (options.ecmaVersion == null) {
    if (!warnedAboutEcmaVersion && typeof console === "object" && console.warn) {
      warnedAboutEcmaVersion = true;
      console.warn("Since Acorn 8.0.0, options.ecmaVersion is required.\nDefaulting to 2020, but this will stop working in the future.");
    ***REMOVED***
    options.ecmaVersion = 11;
  ***REMOVED*** else if (options.ecmaVersion >= 2015) {
    options.ecmaVersion -= 2009;
  ***REMOVED***

  if (options.allowReserved == null)
    { options.allowReserved = options.ecmaVersion < 5; ***REMOVED***

  if (isArray(options.onToken)) {
    var tokens = options.onToken;
    options.onToken = function (token) { return tokens.push(token); ***REMOVED***;
  ***REMOVED***
  if (isArray(options.onComment))
    { options.onComment = pushComment(options, options.onComment); ***REMOVED***
  ...
  return function(block, text, start, end, startLoc, endLoc) {
    ...
    ***REMOVED***;
    if (options.locations)
      { comment.loc = new SourceLocation(this, startLoc, endLoc); ***REMOVED***
    if (options.ranges)
      { comment.range = [start, end]; ***REMOVED***
    array.push(comment);
  ***REMOVED***
***REMOVED***
...
var Parser = function Parser(options, input, startPos) {
  ...
  if (options.allowReserved !== true) {
    ...
    if (options.sourceType === "module") { reserved += " await"; ***REMOVED***
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  // And, if locations are used, the {line, column***REMOVED*** object
  ...
  if (this.pos === 0 && options.allowHashBang && this.input.slice(0, 2) === "#!")
    { this.skipLineComment(2); ***REMOVED***
...
var prototypeAccessors = { inFunction: { configurable: true ***REMOVED***,inGenerator: { configurable: true ***REMOVED***,inAsync: { configurable: true ***REMOVED***,canAwait: { configurable: true ***REMOVED***,allowSuper: { configurable: true ***REMOVED***,allowDirectSuper: { configurable: true ***REMOVED***,treatFunctionsAsVar: { configurable: true ***REMOVED***,allowNewDotTarget: { configurable: true ***REMOVED***,inClassStaticBlock: { configurable: true ***REMOVED*** ***REMOVED***;
...
prototypeAccessors.inFunction.get = function () { return (this.currentVarScope().flags & SCOPE_FUNCTION) > 0 ***REMOVED***;

prototypeAccessors.inGenerator.get = function () { return (this.currentVarScope().flags & SCOPE_GENERATOR) > 0 && !this.currentVarScope().inClassFieldInit ***REMOVED***;

prototypeAccessors.inAsync.get = function () { return (this.currentVarScope().flags & SCOPE_ASYNC) > 0 && !this.currentVarScope().inClassFieldInit ***REMOVED***;

prototypeAccessors.canAwait.get = function () {
  for (var i = this.scopeStack.length - 1; i >= 0; i--) {
    var scope = this.scopeStack[i];
    if (scope.inClassFieldInit || scope.flags & SCOPE_CLASS_STATIC_BLOCK) { return false ***REMOVED***
    if (scope.flags & SCOPE_FUNCTION) { return (scope.flags & SCOPE_ASYNC) > 0 ***REMOVED***
  ***REMOVED***
...
prototypeAccessors.allowDirectSuper.get = function () { return (this.currentThisScope().flags & SCOPE_DIRECT_SUPER) > 0 ***REMOVED***;

prototypeAccessors.treatFunctionsAsVar.get = function () { return this.treatFunctionsAsVarInScope(this.currentScope()) ***REMOVED***;
...
Parser.extend = function extend () {
  ...
  var cls = this;
  for (var i = 0; i < plugins.length; i++) { cls = plugins[i](cls); ***REMOVED***
  return cls
***REMOVED***;
...
pp$9.strictDirective = function(start) {
  if (this.options.ecmaVersion < 5) { return false ***REMOVED***
  for (;;) {
    // Try to find string literal.
    skipWhiteSpace.lastIndex = start;
    start += skipWhiteSpace.exec(this.input)[0].length;
    var match = literal.exec(this.input.slice(start));
    if (!match) { return false ***REMOVED***
...
pp$9.eatContextual = function(name) {
  if (!this.isContextual(name)) { return false ***REMOVED***
  this.next();
  return true
***REMOVED***;
...
pp$9.expectContextual = function(name) {
  if (!this.eatContextual(name)) { this.unexpected(); ***REMOVED***
***REMOVED***;
...
pp$9.insertSemicolon = function() {
  if (this.canInsertSemicolon()) {
    if (this.options.onInsertedSemicolon)
      { this.options.onInsertedSemicolon(this.lastTokEnd, this.lastTokEndLoc); ***REMOVED***
    return true
  ***REMOVED***
***REMOVED***;
...
pp$9.semicolon = function() {
  if (!this.eat(types$1.semi) && !this.insertSemicolon()) { this.unexpected(); ***REMOVED***
***REMOVED***;

pp$9.afterTrailingComma = function(tokType, notNext) {
  if (this.type === tokType) {
    if (this.options.onTrailingComma)
      { this.options.onTrailingComma(this.lastTokStart, this.lastTokStartLoc); ***REMOVED***
    if (!notNext)
      { this.next(); ***REMOVED***
    return true
  ***REMOVED***
...
pp$9.checkPatternErrors = function(refDestructuringErrors, isAssign) {
  if (!refDestructuringErrors) { return ***REMOVED***
  if (refDestructuringErrors.trailingComma > -1)
    { this.raiseRecoverable(refDestructuringErrors.trailingComma, "Comma is not permitted after the rest element"); ***REMOVED***
  var parens = isAssign ? refDestructuringErrors.parenthesizedAssign : refDestructuringErrors.parenthesizedBind;
  if (parens > -1) { this.raiseRecoverable(parens, "Parenthesized pattern"); ***REMOVED***
***REMOVED***;

pp$9.checkExpressionErrors = function(refDestructuringErrors, andThrow) {
  if (!refDestructuringErrors) { return false ***REMOVED***
  var shorthandAssign = refDestructuringErrors.shorthandAssign;
  var doubleProto = refDestructuringErrors.doubleProto;
  if (!andThrow) { return shorthandAssign >= 0 || doubleProto >= 0 ***REMOVED***
  if (shorthandAssign >= 0)
    { this.raise(shorthandAssign, "Shorthand property assignments are valid only in destructuring patterns"); ***REMOVED***
  if (doubleProto >= 0)
    { this.raiseRecoverable(doubleProto, "Redefinition of __proto__ property"); ***REMOVED***
***REMOVED***;

pp$9.checkYieldAwaitInDefaultParams = function() {
  if (this.yieldPos && (!this.awaitPos || this.yieldPos < this.awaitPos))
    { this.raise(this.yieldPos, "Yield expression cannot be a default value"); ***REMOVED***
  if (this.awaitPos)
    { this.raise(this.awaitPos, "Await expression cannot be a default value"); ***REMOVED***
***REMOVED***;

pp$9.isSimpleAssignTarget = function(expr) {
  if (expr.type === "ParenthesizedExpression")
    { return this.isSimpleAssignTarget(expr.expression) ***REMOVED***
...
pp$8.parseTopLevel = function(node) {
  ...
  if (!node.body) { node.body = []; ***REMOVED***
...
var loopLabel = {kind: "loop"***REMOVED***, switchLabel = {kind: "switch"***REMOVED***;

pp$8.isLet = function(context) {
  if (this.options.ecmaVersion < 6 || !this.isContextual("let")) { return false ***REMOVED***
  ...
  if (nextCh === 91 || nextCh === 92 || nextCh > 0xd7ff && nextCh < 0xdc00) { return true ***REMOVED*** // '[', '/', astral
  if (context) { return false ***REMOVED***

  if (nextCh === 123) { return true ***REMOVED*** // '{'
  if (isIdentifierStart(nextCh, true)) {
    var pos = next + 1;
    while (isIdentifierChar(nextCh = this.input.charCodeAt(pos), true)) { ++pos; ***REMOVED***
    if (nextCh === 92 || nextCh > 0xd7ff && nextCh < 0xdc00) { return true ***REMOVED***
    var ident = this.input.slice(next, pos);
    if (!keywordRelationalOperator.test(ident)) { return true ***REMOVED***
  ***REMOVED***
  return false
***REMOVED***;
...
pp$8.parseStatement = function(context, topLevel, exports) {
  ...
  ***REMOVED***
  ...
  case types$1._function:
    ...
    if ((context && (this.strict || context !== "if" && context !== "label")) && this.options.ecmaVersion >= 6) { this.unexpected(); ***REMOVED***
    return this.parseFunctionStatement(node, false, !context)
  case types$1._class:
    if (context) { this.unexpected(); ***REMOVED***
  ...
  case types$1._const: case types$1._var:
    ...
    if (context && kind !== "var") { this.unexpected(); ***REMOVED***
  ...
  case types$1._import:
    if (this.options.ecmaVersion > 10 && starttype === types$1._import) {
      ...
      if (nextCh === 40 || nextCh === 46) // '(' or '.'
        { return this.parseExpressionStatement(node, this.parseExpression()) ***REMOVED***
    ***REMOVED***

    if (!this.options.allowImportExportEverywhere) {
      if (!topLevel)
        { this.raise(this.start, "'import' and 'export' may only appear at the top level"); ***REMOVED***
      if (!this.inModule)
        { this.raise(this.start, "'import' and 'export' may appear only with 'sourceType: module'"); ***REMOVED***
    ***REMOVED***
  ...
  default:
    ...
    ***REMOVED***
    ...
    if (starttype === types$1.name && expr.type === "Identifier" && this.eat(types$1.colon))
      { return this.parseLabeledStatement(node, maybeName, expr, context) ***REMOVED***
    else { return this.parseExpressionStatement(node, expr) ***REMOVED***
  ***REMOVED***
***REMOVED***;
...
pp$8.parseBreakContinueStatement = function(node, keyword) {
  ...
  this.next();
  if (this.eat(types$1.semi) || this.insertSemicolon()) { node.label = null; ***REMOVED***
  else if (this.type !== types$1.name) { this.unexpected(); ***REMOVED***
  else {
  ...
  ***REMOVED***
  ...
  for (; i < this.labels.length; ++i) {
    ...
    if (node.label == null || lab.name === node.label.name) {
      if (lab.kind != null && (isBreak || lab.kind === "loop")) { break ***REMOVED***
      if (node.label && isBreak) { break ***REMOVED***
    ***REMOVED***
  ***REMOVED***
  if (i === this.labels.length) { this.raise(node.start, "Unsyntactic " + keyword); ***REMOVED***
...
pp$8.parseDoStatement = function(node) {
  ...
  if (this.options.ecmaVersion >= 6)
    { this.eat(types$1.semi); ***REMOVED***
  else
    { this.semicolon(); ***REMOVED***
...
pp$8.parseForStatement = function(node) {
  ...
  if (this.type === types$1.semi) {
    if (awaitAt > -1) { this.unexpected(awaitAt); ***REMOVED***
    ...
    if ((this.type === types$1._in || (this.options.ecmaVersion >= 6 && this.isContextual("of"))) && init$1.declarations.length === 1) {
      if (this.options.ecmaVersion >= 9) {
        if (this.type === types$1._in) {
          if (awaitAt > -1) { this.unexpected(awaitAt); ***REMOVED***
        ***REMOVED*** else { node.await = awaitAt > -1; ***REMOVED***
      ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (this.type === types$1._in || (isForOf = this.options.ecmaVersion >= 6 && this.isContextual("of"))) {
    if (this.options.ecmaVersion >= 9) {
      ...
      ***REMOVED*** else { node.await = awaitAt > -1; ***REMOVED***
    ***REMOVED***
    if (startsWithLet && isForOf) { this.raise(init.start, "The left-hand side of a for-of loop may not start with 'let'."); ***REMOVED***
...
pp$8.parseReturnStatement = function(node) {
  if (!this.inFunction && !this.options.allowReturnOutsideFunction)
    { this.raise(this.start, "'return' outside of function"); ***REMOVED***
  this.next();
  ...
  if (this.eat(types$1.semi) || this.insertSemicolon()) { node.argument = null; ***REMOVED***
  else { node.argument = this.parseExpression(); this.semicolon(); ***REMOVED***
...
pp$8.parseSwitchStatement = function(node) {
  ...
  for (var sawDefault = false; this.type !== types$1.braceR;) {
    if (this.type === types$1._case || this.type === types$1._default) {
      ...
      if (cur) { this.finishNode(cur, "SwitchCase"); ***REMOVED***
      ...
      ***REMOVED*** else {
        if (sawDefault) { this.raiseRecoverable(this.lastTokStart, "Multiple default clauses"); ***REMOVED***
        sawDefault = true;
        cur.test = null;
      ***REMOVED***
      this.expect(types$1.colon);
    ***REMOVED*** else {
      if (!cur) { this.unexpected(); ***REMOVED***
...
pp$8.parseThrowStatement = function(node) {
  ...
  if (lineBreak.test(this.input.slice(this.lastTokEnd, this.start)))
    { this.raise(this.lastTokEnd, "Illegal newline after throw"); ***REMOVED***
...
pp$8.parseTryStatement = function(node) {
  ...
  if (this.type === types$1._catch) {
    ...
    ***REMOVED*** else {
      if (this.options.ecmaVersion < 10) { this.unexpected(); ***REMOVED***
      clause.param = null;
      this.enterScope(0);
    ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (!node.handler && !node.finalizer)
    { this.raise(node.start, "Missing catch or finally clause"); ***REMOVED***
...
pp$8.parseWithStatement = function(node) {
  if (this.strict) { this.raise(this.start, "'with' in strict mode"); ***REMOVED***
  this.next();
  ...
  ***REMOVED*** ***REMOVED***
  ...
  for (var i = this.labels.length - 1; i >= 0; i--) {
    ...
    if (label$1.statementStart === node.start) {
      ...
      label$1.kind = kind;
    ***REMOVED*** else { break ***REMOVED***
  ***REMOVED***
  this.labels.push({name: maybeName, kind: kind, statementStart: this.start***REMOVED***);
...
pp$8.parseBlock = function(createNewLexicalScope, node, exitStrict) {
  ...
  node.body = [];
  this.expect(types$1.braceL);
  if (createNewLexicalScope) { this.enterScope(0); ***REMOVED***
  while (this.type !== types$1.braceR) {
    var stmt = this.parseStatement(null);
    node.body.push(stmt);
  ***REMOVED***
  if (exitStrict) { this.strict = false; ***REMOVED***
  this.next();
  if (createNewLexicalScope) { this.exitScope(); ***REMOVED***
...
pp$8.parseVar = function(node, isFor, kind) {
  ...
  for (;;) {
    ...
    ***REMOVED*** else if (kind === "const" && !(this.type === types$1._in || (this.options.ecmaVersion >= 6 && this.isContextual("of")))) {
      this.unexpected();
    ***REMOVED*** else if (decl.id.type !== "Identifier" && !(isFor && (this.type === types$1._in || this.isContextual("of")))) {
      this.raise(this.lastTokEnd, "Complex binding patterns require an initialization value");
    ***REMOVED*** else {
      decl.init = null;
    ***REMOVED***
    node.declarations.push(this.finishNode(decl, "VariableDeclarator"));
    if (!this.eat(types$1.comma)) { break ***REMOVED***
  ***REMOVED***
  return node
***REMOVED***;
...
pp$8.parseFunction = function(node, statement, allowExpressionBody, isAsync, forInit) {
  ...
  if (this.options.ecmaVersion >= 9 || this.options.ecmaVersion >= 6 && !isAsync) {
    if (this.type === types$1.star && (statement & FUNC_HANGING_STATEMENT))
      { this.unexpected(); ***REMOVED***
    node.generator = this.eat(types$1.star);
  ***REMOVED***
  if (this.options.ecmaVersion >= 8)
    { node.async = !!isAsync; ***REMOVED***
  ...
  if (statement & FUNC_STATEMENT) {
    ...
    if (node.id && !(statement & FUNC_HANGING_STATEMENT))
      ...
      // treatFunctionsAsVar).
      { this.checkLValSimple(node.id, (this.strict || node.generator || node.async) ? this.treatFunctionsAsVar ? BIND_VAR : BIND_LEXICAL : BIND_FUNCTION); ***REMOVED***
  ***REMOVED***
  ...
  if (!(statement & FUNC_STATEMENT))
    { node.id = this.type === types$1.name ? this.parseIdent() : null; ***REMOVED***
  ...
  while (this.type !== types$1.braceR) {
    ...
    if (element) {
      ...
      if (element.type === "MethodDefinition" && element.kind === "constructor") {
        if (hadConstructor) { this.raise(element.start, "Duplicate constructor in the same class"); ***REMOVED***
        hadConstructor = true;
      ***REMOVED*** else if (element.key && element.key.type === "PrivateIdentifier" && isPrivateNameConflicted(privateNameMap, element)) {
...
pp$8.parseClassElement = function(constructorAllowsSuper) {
  if (this.eat(types$1.semi)) { return null ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (ecmaVersion < 13 || this.type === types$1.parenL || kind !== "method" || isGenerator || isAsync) {
    ...
    if (isConstructor && kind !== "method") { this.raise(node.key.start, "Constructor can't have get/set modifier"); ***REMOVED***
...
pp$8.parseClassMethod = function(method, isGenerator, isAsync, allowsDirectSuper) {
  ...
  if (method.kind === "constructor") {
    if (isGenerator) { this.raise(key.start, "Constructor can't be a generator"); ***REMOVED***
    if (isAsync) { this.raise(key.start, "Constructor can't be an async method"); ***REMOVED***
  ***REMOVED*** else if (method.static && checkKeyName(method, "prototype")) {
  ...
  ***REMOVED***
  ...
  if (method.kind === "get" && value.params.length !== 0)
    { this.raiseRecoverable(value.start, "getter should have no params"); ***REMOVED***
  if (method.kind === "set" && value.params.length !== 1)
    { this.raiseRecoverable(value.start, "setter should have exactly one param"); ***REMOVED***
  if (method.kind === "set" && value.params[0].type === "RestElement")
    { this.raiseRecoverable(value.params[0].start, "Setter cannot use rest params"); ***REMOVED***
...
pp$8.parseClassField = function(field) {
  ...
  ***REMOVED*** else if (field.static && checkKeyName(field, "prototype")) {
...
pp$8.parseClassId = function(node, isStatement) {
  if (this.type === types$1.name) {
    ...
    if (isStatement)
      { this.checkLValSimple(node.id, BIND_LEXICAL, false); ***REMOVED***
  ***REMOVED*** else {
    if (isStatement === true)
      { this.unexpected(); ***REMOVED***
    node.id = null;
...
pp$8.enterClassBody = function() {
  var element = {declared: Object.create(null), used: []***REMOVED***;
...
function isPrivateNameConflicted(privateNameMap, element) {
  ...
  ***REMOVED***
  ...
  // `class { get #a(){***REMOVED***; static set #a(_){***REMOVED*** ***REMOVED***` is also conflict.
  if (
    curr === "iget" && next === "iset" ||
    curr === "iset" && next === "iget" ||
    curr === "sget" && next === "sset" ||
...
pp$8.parseExport = function(node, exports) {
  ...
  if (this.eat(types$1.star)) {
    ...
    ***REMOVED***
    ...
    if (this.type !== types$1.string) { this.unexpected(); ***REMOVED***
  ...
  ***REMOVED***
  if (this.eat(types$1._default)) { // export default ...
    ...
    if (this.type === types$1._function || (isAsync = this.isAsyncFunction())) {
      ...
      this.next();
      if (isAsync) { this.next(); ***REMOVED***
      node.declaration = this.parseFunction(fNode, FUNC_STATEMENT | FUNC_NULLABLE_ID, false, isAsync);
    ***REMOVED*** else if (this.type === types$1._class) {
  ...
  ***REMOVED***
  ...
  if (this.shouldParseExportStatement()) {
    ...
    if (node.declaration.type === "VariableDeclaration")
      { this.checkVariableExport(exports, node.declaration.declarations); ***REMOVED***
    else
      { this.checkExport(exports, node.declaration.id, node.declaration.id.start); ***REMOVED***
    node.specifiers = [];
    node.source = null;
  ***REMOVED*** else { // export { x, y as z ***REMOVED*** [from '...']
...
pp$8.checkExport = function(exports, name, pos) {
  if (!exports) { return ***REMOVED***
  if (typeof name !== "string")
    { name = name.type === "Identifier" ? name.name : name.value; ***REMOVED***
  if (hasOwn(exports, name))
    { this.raiseRecoverable(pos, "Duplicate export '" + name + "'"); ***REMOVED***
  exports[name] = true;
***REMOVED***;
...
pp$8.checkPatternExport = function(exports, pat) {
  var type = pat.type;
  if (type === "Identifier")
    { this.checkExport(exports, pat, pat.start); ***REMOVED***
    ...
    { for (var i$1 = 0, list$1 = pat.elements; i$1 < list$1.length; i$1 += 1) {
      var elt = list$1[i$1];
        ...
        if (elt) { this.checkPatternExport(exports, elt); ***REMOVED***
    ***REMOVED*** ***REMOVED***
  else if (type === "Property")
    { this.checkPatternExport(exports, pat.value); ***REMOVED***
  else if (type === "AssignmentPattern")
    { this.checkPatternExport(exports, pat.left); ***REMOVED***
  else if (type === "RestElement")
    { this.checkPatternExport(exports, pat.argument); ***REMOVED***
  else if (type === "ParenthesizedExpression")
    { this.checkPatternExport(exports, pat.expression); ***REMOVED***
***REMOVED***;
...
pp$8.parseExportSpecifiers = function(exports) {
  ...
  // export { x, y as z ***REMOVED*** [from '...']
  this.expect(types$1.braceL);
  while (!this.eat(types$1.braceR)) {
    if (!first) {
      this.expect(types$1.comma);
      if (this.afterTrailingComma(types$1.braceR)) { break ***REMOVED***
    ***REMOVED*** else { first = false; ***REMOVED***
...
pp$8.parseImportSpecifiers = function() {
  var nodes = [], first = true;
  if (this.type === types$1.name) {
    // import defaultObj, { x, y as z ***REMOVED*** from '...'
    var node = this.startNode();
    node.local = this.parseIdent();
    this.checkLValSimple(node.local, BIND_LEXICAL);
    nodes.push(this.finishNode(node, "ImportDefaultSpecifier"));
    if (!this.eat(types$1.comma)) { return nodes ***REMOVED***
  ***REMOVED***
  ...
  if (this.options.ecmaVersion >= 6 && node) {
    ...
    case "Identifier":
      if (this.inAsync && node.name === "await")
        { this.raise(node.start, "Cannot use 'await' as identifier inside an async function"); ***REMOVED***
      break
    ...
    case "ObjectExpression":
      ...
      if (refDestructuringErrors) { this.checkPatternErrors(refDestructuringErrors, true); ***REMOVED***
      for (var i = 0, list = node.properties; i < list.length; i += 1) {
        var prop = list[i];
    ...
    case "Property":
      ...
      if (node.kind !== "init") { this.raise(node.key.start, "Object pattern can't contain getter or setter"); ***REMOVED***
    ...
    case "SpreadElement":
      ...
      if (node.argument.type === "AssignmentPattern")
        { this.raise(node.argument.start, "Rest elements cannot have a default value"); ***REMOVED***
      break

    case "AssignmentExpression":
      if (node.operator !== "=") { this.raise(node.left.end, "Only '=' operator can be used for specifying default value."); ***REMOVED***
    ...
    case "MemberExpression":
      if (!isBinding) { break ***REMOVED***

    default:
      this.raise(node.start, "Assigning to rvalue");
    ***REMOVED***
  ***REMOVED*** else if (refDestructuringErrors) { this.checkPatternErrors(refDestructuringErrors, true); ***REMOVED***
  return node
***REMOVED***;
...
pp$7.toAssignableList = function(exprList, isBinding) {
  ...
  for (var i = 0; i < end; i++) {
    var elt = exprList[i];
    if (elt) { this.toAssignable(elt, isBinding); ***REMOVED***
  ***REMOVED***
  if (end) {
    var last = exprList[end - 1];
    if (this.options.ecmaVersion === 6 && isBinding && last && last.type === "RestElement" && last.argument.type !== "Identifier")
      { this.unexpected(last.argument.start); ***REMOVED***
  ***REMOVED***
  return exprList
***REMOVED***;
...
pp$7.parseBindingList = function(close, allowEmpty, allowTrailingComma) {
  ...
  while (!this.eat(close)) {
    if (first) { first = false; ***REMOVED***
    else { this.expect(types$1.comma); ***REMOVED***
    if (allowEmpty && this.type === types$1.comma) {
      elts.push(null);
    ***REMOVED*** else if (allowTrailingComma && this.afterTrailingComma(close)) {
      break
    ***REMOVED*** else if (this.type === types$1.ellipsis) {
      var rest = this.parseRestBinding();
      this.parseBindingListItem(rest);
      elts.push(rest);
      if (this.type === types$1.comma) { this.raise(this.start, "Comma is not permitted after the rest element"); ***REMOVED***
      this.expect(close);
      break
    ***REMOVED*** else {
...
pp$7.parseMaybeDefault = function(startPos, startLoc, left) {
  ...
  if (this.options.ecmaVersion < 6 || !this.eat(types$1.eq)) { return left ***REMOVED***
...
pp$7.checkLValSimple = function(expr, bindingType, checkClashes) {
  ...
  case "Identifier":
    if (this.strict && this.reservedWordsStrictBind.test(expr.name))
      { this.raiseRecoverable(expr.start, (isBind ? "Binding " : "Assigning to ") + expr.name + " in strict mode"); ***REMOVED***
    if (isBind) {
      if (bindingType === BIND_LEXICAL && expr.name === "let")
        { this.raiseRecoverable(expr.start, "let is disallowed as a lexically bound name"); ***REMOVED***
      if (checkClashes) {
        if (hasOwn(checkClashes, expr.name))
          { this.raiseRecoverable(expr.start, "Argument name clash"); ***REMOVED***
        checkClashes[expr.name] = true;
      ***REMOVED***
      if (bindingType !== BIND_OUTSIDE) { this.declareName(expr.name, bindingType, expr.start); ***REMOVED***
    ***REMOVED***
    break
  ...
  case "MemberExpression":
    if (isBind) { this.raiseRecoverable(expr.start, "Binding member expression"); ***REMOVED***
    break

  case "ParenthesizedExpression":
    if (isBind) { this.raiseRecoverable(expr.start, "Binding parenthesized expression"); ***REMOVED***
...
pp$7.checkLValPattern = function(expr, bindingType, checkClashes) {
  ...
  case "ArrayPattern":
    ...
    if (elem) { this.checkLValInnerPattern(elem, bindingType, checkClashes); ***REMOVED***
    ***REMOVED***
    break
...
var types = {
  ...
  q_tmpl: new TokContext("`", true, true, function (p) { return p.tryReadTemplateToken(); ***REMOVED***),
...
pp$6.braceIsBlock = function(prevType) {
  ...
  if (prevType === types$1.colon && (parent === types.b_stat || parent === types.b_expr))
    { return !parent.isExpr ***REMOVED***

  // The check for `tt.name && exprAllowed` detects whether we are
  // after a `yield` or `of` construct. See the `updateContext` for
  // `tt.name`.
  if (prevType === types$1._return || prevType === types$1.name && this.exprAllowed)
    { return lineBreak.test(this.input.slice(this.lastTokEnd, this.start)) ***REMOVED***
  if (prevType === types$1._else || prevType === types$1.semi || prevType === types$1.eof || prevType === types$1.parenR || prevType === types$1.arrow)
    { return true ***REMOVED***
  if (prevType === types$1.braceL)
    { return parent === types.b_stat ***REMOVED***
...
pp$6.inGeneratorContext = function() {
  for (var i = this.context.length - 1; i >= 1; i--) {
    ...
    if (context.token === "function")
      { return context.generator ***REMOVED***
  ***REMOVED***
  return false
***REMOVED***;
...
pp$6.updateContext = function(prevType) {
  ...
  if (type.keyword && prevType === types$1.dot)
    { this.exprAllowed = false; ***REMOVED***
  else if (update = type.updateContext)
    { update.call(this, prevType); ***REMOVED***
  else
    { this.exprAllowed = type.beforeExpr; ***REMOVED***
***REMOVED***;
...
types$1._function.updateContext = types$1._class.updateContext = function(prevType) {
  if (prevType.beforeExpr && prevType !== types$1._else &&
    ...
    { this.context.push(types.f_expr); ***REMOVED***
  else
    { this.context.push(types.f_stat); ***REMOVED***
  this.exprAllowed = false;
***REMOVED***;

types$1.backQuote.updateContext = function() {
  if (this.curContext() === types.q_tmpl)
    { this.context.pop(); ***REMOVED***
  else
    { this.context.push(types.q_tmpl); ***REMOVED***
  this.exprAllowed = false;
***REMOVED***;
...
types$1.star.updateContext = function(prevType) {
  if (prevType === types$1._function) {
    ...
    if (this.context[index] === types.f_expr)
      { this.context[index] = types.f_expr_gen; ***REMOVED***
    else
      { this.context[index] = types.f_gen; ***REMOVED***
  ***REMOVED***
  this.exprAllowed = true;
***REMOVED***;
...
pp$5.checkPropClash = function(prop, propHash, refDestructuringErrors) {
  ...
  ***REMOVED***
  ...
  if (other) {
    ...
    ***REMOVED***
    if (redefinition)
      { this.raiseRecoverable(key.start, "Redefinition of property"); ***REMOVED***
  ***REMOVED*** else {
    other = propHash[name] = {
      init: false,
      get: false,
...
pp$5.parseExpression = function(forInit, refDestructuringErrors) {
  ...
  if (this.type === types$1.comma) {
    ...
    while (this.eat(types$1.comma)) { node.expressions.push(this.parseMaybeAssign(forInit, refDestructuringErrors)); ***REMOVED***
...
pp$5.parseMaybeAssign = function(forInit, refDestructuringErrors, afterLeftParse) {
  if (this.isContextual("yield")) {
    if (this.inGenerator) { return this.parseYield(forInit) ***REMOVED***
    // The tokenizer will assume an expression is allowed after
    // `yield`, but this isn't that kind of yield
    else { this.exprAllowed = false; ***REMOVED***
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (afterLeftParse) { left = afterLeftParse.call(this, left, startPos, startLoc); ***REMOVED***
  if (this.type.isAssign) {
    var node = this.startNodeAt(startPos, startLoc);
    node.operator = this.value;
    if (this.type === types$1.eq)
      { left = this.toAssignable(left, false, refDestructuringErrors); ***REMOVED***
    if (!ownDestructuringErrors) {
      refDestructuringErrors.parenthesizedAssign = refDestructuringErrors.trailingComma = refDestructuringErrors.doubleProto = -1;
    ***REMOVED***
    if (refDestructuringErrors.shorthandAssign >= left.start)
      { refDestructuringErrors.shorthandAssign = -1; ***REMOVED*** // reset because shorthand default was used correctly
    if (this.type === types$1.eq)
      { this.checkLValPattern(left); ***REMOVED***
    else
      { this.checkLValSimple(left); ***REMOVED***
    node.left = left;
    this.next();
    node.right = this.parseMaybeAssign(forInit);
    if (oldDoubleProto > -1) { refDestructuringErrors.doubleProto = oldDoubleProto; ***REMOVED***
    return this.finishNode(node, "AssignmentExpression")
  ***REMOVED*** else {
    if (ownDestructuringErrors) { this.checkExpressionErrors(refDestructuringErrors, true); ***REMOVED***
  ***REMOVED***
  if (oldParenAssign > -1) { refDestructuringErrors.parenthesizedAssign = oldParenAssign; ***REMOVED***
  if (oldTrailingComma > -1) { refDestructuringErrors.trailingComma = oldTrailingComma; ***REMOVED***
  return left
***REMOVED***;
...
pp$5.parseMaybeConditional = function(forInit, refDestructuringErrors) {
  ...
  if (this.checkExpressionErrors(refDestructuringErrors)) { return expr ***REMOVED***
...
pp$5.buildBinary = function(startPos, startLoc, left, right, op, logical) {
  if (right.type === "PrivateIdentifier") { this.raise(right.start, "Private identifier can only be left side of binary expression"); ***REMOVED***
  var node = this.startNodeAt(startPos, startLoc);
  node.left = left;
  node.operator = op;
  node.right = right;
...
pp$5.parseMaybeUnary = function(refDestructuringErrors, sawUnary, incDec, forInit) {
  ...
  if (this.isContextual("await") && this.canAwait) {
    ...
    sawUnary = true;
  ***REMOVED*** else if (this.type.prefix) {
    ...
    this.checkExpressionErrors(refDestructuringErrors, true);
    if (update) { this.checkLValSimple(node.argument); ***REMOVED***
    else if (this.strict && node.operator === "delete" &&
             node.argument.type === "Identifier")
      { this.raiseRecoverable(node.start, "Deleting local variable in strict mode"); ***REMOVED***
    else if (node.operator === "delete" && isPrivateFieldAccess(node.argument))
      { this.raiseRecoverable(node.start, "Private fields can not be deleted"); ***REMOVED***
    else { sawUnary = true; ***REMOVED***
    expr = this.finishNode(node, update ? "UpdateExpression" : "UnaryExpression");
  ***REMOVED*** else if (!sawUnary && this.type === types$1.privateId) {
    if (forInit || this.privateNameStack.length === 0) { this.unexpected(); ***REMOVED***
    expr = this.parsePrivateIdent();
    // only could be private fields in 'in', such as #x in obj
    if (this.type !== types$1._in) { this.unexpected(); ***REMOVED***
  ***REMOVED*** else {
  ...
  ***REMOVED***
  ...
  if (!incDec && this.eat(types$1.starstar)) {
    if (sawUnary)
      { this.unexpected(this.lastTokStart); ***REMOVED***
    else
      { return this.buildBinary(startPos, startLoc, expr, this.parseMaybeUnary(null, false, false, forInit), "**", false) ***REMOVED***
  ***REMOVED*** else {
    return expr
  ***REMOVED***
***REMOVED***;
...
pp$5.parseExprSubscripts = function(refDestructuringErrors, forInit) {
  ...
  if (refDestructuringErrors && result.type === "MemberExpression") {
    if (refDestructuringErrors.parenthesizedAssign >= result.start) { refDestructuringErrors.parenthesizedAssign = -1; ***REMOVED***
    if (refDestructuringErrors.parenthesizedBind >= result.start) { refDestructuringErrors.parenthesizedBind = -1; ***REMOVED***
    if (refDestructuringErrors.trailingComma >= result.start) { refDestructuringErrors.trailingComma = -1; ***REMOVED***
  ***REMOVED***
  return result
***REMOVED***;
...
pp$5.parseSubscripts = function(base, startPos, startLoc, noCalls, forInit) {
  ...
  while (true) {
    ...
    if (element.optional) { optionalChained = true; ***REMOVED***
...
pp$5.parseSubscript = function(base, startPos, startLoc, noCalls, maybeAsyncArrow, optionalChained, forInit) {
  ...
  if (noCalls && optional) { this.raise(this.lastTokStart, "Optional chaining cannot appear in the callee of new expressions"); ***REMOVED***
  ...
  if (computed || (optional && this.type !== types$1.parenL && this.type !== types$1.backQuote) || this.eat(types$1.dot)) {
    ...
    ***REMOVED*** else if (this.type === types$1.privateId && base.type !== "Super") {
  ...
  ***REMOVED*** else if (!noCalls && this.eat(types$1.parenL)) {
    ...
    if (maybeAsyncArrow && !optional && !this.canInsertSemicolon() && this.eat(types$1.arrow)) {
      ...
      if (this.awaitIdentPos > 0)
        { this.raise(this.awaitIdentPos, "Cannot use 'await' as identifier inside an async function"); ***REMOVED***
      this.yieldPos = oldYieldPos;
      this.awaitPos = oldAwaitPos;
      this.awaitIdentPos = oldAwaitIdentPos;
  ...
  ***REMOVED*** else if (this.type === types$1.backQuote) {
    if (optional || optionalChained) {
      this.raise(this.start, "Optional chaining cannot appear in the tag of tagged template expressions");
    ***REMOVED***
    var node$2 = this.startNodeAt(startPos, startLoc);
    node$2.tag = base;
    node$2.quasi = this.parseTemplate({isTagged: true***REMOVED***);
...
pp$5.parseExprAtom = function(refDestructuringErrors, forInit) {
  ...
  if (this.type === types$1.slash) { this.readRegexp(); ***REMOVED***

  var node, canBeArrow = this.potentialArrowAt === this.start;
  switch (this.type) {
  case types$1._super:
    if (!this.allowSuper)
      { this.raise(this.start, "'super' keyword outside a method"); ***REMOVED***
    node = this.startNode();
    this.next();
    if (this.type === types$1.parenL && !this.allowDirectSuper)
      { this.raise(node.start, "super() call outside constructor of a subclass"); ***REMOVED***
    ...
    ***REMOVED***
    if (canBeArrow && !this.canInsertSemicolon()) {
      if (this.eat(types$1.arrow))
        { return this.parseArrowExpression(this.startNodeAt(startPos, startLoc), [id], false, forInit) ***REMOVED***
  ...
  case types$1.regexp:
    ...
    node.regex = {pattern: value.pattern, flags: value.flags***REMOVED***;
    return node
  ...
  case types$1.parenL:
    ...
    if (refDestructuringErrors) {
      if (refDestructuringErrors.parenthesizedAssign < 0 && !this.isSimpleAssignTarget(expr))
        { refDestructuringErrors.parenthesizedAssign = start; ***REMOVED***
      if (refDestructuringErrors.parenthesizedBind < 0)
        { refDestructuringErrors.parenthesizedBind = start; ***REMOVED***
    ***REMOVED***
    return expr
...
pp$5.parseExprImport = function() {
  ...
  if (this.containsEsc) { this.raiseRecoverable(this.start, "Escape sequence in keyword import"); ***REMOVED***
...
pp$5.parseImportMeta = function(node) {
  ...
  if (node.property.name !== "meta")
    { this.raiseRecoverable(node.property.start, "The only valid meta property for import is 'import.meta'"); ***REMOVED***
  if (containsEsc)
    { this.raiseRecoverable(node.start, "'import.meta' must not contain escaped characters"); ***REMOVED***
  if (this.options.sourceType !== "module" && !this.options.allowImportExportEverywhere)
    { this.raiseRecoverable(node.start, "Cannot use 'import.meta' outside a module"); ***REMOVED***
...
pp$5.parseLiteral = function(value) {
  ...
  if (node.raw.charCodeAt(node.raw.length - 1) === 110) { node.bigint = node.raw.slice(0, -1).replace(/_/g, ""); ***REMOVED***
  this.next();
  return this.finishNode(node, "Literal")
***REMOVED***;
...
pp$5.parseParenAndDistinguishExpression = function(canBeArrow, forInit) {
  ...
  if (this.options.ecmaVersion >= 6) {
    ...
    ***REMOVED***
    ...
    if (!exprList.length || lastIsComma) { this.unexpected(this.lastTokStart); ***REMOVED***
    if (spreadStart) { this.unexpected(spreadStart); ***REMOVED***
    this.checkExpressionErrors(refDestructuringErrors, true);
    this.yieldPos = oldYieldPos || this.yieldPos;
    this.awaitPos = oldAwaitPos || this.awaitPos;
...
pp$5.parseNew = function() {
  if (this.containsEsc) { this.raiseRecoverable(this.start, "Escape sequence in keyword new"); ***REMOVED***
  var node = this.startNode();
  var meta = this.parseIdent(true);
  if (this.options.ecmaVersion >= 6 && this.eat(types$1.dot)) {
    ...
    if (node.property.name !== "target")
      { this.raiseRecoverable(node.property.start, "The only valid meta property for new is 'new.target'"); ***REMOVED***
    if (containsEsc)
      { this.raiseRecoverable(node.start, "'new.target' must not contain escaped characters"); ***REMOVED***
    if (!this.allowNewDotTarget)
      { this.raiseRecoverable(node.start, "'new.target' can only be used in functions and class static block"); ***REMOVED***
    return this.finishNode(node, "MetaProperty")
  ***REMOVED***
  ...
  ***REMOVED***
  if (this.eat(types$1.parenL)) { node.arguments = this.parseExprList(types$1.parenR, this.options.ecmaVersion >= 8, false); ***REMOVED***
  else { node.arguments = empty; ***REMOVED***
...
pp$5.parseTemplate = function(ref) {
  if ( ref === void 0 ) ref = {***REMOVED***;
  var isTagged = ref.isTagged; if ( isTagged === void 0 ) isTagged = false;

  var node = this.startNode();
  this.next();
  node.expressions = [];
  var curElt = this.parseTemplateElement({isTagged: isTagged***REMOVED***);
  node.quasis = [curElt];
  while (!curElt.tail) {
    if (this.type === types$1.eof) { this.raise(this.pos, "Unterminated template literal"); ***REMOVED***
    this.expect(types$1.dollarBraceL);
    node.expressions.push(this.parseExpression());
    this.expect(types$1.braceR);
    node.quasis.push(curElt = this.parseTemplateElement({isTagged: isTagged***REMOVED***));
  ***REMOVED***
  this.next();
...
pp$5.parseObj = function(isPattern, refDestructuringErrors) {
  var node = this.startNode(), first = true, propHash = {***REMOVED***;
  node.properties = [];
  this.next();
  while (!this.eat(types$1.braceR)) {
    if (!first) {
      this.expect(types$1.comma);
      if (this.options.ecmaVersion >= 5 && this.afterTrailingComma(types$1.braceR)) { break ***REMOVED***
    ***REMOVED*** else { first = false; ***REMOVED***

    var prop = this.parseProperty(isPattern, refDestructuringErrors);
    if (!isPattern) { this.checkPropClash(prop, propHash, refDestructuringErrors); ***REMOVED***
...
pp$5.parseProperty = function(isPattern, refDestructuringErrors) {
  ...
  ***REMOVED***
  if (this.options.ecmaVersion >= 6) {
    ...
    ***REMOVED***
    if (!isPattern)
      { isGenerator = this.eat(types$1.star); ***REMOVED***
  ***REMOVED***
  var containsEsc = this.containsEsc;
...
pp$5.parsePropertyValue = function(prop, isPattern, isGenerator, isAsync, startPos, startLoc, refDestructuringErrors, containsEsc) {
  ...
  if (this.eat(types$1.colon)) {
    ...
    prop.kind = "init";
  ***REMOVED*** else if (this.options.ecmaVersion >= 6 && this.type === types$1.parenL) {
    if (isPattern) { this.unexpected(); ***REMOVED***
    prop.kind = "init";
    prop.method = true;
  ...
  ***REMOVED*** else if (!isPattern && !containsEsc &&
    ...
    if (isGenerator || isAsync) { this.unexpected(); ***REMOVED***
    ...
    if (prop.value.params.length !== paramCount) {
      ...
      if (prop.kind === "get")
        { this.raiseRecoverable(start, "getter should have no params"); ***REMOVED***
      else
        { this.raiseRecoverable(start, "setter should have exactly one param"); ***REMOVED***
    ***REMOVED*** else {
      if (prop.kind === "set" && prop.value.params[0].type === "RestElement")
        { this.raiseRecoverable(prop.value.params[0].start, "Setter cannot use rest params"); ***REMOVED***
    ***REMOVED***
  ***REMOVED*** else if (this.options.ecmaVersion >= 6 && !prop.computed && prop.key.type === "Identifier") {
    if (isGenerator || isAsync) { this.unexpected(); ***REMOVED***
    this.checkUnreserved(prop.key);
    if (prop.key.name === "await" && !this.awaitIdentPos)
      { this.awaitIdentPos = startPos; ***REMOVED***
    prop.kind = "init";
    if (isPattern) {
      prop.value = this.parseMaybeDefault(startPos, startLoc, this.copyNode(prop.key));
    ***REMOVED*** else if (this.type === types$1.eq && refDestructuringErrors) {
      if (refDestructuringErrors.shorthandAssign < 0)
        { refDestructuringErrors.shorthandAssign = this.start; ***REMOVED***
      prop.value = this.parseMaybeDefault(startPos, startLoc, this.copyNode(prop.key));
    ***REMOVED*** else {
      prop.value = this.copyNode(prop.key);
    ***REMOVED***
    prop.shorthand = true;
  ***REMOVED*** else { this.unexpected(); ***REMOVED***
***REMOVED***;
...
pp$5.initFunction = function(node) {
  node.id = null;
  if (this.options.ecmaVersion >= 6) { node.generator = node.expression = false; ***REMOVED***
  if (this.options.ecmaVersion >= 8) { node.async = false; ***REMOVED***
***REMOVED***;
...
pp$5.parseMethod = function(isGenerator, isAsync, allowDirectSuper) {
  ...
  this.initFunction(node);
  if (this.options.ecmaVersion >= 6)
    { node.generator = isGenerator; ***REMOVED***
  if (this.options.ecmaVersion >= 8)
    { node.async = !!isAsync; ***REMOVED***
...
pp$5.parseArrowExpression = function(node, params, isAsync, forInit) {
  ...
  this.initFunction(node);
  if (this.options.ecmaVersion >= 8) { node.async = !!isAsync; ***REMOVED***
  ...
  ***REMOVED*** else {
    ...
    if (!oldStrict || nonSimple) {
      ...
      if (useStrict && nonSimple)
        { this.raiseRecoverable(node.start, "Illegal 'use strict' directive in function with non-simple parameter list"); ***REMOVED***
    ***REMOVED***
    // Start a new scope with regard to labels and the `inFunction`
    // flag (restore them to their old value afterwards).
    var oldLabels = this.labels;
    this.labels = [];
    if (useStrict) { this.strict = true; ***REMOVED***

    // Add the params to varDeclaredNames to ensure that an error is thrown
    // if a let/const declaration in the function clashes with one of the params.
    this.checkParams(node, !oldStrict && !useStrict && !isArrowFunction && !isMethod && this.isSimpleParamList(node.params));
    // Ensure the function name isn't a forbidden identifier in strict mode, e.g. 'eval'
    if (this.strict && node.id) { this.checkLValSimple(node.id, BIND_OUTSIDE); ***REMOVED***
...
pp$5.parseExprList = function(close, allowTrailingComma, allowEmpty, refDestructuringErrors) {
  ...
  while (!this.eat(close)) {
    if (!first) {
      this.expect(types$1.comma);
      if (allowTrailingComma && this.afterTrailingComma(close)) { break ***REMOVED***
    ***REMOVED*** else { first = false; ***REMOVED***
    ...
    if (allowEmpty && this.type === types$1.comma)
      ...
      if (refDestructuringErrors && this.type === types$1.comma && refDestructuringErrors.trailingComma < 0)
        { refDestructuringErrors.trailingComma = this.start; ***REMOVED***
    ***REMOVED*** else {
...
pp$5.checkUnreserved = function(ref) {
  ...
  if (this.inGenerator && name === "yield")
    { this.raiseRecoverable(start, "Cannot use 'yield' as identifier inside a generator"); ***REMOVED***
  if (this.inAsync && name === "await")
    { this.raiseRecoverable(start, "Cannot use 'await' as identifier inside an async function"); ***REMOVED***
  if (this.currentThisScope().inClassFieldInit && name === "arguments")
    { this.raiseRecoverable(start, "Cannot use 'arguments' in class field initializer"); ***REMOVED***
  if (this.inClassStaticBlock && (name === "arguments" || name === "await"))
    { this.raise(start, ("Cannot use " + name + " in class static initialization block")); ***REMOVED***
  if (this.keywords.test(name))
    { this.raise(start, ("Unexpected keyword '" + name + "'")); ***REMOVED***
  if (this.options.ecmaVersion < 6 &&
    this.input.slice(start, end).indexOf("\\") !== -1) { return ***REMOVED***
  var re = this.strict ? this.reservedWordsStrict : this.reservedWords;
  if (re.test(name)) {
    if (!this.inAsync && name === "await")
      { this.raiseRecoverable(start, "Cannot use keyword 'await' outside an async function"); ***REMOVED***
...
pp$5.parseIdent = function(liberal, isBinding) {
  ...
  ***REMOVED*** else if (this.type.keyword) {
  ...
  ***REMOVED***
  ...
  if (!liberal) {
    ...
    if (node.name === "await" && !this.awaitIdentPos)
      { this.awaitIdentPos = node.start; ***REMOVED***
  ***REMOVED***
  return node
***REMOVED***;
...
pp$5.parseYield = function(forInit) {
  if (!this.yieldPos) { this.yieldPos = this.start; ***REMOVED***
...
pp$5.parseAwait = function(forInit) {
  if (!this.awaitPos) { this.awaitPos = this.start; ***REMOVED***
...
pp$3.declareName = function(name, bindingType, pos) {
  ...
  if (bindingType === BIND_LEXICAL) {
    ...
    if (this.inModule && (scope.flags & SCOPE_TOP))
      { delete this.undefinedExports[name]; ***REMOVED***
  ***REMOVED*** else if (bindingType === BIND_SIMPLE_CATCH) {
    var scope$1 = this.currentScope();
    scope$1.lexical.push(name);
  ***REMOVED*** else if (bindingType === BIND_FUNCTION) {
    var scope$2 = this.currentScope();
    if (this.treatFunctionsAsVar)
      { redeclared = scope$2.lexical.indexOf(name) > -1; ***REMOVED***
    else
      { redeclared = scope$2.lexical.indexOf(name) > -1 || scope$2.var.indexOf(name) > -1; ***REMOVED***
    ...
    for (var i = this.scopeStack.length - 1; i >= 0; --i) {
      ...
      ***REMOVED***
      ...
      if (this.inModule && (scope$3.flags & SCOPE_TOP))
        { delete this.undefinedExports[name]; ***REMOVED***
      if (scope$3.flags & SCOPE_VAR) { break ***REMOVED***
    ***REMOVED***
  ***REMOVED***
  if (redeclared) { this.raiseRecoverable(pos, ("Identifier '" + name + "' has already been declared")); ***REMOVED***
***REMOVED***;
...
pp$3.currentVarScope = function() {
  for (var i = this.scopeStack.length - 1;; i--) {
    var scope = this.scopeStack[i];
    if (scope.flags & SCOPE_VAR) { return scope ***REMOVED***
  ***REMOVED***
***REMOVED***;
...
pp$3.currentThisScope = function() {
  for (var i = this.scopeStack.length - 1;; i--) {
    var scope = this.scopeStack[i];
    if (scope.flags & SCOPE_VAR && !(scope.flags & SCOPE_ARROW)) { return scope ***REMOVED***
  ***REMOVED***
***REMOVED***;
...
var Node = function Node(parser, pos, loc) {
  ...
  if (parser.options.locations)
    { this.loc = new SourceLocation(parser, loc); ***REMOVED***
  if (parser.options.directSourceFile)
    { this.sourceFile = parser.options.directSourceFile; ***REMOVED***
  if (parser.options.ranges)
    { this.range = [pos, 0]; ***REMOVED***
***REMOVED***;
...
function finishNodeAt(node, type, pos, loc) {
  ...
  node.end = pos;
  if (this.options.locations)
    { node.loc.end = loc; ***REMOVED***
  if (this.options.ranges)
    { node.range[1] = pos; ***REMOVED***
  return node
***REMOVED***
...
pp$2.copyNode = function(node) {
  ...
  for (var prop in node) { newNode[prop] = node[prop]; ***REMOVED***
  return newNode
***REMOVED***;
 ...
 *
 * @param {RegExpValidationState***REMOVED*** state The state to validate RegExp.
...
pp$1.regexp_pattern = function(state) {
  ...
  if (state.pos !== state.source.length) {
    ...
    ***REMOVED***
    if (state.eat(0x5D /* ] */) || state.eat(0x7D /* ***REMOVED*** */)) {
...
pp$1.regexp_eatBracedQuantifier = function(state, noError) {
  ...
  if (state.eat(0x7B /* { */)) {
    ...
    if (this.regexp_eatDecimalDigits(state)) {
      ...
      ***REMOVED***
      if (state.eat(0x7D /* ***REMOVED*** */)) {
        // SyntaxError in https://www.ecma-international.org/ecma-262/8.0/#sec-term
        if (max !== -1 && max < min && !noError) {
          state.raise("numbers out of order in {***REMOVED*** quantifier");
        ***REMOVED***
        return true
      ***REMOVED***
    ***REMOVED***
...
pp$1.regexp_eatCapturingGroup = function(state) {
  if (state.eat(0x28 /* ( */)) {
    ...
    ***REMOVED*** else if (state.current() === 0x3F /* ? */) {
...
pp$1.regexp_validateUnicodePropertyNameAndValue = function(state, name, value) {
  if (!hasOwn(state.unicodeProperties.nonBinary, name))
    { state.raise("Invalid property name"); ***REMOVED***
  if (!state.unicodeProperties.nonBinary[name].test(value))
    { state.raise("Invalid property value"); ***REMOVED***
***REMOVED***;
...
var Token = function Token(p) {
  ...
  if (p.options.locations)
    { this.loc = new SourceLocation(p, p.startLoc, p.endLoc); ***REMOVED***
  if (p.options.ranges)
    { this.range = [p.start, p.end]; ***REMOVED***
***REMOVED***;
...
pp.next = function(ignoreEscapeSequenceInKeyword) {
  if (!ignoreEscapeSequenceInKeyword && this.type.keyword && this.containsEsc)
    { this.raiseRecoverable(this.start, "Escape sequence in keyword " + this.type.keyword); ***REMOVED***
  if (this.options.onToken)
    { this.options.onToken(new Token(this)); ***REMOVED***
...
pp.nextToken = function() {
  ...
  if (!curContext || !curContext.preserveSpace) { this.skipSpace(); ***REMOVED***

  this.start = this.pos;
  if (this.options.locations) { this.startLoc = this.curPosition(); ***REMOVED***
  if (this.pos >= this.input.length) { return this.finishToken(types$1.eof) ***REMOVED***

  if (curContext.override) { return curContext.override(this) ***REMOVED***
  else { this.readToken(this.fullCharCodeAtPos()); ***REMOVED***
***REMOVED***;
...
pp.fullCharCodeAtPos = function() {
  ...
  if (code <= 0xd7ff || code >= 0xdc00) { return code ***REMOVED***
...
pp.skipBlockComment = function() {
  ...
  if (end === -1) { this.raise(this.pos - 2, "Unterminated comment"); ***REMOVED***
  this.pos = end + 2;
  if (this.options.locations) {
...
pp.finishToken = function(type, val) {
  this.end = this.pos;
  if (this.options.locations) { this.endLoc = this.curPosition(); ***REMOVED***
...
pp.readToken_dot = function() {
  var next = this.input.charCodeAt(this.pos + 1);
  if (next >= 48 && next <= 57) { return this.readNumber(true) ***REMOVED***
...
pp.readToken_slash = function() { // '/'
  var next = this.input.charCodeAt(this.pos + 1);
  if (this.exprAllowed) { ++this.pos; return this.readRegexp() ***REMOVED***
  if (next === 61) { return this.finishOp(types$1.assign, 2) ***REMOVED***
...
pp.readToken_mult_modulo_exp = function(code) { // '%*'
  ...
  ***REMOVED***
  ...
  if (next === 61) { return this.finishOp(types$1.assign, size + 1) ***REMOVED***
...
pp.readToken_pipe_amp = function(code) { // '|&'
  ...
  if (next === code) {
    if (this.options.ecmaVersion >= 12) {
      var next2 = this.input.charCodeAt(this.pos + 2);
      if (next2 === 61) { return this.finishOp(types$1.assign, 3) ***REMOVED***
    ***REMOVED***
...
pp.readToken_lt_gt = function(code) { // '<>'
  ...
  if (next === code) {
    ...
    if (this.input.charCodeAt(this.pos + size) === 61) { return this.finishOp(types$1.assign, size + 1) ***REMOVED***
  ...
  ***REMOVED***
  if (next === 33 && code === 60 && !this.inModule && this.input.charCodeAt(this.pos + 2) === 45 &&
    ...
    return this.nextToken()
  ***REMOVED***
  if (next === 61) { size = 2; ***REMOVED***
  return this.finishOp(types$1.relational, size)
***REMOVED***;

pp.readToken_eq_excl = function(code) { // '=!'
  var next = this.input.charCodeAt(this.pos + 1);
  if (next === 61) { return this.finishOp(types$1.equality, this.input.charCodeAt(this.pos + 2) === 61 ? 3 : 2) ***REMOVED***
...
pp.readToken_question = function() { // '?'
  ...
  if (ecmaVersion >= 11) {
    ...
    if (next === 46) {
      var next2 = this.input.charCodeAt(this.pos + 2);
      if (next2 < 48 || next2 > 57) { return this.finishOp(types$1.questionDot, 2) ***REMOVED***
    ***REMOVED***
    if (next === 63) {
      if (ecmaVersion >= 12) {
        var next2$1 = this.input.charCodeAt(this.pos + 2);
        if (next2$1 === 61) { return this.finishOp(types$1.assign, 3) ***REMOVED***
      ***REMOVED***
...
pp.getTokenFromCode = function(code) {
  ...
  case 96: // '`'
    if (this.options.ecmaVersion < 6) { break ***REMOVED***
    ++this.pos;
    return this.finishToken(types$1.backQuote)

  case 48: // '0'
    var next = this.input.charCodeAt(this.pos + 1);
    if (next === 120 || next === 88) { return this.readRadixNumber(16) ***REMOVED*** // '0x', '0X' - hex number
    if (this.options.ecmaVersion >= 6) {
      if (next === 111 || next === 79) { return this.readRadixNumber(8) ***REMOVED*** // '0o', '0O' - octal number
      if (next === 98 || next === 66) { return this.readRadixNumber(2) ***REMOVED*** // '0b', '0B' - binary number
    ***REMOVED***
...
pp.readRegexp = function() {
  ...
  for (;;) {
    if (this.pos >= this.input.length) { this.raise(start, "Unterminated regular expression"); ***REMOVED***
    var ch = this.input.charAt(this.pos);
    if (lineBreak.test(ch)) { this.raise(start, "Unterminated regular expression"); ***REMOVED***
    if (!escaped) {
      if (ch === "[") { inClass = true; ***REMOVED***
      else if (ch === "]" && inClass) { inClass = false; ***REMOVED***
      else if (ch === "/" && !inClass) { break ***REMOVED***
      escaped = ch === "\\";
    ***REMOVED*** else { escaped = false; ***REMOVED***
    ++this.pos;
  ***REMOVED***
  ...
  if (this.containsEsc) { this.unexpected(flagsStart); ***REMOVED***
  ...
  ***REMOVED***
  ...
  return this.finishToken(types$1.regexp, {pattern: pattern, flags: flags, value: value***REMOVED***)
***REMOVED***;
...
pp.readInt = function(radix, len, maybeLegacyOctalNumericLiteral) {
  ...
  for (var i = 0, e = len == null ? Infinity : len; i < e; ++i, ++this.pos) {
    ...
    if (allowSeparators && code === 95) {
      if (isLegacyOctalNumericLiteral) { this.raiseRecoverable(this.pos, "Numeric separator is not allowed in legacy octal numeric literals"); ***REMOVED***
      if (lastCode === 95) { this.raiseRecoverable(this.pos, "Numeric separator must be exactly one underscore"); ***REMOVED***
      if (i === 0) { this.raiseRecoverable(this.pos, "Numeric separator is not allowed at the first of digits"); ***REMOVED***
      lastCode = code;
      continue
    ***REMOVED***

    if (code >= 97) { val = code - 97 + 10; ***REMOVED*** // a
    else if (code >= 65) { val = code - 65 + 10; ***REMOVED*** // A
    else if (code >= 48 && code <= 57) { val = code - 48; ***REMOVED*** // 0-9
    else { val = Infinity; ***REMOVED***
    if (val >= radix) { break ***REMOVED***
    lastCode = code;
    total = total * radix + val;
  ***REMOVED***

  if (allowSeparators && lastCode === 95) { this.raiseRecoverable(this.pos - 1, "Numeric separator is not allowed at the last of digits"); ***REMOVED***
  if (this.pos === start || len != null && this.pos - start !== len) { return null ***REMOVED***
...
pp.readRadixNumber = function(radix) {
  ...
  if (val == null) { this.raise(this.start + 2, "Expected number in radix " + radix); ***REMOVED***
  if (this.options.ecmaVersion >= 11 && this.input.charCodeAt(this.pos) === 110) {
    val = stringToBigInt(this.input.slice(start, this.pos));
    ++this.pos;
  ***REMOVED*** else if (isIdentifierStart(this.fullCharCodeAtPos())) { this.raise(this.pos, "Identifier directly after number"); ***REMOVED***
...
pp.readNumber = function(startsWithDot) {
  var start = this.pos;
  if (!startsWithDot && this.readInt(10, undefined, true) === null) { this.raise(start, "Invalid number"); ***REMOVED***
  var octal = this.pos - start >= 2 && this.input.charCodeAt(start) === 48;
  if (octal && this.strict) { this.raise(start, "Invalid number"); ***REMOVED***
  var next = this.input.charCodeAt(this.pos);
  if (!octal && !startsWithDot && this.options.ecmaVersion >= 11 && next === 110) {
    var val$1 = stringToBigInt(this.input.slice(start, this.pos));
    ++this.pos;
    if (isIdentifierStart(this.fullCharCodeAtPos())) { this.raise(this.pos, "Identifier directly after number"); ***REMOVED***
    return this.finishToken(types$1.num, val$1)
  ***REMOVED***
  if (octal && /[89]/.test(this.input.slice(start, this.pos))) { octal = false; ***REMOVED***
  if (next === 46 && !octal) { // '.'
    ++this.pos;
    this.readInt(10);
  ...
  ***REMOVED***
  if ((next === 69 || next === 101) && !octal) { // 'eE'
    next = this.input.charCodeAt(++this.pos);
    if (next === 43 || next === 45) { ++this.pos; ***REMOVED*** // '+-'
    if (this.readInt(10) === null) { this.raise(start, "Invalid number"); ***REMOVED***
  ***REMOVED***
  if (isIdentifierStart(this.fullCharCodeAtPos())) { this.raise(this.pos, "Identifier directly after number"); ***REMOVED***
...
pp.readCodePoint = function() {
  ...
  if (ch === 123) { // '{'
    if (this.options.ecmaVersion < 6) { this.unexpected(); ***REMOVED***
    var codePos = ++this.pos;
    code = this.readHexChar(this.input.indexOf("***REMOVED***", this.pos) - this.pos);
    ++this.pos;
    if (code > 0x10FFFF) { this.invalidStringToken(codePos, "Code point out of bounds"); ***REMOVED***
  ***REMOVED*** else {
...
pp.readString = function(quote) {
  ...
  for (;;) {
    if (this.pos >= this.input.length) { this.raise(this.start, "Unterminated string constant"); ***REMOVED***
    var ch = this.input.charCodeAt(this.pos);
    if (ch === quote) { break ***REMOVED***
    if (ch === 92) { // '\'
      out += this.input.slice(chunkStart, this.pos);
      out += this.readEscapedChar(false);
      chunkStart = this.pos;
    ***REMOVED*** else if (ch === 0x2028 || ch === 0x2029) {
      if (this.options.ecmaVersion < 10) { this.raise(this.start, "Unterminated string constant"); ***REMOVED***
      ++this.pos;
      if (this.options.locations) {
        this.curLine++;
        this.lineStart = this.pos;
      ***REMOVED***
    ***REMOVED*** else {
      if (isNewLine(ch)) { this.raise(this.start, "Unterminated string constant"); ***REMOVED***
      ++this.pos;
    ***REMOVED***
  ***REMOVED***
  out += this.input.slice(chunkStart, this.pos++);
...
var INVALID_TEMPLATE_ESCAPE_ERROR = {***REMOVED***;
...
pp.readTmplToken = function() {
  var out = "", chunkStart = this.pos;
  for (;;) {
    if (this.pos >= this.input.length) { this.raise(this.start, "Unterminated template"); ***REMOVED***
    var ch = this.input.charCodeAt(this.pos);
    ...
    ***REMOVED***
    if (ch === 92) { // '\'
      ...
      chunkStart = this.pos;
    ***REMOVED*** else if (isNewLine(ch)) {
      out += this.input.slice(chunkStart, this.pos);
      ++this.pos;
      switch (ch) {
      case 13:
        if (this.input.charCodeAt(this.pos) === 10) { ++this.pos; ***REMOVED***
      case 10:
        out += "\n";
        break
      default:
...
pp.readEscapedChar = function(inTemplate) {
  ...
  case 13: if (this.input.charCodeAt(this.pos) === 10) { ++this.pos; ***REMOVED*** // '\r\n'
  case 10: // ' \n'
    if (this.options.locations) { this.lineStart = this.pos; ++this.curLine; ***REMOVED***
    return ""
  case 56:
  case 57:
    if (this.strict) {
...
pp.readHexChar = function(len) {
  ...
  if (n === null) { this.invalidStringToken(codePos, "Bad character escape sequence"); ***REMOVED***
  return n
***REMOVED***;
...
pp.readWord1 = function() {
  ...
  while (this.pos < this.input.length) {
    ...
    ***REMOVED*** else if (ch === 92) { // "\"
      this.containsEsc = true;
      word += this.input.slice(chunkStart, this.pos);
      var escStart = this.pos;
      if (this.input.charCodeAt(++this.pos) !== 117) // "u"
        { this.invalidStringToken(this.pos, "Expecting Unicode escape sequence \\uXXXX"); ***REMOVED***
      ++this.pos;
      var esc = this.readCodePoint();
      if (!(first ? isIdentifierStart : isIdentifierChar)(esc, astral))
        { this.invalidStringToken(escStart, "Invalid Unicode escape"); ***REMOVED***
    ...
    if (skip) {
        ...
        ***REMOVED***
        replaceContext = (pluginContext, plugin) => ({
            ...pluginContext,
            resolve: (source, importer, { custom, isEntry, skipSelf ***REMOVED*** = BLANK) => {
                return moduleLoaderResolveId(source, importer, custom, isEntry, skipSelf ? [...skip, { importer, plugin, source ***REMOVED***] : skip);
            ***REMOVED***
        ***REMOVED***);
    ***REMOVED***
    return pluginDriver.hookFirst('resolveId', [source, importer, { custom: customOptions, isEntry ***REMOVED***], replaceContext, skipped);
***REMOVED***
...
function throwPluginError(err, plugin, { hook, id ***REMOVED*** = {***REMOVED***) {
    if (typeof err === 'string')
        err = { message: err ***REMOVED***;
...
const deprecatedHooks = [
    { active: true, deprecated: 'resolveAssetUrl', replacement: 'resolveFileUrl' ***REMOVED***
];
function warnDeprecatedHooks(plugins, options) {
    for (const { active, deprecated, replacement ***REMOVED*** of deprecatedHooks) {
        for (const plugin of plugins) {
            if (deprecated in plugin) {
                warnDeprecation({
                    message: `The "${deprecated***REMOVED***" hook used by plugin ${plugin.name***REMOVED*** is deprecated. The "${replacement***REMOVED***" hook should be used instead.`,
                    plugin: plugin.name
                ***REMOVED***, active, options);
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***
***REMOVED***
...
function uncacheablePluginError(pluginName) {
    ...
    ***REMOVED***
    return error({
        ...
        message: `The plugin name ${pluginName***REMOVED*** is being used twice in the same build. Plugin names must be distinct or provide a cacheKey (please post an issue to the plugin if you are a plugin user).`
    ***REMOVED***);
***REMOVED***
...
async function transform(source, module, pluginDriver, warn) {
            ...
            return {
                ...
                ***REMOVED***,
                ...
                emitAsset(name, source) {
                    emittedFiles.push({ name, source, type: 'asset' ***REMOVED***);
                    return pluginContext.emitAsset(name, source);
                ***REMOVED***,
                emitChunk(id, options) {
                    emittedFiles.push({ id, name: options && options.name, type: 'chunk' ***REMOVED***);
                ...
                ***REMOVED***,
                getCombinedSourcemap() {
                    ...
                    if (!combinedMap) {
                        ...
                        return magicString.generateMap({ hires: true, includeContent: true, source: id ***REMOVED***);
                    ***REMOVED***
    ...
    ***REMOVED***
        ...
        throwPluginError(err, pluginName, { hook: 'transform', id ***REMOVED***);
    ***REMOVED***
...
class ModuleLoader {
    ...
    ***REMOVED***
    async addEntryModules(unresolvedEntryModules, isUserDefined) {
        ...
        const newEntryModules = await this.extendLoadModulesPromise(Promise.all(unresolvedEntryModules.map(({ id, importer ***REMOVED***) => this.loadEntryModule(id, true, importer, null))).then(entryModules => {
            for (let index = 0; index < entryModules.length; index++) {
                ...
                ***REMOVED***
            ***REMOVED***
            this.indexedEntryModules.sort(({ index: indexA ***REMOVED***, { index: indexB ***REMOVED***) => indexA > indexB ? 1 : -1);
            return entryModules;
        ***REMOVED***));
        await this.awaitLoadModulesPromise();
        return {
            entryModules: this.indexedEntryModules.map(({ module ***REMOVED***) => module),
            implicitEntryModules: [...this.implicitEntryModules],
            newEntryModules
        ***REMOVED***;
    ***REMOVED***
    async emitChunk({ fileName, id, importer, name, implicitlyLoadedAfterOneOf, preserveSignature ***REMOVED***) {
    ...
    ***REMOVED***
    async addModuleSource(id, importer, module) {
        ...
        let source;
        try {
            source = await this.graph.fileOperationQueue.run(async () => { var _a; return (_a = (await this.pluginDriver.hookFirst('load', [id]))) !== null && _a !== void 0 ? _a : (await promises.readFile(id, 'utf8')); ***REMOVED***);
        ***REMOVED***
        catch (err) {
            timeEnd('load modules', 3);
            let msg = `Could not load ${id***REMOVED***`;
            if (importer)
                msg += ` (imported by ${relativeId(importer)***REMOVED***)`;
            msg += `: ${err.message***REMOVED***`;
            err.message = msg;
            throw err;
        ***REMOVED***
        timeEnd('load modules', 3);
    ...
    ***REMOVED***
    async awaitLoadModulesPromise() {
        ...
        ***REMOVED*** while (startingPromise !== this.latestLoadModulesPromise);
    ***REMOVED***
    ...
    ***REMOVED***
    ...
    async fetchModule({ id, meta, moduleSideEffects, syntheticNamedExports ***REMOVED***, importer, isEntry, isPreload) {
    ...
    ***REMOVED***
    fetchResolvedDependency(source, importer, resolvedId) {
        if (resolvedId.external) {
            const { external, id, moduleSideEffects, meta ***REMOVED*** = resolvedId;
    ...
    ***REMOVED***
    getNormalizedResolvedIdWithoutDefaults(resolveIdResult, importer, source) {
        const { makeAbsoluteExternalsRelative ***REMOVED*** = this.options;
        if (resolveIdResult) {
    ...
    async loadEntryModule(unresolvedId, isEntry, importer, implicitlyLoadedBefore) {
        ...
        ***REMOVED***
        return this.fetchModule(this.getResolvedIdWithDefaults(typeof resolveIdResult === 'object'
            ? resolveIdResult
            : { id: resolveIdResult ***REMOVED***), undefined, isEntry, false);
    ***REMOVED***
...
function normalizeRelativeExternalId(source, importer) {
    return isRelative(source)
        ...
        : source;
***REMOVED***
function addChunkNamesToModule(module, { fileName, name ***REMOVED***, isUserDefined, priority) {
    var _a;
    if (fileName !== null) {
    ...
    ***REMOVED***
        ...
        while (((_a = module.chunkNames[namePosition]) === null || _a === void 0 ? void 0 : _a.priority) < priority)
            namePosition++;
        module.chunkNames.splice(namePosition, 0, { isUserDefined, name, priority ***REMOVED***);
    ***REMOVED***
***REMOVED***
...
function generateAssetFileName(name, source, outputOptions, bundle) {
    ...
    return makeUnique(renderNamePattern(typeof outputOptions.assetFileNames === 'function'
        ? outputOptions.assetFileNames({ name, source, type: 'asset' ***REMOVED***)
...
function getValidSource(source, emittedFile, fileReferenceId) {
    if (!(typeof source === 'string' || source instanceof Uint8Array)) {
        ...
        return error(errFailedValidation(`Could not set source for ${typeof assetName === 'string' ? `asset "${assetName***REMOVED***"` : 'unnamed asset'***REMOVED***, asset source needs to be a string, Uint8Array or Buffer.`));
    ***REMOVED***
    return source;
***REMOVED***
...
class FileEmitter {
    constructor(graph, options, baseFileEmitter) {
        ...
        ***REMOVED***;
        this.emitFile = (emittedFile) => {
            if (!hasValidType(emittedFile)) {
                return error(errFailedValidation(`Emitted files must be of type "asset" or "chunk", received "${emittedFile && emittedFile.type***REMOVED***".`));
            ***REMOVED***
            if (!hasValidName(emittedFile)) {
                return error(errFailedValidation(`The "fileName" or "name" properties of emitted files must be strings that are neither absolute nor relative paths, received "${emittedFile.fileName || emittedFile.name***REMOVED***".`));
            ***REMOVED***
        ...
        ***REMOVED***;
        this.setAssetSource = (referenceId, requestedSource) => {
            ...
            if (consumedFile.type !== 'asset') {
                return error(errFailedValidation(`Asset sources can only be set for emitted assets but "${referenceId***REMOVED***" is an emitted chunk.`));
            ***REMOVED***
        ...
        ***REMOVED***;
        this.setOutputBundle = (bundle, outputOptions, facadeChunkByModule) => {
            ...
            this.bundle = bundle;
            this.facadeChunkByModule = facadeChunkByModule;
            for (const { fileName ***REMOVED*** of this.filesByReferenceId.values()) {
                if (fileName) {
    ...
    assignReferenceId(file, idBase) {
        ...
        do {
            referenceId = createHash()
                ...
                .digest('hex')
                .substring(0, 8);
        ***REMOVED*** while (this.filesByReferenceId.has(referenceId));
    ...
    emitChunk(emittedChunk) {
        ...
        ***REMOVED***
        if (typeof emittedChunk.id !== 'string') {
            return error(errFailedValidation(`Emitted chunks need to have a valid string id, received "${emittedChunk.id***REMOVED***"`));
        ***REMOVED***
        const consumedChunk = {
    ...
    ***REMOVED***
    finalizeAsset(consumedFile, source, referenceId, bundle) {
        ...
        const assetWithFileName = { ...consumedFile, fileName, source ***REMOVED***;
...
function getDeprecatedContextHandler(handler, handlerName, newHandlerName, pluginName, activeDeprecation, options) {
    ...
    return ((...args) => {
        if (!deprecationWarningShown) {
            ...
            warnDeprecation({
                message: `The "this.${handlerName***REMOVED***" plugin context function used by plugin ${pluginName***REMOVED*** is deprecated. The "this.${newHandlerName***REMOVED***" plugin context function should be used instead.`,
                plugin: pluginName
            ***REMOVED***, activeDeprecation, options);
        ***REMOVED***
        return handler(...args);
    ***REMOVED***);
***REMOVED***
function getPluginContext(plugin, pluginCache, graph, options, fileEmitter, existingPluginNames) {
    ...
    ***REMOVED***
    return {
        ...
        ***REMOVED***,
        cache: cacheInstance,
        emitAsset: getDeprecatedContextHandler((name, source) => fileEmitter.emitFile({ name, source, type: 'asset' ***REMOVED***), 'emitAsset', 'emitFile', plugin.name, true, options),
        emitChunk: getDeprecatedContextHandler((id, options) => fileEmitter.emitFile({ id, name: options && options.name, type: 'chunk' ***REMOVED***), 'emitChunk', 'emitFile', plugin.name, true, options),
        ...
        ***REMOVED***,
        get moduleIds() {
            function* wrappedModuleIds() {
                ...
                warnDeprecation({
                    message: `Accessing "this.moduleIds" on the plugin context by plugin ${plugin.name***REMOVED*** is deprecated. The "this.getModuleIds" plugin context function should be used instead.`,
                    plugin: plugin.name
                ***REMOVED***, false, options);
                yield* moduleIds;
            ***REMOVED***
        ...
        ***REMOVED***,
        ...
        resolve(source, importer, { custom, isEntry, skipSelf ***REMOVED*** = BLANK) {
            return graph.moduleLoader.resolveId(source, importer, custom, isEntry, skipSelf ? [{ importer, plugin, source ***REMOVED***] : null);
        ***REMOVED***,
    ...
    runHook(hookName, args, plugin, replaceContext) {
        ...
        ***REMOVED***)
            ...
            if (action !== null) {
                ...
                this.unfulfilledActions.delete(action);
            ***REMOVED***
            return throwPluginError(err, plugin.name, { hook: hookName ***REMOVED***);
        ***REMOVED***);
    ***REMOVED***
    /**
    ...
    ***REMOVED***
    run(task) {
        return new Promise((resolve, reject) => {
            this.queue.push({ reject, resolve, task ***REMOVED***);
            this.work();
        ***REMOVED***);
    ***REMOVED***
    async work() {
        ...
        while ((entry = this.queue.shift())) {
            const { reject, resolve, task ***REMOVED*** = entry;
            try {
...
class Graph {
    ...
    ***REMOVED***
    contextParse(code, options = {***REMOVED***) {
        ...
        if (onCommentOrig && typeof onCommentOrig == 'function') {
            options.onComment = (block, text, start, end, ...args) => {
                comments.push({ end, start, type: block ? 'Block' : 'Line', value: text ***REMOVED***);
    ...
    ***REMOVED***
    async generateModuleGraph() {
        ({ entryModules: this.entryModules, implicitEntryModules: this.implicitEntryModules ***REMOVED*** =
    ...
    includeStatements() {
        ...
        ***REMOVED***
        if (this.options.treeshake) {
            ...
            do {
                timeStart(`treeshaking pass ${treeshakingPass***REMOVED***`, 3);
                this.needsTreeshakingPass = false;
                ...
                ***REMOVED***
                if (treeshakingPass === 1) {
                    ...
                    ***REMOVED***
                ***REMOVED***
                timeEnd(`treeshaking pass ${treeshakingPass++***REMOVED***`, 3);
            ***REMOVED*** while (this.needsTreeshakingPass);
        ***REMOVED***
        else {
            for (const module of this.modules)
                module.includeAllInBundle();
    ...
    ***REMOVED***
    sortModules() {
        const { orderedModules, cyclePaths ***REMOVED*** = analyseModuleExecution(this.entryModules);
        for (const cyclePath of cyclePaths) {
            this.options.onwarn({
                code: 'CIRCULAR_DEPENDENCY',
                cycle: cyclePath,
                importer: cyclePath[0],
                message: `Circular dependency: ${cyclePath.join(' -> ')***REMOVED***`
            ***REMOVED***);
        ***REMOVED***
            ...
            for (const importDescription of module.importDescriptions.values()) {
                if (importDescription.name !== '*' &&
                    ...
                    module.warn({
                        ...
                        message: `Non-existent export '${importDescription.name***REMOVED***' is imported from ${relativeId(importDescription.module.id)***REMOVED***`,
                        name: importDescription.name,
                        source: importDescription.module.id
                    ***REMOVED***, importDescription.start);
                ***REMOVED***
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***
...
function formatAction([pluginName, hookName, args]) {
    const action = `(${pluginName***REMOVED***) ${hookName***REMOVED***`;
    const s = JSON.stringify;
    switch (hookName) {
        case 'resolveId':
            return `${action***REMOVED*** ${s(args[0])***REMOVED*** ${s(args[1])***REMOVED***`;
        case 'load':
            return `${action***REMOVED*** ${s(args[0])***REMOVED***`;
        case 'transform':
            return `${action***REMOVED*** ${s(args[1])***REMOVED***`;
        case 'shouldTransformCachedModule':
            return `${action***REMOVED*** ${s(args[0].id)***REMOVED***`;
        case 'moduleParsed':
            return `${action***REMOVED*** ${s(args[0].id)***REMOVED***`;
    ***REMOVED***
    return action;
...
function warnUnknownOptions(passedOptions, validOptions, optionType, warn, ignoredKeys = /$./) {
    ...
    if (unknownOptions.length > 0) {
        warn({
            code: 'UNKNOWN_OPTION',
            message: `Unknown ${optionType***REMOVED***: ${unknownOptions.join(', ')***REMOVED***. Allowed options: ${[
                ...validOptionSet
            ]
                .sort()
                .join(', ')***REMOVED***`
...
const generatedCodePresets = {
    ...
    ***REMOVED***
***REMOVED***;
const objectifyOption = (value) => value && typeof value === 'object' ? value : {***REMOVED***;
const objectifyOptionWithPresets = (presets, optionName, additionalValues) => (value) => {
    if (typeof value === 'string') {
        ...
        if (preset) {
            return preset;
        ***REMOVED***
        error(errInvalidOption(optionName, getHashFromObjectOption(optionName), `valid values are ${additionalValues***REMOVED***${printQuotedStringList(Object.keys(presets))***REMOVED***. You can also supply an object for more fine-grained control`, value));
    ***REMOVED***
...
const getOptionWithPreset = (value, presets, optionName, additionalValues) => {
    ...
    if (presetName) {
        ...
        if (preset) {
            return { ...preset, ...value ***REMOVED***;
        ***REMOVED***
        else {
            error(errInvalidOption(`${optionName***REMOVED***.preset`, getHashFromObjectOption(optionName), `valid values are ${printQuotedStringList(Object.keys(presets))***REMOVED***`, presetName));
        ***REMOVED***
    ***REMOVED***
...
function normalizeInputOptions(config) {
    ...
    ***REMOVED***;
    ...
    return { options, unsetOptions ***REMOVED***;
***REMOVED***
const getOnwarn = (config) => {
    const { onwarn ***REMOVED*** = config;
    return onwarn
        ? warning => {
            warning.toString = () => {
                let str = '';
                if (warning.plugin)
                    str += `(${warning.plugin***REMOVED*** plugin) `;
                if (warning.loc)
                    str += `${relativeId(warning.loc.file)***REMOVED*** (${warning.loc.line***REMOVED***:${warning.loc.column***REMOVED***) `;
                str += warning.message;
                return str;
            ***REMOVED***;
...
const getCache = (config) => { var _a; return ((_a = config.cache) === null || _a === void 0 ? void 0 : _a.cache) || config.cache; ***REMOVED***;
...
const getModuleContext = (config, context) => {
    ...
    if (typeof configModuleContext === 'function') {
        return id => { var _a; return (_a = configModuleContext(id)) !== null && _a !== void 0 ? _a : context; ***REMOVED***;
    ***REMOVED***
...
const INVALID_CHAR_REGEX = /[\x00-\x1F\x7F<>*#"{***REMOVED***|^[\]`;?:&=+$,]/g;
...
function normalizeOutputOptions(config, inputOptions, unsetInputOptions) {
    ...
    const outputOptions = {
        ...
        generatedCode,
        globals: config.globals || {***REMOVED***,
    ...
    ***REMOVED***;
    ...
    return { options: outputOptions, unsetOptions ***REMOVED***;
***REMOVED***
const getFile = (config, preserveModules, inputOptions) => {
    const { file ***REMOVED*** = config;
...
const getInlineDynamicImports = (config, inputOptions) => {
    ...
    const { input ***REMOVED*** = inputOptions;
...
const getPreserveModulesRoot = (config) => {
    const { preserveModulesRoot ***REMOVED*** = config;
    ...
    const validateInterop = (interop) => {
        if (!validatedInteropTypes.has(interop)) {
            ...
            if (!ALLOWED_INTEROP_TYPES.has(interop)) {
                return error(errInvalidOption('output.interop', 'outputinterop', `use one of ${Array.from(ALLOWED_INTEROP_TYPES, value => JSON.stringify(value)).join(', ')***REMOVED***`, interop));
            ***REMOVED***
            if (typeof interop === 'boolean') {
                warnDeprecation({
                    message: `The boolean value "${interop***REMOVED***" for the "output.interop" option is deprecated. Use ${interop ? '"auto"' : '"esModule", "default" or "defaultOnly"'***REMOVED*** instead.`,
                    url: 'https://rollupjs.org/guide/en/#outputinterop'
                ***REMOVED***, false, inputOptions);
            ***REMOVED***
        ***REMOVED***
        return interop;
    ***REMOVED***;
...
const getManualChunks = (config, inlineDynamicImports, preserveModules, inputOptions) => {
    ...
    if (configManualChunks) {
        ...
        ***REMOVED***
    ***REMOVED***
    return configManualChunks || {***REMOVED***;
***REMOVED***;
const getMinifyInternalExports = (config, format, compact) => { var _a; return (_a = config.minifyInternalExports) !== null && _a !== void 0 ? _a : (compact || format === 'es' || format === 'system'); ***REMOVED***;
...
const getSourcemapBaseUrl = (config) => {
    const { sourcemapBaseUrl ***REMOVED*** = config;
    if (sourcemapBaseUrl) {
        if (isValidUrl(sourcemapBaseUrl)) {
            return sourcemapBaseUrl;
        ***REMOVED***
        return error(errInvalidOption('output.sourcemapBaseUrl', 'outputsourcemapbaseurl', `must be a valid URL, received ${JSON.stringify(sourcemapBaseUrl)***REMOVED***`));
    ***REMOVED***
***REMOVED***;
...
async function rollupInternal(rawInputOptions, watcher) {
    const { options: inputOptions, unsetOptions: unsetInputOptions ***REMOVED*** = await getInputOptions(rawInputOptions, watcher !== null);
...
async function getInputOptions(rawInputOptions, watchMode) {
    ...
    ***REMOVED***
    ...
    const { options, unsetOptions ***REMOVED*** = normalizeInputOptions(await rawPlugins.reduce(applyOptionHook(watchMode), Promise.resolve(rawInputOptions)));
...
function applyOptionHook(watchMode) {
    return async (inputOptions, plugin) => {
        ...
        return ((await handler.call({ meta: { rollupVersion: version$1, watchMode ***REMOVED*** ***REMOVED***, await inputOptions)) || inputOptions);
    ***REMOVED***;
***REMOVED***
function normalizePlugins(plugins, anonymousPrefix) {
    plugins.forEach((plugin, index) => {
        if (!plugin.name) {
            plugin.name = `${anonymousPrefix***REMOVED***${index + 1***REMOVED***`;
        ***REMOVED***
    ***REMOVED***);
***REMOVED***
function handleGenerateWrite(isWrite, inputOptions, unsetInputOptions, rawOutputOptions, graph) {
    const { options: outputOptions, outputPluginDriver, unsetOptions ***REMOVED*** = getOutputOptionsAndPluginDriver(rawOutputOptions, graph.pluginDriver, inputOptions, unsetInputOptions);
...
function getOutputOptions(inputOptions, unsetInputOptions, rawOutputOptions, outputPluginDriver) {
    return normalizeOutputOptions(outputPluginDriver.hookReduceArg0Sync('outputOptions', [rawOutputOptions.output || rawOutputOptions], (outputOptions, result) => result || outputOptions, pluginContext => {
        ...
        ***REMOVED***;
    ***REMOVED***), inputOptions, unsetInputOptions);
***REMOVED***
...
***REMOVED***)(SortingFileType || (SortingFileType = {***REMOVED***));
...
async function writeOutputFile(outputFile, outputOptions) {
    ...
    await promises.mkdir(dirname(fileName), { recursive: true ***REMOVED***);
    ...
    ***REMOVED***
        ...
        if (outputOptions.sourcemap && outputFile.map) {
            ...
            ***REMOVED***
                ...
                const { sourcemapBaseUrl ***REMOVED*** = outputOptions;
                const sourcemapFileName = `${basename(outputFile.fileName)***REMOVED***.map`;
                url = sourcemapBaseUrl
                    ? new URL(sourcemapFileName, sourcemapBaseUrl).toString()
                    : sourcemapFileName;
                writeSourceMapPromise = promises.writeFile(`${fileName***REMOVED***.map`, outputFile.map.toString());
            ***REMOVED***
            if (outputOptions.sourcemap !== 'hidden') {
                source += `//# ${SOURCEMAPPING_URL***REMOVED***=${url***REMOVED***\n`;
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***
...
async function loadFsEvents() {
    try {
        ({ default: fsEvents ***REMOVED*** = await import('fsevents'));
    ***REMOVED***
    catch (err) {
...
const fseventsImporter = /*#__PURE__*/Object.defineProperty({
  ...
  loadFsEvents,
  getFsEvents
***REMOVED***, Symbol.toStringTag, { value: 'Module' ***REMOVED***);
...
function watch(configs) {
    ...
    ***REMOVED***
    loadFsEvents()
        ...
        .then(({ Watcher ***REMOVED***) => new Watcher(watchConfigs, emitter));
    return emitter;
***REMOVED***

export { createFilter, defaultOnWarn, defineConfig, ensureArray, fseventsImporter, generatedCodePresets, getAugmentedNamespace, objectifyOption, objectifyOptionWithPresets, picomatch$1 as picomatch, rollup, rollupInternal, treeshakePresets, version$1 as version, warnUnknownOptions, watch ***REMOVED***;
