...
const JAKE_CMD = `${PROJECT_DIR***REMOVED***/bin/cli.js`;

let assert = require('assert');
let exec = require('child_process').execSync;
let fs = require('fs');
let { Rule ***REMOVED*** = require(`${PROJECT_DIR***REMOVED***/lib/rule`);
let { rmRf ***REMOVED*** = require(`${PROJECT_DIR***REMOVED***/lib/jake`);
...
suite('rule', function () {
  ...
  ***REMOVED***);
  ...
  test('rule w/o pattern', function () {
    let out = exec( `${JAKE_CMD***REMOVED*** -q  tmp`).toString().trim();
    let output = [
      "tmp_dep2.c task"
      , "tmp_dep1.c task"
  ...
  ***REMOVED***);
  ...
  test('rule w pattern w/o folder w/o namespace', function () {
    let out = exec( `${JAKE_CMD***REMOVED***  -q  tmp_p`).toString().trim();
    let output = [
      "tmp_dep2.c task"
      , "tmp_dep1.c task"
  ...
  ***REMOVED***);
  ...
  test('rule w pattern w folder w/o namespace', function () {
    let out = exec( `${JAKE_CMD***REMOVED***  -q  tmp_pf`).toString().trim();
    let output = [
  ...
  ***REMOVED***);
  ...
  test.skip('rule w pattern w folder w namespace', function () {
    let out = exec( `${JAKE_CMD***REMOVED*** -q   tmp_ns`).toString().trim();
    let output = [
  ...
  ***REMOVED***);
  ...
  test.skip('rule w chain w pattern w folder w namespace', function () {
    let out = exec( `${JAKE_CMD***REMOVED*** -q tmp_cr`).toString().trim();
    let output = [
      "chainrule init task"
  ...
  ***REMOVED***);
  ...
  ['precedence', 'regexPattern', 'sourceFunction'].forEach(function (key) {
    ...
    test('rule with source file not created yet (' + key  + ')', function () {
      ...
      rmRf('foo.txt', {silent: true***REMOVED***);
      rmRf('foo.html', {silent: true***REMOVED***);
      try {
        exec(`${JAKE_CMD***REMOVED***  ` + key + ':test');
      ***REMOVED***
      catch(err) {
    ...
    ***REMOVED***);
    ...
    test('rule with source file now created (' + key  + ')', function () {
      ...
      let out = exec(`${JAKE_CMD***REMOVED*** -q  ` + key + ':test').toString().trim();
    ...
    ***REMOVED***);
    ...
    test('rule with source file modified (' + key  + ')', function (next) {
      setTimeout(function () {
        ...
        //next();
        cleanUpAndNext(next);
      ***REMOVED***, 1000); // Wait to do the touch to ensure mod-time is different
    ***REMOVED***);
