'use strict';Object.defineProperty(exports, "__esModule", { value: true ***REMOVED***);exports['default'] =
...
function stringifyReplacerFn(_, value) {if (typeof value === 'function') {return String(value);***REMOVED***return value;***REMOVED*** /**
                                                                                                                * don't hold full context object in memory, just grab what we need.
                                                                                                                * also calculate a cacheKey, where parts of the cacheKey hash are memoized
                                                                                                                */function childContext(path, context) {var settings = context.settings,parserOptions = context.parserOptions,parserPath = context.parserPath,languageOptions = context.languageOptions;if (JSON.stringify(settings) !== prevSettings) {settingsHash = (0, _hash.hashObject)({ settings: settings ***REMOVED***).digest('hex');
  ...
  ***REMOVED***
  ...
  if (!parserPath && languageOptions) {
    if (JSON.stringify(languageOptions, stringifyReplacerFn) !== prevOptions) {
      optionsHash = (0, _hash.hashObject)({ languageOptions: languageOptions ***REMOVED***).digest('hex');
  ...
  ***REMOVED*** else {
    if (JSON.stringify(parserOptions) !== prevOptions) {
      optionsHash = (0, _hash.hashObject)({ parserOptions: parserOptions ***REMOVED***).digest('hex');
  ...
  ***REMOVED***
  ...
  return {
    ...
    path: path,
    languageOptions: languageOptions ***REMOVED***;
