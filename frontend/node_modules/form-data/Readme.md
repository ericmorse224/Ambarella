...
var form = new FormData({ maxDataSize: 20971520 ***REMOVED***);
form.append('my_field', 'my value');
...
form.submit({
  host: 'example.com',
  path: '/surelynot.php',
  headers: {'x-test-header': 'test-header-value'***REMOVED***
***REMOVED***, function(err, res) {
  console.log(res.statusCode);
***REMOVED***);
```
...
// provide an object.
form.append( 'my_file', fs.createReadStream('/foo/bar.jpg'), {filename: 'bar.jpg', contentType: 'image/jpeg', knownLength: 19806***REMOVED*** );
```
...
request.post({url:'http://service.com/upload', formData: formData***REMOVED***, function(err, httpResponse, body) {
  if (err) {
...
fetch('http://example.com', { method: 'POST', body: form ***REMOVED***)
    .then(function(res) {
        return res.json();
