...
const printCloseTo = (receivedDiff, expectedDiff, precision, isNot) => {
  ...
  return (
    `Expected precision:  ${isNot ? '    ' : ''***REMOVED***  ${(0,
    _jestMatcherUtils.stringify)(precision)***REMOVED***\n` +
    `Expected difference: ${isNot ? 'not ' : ''***REMOVED***< ${(0,
    _jestMatcherUtils.EXPECTED_COLOR)(expectedDiffString)***REMOVED***\n` +
    `Received difference: ${isNot ? '    ' : ''***REMOVED***  ${(0,
    _jestMatcherUtils.RECEIVED_COLOR)(receivedDiffString)***REMOVED***`
  );
***REMOVED***;
    ...
    ? printConstructorName(label, received, true, false) +
      ` ${
        Object.getPrototypeOf(received) === expected
          ? 'extends'
          : 'extends … extends'
      ***REMOVED*** ${(0, _jestMatcherUtils.EXPECTED_COLOR)(expected.name)***REMOVED***` +
      '\n'
    : printConstructorName(label, received, false, false) + '\n';
...
const printConstructorName = (label, constructor, isNot, isExpected) =>
  typeof constructor.name !== 'string'
    ? `${label***REMOVED*** name is not a string`
    : constructor.name.length === 0
    ? `${label***REMOVED*** name is an empty string`
    : `${label***REMOVED***: ${!isNot ? '' : isExpected ? 'not ' : '    '***REMOVED***${
        isExpected
