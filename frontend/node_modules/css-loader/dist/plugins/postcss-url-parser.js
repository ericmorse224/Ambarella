...
function shouldHandleURL(url, declaration, result, options) {
  if (url.length === 0) {
    result.warn(`Unable to find uri in '${declaration.toString()***REMOVED***'`, {
      node: declaration
    ***REMOVED***);
    return {
      requestable: false,
    ...
    if (isUrlFunc.test(valueNode.value)) {
      ...
      ***REMOVED*** = valueNode;
      ...
      const {
        requestable,
        needResolve
      ***REMOVED*** = shouldHandleURL(url, declaration, result, options);
      ...
      ***REMOVED***);
      ...
      // eslint-disable-next-line consistent-return
      return false;
    ***REMOVED*** else if (isImageSetFunc.test(valueNode.value)) {
          ...
          ***REMOVED***
          const resolvedDeclarations = await Promise.all(parsedDeclarations.map(async parsedDeclaration => {
            ...
            ***REMOVED***
            ...
            let hash = query ? "?" : "";
            hash += hashOrQuery ? `#${hashOrQuery***REMOVED***` : "";
            const {
              resolver,
              rootContext
            ***REMOVED*** = options;
          ...
          for (let index = 0; index <= resolvedDeclarations.length - 1; index++) {
            ...
            ***REMOVED***
            const {
              ...
              prefix
            ***REMOVED*** = item;
            const newUrl = prefix ? `${prefix***REMOVED***!${url***REMOVED***` : url;
            let importName = urlToNameMap.get(newUrl);
            if (!importName) {
              importName = `___CSS_LOADER_URL_IMPORT_${urlToNameMap.size***REMOVED***___`;
            ...
            ***REMOVED***);
            ...
            if (!replacementName) {
              replacementName = `___CSS_LOADER_URL_REPLACEMENT_${urlToReplacementMap.size***REMOVED***___`;
