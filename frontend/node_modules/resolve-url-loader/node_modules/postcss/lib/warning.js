"use strict";

exports.__esModule = true;
exports.default = void 0;

/**
 * Represents a plugin’s warning. It can be created using {@link Node#warn***REMOVED***.
 *
 * @example
 * if (decl.important) {
 *   decl.warn(result, 'Avoid !important', { word: '!important' ***REMOVED***)
 * ***REMOVED***
 */
var Warning = /*#__PURE__*/function () {
  /**
   * @param {string***REMOVED*** text        Warning message.
   * @param {Object***REMOVED*** [opts]      Warning options.
   * @param {Node***REMOVED***   opts.node   CSS node that caused the warning.
   * @param {string***REMOVED*** opts.word   Word in CSS source that caused the warning.
   * @param {number***REMOVED*** opts.index  Index in CSS node string that caused
   *                             the warning.
   * @param {string***REMOVED*** opts.plugin Name of the plugin that created
   *                             this warning. {@link Result#warn***REMOVED*** fills
   *                             this property automatically.
   */
  function Warning(text, opts) {
    if (opts === void 0) {
      opts = {***REMOVED***;
    ***REMOVED***

    /**
     * Type to filter warnings from {@link Result#messages***REMOVED***.
     * Always equal to `"warning"`.
     *
     * @type {string***REMOVED***
     *
     * @example
     * const nonWarning = result.messages.filter(i => i.type !== 'warning')
     */
    this.type = 'warning';
    /**
     * The warning message.
     *
     * @type {string***REMOVED***
     *
     * @example
     * warning.text //=> 'Try to avoid !important'
     */

    this.text = text;

    if (opts.node && opts.node.source) {
      var pos = opts.node.positionBy(opts);
      /**
       * Line in the input file with this warning’s source.
       * @type {number***REMOVED***
       *
       * @example
       * warning.line //=> 5
       */

      this.line = pos.line;
      /**
       * Column in the input file with this warning’s source.
       *
       * @type {number***REMOVED***
       *
       * @example
       * warning.column //=> 6
       */

      this.column = pos.column;
    ***REMOVED***

    for (var opt in opts) {
      this[opt] = opts[opt];
    ***REMOVED***
  ***REMOVED***
  /**
   * Returns a warning position and message.
   *
   * @example
   * warning.toString() //=> 'postcss-lint:a.css:10:14: Avoid !important'
   *
   * @return {string***REMOVED*** Warning position and message.
   */


  var _proto = Warning.prototype;

  _proto.toString = function toString() {
    if (this.node) {
      return this.node.error(this.text, {
        plugin: this.plugin,
        index: this.index,
        word: this.word
      ***REMOVED***).message;
    ***REMOVED***

    if (this.plugin) {
      return this.plugin + ': ' + this.text;
    ***REMOVED***

    return this.text;
  ***REMOVED***
  /**
   * @memberof Warning#
   * @member {string***REMOVED*** plugin The name of the plugin that created
   *                         it will fill this property automatically.
   *                         this warning. When you call {@link Node#warn***REMOVED***
   *
   * @example
   * warning.plugin //=> 'postcss-important'
   */

  /**
   * @memberof Warning#
   * @member {Node***REMOVED*** node Contains the CSS node that caused the warning.
   *
   * @example
   * warning.node.toString() //=> 'color: white !important'
   */
  ;

  return Warning;
***REMOVED***();

var _default = Warning;
exports.default = _default;
module.exports = exports.default;
//# sourceMappingURL=data:application/json;charset=utf8;base64,
