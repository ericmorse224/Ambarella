    ...
    return value.replace(/(calc|min|max|clamp)\(.+\)/g, (match)=>{
        ...
        ***REMOVED***
        for(let i = 0; i < match.length; i++){
            ...
            ***REMOVED*** else if (preventFormattingKeywords.some((keyword)=>peek(keyword))) {
                let keyword = preventFormattingKeywords.find((keyword)=>peek(keyword));
                result += keyword;
                i += keyword.length - 1;
            ***REMOVED*** else if (preventFormattingInFunctions.some((fn)=>peek(fn))) {
                result += consumeUntil([
                    ")"
                ]);
            ***REMOVED*** else if (peek("[")) {
                result += consumeUntil([
                    "]"
                ]);
            ***REMOVED*** else if ([
...
function length(value) {
    return value === "0" || new RegExp(`^[+-]?[0-9]*\.?[0-9]+(?:[eE][+-]?[0-9]+)?${lengthUnitsPattern***REMOVED***$`).test(value) || isCSSFunction(value);
***REMOVED***
...
function color(value) {
    ...
    let result = (0, _splitAtTopLevelOnly.splitAtTopLevelOnly)(value, "_").every((part)=>{
        ...
        if ((0, _color.parseColor)(part, {
            loose: true
        ***REMOVED***) !== null) return colors++, true;
        return false;
    ***REMOVED***);
