 ...
 * because minifiers treat quoted accessors differently.  e.g. import { a ***REMOVED*** from "./module"; a["b"] vs a.b
 * @param {string[]***REMOVED*** untrimmedIds chained ids
 * @param {Range***REMOVED*** untrimmedRange range encompassing allIds
 * @param {Range[] | undefined***REMOVED*** ranges cumulative range of ids for each of allIds
 * @param {ModuleGraph***REMOVED*** moduleGraph moduleGraph
 * @param {Dependency***REMOVED*** dependency dependency
 * @returns {{trimmedIds: string[], trimmedRange: Range***REMOVED******REMOVED*** computed trimmed ids and cumulative range of those ids
 */
module.exports.getTrimmedIdsAndRange = (
	...
	if (trimmedIds.length !== untrimmedIds.length) {
		...
		if (idx < 0 || idx >= /** @type {Range[]***REMOVED*** */ (ranges).length) {
		...
		***REMOVED*** else {
			trimmedRange = /** @type {Range[]***REMOVED*** */ (ranges)[idx];
		***REMOVED***
	***REMOVED***

	return { trimmedIds, trimmedRange ***REMOVED***;
***REMOVED***;
 ...
 * @param {string[]***REMOVED*** ids untrimmed ids
 * @param {ModuleGraph***REMOVED*** moduleGraph moduleGraph
 * @param {Dependency***REMOVED*** dependency dependency
 * @returns {string[]***REMOVED*** trimmed ids
 */
function trimIdsToThoseImported(ids, moduleGraph, dependency) {
	/** @type {string[]***REMOVED*** */
	let trimmedIds = [];
	let currentExportsInfo = moduleGraph.getExportsInfo(
		/** @type {Module***REMOVED*** */ (moduleGraph.getModule(dependency))
	);
	...
	***REMOVED***
	// Never trim to nothing.  This can happen for invalid imports (e.g. import { notThere ***REMOVED*** from "./module", or import { anything ***REMOVED*** from "./missingModule")
