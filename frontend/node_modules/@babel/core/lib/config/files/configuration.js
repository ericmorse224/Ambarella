...
function* readConfigCode(filepath, data) {
  ...
  if (typeof options === "function") {
    ...
    ***REMOVED*** = yield* runConfig(options, data));
  ***REMOVED***
  if (!options || typeof options !== "object" || Array.isArray(options)) {
    throw new _configError.default(`Configuration should be an exported JavaScript object.`, filepath);
  ***REMOVED***
  if (typeof options.then === "function") {
    options.catch == null || options.catch(() => {***REMOVED***);
...
const readConfigJSON5 = (0, _utils.makeStaticFileCache)((filepath, content) => {
  ...
  ***REMOVED*** catch (err) {
    throw new _configError.default(`Error while parsing config - ${err.message***REMOVED***`, filepath);
  ***REMOVED***
  if (!options) throw new _configError.default(`No config detected`, filepath);
  if (typeof options !== "object") {
    throw new _configError.default(`Config returned typeof ${typeof options***REMOVED***`, filepath);
  ***REMOVED***
...
function* loadOneConfig(names, dirname, envName, caller, previousConfig = null) {
  ...
  const config = configs.reduce((previousConfig, config) => {
    if (config && previousConfig) {
      throw new _configError.default(`Multiple configuration files found. Please remove one:\n` + ` - ${_path().basename(previousConfig.filepath)***REMOVED***\n` + ` - ${config.filepath***REMOVED***\n` + `from ${dirname***REMOVED***`);
    ***REMOVED***
    return config || previousConfig;
  ***REMOVED***, previousConfig);
  if (config) {
...
function* loadConfig(name, dirname, envName, caller) {
  const filepath = (((v, w) => (v = v.split("."), w = w.split("."), +v[0] > +w[0] || v[0] == w[0] && +v[1] >= +w[1]))(process.versions.node, "8.9") ? require.resolve : (r, {
    paths: [b]
  ***REMOVED***, M = require("module")) => {
    let f = M._findPath(r, M._nodeModulePaths(b).concat(b));
    if (f) return f;
    f = new Error(`Cannot resolve module '${r***REMOVED***'`);
...
function* resolveShowConfigPath(dirname) {
  ...
  if (targetPath != null) {
    ...
    if (!stats.isFile()) {
      throw new Error(`${absolutePath***REMOVED***: BABEL_SHOW_CONFIG_FOR must refer to a regular file, directories are not supported.`);
    ***REMOVED***
    return absolutePath;
  ***REMOVED***
  return null;
