...
parse('{a: String, b: Number***REMOVED***', '{a: str, b: 2***REMOVED***'); // {a: 'str', b: 2***REMOVED***

// at the top level, you can ommit surrounding delimiters
parse('[Number]', '1,2,3');                      // [1,2,3]
parse('(String, Boolean)', 'hi, false');         // ['hi', false]
parse('{a: String, b: Number***REMOVED***', 'a: str, b: 2'); // {a: 'str', b: 2***REMOVED***

// wildcard - auto choose type
parse('*', '[hi,(null,[42]),{k: true***REMOVED***]'); // ['hi', [null, [42]], {k: true***REMOVED***]
```
## Usage
...
* `{key1: val1, key2: val2, ...***REMOVED***` is an Object, and the casting procedure is recursively applied to each property. Eg. `{a: 1, b: 2***REMOVED***` is `{a: 1, b: 2***REMOVED***`.
* Any test which does not fall under the above, and which does not contain special characters (`[``]``(``)``{``***REMOVED***``:``,`) is a string, eg. `$12- blah` is `"$12- blah"`.
...
* If a String is expected, and only a String, all characters of the input (including any special ones) will become part of the output. Eg. `[({***REMOVED***)]` is `"[({***REMOVED***)]"`, and `"hi"` is `'"hi"'`.
...
* If an Object is expected, and it is the root node (at the top level), the opening `{` and closing `***REMOVED***` can be omitted. Eg `a: 1, b: 2` is `{a: 1, b: 2***REMOVED***`.
...
```js
parse('RegExp', 're', {explicit: false***REMOVED***);          // /re/
parse('RegExp', 're', {explicit: true***REMOVED***);           // Error: ... does not type check...
parse('RegExp | String', 're', {explicit: true***REMOVED***);  // 're'
```
...
An `Object`. Empty `{***REMOVED***` by default.
...
var options = {
  customTypes: {
    Even: {
      ...
      ***REMOVED***,
      cast: function (x) {
        return {type: 'Just', value: parseInt(x)***REMOVED***;
      ***REMOVED***
    ***REMOVED***
  ***REMOVED***
***REMOVED***
    ...
    Person: {
      ...
      ***REMOVED***,
      cast: function (value, options, typesCast) {
        var name, age;
        if ({***REMOVED***.toString.call(value).slice(8, -1) !== 'Object') {
          return {type: 'Nothing'***REMOVED***;
        ***REMOVED***
        name = typesCast(value.name, [{type: 'String'***REMOVED***], options);
        age = typesCast(value.age, [{type: 'Numger'***REMOVED***], options);
        return {type: 'Just', value: new Person(name, age)***REMOVED***;
    ***REMOVED***
  ***REMOVED***
***REMOVED***
parse('Person', '{name: Laura, age: 25***REMOVED***', options); // Person {name: 'Laura', age: 25***REMOVED***
```
...
`cast` is a function which receives three arguments, the value under question, options, and the typesCast function. In `cast`, attempt to cast the value into the specified type. If you are successful, return an object in the format `{type: 'Just', value: CAST-VALUE***REMOVED***`, if you know it won't work, return `{type: 'Nothing'***REMOVED***`.  You can use the `typesCast` function to cast any child values. Remember to pass `options` to it. In your function you can also check for `options.explicit` and act accordingly.
