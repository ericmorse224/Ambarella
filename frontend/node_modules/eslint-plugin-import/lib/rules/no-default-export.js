'use strict';var _contextCompat = require('eslint-module-utils/contextCompat');

var _docsUrl = require('../docsUrl');var _docsUrl2 = _interopRequireDefault(_docsUrl);
var _sourceType = require('../core/sourceType');var _sourceType2 = _interopRequireDefault(_sourceType);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { 'default': obj ***REMOVED***;***REMOVED***

module.exports = {
  meta: {
    type: 'suggestion',
    docs: {
      category: 'Style guide',
      description: 'Forbid default exports.',
      url: (0, _docsUrl2['default'])('no-default-export') ***REMOVED***,

    schema: [] ***REMOVED***,


  create: function () {function create(context) {
      // ignore non-modules
      if ((0, _sourceType2['default'])(context) !== 'module') {
        return {***REMOVED***;
      ***REMOVED***

      var preferNamed = 'Prefer named exports.';
      var noAliasDefault = function () {function noAliasDefault(_ref) {var local = _ref.local;return 'Do not alias `' + String(local.name) + '` as `default`. Just export `' + String(local.name) + '` itself instead.';***REMOVED***return noAliasDefault;***REMOVED***();

      return {
        ExportDefaultDeclaration: function () {function ExportDefaultDeclaration(node) {var _ref2 =
            (0, _contextCompat.getSourceCode)(context).getFirstTokens(node)[1] || {***REMOVED***,loc = _ref2.loc;
            context.report({ node: node, message: preferNamed, loc: loc ***REMOVED***);
          ***REMOVED***return ExportDefaultDeclaration;***REMOVED***(),

        ExportNamedDeclaration: function () {function ExportNamedDeclaration(node) {
            node.specifiers.
            filter(function (specifier) {return (specifier.exported.name || specifier.exported.value) === 'default';***REMOVED***).
            forEach(function (specifier) {var _ref3 =
              (0, _contextCompat.getSourceCode)(context).getFirstTokens(node)[1] || {***REMOVED***,loc = _ref3.loc;
              if (specifier.type === 'ExportDefaultSpecifier') {
                context.report({ node: node, message: preferNamed, loc: loc ***REMOVED***);
              ***REMOVED*** else if (specifier.type === 'ExportSpecifier') {
                context.report({ node: node, message: noAliasDefault(specifier), loc: loc ***REMOVED***);
              ***REMOVED***
            ***REMOVED***);
          ***REMOVED***return ExportNamedDeclaration;***REMOVED***() ***REMOVED***;

    ***REMOVED***return create;***REMOVED***() ***REMOVED***;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,