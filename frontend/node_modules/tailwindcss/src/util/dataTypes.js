import { parseColor ***REMOVED*** from './color'
import { parseBoxShadowValue ***REMOVED*** from './parseBoxShadowValue'
import { splitAtTopLevelOnly ***REMOVED*** from './splitAtTopLevelOnly'
...
function isCSSFunction(value) {
  return cssFunctions.some((fn) => new RegExp(`^${fn***REMOVED***\\(.*\\)`).test(value))
***REMOVED***
      ...
      ***REMOVED***
      ...
      if (match.length > 2) {
        ...
        if (
          ...
          return `="${match.slice(1, -2)***REMOVED***" ${match[match.length - 1]***REMOVED***`
        ***REMOVED***
      ***REMOVED***

      return `="${match.slice(1)***REMOVED***"`
    ***REMOVED***)
  ***REMOVED***
  return value
***REMOVED***
...
let lengthUnitsPattern = `(?:${lengthUnits.join('|')***REMOVED***)`
export function length(value) {
  return (
    value === '0' ||
    new RegExp(`^[+-]?[0-9]*\.?[0-9]+(?:[eE][+-]?[0-9]+)?${lengthUnitsPattern***REMOVED***$`).test(value) ||
    isCSSFunction(value)
  )
***REMOVED***
...
export function color(value) {
  ...
  let result = splitAtTopLevelOnly(value, '_').every((part) => {
    ...
    if (parseColor(part, { loose: true ***REMOVED***) !== null) return colors++, true
...
export function gradient(value) {
  ...
  for (let type of gradientTypes) {
    if (value.startsWith(`${type***REMOVED***(`)) {
      return true
    ***REMOVED***
  ***REMOVED***
  return false
