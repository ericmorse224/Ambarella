...
async function getFileManifestEntries({ additionalManifestEntries, dontCacheBustURLsMatching, globDirectory, globFollow, globIgnores, globPatterns = [], globStrict, manifestTransforms, maximumFileSizeToCacheInBytes, modifyURLPrefix, templatedURLs, ***REMOVED***) {
    const warnings = [];
    const allFileDetails = new Map();
    try {
        for (const globPattern of globPatterns) {
            const { globbedFileDetails, warning ***REMOVED*** = (0, get_file_details_1.getFileDetails)({
                globDirectory,
                globFollow,
                globIgnores,
                globPattern,
            ...
            if (Array.isArray(dependencies)) {
                const details = dependencies.reduce((previous, globPattern) => {
                    ...
                    ***REMOVED***
                        ...
                        const debugObj = {***REMOVED***;
                        debugObj[url] = dependencies;
                        throw new Error(`${errors_1.errors['bad-template-urls-asset']***REMOVED*** ` +
                            `'${globPattern***REMOVED***' from '${JSON.stringify(debugObj)***REMOVED***':\n` +
                            `${error instanceof Error ? error.toString() : ''***REMOVED***`);
                    ***REMOVED***
                ***REMOVED***, []);
                if (details.length === 0) {
                    throw new Error(`${errors_1.errors['bad-template-urls-asset']***REMOVED*** The glob ` +
