"use strict";

exports.__esModule = true;
exports.default = void 0;

var _warning = _interopRequireDefault(require("./warning"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj ***REMOVED***; ***REMOVED***

function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); ***REMOVED*** ***REMOVED***

function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); return Constructor; ***REMOVED***

/**
 * Provides the result of the PostCSS transformations.
 *
 * A Result instance is returned by {@link LazyResult#then***REMOVED***
 * or {@link Root#toResult***REMOVED*** methods.
 *
 * @example
 * postcss([autoprefixer]).process(css).then(result => {
 *  console.log(result.css)
 * ***REMOVED***)
 *
 * @example
 * const result2 = postcss.parse(css).toResult()
 */
var Result = /*#__PURE__*/function () {
  /**
   * @param {Processor***REMOVED*** processor Processor used for this transformation.
   * @param {Root***REMOVED***      root      Root node after all transformations.
   * @param {processOptions***REMOVED*** opts Options from the {@link Processor#process***REMOVED***
   *                              or {@link Root#toResult***REMOVED***.
   */
  function Result(processor, root, opts) {
    /**
     * The Processor instance used for this transformation.
     *
     * @type {Processor***REMOVED***
     *
     * @example
     * for (const plugin of result.processor.plugins) {
     *   if (plugin.postcssPlugin === 'postcss-bad') {
     *     throw 'postcss-good is incompatible with postcss-bad'
     *   ***REMOVED***
     * ***REMOVED***)
     */
    this.processor = processor;
    /**
     * Contains messages from plugins (e.g., warnings or custom messages).
     * Each message should have type and plugin properties.
     *
     * @type {Message[]***REMOVED***
     *
     * @example
     * postcss.plugin('postcss-min-browser', () => {
     *   return (root, result) => {
     *     const browsers = detectMinBrowsersByCanIUse(root)
     *     result.messages.push({
     *       type: 'min-browser',
     *       plugin: 'postcss-min-browser',
     *       browsers
     *     ***REMOVED***)
     *   ***REMOVED***
     * ***REMOVED***)
     */

    this.messages = [];
    /**
     * Root node after all transformations.
     *
     * @type {Root***REMOVED***
     *
     * @example
     * root.toResult().root === root
     */

    this.root = root;
    /**
     * Options from the {@link Processor#process***REMOVED*** or {@link Root#toResult***REMOVED*** call
     * that produced this Result instance.
     *
     * @type {processOptions***REMOVED***
     *
     * @example
     * root.toResult(opts).opts === opts
     */

    this.opts = opts;
    /**
     * A CSS string representing of {@link Result#root***REMOVED***.
     *
     * @type {string***REMOVED***
     *
     * @example
     * postcss.parse('a{***REMOVED***').toResult().css //=> "a{***REMOVED***"
     */

    this.css = undefined;
    /**
     * An instance of `SourceMapGenerator` class from the `source-map` library,
     * representing changes to the {@link Result#root***REMOVED*** instance.
     *
     * @type {SourceMapGenerator***REMOVED***
     *
     * @example
     * result.map.toJSON() //=> { version: 3, file: 'a.css', … ***REMOVED***
     *
     * @example
     * if (result.map) {
     *   fs.writeFileSync(result.opts.to + '.map', result.map.toString())
     * ***REMOVED***
     */

    this.map = undefined;
  ***REMOVED***
  /**
   * Returns for @{link Result#css***REMOVED*** content.
   *
   * @example
   * result + '' === result.css
   *
   * @return {string***REMOVED*** String representing of {@link Result#root***REMOVED***.
   */


  var _proto = Result.prototype;

  _proto.toString = function toString() {
    return this.css;
  ***REMOVED***
  /**
   * Creates an instance of {@link Warning***REMOVED*** and adds it
   * to {@link Result#messages***REMOVED***.
   *
   * @param {string***REMOVED*** text        Warning message.
   * @param {Object***REMOVED*** [opts]      Warning options.
   * @param {Node***REMOVED***   opts.node   CSS node that caused the warning.
   * @param {string***REMOVED*** opts.word   Word in CSS source that caused the warning.
   * @param {number***REMOVED*** opts.index  Index in CSS node string that caused
   *                             the warning.
   * @param {string***REMOVED*** opts.plugin Name of the plugin that created
   *                             this warning. {@link Result#warn***REMOVED*** fills
   *                             this property automatically.
   *
   * @return {Warning***REMOVED*** Created warning.
   */
  ;

  _proto.warn = function warn(text, opts) {
    if (opts === void 0) {
      opts = {***REMOVED***;
    ***REMOVED***

    if (!opts.plugin) {
      if (this.lastPlugin && this.lastPlugin.postcssPlugin) {
        opts.plugin = this.lastPlugin.postcssPlugin;
      ***REMOVED***
    ***REMOVED***

    var warning = new _warning.default(text, opts);
    this.messages.push(warning);
    return warning;
  ***REMOVED***
  /**
     * Returns warnings from plugins. Filters {@link Warning***REMOVED*** instances
     * from {@link Result#messages***REMOVED***.
     *
     * @example
     * result.warnings().forEach(warn => {
     *   console.warn(warn.toString())
     * ***REMOVED***)
     *
     * @return {Warning[]***REMOVED*** Warnings from plugins.
     */
  ;

  _proto.warnings = function warnings() {
    return this.messages.filter(function (i) {
      return i.type === 'warning';
    ***REMOVED***);
  ***REMOVED***
  /**
   * An alias for the {@link Result#css***REMOVED*** property.
   * Use it with syntaxes that generate non-CSS output.
   *
   * @type {string***REMOVED***
   *
   * @example
   * result.css === result.content
   */
  ;

  _createClass(Result, [{
    key: "content",
    get: function get() {
      return this.css;
    ***REMOVED***
  ***REMOVED***]);

  return Result;
***REMOVED***();

var _default = Result;
/**
 * @typedef  {object***REMOVED*** Message
 * @property {string***REMOVED*** type   Message type.
 * @property {string***REMOVED*** plugin Source PostCSS plugin name.
 */

exports.default = _default;
module.exports = exports.default;
//# sourceMappingURL=data:application/json;charset=utf8;base64,
