...
var enumValues = { 'enum': ['always', 'ignorePackages', 'never'] ***REMOVED***;
var patternProperties = {
  type: 'object',
  patternProperties: { '.*': enumValues ***REMOVED*** ***REMOVED***;

var properties = {
  type: 'object',
  properties: {
    pattern: patternProperties,
    checkTypeImports: { type: 'boolean' ***REMOVED***,
    ignorePackages: { type: 'boolean' ***REMOVED*** ***REMOVED*** ***REMOVED***;
...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      url: (0, _docsUrl2['default'])('extensions') ***REMOVED***,
  ...
  create: function () {function create(context) {
      ...
      ***REMOVED***
      ...
      function isExternalRootModule(file) {
        if (file === '.' || file === '..') {return false;***REMOVED***
        var slashCount = file.split('/').length - 1;

        if (slashCount === 0) {return true;***REMOVED***
        if ((0, _importType.isScoped)(file) && slashCount <= 1) {return true;***REMOVED***
        return false;
      ***REMOVED***

      function checkFileExtension(source, node) {
        // bail if the declaration doesn't have a source, e.g. "export { foo ***REMOVED***;", or if it's only partially typed like in an editor
        if (!source || !source.value) {return;***REMOVED***

        var importPathWithQueryString = source.value;

        // don't enforce anything on builtins
        if ((0, _importType.isBuiltIn)(importPathWithQueryString, context.settings)) {return;***REMOVED***

        var importPath = importPathWithQueryString.replace(/\?(.*)$/, '');

        // don't enforce in root external packages as they may have names with `.js`.
        // Like `import Decimal from decimal.js`)
        if (isExternalRootModule(importPath)) {return;***REMOVED***
        ...
        if (!extension || !importPath.endsWith('.' + String(extension))) {
          ...
          if (!props.checkTypeImports && (node.importKind === 'type' || node.exportKind === 'type')) {return;***REMOVED***
          ...
          if (extensionRequired && !extensionForbidden) {
            context.report({
              ...
              extension ? '"' + String(extension) + '" ' : '') + 'for "' + String(importPathWithQueryString) + '"' ***REMOVED***);
        ...
        ***REMOVED*** else if (extension) {
          if (isUseOfExtensionForbidden(extension) && isResolvableWithoutExtension(importPath)) {
            context.report({
              node: source,
              message: 'Unexpected use of file extension "' + String(extension) + '" for "' + String(importPathWithQueryString) + '"' ***REMOVED***);

          ***REMOVED***
        ***REMOVED***
      ***REMOVED***

      return (0, _moduleVisitor2['default'])(checkFileExtension, { commonjs: true ***REMOVED***);
