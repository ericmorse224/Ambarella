'use strict';Object.defineProperty(exports, "__esModule", { value: true ***REMOVED***);exports.






























isEsModuleInterop = isEsModuleInterop;var _path = require('path');var _tsconfigLoader = require('tsconfig-paths/lib/tsconfig-loader');var _hash = require('eslint-module-utils/hash');var ts = void 0;var tsconfigCache = new Map();function readTsConfig(context) {var tsconfigInfo = (0, _tsconfigLoader.tsConfigLoader)({ cwd: context.parserOptions && context.parserOptions.tsconfigRootDir || process.cwd(), getEnv: function () {function getEnv(key) {return process.env[key];***REMOVED***return getEnv;***REMOVED***() ***REMOVED***);try {if (tsconfigInfo.tsConfigPath !== undefined) {// Projects not using TypeScript won't have `typescript` installed.
      if (!ts) {ts = require('typescript');***REMOVED*** // eslint-disable-line import/no-extraneous-dependencies
      var configFile = ts.readConfigFile(tsconfigInfo.tsConfigPath, ts.sys.readFile);return ts.parseJsonConfigFileContent(configFile.config, ts.sys, (0, _path.dirname)(tsconfigInfo.tsConfigPath));***REMOVED******REMOVED*** catch (e) {// Catch any errors
  ***REMOVED***return null;***REMOVED***function isEsModuleInterop(context) {var cacheKey = (0, _hash.hashObject)({ tsconfigRootDir: context.parserOptions && context.parserOptions.tsconfigRootDir ***REMOVED***).digest('hex');
  var tsConfig = tsconfigCache.get(cacheKey);
  if (typeof tsConfig === 'undefined') {
    tsConfig = readTsConfig(context);
    tsconfigCache.set(cacheKey, tsConfig);
  ***REMOVED***

  return tsConfig && tsConfig.options ? tsConfig.options.esModuleInterop : false;
***REMOVED***
//# sourceMappingURL=data:application/json;charset=utf-8;base64,