...
declare class Keyv<Value = any, Options extends Record<string, any> = Record<string, unknown>> extends EventEmitter {
	...
	/** Returns the value. */
	get(key: string, options?: {raw?: false***REMOVED***): Promise<Value | undefined>;
	/** Returns the raw value. */
	get(key: string, options: {raw: true***REMOVED***): Promise<Keyv.DeserializedData<Value> | undefined>;

	/** Returns an array of values. Uses `store.getMany` if it exists, otherwise uses parallel calls to `store.get`. */
	get(key: string[], options?: {raw?: false***REMOVED***): Promise<Array<Value | undefined>>;
	/** Returns an array of raw values. Uses `store.getMany` if it exists, otherwise uses parallel calls to `store.get`. */
	get(key: string[], options: {raw: true***REMOVED***): Promise<Array<Keyv.DeserializedData<Value> | undefined>>;
