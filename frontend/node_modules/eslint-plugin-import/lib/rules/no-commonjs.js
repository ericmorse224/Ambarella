'use strict';




var _contextCompat = require('eslint-module-utils/contextCompat');

var _docsUrl = require('../docsUrl');var _docsUrl2 = _interopRequireDefault(_docsUrl);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { 'default': obj ***REMOVED***;***REMOVED*** /**
                                                                                                                                                                                       * @fileoverview Rule to prefer ES6 to CJS
                                                                                                                                                                                       * @author Jamund Ferguson
                                                                                                                                                                                       */var EXPORT_MESSAGE = 'Expected "export" or "export default"';var IMPORT_MESSAGE = 'Expected "import" instead of "require()"';

function normalizeLegacyOptions(options) {
  if (options.indexOf('allow-primitive-modules') >= 0) {
    return { allowPrimitiveModules: true ***REMOVED***;
  ***REMOVED***
  return options[0] || {***REMOVED***;
***REMOVED***

function allowPrimitive(node, options) {
  if (!options.allowPrimitiveModules) {return false;***REMOVED***
  if (node.parent.type !== 'AssignmentExpression') {return false;***REMOVED***
  return node.parent.right.type !== 'ObjectExpression';
***REMOVED***

function allowRequire(node, options) {
  return options.allowRequire;
***REMOVED***

function allowConditionalRequire(node, options) {
  return options.allowConditionalRequire !== false;
***REMOVED***

function validateScope(scope) {
  return scope.variableScope.type === 'module';
***REMOVED***

// https://github.com/estree/estree/blob/HEAD/es5.md
function isConditional(node) {
  if (
  node.type === 'IfStatement' ||
  node.type === 'TryStatement' ||
  node.type === 'LogicalExpression' ||
  node.type === 'ConditionalExpression')
  {
    return true;
  ***REMOVED***
  if (node.parent) {return isConditional(node.parent);***REMOVED***
  return false;
***REMOVED***

function isLiteralString(node) {
  return node.type === 'Literal' && typeof node.value === 'string' ||
  node.type === 'TemplateLiteral' && node.expressions.length === 0;
***REMOVED***

//------------------------------------------------------------------------------
// Rule Definition
//------------------------------------------------------------------------------

var schemaString = { 'enum': ['allow-primitive-modules'] ***REMOVED***;
var schemaObject = {
  type: 'object',
  properties: {
    allowPrimitiveModules: { type: 'boolean' ***REMOVED***,
    allowRequire: { type: 'boolean' ***REMOVED***,
    allowConditionalRequire: { type: 'boolean' ***REMOVED*** ***REMOVED***,

  additionalProperties: false ***REMOVED***;


module.exports = {
  meta: {
    type: 'suggestion',
    docs: {
      category: 'Module systems',
      description: 'Forbid CommonJS `require` calls and `module.exports` or `exports.*`.',
      url: (0, _docsUrl2['default'])('no-commonjs') ***REMOVED***,


    schema: {
      anyOf: [
      {
        type: 'array',
        items: [schemaString],
        additionalItems: false ***REMOVED***,

      {
        type: 'array',
        items: [schemaObject],
        additionalItems: false ***REMOVED***] ***REMOVED*** ***REMOVED***,





  create: function () {function create(context) {
      var options = normalizeLegacyOptions(context.options);

      return {

        MemberExpression: function () {function MemberExpression(node) {

            // module.exports
            if (node.object.name === 'module' && node.property.name === 'exports') {
              if (allowPrimitive(node, options)) {return;***REMOVED***
              context.report({ node: node, message: EXPORT_MESSAGE ***REMOVED***);
            ***REMOVED***

            // exports.
            if (node.object.name === 'exports') {
              var isInScope = (0, _contextCompat.getScope)(context, node).
              variables.
              some(function (variable) {return variable.name === 'exports';***REMOVED***);
              if (!isInScope) {
                context.report({ node: node, message: EXPORT_MESSAGE ***REMOVED***);
              ***REMOVED***
            ***REMOVED***

          ***REMOVED***return MemberExpression;***REMOVED***(),
        CallExpression: function () {function CallExpression(call) {
            if (!validateScope((0, _contextCompat.getScope)(context, call))) {return;***REMOVED***

            if (call.callee.type !== 'Identifier') {return;***REMOVED***
            if (call.callee.name !== 'require') {return;***REMOVED***

            if (call.arguments.length !== 1) {return;***REMOVED***
            if (!isLiteralString(call.arguments[0])) {return;***REMOVED***

            if (allowRequire(call, options)) {return;***REMOVED***

            if (allowConditionalRequire(call, options) && isConditional(call.parent)) {return;***REMOVED***

            // keeping it simple: all 1-string-arg `require` calls are reported
            context.report({
              node: call.callee,
              message: IMPORT_MESSAGE ***REMOVED***);

          ***REMOVED***return CallExpression;***REMOVED***() ***REMOVED***;


    ***REMOVED***return create;***REMOVED***() ***REMOVED***;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,