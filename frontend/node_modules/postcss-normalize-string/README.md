...
```css
p:after{ content: '\\'string\\' is intact' ***REMOVED***
```
...
```js
var css = 'p:after{content:""***REMOVED***';
console.log(postcss(normalize({preferredQuote: 'single'***REMOVED***)).process(css).css);
//=> p:after{content:''***REMOVED***
```
