...
```js
const { window ***REMOVED*** = new JSDOM(`...`);
// or even
const { document ***REMOVED*** = (new JSDOM(`...`)).window;
```
...
const dom = new JSDOM(`<body>
  <script>document.body.appendChild(document.createElement("hr"));</script>
</body>`, { runScripts: "dangerously" ***REMOVED***);
...
```js
const { window ***REMOVED*** = new JSDOM(``, { runScripts: "outside-only" ***REMOVED***);
...
```js
const window = (new JSDOM(``, { pretendToBeVisual: true ***REMOVED***)).window;
...
***REMOVED***);
const dom = new JSDOM(``, { resources: resourceLoader ***REMOVED***);
```
...
- `userAgent` affects the `User-Agent` header sent, and thus the resulting value for `navigator.userAgent`. It defaults to <code>\`Mozilla/5.0 (${process.platform || "unknown OS"***REMOVED***) AppleWebKit/537.36 (KHTML, like Gecko) jsdom/${jsdomVersion***REMOVED***\`</code>.
...
class CustomResourceLoader extends jsdom.ResourceLoader {
  fetch(url, options) {
    if (options.element) {
      console.log(`Element ${options.element.localName***REMOVED*** is requesting the url ${url***REMOVED***`);
    ***REMOVED***
...
const dom = new JSDOM(``, { virtualConsole ***REMOVED***);
```
...
```js
virtualConsole.on("error", () => { ... ***REMOVED***);
virtualConsole.on("warn", () => { ... ***REMOVED***);
virtualConsole.on("info", () => { ... ***REMOVED***);
virtualConsole.on("dir", () => { ... ***REMOVED***);
...
```js
virtualConsole.sendTo(c, { omitJSDOMErrors: true ***REMOVED***);
```
...
const dom = new JSDOM(``, { cookieJar ***REMOVED***);
```
...
const dom = new JSDOM(
  ...
  { includeNodeLocations: true ***REMOVED***
);
...
console.log(dom.nodeLocation(pEl));      // { startOffset: 0, endOffset: 39, startTag: ..., endTag: ... ***REMOVED***
console.log(dom.nodeLocation(textNode)); // { startOffset: 3, endOffset: 13 ***REMOVED***
console.log(dom.nodeLocation(imgEl));    // { startOffset: 13, endOffset: 32 ***REMOVED***
```
...
```js
const { Script ***REMOVED*** = require("vm");

const dom = new JSDOM(``, { runScripts: "outside-only" ***REMOVED***);
...
dom.reconfigure({ windowTop: myFakeTopForTesting, url: "https://example.com/" ***REMOVED***);
