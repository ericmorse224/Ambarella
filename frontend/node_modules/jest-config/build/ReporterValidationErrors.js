...
const ERROR = `${_utils.BULLET***REMOVED***Reporter Validation Error`;
/**
...
function createReporterError(reporterIndex, reporterValue) {
  const errorMessage =
    `  Reporter at index ${reporterIndex***REMOVED*** must be of type:\n` +
    `    ${_chalk().default.bold.green(validReporterTypes.join(' or '))***REMOVED***\n` +
...
function createArrayReporterError(
  ...
  const errorMessage =
    `  Unexpected value for ${valueName***REMOVED*** ` +
    `at index ${valueIndex***REMOVED*** of reporter at index ${reporterIndex***REMOVED***\n` +
    '  Expected:\n' +
    `    ${_chalk().default.bold.red(expectedType)***REMOVED***\n` +
    '  Got:\n' +
    `    ${_chalk().default.bold.green((0, _jestGetType().getType)(value))***REMOVED***\n` +
...
function validateReporters(reporterConfig) {
  return reporterConfig.every((reporter, index) => {
    ...
    ***REMOVED*** else if (typeof reporter !== 'string') {
