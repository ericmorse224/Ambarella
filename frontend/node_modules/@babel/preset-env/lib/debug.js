...
const logPlugin = (item, targetVersions, list) => {
  ...
  const support = list[item];
  {
    if (item.startsWith("transform-")) {
      const proposalName = `proposal-${item.slice(10)***REMOVED***`;
  ...
  ***REMOVED***
  ...
  for (const target of Object.keys(filteredList)) {
    ...
    first = false;
    formattedTargets += ` ${target***REMOVED***`;
    if (support[target]) formattedTargets += ` < ${support[target]***REMOVED***`;
  ***REMOVED***
  formattedTargets += ` ***REMOVED***`;
  console.log(`  ${item***REMOVED*** ${formattedTargets***REMOVED***`);
***REMOVED***;
