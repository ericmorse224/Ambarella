import {OperationOptions***REMOVED*** from 'retry';
...
declare const pRetry: {
	...
	***REMOVED***;
	...
	(async () => {
		console.log(await pRetry(run, {retries: 5***REMOVED***));

		// With the `onFailedAttempt` option:
		const result = await pRetry(run, {
			onFailedAttempt: error => {
				console.log(`Attempt ${error.attemptNumber***REMOVED*** failed. There are ${error.retriesLeft***REMOVED*** retries left.`);
