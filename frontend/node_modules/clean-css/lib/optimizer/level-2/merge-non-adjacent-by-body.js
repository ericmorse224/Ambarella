...
function mergeNonAdjacentByBody(tokens, context) {
  ...
  for (var i = tokens.length - 1; i >= 0; i--) {
    ...
    if (token[2].length > 0 && (!mergeSemantically && unsafeSelector(serializeRules(token[1])))) { candidates = {***REMOVED***; ***REMOVED***

    if (token[2].length > 0 && mergeSemantically && isBemElement(token)) { removeAnyUnsafeElements(token, candidates); ***REMOVED***
