  ...
  return {
    ...
    async OnceExit(root) {
      ...
      const {
        icssImports,
        icssExports
      ***REMOVED*** = (0, _icssUtils.extractICSS)(root);
      ...
      ***REMOVED***
      ...
      for (let index = 0; index <= results.length - 1; index++) {
        ...
        if (!item) {
          ...
          continue;
        ***REMOVED***
        const newUrl = item.prefix ? `${item.prefix***REMOVED***!${item.url***REMOVED***` : item.url;
        const importKey = newUrl;
        let importName = imports.get(importKey);
        if (!importName) {
          importName = `___CSS_LOADER_ICSS_IMPORT_${imports.size***REMOVED***___`;
        ...
        ***REMOVED***
        for (const [replacementIndex, token] of Object.keys(item.tokens).entries()) {
          const replacementName = `___CSS_LOADER_ICSS_IMPORT_${index***REMOVED***_REPLACEMENT_${replacementIndex***REMOVED***___`;
