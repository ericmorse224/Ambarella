'use strict';Object.defineProperty(exports, "__esModule", { value: true ***REMOVED***);exports['default'] = processSpecifier;function processSpecifier(specifier, astNode, exportMap, namespace) {
  var nsource = astNode.source && astNode.source.value;
  var exportMeta = {***REMOVED***;
  var local = void 0;

  switch (specifier.type) {
    case 'ExportDefaultSpecifier':
      if (!nsource) {return;***REMOVED***
      local = 'default';
      break;
    case 'ExportNamespaceSpecifier':
      exportMap.namespace.set(specifier.exported.name, Object.defineProperty(exportMeta, 'namespace', {
        get: function () {function get() {return namespace.resolveImport(nsource);***REMOVED***return get;***REMOVED***() ***REMOVED***));

      return;
    case 'ExportAllDeclaration':
      exportMap.namespace.set(specifier.exported.name || specifier.exported.value, namespace.add(exportMeta, specifier.source.value));
      return;
    case 'ExportSpecifier':
      if (!astNode.source) {
        exportMap.namespace.set(specifier.exported.name || specifier.exported.value, namespace.add(exportMeta, specifier.local));
        return;
      ***REMOVED***
    // else falls through
    default:
      local = specifier.local.name;
      break;***REMOVED***


  // todo: JSDoc
  exportMap.reexports.set(specifier.exported.name, { local: local, getImport: function () {function getImport() {return namespace.resolveImport(nsource);***REMOVED***return getImport;***REMOVED***() ***REMOVED***);
***REMOVED***
//# sourceMappingURL=data:application/json;charset=utf-8;base64,