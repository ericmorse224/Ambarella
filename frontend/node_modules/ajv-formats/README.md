...
```javascript
const ajv = new Ajv((formats: {date: true, time: true***REMOVED***)) // to ignore "date" and "time" formats in schemas.
```
...
```javascript
addFormats(ajv, {mode: "fast"***REMOVED***)
```

or

```javascript
addFormats(ajv, {mode: "fast", formats: ["date", "time"], keywords: true***REMOVED***)
```
