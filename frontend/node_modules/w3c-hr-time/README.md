...
```js
const { Performance ***REMOVED*** = require("w3c-hr-time");
...
This module does not aim for full [Web IDL][WEBIDL] conformance, so things like `performance.toJSON.call({***REMOVED***)` will not throw `TypeError`s like it does in browsers. If you need full Web IDL conformance, you may be interested in the [webidl2js][] module.
...
```js
const { getGlobalMonotonicClockMS ***REMOVED*** = require("w3c-hr-time");
...
```js
const { Performance, clockIsAccurate ***REMOVED*** = require("w3c-hr-time");
