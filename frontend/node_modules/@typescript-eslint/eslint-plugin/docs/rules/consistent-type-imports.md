...
Examples of **correct** code with `{prefer: 'type-imports'***REMOVED***`, and **incorrect** code with `{prefer: 'no-type-imports'***REMOVED***`.
...
Examples of **incorrect** code with `{prefer: 'type-imports'***REMOVED***`, and **correct** code with `{prefer: 'no-type-imports'***REMOVED***`.
...
- `separate-type-imports` will add the type keyword after the import keyword `import type { A ***REMOVED*** from '...'`. It is the default.
- `inline-type-imports` will inline the type keyword `import { type A ***REMOVED*** from '...'` and is only available in TypeScript 4.5 and onwards. See [documentation here](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-4-5.html#type-modifiers-on-import-names 'TypeScript 4.5 documentation on type modifiers and import names').
...
Examples of **incorrect** code with `{disallowTypeAnnotations: true***REMOVED***`:
