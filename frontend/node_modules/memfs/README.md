...
```js
import { fs, vol ***REMOVED*** from 'memfs';
...
vol.toJSON(); // {"/script.sh": "sudo rm -rf *"***REMOVED***
```
...
expect(vol.toJSON()).toEqual({ '/foo': 'bar' ***REMOVED***);
```

Create as many filesystem volumes as you need:

```js
import { Volume ***REMOVED*** from 'memfs';

const vol = Volume.fromJSON({ '/foo': 'bar' ***REMOVED***);
vol.readFileSync('/foo'); // bar

const vol2 = Volume.fromJSON({ '/foo': 'bar 2' ***REMOVED***);
