'use strict';var _contextCompat = require('eslint-module-utils/contextCompat');

var _docsUrl = require('../docsUrl');var _docsUrl2 = _interopRequireDefault(_docsUrl);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { 'default': obj ***REMOVED***;***REMOVED***

/** @type {import('eslint').Rule.RuleModule***REMOVED*** */
module.exports = {
  meta: {
    type: 'suggestion',
    docs: {
      category: 'Helpful warnings',
      description: 'Forbid the use of mutable exports with `var` or `let`.',
      url: (0, _docsUrl2['default'])('no-mutable-exports') ***REMOVED***,

    schema: [] ***REMOVED***,


  create: function () {function create(context) {
      function checkDeclaration(node) {var
        kind = node.kind;
        if (kind === 'var' || kind === 'let') {
          context.report(node, 'Exporting mutable \'' + String(kind) + '\' binding, use \'const\' instead.');
        ***REMOVED***
      ***REMOVED***

      /** @type {(scope: import('eslint').Scope.Scope, name: string) => void***REMOVED*** */
      function checkDeclarationsInScope(_ref, name) {var variables = _ref.variables;
        variables.
        filter(function (variable) {return variable.name === name;***REMOVED***).
        forEach(function (variable) {
          variable.defs.
          filter(function (def) {return def.type === 'Variable' && def.parent;***REMOVED***).
          forEach(function (def) {
            checkDeclaration(def.parent);
          ***REMOVED***);
        ***REMOVED***);
      ***REMOVED***

      return {
        /** @param {import('estree').ExportDefaultDeclaration***REMOVED*** node */
        ExportDefaultDeclaration: function () {function ExportDefaultDeclaration(node) {
            var scope = (0, _contextCompat.getScope)(context, node);

            if ('name' in node.declaration && node.declaration.name) {
              checkDeclarationsInScope(scope, node.declaration.name);
            ***REMOVED***
          ***REMOVED***return ExportDefaultDeclaration;***REMOVED***(),

        /** @param {import('estree').ExportNamedDeclaration***REMOVED*** node */
        ExportNamedDeclaration: function () {function ExportNamedDeclaration(node) {
            var scope = (0, _contextCompat.getScope)(context, node);

            if ('declaration' in node && node.declaration) {
              checkDeclaration(node.declaration);
            ***REMOVED*** else if (!('source' in node) || !node.source) {
              node.specifiers.forEach(function (specifier) {
                checkDeclarationsInScope(scope, specifier.local.name);
              ***REMOVED***);
            ***REMOVED***
          ***REMOVED***return ExportNamedDeclaration;***REMOVED***() ***REMOVED***;

    ***REMOVED***return create;***REMOVED***() ***REMOVED***;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,