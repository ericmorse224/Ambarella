...
import { resolveMatches ***REMOVED*** from './generateRules'
import escapeClassName from '../util/escapeClassName'
import { applyImportantSelector ***REMOVED*** from '../util/applyImportantSelector'
import { movePseudos ***REMOVED*** from '../util/pseudoElements'

/** @typedef {Map<string, [any, import('postcss').Rule[]]>***REMOVED*** ApplyCache */

function extractClasses(node) {
  /** @type {Map<string, Set<string>>***REMOVED*** */
  let groups = new Map()

  let container = postcss.root({ nodes: [node.clone()] ***REMOVED***)
  ...
  ***REMOVED***)
  ...
  return Object.assign(classes, { groups: normalizedGroups ***REMOVED***)
***REMOVED***
...
function processApply(root, context, localCache) {
  ...
  ***REMOVED***
  ...
  function replaceSelector(selector, utilitySelectors, candidate) {
    ...
    let candidateList = extractSelectors(`.${escapeClassName(candidate)***REMOVED***`)
    let candidateClass = candidateList.nodes[0].nodes[0]

    selectorList.each((sel) => {
      /** @type {Set<import('postcss-selector-parser').Selector>***REMOVED*** */
      ...
      if (apply.parent.name === 'screen') {
        ...
        throw apply.error(
          `@apply is not supported within nested at-rules like @screen. We suggest you write this as @apply ${applyCandidates
            .map((c) => `${screenType***REMOVED***:${c***REMOVED***`)
            .join(' ')***REMOVED*** instead.`
        )
      ***REMOVED***

      throw apply.error(
        `@apply is not supported within nested at-rules like @${apply.parent.name***REMOVED***. You can fix this by un-nesting @${apply.parent.name***REMOVED***.`
      )
    ***REMOVED***
    ...
    for (let applyCandidate of applyCandidates) {
      ...
      ***REMOVED***
      ...
      if (!applyClassCache.has(applyCandidate)) {
        throw apply.error(
          `The \`${applyCandidate***REMOVED***\` class does not exist. If \`${applyCandidate***REMOVED***\` is a custom class, make sure it is defined within a \`@layer\` directive.`
        )
      ***REMOVED***
  ...
  for (let [parent, [candidates, atApplySource]] of perParentApplies) {
    ...
    for (let [applyCandidate, important, rules] of candidates) {
      ...
      for (let [meta, node] of rules) {
        ...
        if (intersects) {
          throw node.error(
            `You cannot \`@apply\` the \`${applyCandidate***REMOVED***\` utility here because it creates a circular dependency.`
          )
        ***REMOVED***

        let root = postcss.root({ nodes: [node.clone()] ***REMOVED***)
