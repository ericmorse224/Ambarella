...
const printDisplayName = config => {
  const {displayName***REMOVED*** = config;
  ...
  ***REMOVED***
  ...
  const chosenColor = _chalk().default.reset.inverse[color]
    ...
    : white;
  return _chalk().default.supportsColor ? chosenColor(` ${name***REMOVED*** `) : name;
***REMOVED***;
...
const trimAndFormatPath = (pad, config, testPath, columns) => {
  ...
  const {basename***REMOVED*** = relative;
  let {dirname***REMOVED*** = relative; // length is ok
  ...
  ***REMOVED*** // we can fit trimmed dirname and full basename
  ...
  ***REMOVED*** // can't fit dirname, but can fit trimmed basename
...
const formatTestPath = (config, testPath) => {
  const {dirname, basename***REMOVED*** = relativePath(config, testPath);
  return (0, _slash().default)(
...
const getSummary = (aggregatedResults, options) => {
  ...
  ***REMOVED***
  ...
  const suites =
    ...
    (suitesFailed
      ? _chalk().default.bold.red(`${suitesFailed***REMOVED*** failed`) + ', '
      : '') +
    (suitesPending
      ? _chalk().default.bold.yellow(`${suitesPending***REMOVED*** skipped`) + ', '
      : '') +
    (suitesPassed
      ? _chalk().default.bold.green(`${suitesPassed***REMOVED*** passed`) + ', '
      : '') +
  ...
  const tests =
    ...
    (updatedTestsFailed > 0
      ? _chalk().default.bold.red(`${updatedTestsFailed***REMOVED*** failed`) + ', '
      : '') +
    (updatedTestsPending > 0
      ? _chalk().default.bold.yellow(`${updatedTestsPending***REMOVED*** skipped`) + ', '
      : '') +
    (updatedTestsTodo > 0
      ? _chalk().default.bold.magenta(`${updatedTestsTodo***REMOVED*** todo`) + ', '
      : '') +
    (updatedTestsPassed > 0
      ? _chalk().default.bold.green(`${updatedTestsPassed***REMOVED*** passed`) + ', '
      : '') +
    `${updatedTestsTotal***REMOVED*** total`;
  const snapshots =
    _chalk().default.bold('Snapshots:   ') +
    (snapshotsFailed
      ? _chalk().default.bold.red(`${snapshotsFailed***REMOVED*** failed`) + ', '
      : '') +
    (snapshotsOutdated && !snapshotsDidUpdate
      ? _chalk().default.bold.yellow(`${snapshotsOutdated***REMOVED*** obsolete`) + ', '
      : '') +
    (snapshotsOutdated && snapshotsDidUpdate
      ? _chalk().default.bold.green(`${snapshotsOutdated***REMOVED*** removed`) + ', '
      : '') +
    ...
    (snapshotsFilesRemoved && snapshotsDidUpdate
      ...
      : '') +
    (snapshotsUpdated
      ? _chalk().default.bold.green(`${snapshotsUpdated***REMOVED*** updated`) + ', '
      : '') +
    (snapshotsAdded
      ? _chalk().default.bold.green(`${snapshotsAdded***REMOVED*** written`) + ', '
      : '') +
    (snapshotsPassed
      ? _chalk().default.bold.green(`${snapshotsPassed***REMOVED*** passed`) + ', '
      : '') +
    `${snapshotsTotal***REMOVED*** total`;
...
const renderTime = (runTime, estimatedTime, width) => {
  ...
  let time = _chalk().default.bold('Time:') + `        ${renderedTime***REMOVED***`;

  if (runTime < estimatedTime) {
    time += `, estimated ${(0, _jestUtil().formatTime)(estimatedTime, 0)***REMOVED***`;
  ***REMOVED*** // Only show a progress bar if the test run is actually going to take
  // some time.
  ...
  ***REMOVED***
  ...
  return time;
***REMOVED***; // word-wrap a string that contains ANSI escape sequences.
...
const wrapAnsiString = (string, terminalWidth) => {
  ...
  ***REMOVED***
  ...
  const ANSI_REGEXP = /[\u001b\u009b]\[\d{1,2***REMOVED***m/gu;
  const tokens = [];
  let lastIndex = 0;
  let match;
