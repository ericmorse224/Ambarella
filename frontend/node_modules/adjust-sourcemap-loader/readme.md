...
* **`codecs`** : `Array.<{name:string, decode:function, encode:function, root:function***REMOVED***>` Optional Array of codecs. There are a number of built-in codecs available. If you specify you own codecs you will loose those that are built-in. However you can include them from the `codec/` directory.
...
var inBuiltCodecs = require('adjust-sourcemap-loader').codecs,
    myCodecs      = [
      {
        name  : 'foo',
        decode: function(uri) {...***REMOVED***,
        encode: function(absolute) {...***REMOVED***,
