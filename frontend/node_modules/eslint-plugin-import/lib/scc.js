'use strict';Object.defineProperty(exports, "__esModule", { value: true ***REMOVED***);var _createClass = function () {function defineProperties(target, props) {for (var i = 0; i < props.length; i++) {var descriptor = props[i];descriptor.enumerable = descriptor.enumerable || false;descriptor.configurable = true;if ("value" in descriptor) descriptor.writable = true;Object.defineProperty(target, descriptor.key, descriptor);***REMOVED******REMOVED***return function (Constructor, protoProps, staticProps) {if (protoProps) defineProperties(Constructor.prototype, protoProps);if (staticProps) defineProperties(Constructor, staticProps);return Constructor;***REMOVED***;***REMOVED***();var _scc = require('@rtsao/scc');var _scc2 = _interopRequireDefault(_scc);
var _hash = require('eslint-module-utils/hash');
var _resolve = require('eslint-module-utils/resolve');var _resolve2 = _interopRequireDefault(_resolve);
var _builder = require('./exportMap/builder');var _builder2 = _interopRequireDefault(_builder);
var _childContext = require('./exportMap/childContext');var _childContext2 = _interopRequireDefault(_childContext);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { 'default': obj ***REMOVED***;***REMOVED***function _toConsumableArray(arr) {if (Array.isArray(arr)) {for (var i = 0, arr2 = Array(arr.length); i < arr.length; i++) {arr2[i] = arr[i];***REMOVED***return arr2;***REMOVED*** else {return Array.from(arr);***REMOVED******REMOVED***function _classCallCheck(instance, Constructor) {if (!(instance instanceof Constructor)) {throw new TypeError("Cannot call a class as a function");***REMOVED******REMOVED***

var cache = new Map();var

StronglyConnectedComponentsBuilder = function () {function StronglyConnectedComponentsBuilder() {_classCallCheck(this, StronglyConnectedComponentsBuilder);***REMOVED***_createClass(StronglyConnectedComponentsBuilder, null, [{ key: 'clearCache', value: function () {function clearCache()
      {
        cache = new Map();
      ***REMOVED***return clearCache;***REMOVED***() ***REMOVED***, { key: 'get', value: function () {function get(

      source, context) {
        var path = (0, _resolve2['default'])(source, context);
        if (path == null) {return null;***REMOVED***
        return StronglyConnectedComponentsBuilder['for']((0, _childContext2['default'])(path, context));
      ***REMOVED***return get;***REMOVED***() ***REMOVED***, { key: 'for', value: function () {function _for(

      context) {
        var settingsHash = (0, _hash.hashObject)({
          settings: context.settings,
          parserOptions: context.parserOptions,
          parserPath: context.parserPath ***REMOVED***).
        digest('hex');
        var cacheKey = context.path + settingsHash;
        if (cache.has(cacheKey)) {
          return cache.get(cacheKey);
        ***REMOVED***
        var scc = StronglyConnectedComponentsBuilder.calculate(context);
        var visitedFiles = Object.keys(scc);
        visitedFiles.forEach(function (filePath) {return cache.set(filePath + settingsHash, scc);***REMOVED***);
        return scc;
      ***REMOVED***return _for;***REMOVED***() ***REMOVED***, { key: 'calculate', value: function () {function calculate(

      context) {
        var exportMap = _builder2['default']['for'](context);
        var adjacencyList = this.exportMapToAdjacencyList(exportMap);
        var calculatedScc = (0, _scc2['default'])(adjacencyList);
        return StronglyConnectedComponentsBuilder.calculatedSccToPlainObject(calculatedScc);
      ***REMOVED***return calculate;***REMOVED***()

    /** @returns {Map<string, Set<string>>***REMOVED*** for each dep, what are its direct deps */ ***REMOVED***, { key: 'exportMapToAdjacencyList', value: function () {function exportMapToAdjacencyList(
      initialExportMap) {
        var adjacencyList = new Map();
        // BFS
        function visitNode(exportMap) {
          if (!exportMap) {
            return;
          ***REMOVED***
          exportMap.imports.forEach(function (v, importedPath) {
            var from = exportMap.path;
            var to = importedPath;

            // Ignore type-only imports, because we care only about SCCs of value imports
            var toTraverse = [].concat(_toConsumableArray(v.declarations)).filter(function (_ref) {var isOnlyImportingTypes = _ref.isOnlyImportingTypes;return !isOnlyImportingTypes;***REMOVED***);
            if (toTraverse.length === 0) {return;***REMOVED***

            if (!adjacencyList.has(from)) {
              adjacencyList.set(from, new Set());
            ***REMOVED***

            if (adjacencyList.get(from).has(to)) {
              return; // prevent endless loop
            ***REMOVED***
            adjacencyList.get(from).add(to);
            visitNode(v.getter());
          ***REMOVED***);
        ***REMOVED***
        visitNode(initialExportMap);
        // Fill gaps
        adjacencyList.forEach(function (values) {
          values.forEach(function (value) {
            if (!adjacencyList.has(value)) {
              adjacencyList.set(value, new Set());
            ***REMOVED***
          ***REMOVED***);
        ***REMOVED***);
        return adjacencyList;
      ***REMOVED***return exportMapToAdjacencyList;***REMOVED***()

    /** @returns {Record<string, number>***REMOVED*** for each key, its SCC's index */ ***REMOVED***, { key: 'calculatedSccToPlainObject', value: function () {function calculatedSccToPlainObject(
      sccs) {
        var obj = {***REMOVED***;
        sccs.forEach(function (scc, index) {
          scc.forEach(function (node) {
            obj[node] = index;
          ***REMOVED***);
        ***REMOVED***);
        return obj;
      ***REMOVED***return calculatedSccToPlainObject;***REMOVED***() ***REMOVED***]);return StronglyConnectedComponentsBuilder;***REMOVED***();exports['default'] = StronglyConnectedComponentsBuilder;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,