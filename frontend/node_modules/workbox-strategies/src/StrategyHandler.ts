...
class StrategyHandler {
  ...
  private _cacheKeys: Record<string, Request> = {***REMOVED***;
  ...
  async fetch(input: RequestInfo): Promise<Response> {
    ...
    ***REMOVED*** catch (error) {
      if (process.env.NODE_ENV !== 'production') {
        logger.log(
          `Network request for ` +
            `'${getFriendlyURL(request.url)***REMOVED***' threw an error.`,
          error,
        );
      ***REMOVED***
  ...
  ***REMOVED***
  ...
  async cacheMatch(key: RequestInfo): Promise<Response | undefined> {
    ...
    const multiMatchOptions = {...matchOptions, ...{cacheName***REMOVED******REMOVED***;
  ...
  async cachePut(key: RequestInfo, response: Response): Promise<boolean> {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      ***REMOVED***
      ...
      if (vary) {
        logger.debug(
          `The response for ${getFriendlyURL(effectiveRequest.url)***REMOVED*** ` +
    ...
    ***REMOVED***
    ...
    if (!responseToCache) {
      if (process.env.NODE_ENV !== 'production') {
        logger.debug(
          `Response '${getFriendlyURL(effectiveRequest.url)***REMOVED***' ` +
            `will not be cached.`,
          responseToCache,
        );
      ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (process.env.NODE_ENV !== 'production') {
      logger.debug(
        `Updating the '${cacheName***REMOVED***' cache with a new Response ` +
    ...
    for (const plugin of this._strategy.plugins) {
      if (typeof plugin[name] === 'function') {
        ...
        const statefulCallback = (
          ...
          const statefulParam = {...param, state***REMOVED***;
        ...
        if (responseToCache) {
          if (responseToCache.status !== 200) {
            if (responseToCache.status === 0) {
              logger.warn(
                `The response for '${this.request.url***REMOVED***' ` +
