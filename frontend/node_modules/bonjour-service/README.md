...
```js
import { Bonjour ***REMOVED*** from 'bonjour-service'

const instance = new Bonjour()

// advertise an HTTP server on port 3000
instance.publish({ name: 'My Web Server', type: 'http', port: 3000 ***REMOVED***)

// browse for all http services
instance.find({ type: 'http' ***REMOVED***, function (service) {
...
```js
var instance = new Bonjour({ options ***REMOVED***, errorCallback)
```
...
- `txt` (object) - passed into [dns-txt
  ...
  binary: true ***REMOVED***` if you want to keep the TXT records in binary
