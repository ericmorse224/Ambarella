/*
	MIT License http://www.opensource.org/licenses/mit-license.php
	Author Tobias Koppers @sokra
*/

"use strict";

/**
 * @typedef {object***REMOVED*** GroupOptions
 * @property {boolean=***REMOVED*** groupChildren
 * @property {boolean=***REMOVED*** force
 * @property {number=***REMOVED*** targetGroupCount
 */

/**
 * @template T
 * @template R
 * @typedef {object***REMOVED*** GroupConfig
 * @property {(item: T) => string[] | undefined***REMOVED*** getKeys
 * @property {(key: string, children: (R | T)[], items: T[]) => R***REMOVED*** createGroup
 * @property {(name: string, items: T[]) => GroupOptions=***REMOVED*** getOptions
 */

/**
 * @template T
 * @template R
 * @typedef {object***REMOVED*** ItemWithGroups
 * @property {T***REMOVED*** item
 * @property {Set<Group<T, R>>***REMOVED*** groups
 */

/**
 * @template T
 * @template R
 * @typedef {{ config: GroupConfig<T, R>, name: string, alreadyGrouped: boolean, items: Set<ItemWithGroups<T, R>> | undefined ***REMOVED******REMOVED*** Group
 */

/**
 * @template T
 * @template R
 * @param {T[]***REMOVED*** items the list of items
 * @param {GroupConfig<T, R>[]***REMOVED*** groupConfigs configuration
 * @returns {(R | T)[]***REMOVED*** grouped items
 */
const smartGrouping = (items, groupConfigs) => {
	/** @type {Set<ItemWithGroups<T, R>>***REMOVED*** */
	const itemsWithGroups = new Set();
	/** @type {Map<string, Group<T, R>>***REMOVED*** */
	const allGroups = new Map();
	for (const item of items) {
		/** @type {Set<Group<T, R>>***REMOVED*** */
		const groups = new Set();
		for (let i = 0; i < groupConfigs.length; i++) {
			const groupConfig = groupConfigs[i];
			const keys = groupConfig.getKeys(item);
			if (keys) {
				for (const name of keys) {
					const key = `${i***REMOVED***:${name***REMOVED***`;
					let group = allGroups.get(key);
					if (group === undefined) {
						allGroups.set(
							key,
							(group = {
								config: groupConfig,
								name,
								alreadyGrouped: false,
								items: undefined
							***REMOVED***)
						);
					***REMOVED***
					groups.add(group);
				***REMOVED***
			***REMOVED***
		***REMOVED***
		itemsWithGroups.add({
			item,
			groups
		***REMOVED***);
	***REMOVED***
	/**
	 * @param {Set<ItemWithGroups<T, R>>***REMOVED*** itemsWithGroups input items with groups
	 * @returns {(T | R)[]***REMOVED*** groups items
	 */
	const runGrouping = itemsWithGroups => {
		const totalSize = itemsWithGroups.size;
		for (const entry of itemsWithGroups) {
			for (const group of entry.groups) {
				if (group.alreadyGrouped) continue;
				const items = group.items;
				if (items === undefined) {
					group.items = new Set([entry]);
				***REMOVED*** else {
					items.add(entry);
				***REMOVED***
			***REMOVED***
		***REMOVED***
		/** @type {Map<Group<T, R>, { items: Set<ItemWithGroups<T, R>>, options: GroupOptions | false | undefined, used: boolean ***REMOVED***>***REMOVED*** */
		const groupMap = new Map();
		for (const group of allGroups.values()) {
			if (group.items) {
				const items = group.items;
				group.items = undefined;
				groupMap.set(group, {
					items,
					options: undefined,
					used: false
				***REMOVED***);
			***REMOVED***
		***REMOVED***
		/** @type {(T | R)[]***REMOVED*** */
		const results = [];
		for (;;) {
			/** @type {Group<T, R> | undefined***REMOVED*** */
			let bestGroup;
			let bestGroupSize = -1;
			let bestGroupItems;
			let bestGroupOptions;
			for (const [group, state] of groupMap) {
				const { items, used ***REMOVED*** = state;
				let options = state.options;
				if (options === undefined) {
					const groupConfig = group.config;
					state.options = options =
						(groupConfig.getOptions &&
							groupConfig.getOptions(
								group.name,
								Array.from(items, ({ item ***REMOVED***) => item)
							)) ||
						false;
				***REMOVED***

				const force = options && options.force;
				if (!force) {
					if (bestGroupOptions && bestGroupOptions.force) continue;
					if (used) continue;
					if (items.size <= 1 || totalSize - items.size <= 1) {
						continue;
					***REMOVED***
				***REMOVED***
				const targetGroupCount = (options && options.targetGroupCount) || 4;
				const sizeValue = force
					? items.size
					: Math.min(
							items.size,
							(totalSize * 2) / targetGroupCount +
								itemsWithGroups.size -
								items.size
						);
				if (
					sizeValue > bestGroupSize ||
					(force && (!bestGroupOptions || !bestGroupOptions.force))
				) {
					bestGroup = group;
					bestGroupSize = sizeValue;
					bestGroupItems = items;
					bestGroupOptions = options;
				***REMOVED***
			***REMOVED***
			if (bestGroup === undefined) {
				break;
			***REMOVED***
			const items = new Set(bestGroupItems);
			const options = bestGroupOptions;

			const groupChildren = !options || options.groupChildren !== false;

			for (const item of items) {
				itemsWithGroups.delete(item);
				// Remove all groups that items have from the map to not select them again
				for (const group of item.groups) {
					const state = groupMap.get(group);
					if (state !== undefined) {
						state.items.delete(item);
						if (state.items.size === 0) {
							groupMap.delete(group);
						***REMOVED*** else {
							state.options = undefined;
							if (groupChildren) {
								state.used = true;
							***REMOVED***
						***REMOVED***
					***REMOVED***
				***REMOVED***
			***REMOVED***
			groupMap.delete(bestGroup);

			const key = bestGroup.name;
			const groupConfig = bestGroup.config;

			const allItems = Array.from(items, ({ item ***REMOVED***) => item);

			bestGroup.alreadyGrouped = true;
			const children = groupChildren ? runGrouping(items) : allItems;
			bestGroup.alreadyGrouped = false;

			results.push(groupConfig.createGroup(key, children, allItems));
		***REMOVED***
		for (const { item ***REMOVED*** of itemsWithGroups) {
			results.push(item);
		***REMOVED***
		return results;
	***REMOVED***;
	return runGrouping(itemsWithGroups);
***REMOVED***;

module.exports = smartGrouping;
