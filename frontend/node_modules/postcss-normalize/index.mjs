...
***REMOVED***); // export a hashmap of css library filenames by id
...
***REMOVED***); // get the resolved filename of a package/module
...
async function readFile(filename) {
  ...
  return new Promise((resolve, reject) => fs.stat(filename, (statsError, {
    mtime
  ***REMOVED***) => statsError ? reject(statsError) : mtime === cache$1[filename].mtime ? resolve(cache$1[filename].data) : fs.readFile(filename, 'utf8', (readFileError, data) => readFileError ? reject(readFileError) : resolve((cache$1[filename] = {
    data,
    mtime
  ***REMOVED***).data))));
***REMOVED***
...
css => cache[css] = cache[css] || postcss.parse(css, {
  from: filename
***REMOVED***)).then( // clone the cached root
  ...
  return create({
    load(filename, importOptions) {
      ...
      ? parse(filename, commentsTransformer).then(root => root.toResult({
        to: filename,
        map: true
      ***REMOVED***).css) : typeof opts.load === 'function' // otherwise, use the override loader
...
const postcssPlugin = (commentsTransformer, opts) => root => {
  const promises = [];
  const insertedFilenames = {***REMOVED***; // use @import insertion point
  ...
  ***REMOVED***);
  ...
  (all, id) => {
    ...
    ***REMOVED*** else if (typeof id === 'string') {
    ...
    ***REMOVED***
    ...
    return all;
  ***REMOVED***, []).filter( // ignore filenames that have already been inserted
  filename => insertedFilenames[filename] = opts.allowDuplicates || !(filename in insertedFilenames)).map( // parse the file (the file and css are conservatively cached)
  filename => parse(filename, commentsTransformer))).then(roots => {
    if (roots.length) {
      // combine all the library nodes returned by the parsed files
...
export { plugin as default ***REMOVED***;
