...
const messages = {
  [MESSAGE_ID]: '{{propName***REMOVED******REMOVED*** has a/an {{forbiddenType***REMOVED******REMOVED*** as default prop. This could lead to potential infinite render loop in React. Use a variable reference instead of {{forbiddenType***REMOVED******REMOVED***.',
***REMOVED***;
  ...
  properties.filter((prop) => prop.type === 'Property' && prop.value.type === 'AssignmentPattern').forEach((prop) => {
    ...
    ***REMOVED*** else if (
      ...
      report(context, messages[MESSAGE_ID], MESSAGE_ID, {
        ...
        ***REMOVED***,
      ***REMOVED***);
    ***REMOVED*** else if (FORBIDDEN_TYPES.has(propDefaultValueType)) {
      report(context, messages[MESSAGE_ID], MESSAGE_ID, {
        node: propDefaultValue,
        data: {
          propName,
