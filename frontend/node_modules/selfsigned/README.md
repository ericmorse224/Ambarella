...
var attrs = [{ name: 'commonName', value: 'contoso.com' ***REMOVED***];
var pems = selfsigned.generate(attrs, { days: 365 ***REMOVED***);
console.log(pems)
```
...
```js
selfsigned.generate(attrs, { days: 365 ***REMOVED***, function (err, pems) {
  console.log(pems)
***REMOVED***);
```
...
var pems = selfsigned.generate(null, {
  ...
  extensions: [{ name: 'basicConstraints', cA: true ***REMOVED***], // certificate extensions array
...
> You can avoid key pair generation specifying your own keys (`{ keyPair: { publicKey: '-----BEGIN PUBLIC KEY-----...', privateKey: '-----BEGIN RSA PRIVATE KEY-----...' ***REMOVED***`)
...
```js
var pems = selfsigned.generate(null, { clientCertificate: true ***REMOVED***);
console.log(pems)
```
Will return the following like this:
...
```js
var pems = selfsigned.generate(null, { clientCertificate: true, clientCertificateCN: 'FooBar' ***REMOVED***);
```
