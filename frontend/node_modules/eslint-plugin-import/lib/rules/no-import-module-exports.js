'use strict';var _minimatch = require('minimatch');var _minimatch2 = _interopRequireDefault(_minimatch);
var _path = require('path');var _path2 = _interopRequireDefault(_path);
var _contextCompat = require('eslint-module-utils/contextCompat');
var _pkgUp = require('eslint-module-utils/pkgUp');var _pkgUp2 = _interopRequireDefault(_pkgUp);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { 'default': obj ***REMOVED***;***REMOVED***

function getEntryPoint(context) {
  var pkgPath = (0, _pkgUp2['default'])({ cwd: (0, _contextCompat.getPhysicalFilename)(context) ***REMOVED***);
  try {
    return require.resolve(_path2['default'].dirname(pkgPath));
  ***REMOVED*** catch (error) {
    // Assume the package has no entrypoint (e.g. CLI packages)
    // in which case require.resolve would throw.
    return null;
  ***REMOVED***
***REMOVED***

function findScope(context, identifier) {var _getSourceCode =
  (0, _contextCompat.getSourceCode)(context),scopeManager = _getSourceCode.scopeManager;

  return scopeManager && scopeManager.scopes.slice().reverse().find(function (scope) {return scope.variables.some(function (variable) {return variable.identifiers.some(function (node) {return node.name === identifier;***REMOVED***);***REMOVED***);***REMOVED***);
***REMOVED***

function findDefinition(objectScope, identifier) {
  var variable = objectScope.variables.find(function (variable) {return variable.name === identifier;***REMOVED***);
  return variable.defs.find(function (def) {return def.name.name === identifier;***REMOVED***);
***REMOVED***

module.exports = {
  meta: {
    type: 'problem',
    docs: {
      category: 'Module systems',
      description: 'Forbid import statements with CommonJS module.exports.',
      recommended: true ***REMOVED***,

    fixable: 'code',
    schema: [
    {
      type: 'object',
      properties: {
        exceptions: { type: 'array' ***REMOVED*** ***REMOVED***,

      additionalProperties: false ***REMOVED***] ***REMOVED***,



  create: function () {function create(context) {
      var importDeclarations = [];
      var entryPoint = getEntryPoint(context);
      var options = context.options[0] || {***REMOVED***;
      var alreadyReported = false;

      function report(node) {
        var fileName = (0, _contextCompat.getPhysicalFilename)(context);
        var isEntryPoint = entryPoint === fileName;
        var isIdentifier = node.object.type === 'Identifier';
        var hasKeywords = /^(module|exports)$/.test(node.object.name);
        var objectScope = hasKeywords && findScope(context, node.object.name);
        var variableDefinition = objectScope && findDefinition(objectScope, node.object.name);
        var isImportBinding = variableDefinition && variableDefinition.type === 'ImportBinding';
        var hasCJSExportReference = hasKeywords && (!objectScope || objectScope.type === 'module');
        var isException = !!options.exceptions && options.exceptions.some(function (glob) {return (0, _minimatch2['default'])(fileName, glob);***REMOVED***);

        if (isIdentifier && hasCJSExportReference && !isEntryPoint && !isException && !isImportBinding) {
          importDeclarations.forEach(function (importDeclaration) {
            context.report({
              node: importDeclaration,
              message: 'Cannot use import declarations in modules that export using CommonJS (module.exports = \'foo\' or exports.bar = \'hi\')' ***REMOVED***);

          ***REMOVED***);
          alreadyReported = true;
        ***REMOVED***
      ***REMOVED***

      return {
        ImportDeclaration: function () {function ImportDeclaration(node) {
            importDeclarations.push(node);
          ***REMOVED***return ImportDeclaration;***REMOVED***(),
        MemberExpression: function () {function MemberExpression(node) {
            if (!alreadyReported) {
              report(node);
            ***REMOVED***
          ***REMOVED***return MemberExpression;***REMOVED***() ***REMOVED***;

    ***REMOVED***return create;***REMOVED***() ***REMOVED***;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,