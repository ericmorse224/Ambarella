...
const {networkInterfaces***REMOVED*** = require("os");
...
const parse = (stdout, family) => {
  ...
  (stdout || "").trim().split("\n").some(line => {
    ...
    if (gateway && isIP(gateway)) { // default via 1.2.3.4 dev en0
      ...
      return true;
    ***REMOVED*** else if (iface && !gateway) { // default via dev en0
      ...
      addresses.some(addr => {
        if (addr.family.substring(2) === family && isIP(addr.address)) {
          result = {gateway: addr.address, interface: (iface ? iface : null)***REMOVED***;
          return true;
        ***REMOVED***
      ***REMOVED***);
    ***REMOVED***
