...
const distributeUnevenly = (sum, length) => {
    const result = Array.from({ length ***REMOVED***).fill(Math.floor(sum / length));
...
const sequence = (start, end) => {
    return Array.from({ length: end - start + 1 ***REMOVED***, (_, index) => {
        return index + start;
    ***REMOVED***);
***REMOVED***;
...
const extractTruncates = (config) => {
    return config.columns.map(({ truncate ***REMOVED***) => {
        return truncate;
    ***REMOVED***);
***REMOVED***;
...
const calculateRangeCoordinate = (spanningCellConfig) => {
    const { row, col, colSpan = 1, rowSpan = 1 ***REMOVED*** = spanningCellConfig;
    return { bottomRight: { col: col + colSpan - 1,
            row: row + rowSpan - 1 ***REMOVED***,
        topLeft: { col,
            row ***REMOVED*** ***REMOVED***;
***REMOVED***;
...
const isCellInRange = (cell, { topLeft, bottomRight ***REMOVED***) => {
