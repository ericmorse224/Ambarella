import { compareRangeCovs ***REMOVED*** from "./compare";
...
function emitTreeLayer(layer, colMap) {
    const line = [];
    let curIdx = 0;
    for (const { start, end, count ***REMOVED*** of layer) {
    ...
    while (true) {
        ...
        ***REMOVED***
        ...
        if (startOffset === undefined || endOffset === undefined) {
            throw new Error(`Invalid offsets for: ${JSON.stringify(text)***REMOVED***`);
        ***REMOVED***
        result.push({ startOffset, endOffset, count ***REMOVED***);
    ***REMOVED***
    return result;
***REMOVED***
function emitRange(count, len) {
    const rangeStart = `[${count.toString(10)***REMOVED***`;
    const rangeEnd = ")";
    const hyphensLen = len - (rangeStart.length + rangeEnd.length);
    const hyphens = "-".repeat(Math.max(0, hyphensLen));
    return `${rangeStart***REMOVED***${hyphens***REMOVED***${rangeEnd***REMOVED***`;
***REMOVED***
