...
eg. `{ "X-Custom-Header": "yes" ***REMOVED***`
...
eg. `mimeTypes: { phtml: 'text/html' ***REMOVED***`.
...
middleware(compiler, { outputFileSystem: myOutputFileSystem ***REMOVED***);
```
...
instance.waitUntilValid(() => {
  ...
  console.log(`Filename is ${filename***REMOVED***`);
***REMOVED***);
```
...
app.use(middleware(compiler, { serverSideRender: true ***REMOVED***));

// The following middleware would not be invoked until the latest build is finished.
app.use((req, res) => {
  const { devMiddleware ***REMOVED*** = res.locals.webpack;
  const outputFileSystem = devMiddleware.context.outputFileSystem;
  const jsonWebpackStats = devMiddleware.context.stats.toJson();
  const { assetsByChunkName, outputPath ***REMOVED*** = jsonWebpackStats;
...
<html>
  ...
  <body>
    ...
    ${normalizeAssets(assetsByChunkName.main)
      ...
      .map((path) => `<script src="${path***REMOVED***"></script>`)
      .join("\n")***REMOVED***
  </body>
</html>
  `);
...
const { publicPath ***REMOVED*** = webpackConfig.output;

(async () => {
  await fastify.register(require("fastify-express"));
  await fastify.use(devMiddleware(compiler, { publicPath ***REMOVED***));
