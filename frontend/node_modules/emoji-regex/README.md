...
const text = `
\u{231A***REMOVED***: ⌚ default emoji presentation character (Emoji_Presentation)
\u{2194***REMOVED***\u{FE0F***REMOVED***: ↔️ default text presentation character rendered as emoji
\u{1F469***REMOVED***: 👩 emoji modifier base (Emoji_Modifier_Base)
\u{1F469***REMOVED***\u{1F3FF***REMOVED***: 👩🏿 emoji modifier base followed by a modifier
`;
...
while (match = regex.exec(text)) {
  const emoji = match[0];
  console.log(`Matched sequence ${ emoji ***REMOVED*** — code points: ${ [...emoji].length ***REMOVED***`);
***REMOVED***
```
