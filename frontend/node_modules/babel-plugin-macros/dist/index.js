...
function createMacro(macro, options = {***REMOVED***) {
  ...
  function macroWrapper(args) {
    ...
    ***REMOVED*** = args;
    ...
    if (!isBabelMacrosCall) {
      throw new MacroError(`The macro you imported from "${source***REMOVED***" is being executed outside the context of compilation with babel-plugin-macros. ` + `This indicates that you don't have the babel plugin "babel-plugin-macros" configured correctly. ` + `Please see the documentation for how to configure babel-plugin-macros properly: ` + 'https://github.com/kentcdodds/babel-plugin-macros/blob/master/other/docs/user.md');
    ***REMOVED***
          ...
          VariableDeclaration(path) {
            ...
            ***REMOVED***);
            ...
            path.get('declarations').filter(isMacros).forEach(child => {
              ...
              ***REMOVED***] : child.node.id.properties.map(property => ({
...
***REMOVED***) {
  ...
  ***REMOVED***, {***REMOVED***);
  ...
  if (!macro.isBabelMacro) {
    throw new Error(`The macro imported from "${source***REMOVED***" must be wrapped in "createMacro" ` + `which you can get from "babel-plugin-macros". ` + `Please refer to the documentation to see how to do this properly: https://github.com/kentcdodds/babel-plugin-macros/blob/master/other/docs/author.md#writing-a-macro`);
  ***REMOVED***
  ...
  ***REMOVED*** catch (error) {
    ...
    ***REMOVED***
    ...
    error.message = `${source***REMOVED***: ${error.message***REMOVED***`;

    if (!isRelative) {
      error.message = `${error.message***REMOVED*** Learn more: https://www.npmjs.com/package/${source.replace( // remove everything after package name
...
function getConfigFromOptions(configName, options) {
  if (options.hasOwnProperty(configName)) {
    if (options[configName] && typeof options[configName] !== 'object') {
      ...
      console.error(`The macro plugin options' ${configName***REMOVED*** property was not an object or null.`);
    ***REMOVED*** else {
      return {
  ...
  ***REMOVED*** = macro.options;
  ...
  if (configName) {
    ...
    if (optionsConfig.options === undefined && fileConfig.options === undefined && fileConfig.error !== undefined) {
      // eslint-disable-next-line no-console
      console.error(`There was an error trying to load the config "${configName***REMOVED***" ` + `for the macro imported from "${source***REMOVED***. ` + `Please see the error thrown for more information.`);
      throw fileConfig.error;
    ***REMOVED***

    if (fileConfig.options !== undefined && optionsConfig.options !== undefined && typeof fileConfig.options !== 'object') {
      throw new Error(`${fileConfig.path***REMOVED*** specified a ${configName***REMOVED*** config of type ` + `${typeof optionsConfig.options***REMOVED***, but the the macros plugin's ` + `options.${configName***REMOVED*** did contain an object. Both configs must ` + `contain objects for their options to be mergeable.`);
    ***REMOVED***
