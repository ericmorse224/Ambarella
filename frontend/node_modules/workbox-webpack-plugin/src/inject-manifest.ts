...
import {escapeRegExp***REMOVED*** from 'workbox-build/build/lib/escape-regexp';
import {replaceAndUpdateSourceMap***REMOVED*** from 'workbox-build/build/lib/replace-and-update-source-map';
import {validateWebpackInjectManifestOptions***REMOVED*** from 'workbox-build/build/lib/validate-options';
import prettyBytes from 'pretty-bytes';
import stringify from 'fast-json-stable-stringify';
import upath from 'upath';
import webpack from 'webpack';

import {getManifestEntriesFromCompilation***REMOVED*** from './lib/get-manifest-entries-from-compilation';
import {getSourcemapAssetName***REMOVED*** from './lib/get-sourcemap-asset-name';
import {relativeToOutputPath***REMOVED*** from './lib/relative-to-output-path';
import {WebpackInjectManifestOptions***REMOVED*** from 'workbox-build';
...
const {RawSource***REMOVED*** = webpack.sources || require('webpack-sources');
...
class InjectManifest {
  ...
  ***REMOVED***
  ...
  /**
   * @param {Object***REMOVED*** [compiler] default compiler object passed from webpack
   *
   * @private
   */
  ...
  ***REMOVED***
  ...
  apply(compiler: webpack.Compiler): void {
    ...
    ***REMOVED*** else {
      const {PROCESS_ASSETS_STAGE_OPTIMIZE_TRANSFER***REMOVED*** = webpack.Compilation;
  ...
  ***REMOVED***
  ...
  /**
   * @param {Object***REMOVED*** compilation The webpack compilation.
   * @param {Object***REMOVED*** parentCompiler The webpack parent compiler.
   *
   * @private
   */
  ...
  async handleMake(
    ...
    ***REMOVED*** catch (error) {
      if (error instanceof Error) {
        throw new Error(
          `Please check your ${this.constructor.name***REMOVED*** plugin ` +
  ...
  async addAssets(compilation: webpack.Compilation): Promise<void> {
    ...
    if (this.alreadyCalled) {
      const warningMessage =
        `${this.constructor.name***REMOVED*** has been called ` +
    ...
    ***REMOVED***
    ...
    const config = Object.assign({***REMOVED***, this.config);

    // Ensure that we don't precache any of the assets generated by *any*
    // instance of this plugin.
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    config.exclude!.push(({asset***REMOVED***) => _generatedAssetNames.has(asset.name));
    ...
    if (!injectionResults) {
      throw new Error(
        `Can't find ${config.injectionPoint ?? ''***REMOVED*** in your SW source.`,
      );
    ***REMOVED***
    if (injectionResults.length !== 1) {
      throw new Error(
        `Multiple instances of ${config.injectionPoint ?? ''***REMOVED*** were ` +
          `found in your SW source. Include it only once. For more info, see ` +
          `https://github.com/GoogleChrome/workbox/issues/2681`,
      );
    ***REMOVED***

    const {size, sortedEntries***REMOVED*** = await getManifestEntriesFromCompilation(
      compilation,
      config,
    );
    ...
    ***REMOVED***
    ...
    if (sourcemapAssetName) {
      ...
      const {source, map***REMOVED*** = await replaceAndUpdateSourceMap({
    ...
    ***REMOVED***
    ...
    if (compilation.getLogger) {
      ...
      logger.info(`The service worker at ${config.swDest ?? ''***REMOVED*** will precache
        ${sortedEntries.length***REMOVED*** URLs, totaling ${prettyBytes(size)***REMOVED***.`);
    ***REMOVED***
  ***REMOVED***
***REMOVED***

export {InjectManifest***REMOVED***;
