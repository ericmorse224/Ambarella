...
/** @typedef {import("../../declarations/plugins/container/ContainerReferencePlugin").ContainerReferencePluginOptions***REMOVED*** ContainerReferencePluginOptions */
/** @typedef {import("../../declarations/plugins/container/ContainerReferencePlugin").RemotesConfig***REMOVED*** RemotesConfig */
...
class ContainerReferencePlugin {
	/**
	 * @param {ContainerReferencePluginOptions***REMOVED*** options options
	 */
	constructor(options) {
		validate(options);
	...
	***REMOVED***
	 ...
	 */
	apply(compiler) {
		const { _remotes: remotes, _remoteType: remoteType ***REMOVED*** = this;

		/** @type {Record<string, string>***REMOVED*** */
		const remoteExternals = {***REMOVED***;
		for (const [key, config] of remotes) {
			let i = 0;
			for (const external of config.external) {
				if (external.startsWith("internal ")) continue;
				remoteExternals[
					`webpack/container/reference/${key***REMOVED***${i ? `/fallback-${i***REMOVED***` : ""***REMOVED***`
				] = external;
				i++;
			***REMOVED***
		***REMOVED***
			...
			(compilation, { normalModuleFactory ***REMOVED***) => {
				...
				normalModuleFactory.hooks.factorize.tap(PLUGIN_NAME, data => {
					if (!data.request.includes("!")) {
						for (const [key, config] of remotes) {
							if (
								data.request.startsWith(`${key***REMOVED***`) &&
								...
								return new RemoteModule(
									...
									config.external.map((external, i) =>
										external.startsWith("internal ")
											? external.slice(9)
											: `webpack/container/reference/${key***REMOVED***${
													i ? `/fallback-${i***REMOVED***` : ""
												***REMOVED***`
									),
									`.${data.request.slice(key.length)***REMOVED***`,
									config.shareScope
								);
							***REMOVED***
						***REMOVED***
