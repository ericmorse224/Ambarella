...
var input = 'a{font-weight:bold;***REMOVED***';
...
* changes default Internet Explorer compatibility from 10+ to >11, to revert the old default use `{ compatibility: 'ie10' ***REMOVED***` flag;
...
* format options now accepts numerical values for all breaks, which will allow you to have more control over output formatting, e.g. `format: {breaks: {afterComment: 2***REMOVED******REMOVED***` means clean-css will add two line breaks after each comment
...
* adds configurable line breaks via `format: { breakWith: 'lf' ***REMOVED***` option.
...
* `compatibility: { selectors: { mergeLimit: <number> ***REMOVED*** ***REMOVED***` flag in compatibility settings controlling maximum number of selectors in a single rule;
...
* splits `inliner: { request: ..., timeout: ... ***REMOVED***` option into `inlineRequest` and `inlineTimeout` options;
* remote resources without a protocol, e.g. `//fonts.googleapis.com/css?family=Domine:700`, are not inlined anymore;
* changes default Internet Explorer compatibility from 9+ to 10+, to revert the old default use `{ compatibility: 'ie9' ***REMOVED***` flag;
...
* `keepBreaks` option is replaced with `{ format: 'keep-breaks' ***REMOVED***` to ease transition;
...
* `new CleanCSS({ compatibility: '*' ***REMOVED***)` (default) - Internet Explorer 10+ compatibility mode
* `new CleanCSS({ compatibility: 'ie9' ***REMOVED***)` - Internet Explorer 9+ compatibility mode
* `new CleanCSS({ compatibility: 'ie8' ***REMOVED***)` - Internet Explorer 8+ compatibility mode
* `new CleanCSS({ compatibility: 'ie7' ***REMOVED***)` - Internet Explorer 7+ compatibility mode
...
new CleanCSS({
  fetch: function (uri, inlineRequest, inlineTimeout, callback) {
    request(uri, function (error, response, body) {
      if (error) {
        callback(error, null);
      ***REMOVED*** else if (response && response.statusCode != 200) {
...
new CleanCSS({plugins: [myPlugin]***REMOVED***)
...
// Log:
{
  styles: 'a{color:#00f***REMOVED***div{margin:5px***REMOVED***',
  stats: {
...
```js
new CleanCSS({ returnPromise: true ***REMOVED***)
  .minify(source)
  .then(function (output) { console.log(output.styles); ***REMOVED***)
  .catch(function (error) { // deal with errors ***REMOVED***);
```
...
```js
new CleanCSS().minify([
  {'path/to/file/one': {styles: 'contents of file one'***REMOVED******REMOVED***,
  {'path/to/file/two': {styles: 'contents of file two'***REMOVED******REMOVED***
]);
```
...
```js
var output = new CleanCSS({ batch: true ***REMOVED***).minify(['path/to/file/one', 'path/to/file/two']);
...
new CleanCSS({ inline: ['remote'] ***REMOVED***).minify(source, function (error, output) {
  // output.styles
***REMOVED***);
```
...
```css
.block-1{color:red***REMOVED***
.block-special {
  color: transparent
***REMOVED***
.block-2{margin:0***REMOVED***
```
...
```js
new CleanCSS({ sourceMap: true, rebaseTo: pathToOutputDirectory ***REMOVED***)
...
```js
new CleanCSS({ sourceMap: true, rebaseTo: pathToOutputDirectory ***REMOVED***).minify({
  'path/to/source/1': {
    styles: '...styles...',
...
* `restructure` - tries to reorganize different-selector different-rules rulesets so they take less space, e.g. `.one{padding:0***REMOVED***.two{margin:0***REMOVED***.one{margin-bottom:3px***REMOVED***` into `.two{margin:0***REMOVED***.one{padding:0;margin-bottom:3px***REMOVED***`;
...
```js
const { src, dest, series ***REMOVED*** = require('gulp');
const CleanCSS = require('clean-css');
