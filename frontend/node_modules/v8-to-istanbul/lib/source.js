...
const { sliceRange ***REMOVED*** = require('./range')
const { GREATEST_LOWER_BOUND, LEAST_UPPER_BOUND ***REMOVED*** = require('source-map').SourceMapConsumer
...
module.exports = class CovSource {
  ...
  ***REMOVED***
   ...
   *    c8 ignore start
   *    c8 ignore stop
   * @param {string***REMOVED*** lineStr
   * @return {{count?: number, start?: boolean, stop?: boolean***REMOVED***|undefined***REMOVED***
   */
  _parseIgnore (lineStr) {
    const testIgnoreNextLines = lineStr.match(/^\W*\/\* c8 ignore next (?<count>[0-9]+) *\*\/\W*$/)
    if (testIgnoreNextLines) {
      return { count: Number(testIgnoreNextLines.groups.count) ***REMOVED***
    ***REMOVED***

    // Check if comment is on its own line.
    if (lineStr.match(/^\W*\/\* c8 ignore next *\*\/\W*$/)) {
      return { count: 1 ***REMOVED***
    ***REMOVED***
    ...
    ***REMOVED***
    ...
    if (testIgnoreStartStop) {
      if (testIgnoreStartStop.groups.mode === 'start') return { start: true ***REMOVED***
      if (testIgnoreStartStop.groups.mode === 'stop') return { stop: true ***REMOVED***
    ***REMOVED***
  ***REMOVED***
