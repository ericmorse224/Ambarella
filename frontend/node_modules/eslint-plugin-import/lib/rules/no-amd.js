'use strict';




var _contextCompat = require('eslint-module-utils/contextCompat');

var _docsUrl = require('../docsUrl');var _docsUrl2 = _interopRequireDefault(_docsUrl);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { 'default': obj ***REMOVED***;***REMOVED***

//------------------------------------------------------------------------------
// Rule Definition
//------------------------------------------------------------------------------
/**
 * @fileoverview Rule to prefer imports to AMD
 * @author Jamund Ferguson
 */module.exports = { meta: { type: 'suggestion',
    docs: {
      category: 'Module systems',
      description: 'Forbid AMD `require` and `define` calls.',
      url: (0, _docsUrl2['default'])('no-amd') ***REMOVED***,

    schema: [] ***REMOVED***,


  create: function () {function create(context) {
      return {
        CallExpression: function () {function CallExpression(node) {
            if ((0, _contextCompat.getScope)(context, node).type !== 'module') {return;***REMOVED***

            if (node.callee.type !== 'Identifier') {return;***REMOVED***
            if (node.callee.name !== 'require' && node.callee.name !== 'define') {return;***REMOVED***

            // todo: capture define((require, module, exports) => {***REMOVED***) form?
            if (node.arguments.length !== 2) {return;***REMOVED***

            var modules = node.arguments[0];
            if (modules.type !== 'ArrayExpression') {return;***REMOVED***

            // todo: check second arg type? (identifier or callback)

            context.report(node, 'Expected imports instead of AMD ' + String(node.callee.name) + '().');
          ***REMOVED***return CallExpression;***REMOVED***() ***REMOVED***;


    ***REMOVED***return create;***REMOVED***() ***REMOVED***;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,