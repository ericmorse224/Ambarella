    ...
    const parens = node => {
      ...
      const fixParamsWithParenthesis = fixer => {
        ...
        */
        ...
        return fixer.replaceTextRange([firstTokenOfParam.range[0], closingParenToken.range[1]], `${shouldAddSpaceForAsync ? ' ' : ''***REMOVED***${paramToken.value***REMOVED***`);
      ***REMOVED***; // Type parameters without an opening paren is always a parse error, and
      // can therefore be safely ignored.


      if (node.typeParameters) {
        return;
      ***REMOVED*** // Similarly, a predicate always requires parens just like a return type
      // does, and therefore this case can also be safely ignored.


      if (node.predicate) {
        return;
      ***REMOVED*** // "as-needed", { "requireForBlockBody": true ***REMOVED***: x => x
      ...
      ***REMOVED***
      ...
      if (requireForBlockBody && node.body.type === 'BlockStatement') {
        if (!isOpeningParenToken(firstTokenOfParam)) {
          context.report({
            fix(fixer) {
              return fixer.replaceText(firstTokenOfParam, `(${firstTokenOfParam.value***REMOVED***)`);
            ***REMOVED***,
