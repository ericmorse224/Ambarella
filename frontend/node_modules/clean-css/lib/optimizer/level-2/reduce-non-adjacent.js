...
function reduceNonAdjacent(tokens, context) {
  ...
  for (var i = tokens.length - 1; i >= 0; i--) {
    ...
    if (token[0] != Token.RULE) {
      continue;
    ***REMOVED*** else if (token[2].length === 0) {
      continue;
    ***REMOVED***
    ...
    for (var j = 0, m = selectors.length; j < m; j++) {
      ...
      if (!candidates[selector]) { candidates[selector] = []; ***REMOVED*** else { repeated.push(selector); ***REMOVED***
...
function reduceSimpleNonAdjacentCases(tokens, repeated, candidates, options, context) {
  ...
  ***REMOVED***
  ...
  function reduceBody(token, newBody, processedCount, tokenIdx) {
    if (!data[processedCount - tokenIdx - 1].isPartial) { token[2] = newBody; ***REMOVED***
  ***REMOVED***
...
function reduceComplexNonAdjacentCases(tokens, candidates, options, context) {
  ...
  ***REMOVED***
  ...
  function collectReducedBodies(token, newBody, processedCount, tokenIdx) {
    if (tokenIdx === 0) { localContext.reducedBodies.push(newBody); ***REMOVED***
  ***REMOVED***

  allSelectors:
  for (var complexSelector in candidates) {
    var into = candidates[complexSelector];
    if (!into[0].isComplex) { continue; ***REMOVED***
    ...
    for (var j = 0, m = selectors.length; j < m; j++) {
      ...
      if (data.length < 2) { continue allSelectors; ***REMOVED***
...
function reduceSelector(tokens, data, context, options, outerContext) {
  ...
  for (var j = data.length - 1; j >= 0; j--) {
    if (context.filterOut(j, bodies)) { continue; ***REMOVED***
