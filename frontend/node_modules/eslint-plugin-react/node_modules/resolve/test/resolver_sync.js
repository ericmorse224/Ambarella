...
test('foo', function (t) {
    ...
    t.equal(
        resolve.sync('./foo', { basedir: dir ***REMOVED***),
    ...
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./foo', { basedir: dir ***REMOVED***),
            require.resolve('./foo', { paths: [dir] ***REMOVED***),
            './foo: resolve.sync === require.resolve'
        );
    ***REMOVED***

    t.equal(
        resolve.sync('./foo.js', { basedir: dir ***REMOVED***),
        path.join(dir, 'foo.js'),
        './foo.js'
    );
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./foo.js', { basedir: dir ***REMOVED***),
            require.resolve('./foo.js', { paths: [dir] ***REMOVED***),
            './foo.js: resolve.sync === require.resolve'
        );
    ***REMOVED***

    t.equal(
        resolve.sync('./foo.js', { basedir: dir, filename: path.join(dir, 'bar.js') ***REMOVED***),
        path.join(dir, 'foo.js')
    );
    ...
    ***REMOVED***);
    ...
    t.throws(
        function () {
            resolve.sync('foo', { basedir: dir, filename: path.join(dir, 'bar.js') ***REMOVED***);
        ***REMOVED***,
        {
            name: 'Error',
...
test('bar', function (t) {
    ...
    t.equal(
        resolve.sync('foo', { basedir: basedir ***REMOVED***),
    ...
    if (!requireResolveDefaultPathsBroken && requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('foo', { basedir: basedir ***REMOVED***),
            require.resolve('foo', { paths: [basedir] ***REMOVED***),
...
test('baz', function (t) {
    ...
    t.equal(
        resolve.sync('./baz', { basedir: dir ***REMOVED***),
    ...
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./baz', { basedir: dir ***REMOVED***),
            require.resolve('./baz', { paths: [dir] ***REMOVED***),
...
test('biz', function (t) {
    ...
    t.equal(
        resolve.sync('./grux', { basedir: dir ***REMOVED***),
        path.join(dir, 'grux/index.js')
    );
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./grux', { basedir: dir ***REMOVED***),
            require.resolve('./grux', { paths: [dir] ***REMOVED***),
    ...
    ***REMOVED***
    ...
    t.equal(
        resolve.sync('tiv', { basedir: tivDir ***REMOVED***),
        path.join(dir, 'tiv/index.js')
    );
    if (!requireResolveDefaultPathsBroken && requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('tiv', { basedir: tivDir ***REMOVED***),
            require.resolve('tiv', { paths: [tivDir] ***REMOVED***),
    ...
    ***REMOVED***
    ...
    t.equal(
        resolve.sync('grux', { basedir: gruxDir ***REMOVED***),
        path.join(dir, 'grux/index.js')
    );
    if (!requireResolveDefaultPathsBroken && requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('grux', { basedir: gruxDir ***REMOVED***),
            require.resolve('grux', { paths: [gruxDir] ***REMOVED***),
            'grux: resolve.sync === require.resolve'
        );
    ***REMOVED***
...
test('cup', function (t) {
    ...
    t.equal(
        resolve.sync('./cup.coffee', { basedir: dir ***REMOVED***),
        path.join(dir, 'cup.coffee'),
        './cup.coffee'
    );
    ...
    ***REMOVED***);
    ...
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./cup.coffee', { basedir: dir, extensions: ['.js', '.coffee'] ***REMOVED***),
            require.resolve('./cup.coffee', { paths: [dir] ***REMOVED***),
...
test('mug', function (t) {
    ...
    t.equal(
        resolve.sync('./mug', { basedir: dir ***REMOVED***),
    ...
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./mug', { basedir: dir ***REMOVED***),
            require.resolve('./mug', { paths: [dir] ***REMOVED***),
...
test('other path', function (t) {
    ...
    t.throws(function () {
        resolve.sync('root', { basedir: dir ***REMOVED***);
    ***REMOVED***);
...
test('path iterator', function (t) {
    ...
    ***REMOVED***;
    ...
    t.equal(
        resolve.sync('baz', { packageIterator: exactIterator ***REMOVED***),
...
test('incorrect main', function (t) {
    ...
    t.equal(
        resolve.sync('./incorrect_main', { basedir: resolverDir ***REMOVED***),
        path.join(dir, 'index.js')
    );
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./incorrect_main', { basedir: resolverDir ***REMOVED***),
            require.resolve('./incorrect_main', { paths: [resolverDir] ***REMOVED***),
...
test('missing index', function (t) {
    ...
    var resolverDir = path.join(__dirname, 'resolver');
    try {
        resolve.sync('./missing_index', { basedir: resolverDir ***REMOVED***);
        t.fail('did not fail');
    ***REMOVED*** catch (err) {
    ...
    ***REMOVED***
    if (requireResolveSupportsPaths) {
        try {
            require.resolve('./missing_index', { basedir: resolverDir ***REMOVED***);
...
test('missing main', function (t) {
    ...
    t.equal(
        resolve.sync('./missing_main', { basedir: resolverDir ***REMOVED***),
        path.join(dir, 'index.js')
    );
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./missing_main', { basedir: resolverDir ***REMOVED***),
            require.resolve('./missing_main', { paths: [resolverDir] ***REMOVED***),
...
test('null main', function (t) {
    ...
    t.equal(
        resolve.sync('./null_main', { basedir: resolverDir ***REMOVED***),
        path.join(dir, 'index.js')
    );
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./null_main', { basedir: resolverDir ***REMOVED***),
            require.resolve('./null_main', { paths: [resolverDir] ***REMOVED***),
...
test('main: false', function (t) {
    ...
    t.equal(
        resolve.sync('./false_main', { basedir: basedir ***REMOVED***),
        path.join(dir, 'index.js'),
    ...
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./false_main', { basedir: basedir ***REMOVED***),
            require.resolve('./false_main', { paths: [basedir] ***REMOVED***),
...
test('#211 - incorrectly resolves module-paths like "." when from inside a folder with a sibling file of the same name', function (t) {
    ...
    t.equal(
        resolve.sync('./', { basedir: basedir ***REMOVED***),
        path.join(dir, 'same_names/foo/index.js'),
        './'
    );
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./', { basedir: basedir ***REMOVED***),
            require.resolve('./', { paths: [basedir] ***REMOVED***),
...
test('sync: #121 - treating an existing file as a dir when no basedir', function (t) {
    ...
    ***REMOVED***);
    ...
    t.test('with a fake directory', function (st) {
        function run() { return resolve.sync('./' + testFile + '/blah'); ***REMOVED***
...
test('not a directory', function (t) {
    var path = './foo';
    try {
        resolve.sync(path, { basedir: __filename ***REMOVED***);
        t.fail();
    ***REMOVED*** catch (err) {
...
test('non-string "main" field in package.json', function (t) {
    var dir = path.join(__dirname, 'resolver');
    try {
        var result = resolve.sync('./invalid_main', { basedir: dir ***REMOVED***);
...
test('absolute paths', function (t) {
    ...
    t.equal(
        resolve.sync(__filename, { basedir: process.cwd() ***REMOVED***),
        __filename,
    ...
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync(__filename, { basedir: process.cwd() ***REMOVED***),
            require.resolve(__filename, { paths: [process.cwd()] ***REMOVED***),
            'absolute path to this file + basedir: resolve.sync === require.resolve'
        );
    ***REMOVED***

    t.equal(
        resolve.sync(extensionless, { basedir: process.cwd() ***REMOVED***),
        __filename,
    ...
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync(extensionless, { basedir: process.cwd() ***REMOVED***),
            require.resolve(extensionless, { paths: [process.cwd()] ***REMOVED***),
...
test('malformed package.json', { skip: !fs.existsSync(malformedDir) ***REMOVED***, function (t) {
    ...
    t.equal(
        resolve.sync('./index.js', { basedir: basedir ***REMOVED***),
        expected,
    ...
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./index.js', { basedir: basedir ***REMOVED***),
            require.resolve('./index.js', { paths: [basedir] ***REMOVED***),
