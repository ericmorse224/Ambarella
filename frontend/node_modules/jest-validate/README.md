...
```js
import {validate***REMOVED*** from 'jest-validate';

validate(config, validationOptions); // => {hasDeprecationWarnings: boolean, isValid: boolean***REMOVED***
```
...
```js
validate(config, {exampleConfig***REMOVED***);
```
...
```js
import {multipleValidOptions***REMOVED*** from 'jest-validate';
