...
import {QueueStoreEntry, UnidentifiedQueueStoreEntry***REMOVED*** from './lib/QueueDb.js';
...
class Queue {
  ...
  constructor(
    name: string,
    {forceSyncFallback, onSync, maxRetentionTime***REMOVED***: QueueOptions = {***REMOVED***,
  ) {
  ...
  ***REMOVED***
   ...
   */
  async _addRequest(
    {request, metadata, timestamp = Date.now()***REMOVED***: QueueEntry,
    ...
    ***REMOVED***
    ...
    if (process.env.NODE_ENV !== 'production') {
      logger.log(
        `Request for '${getFriendlyURL(request.url)***REMOVED***' has ` +
  ...
  async replayRequests(): Promise<void> {
    ...
    while ((entry = await this.shiftRequest())) {
      try {
        ...
        if (process.env.NODE_ENV !== 'production') {
          logger.log(
            `Request for '${getFriendlyURL(entry.request.url)***REMOVED***' ` +
    ...
    ***REMOVED***
    if (process.env.NODE_ENV !== 'production') {
      logger.log(
        `All requests in queue '${this.name***REMOVED***' have successfully ` +
  ...
  async registerSync(): Promise<void> {
    ...
    if ('sync' in self.registration && !this._forceSyncFallback) {
      ...
      ***REMOVED*** catch (err) {
        ...
        if (process.env.NODE_ENV !== 'production') {
          logger.warn(
            `Unable to register sync event for '${this._name***REMOVED***'.`,
            err,
          );
        ***REMOVED***
      ***REMOVED***
    ...
    if ('sync' in self.registration && !this._forceSyncFallback) {
      self.addEventListener('sync', (event: SyncEvent) => {
        if (event.tag === `${TAG_PREFIX***REMOVED***:${this._name***REMOVED***`) {
          if (process.env.NODE_ENV !== 'production') {
            logger.log(
              `Background sync for tag '${event.tag***REMOVED***' ` + `has been received`,
            );
          ***REMOVED***
