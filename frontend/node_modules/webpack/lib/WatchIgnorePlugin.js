/*
	MIT License http://www.opensource.org/licenses/mit-license.php
	Author Tobias Koppers @sokra
*/

"use strict";

const { groupBy ***REMOVED*** = require("./util/ArrayHelpers");
const createSchemaValidation = require("./util/create-schema-validation");

/** @typedef {import("../declarations/plugins/WatchIgnorePlugin").WatchIgnorePluginOptions***REMOVED*** WatchIgnorePluginOptions */
/** @typedef {import("../declarations/WebpackOptions").WatchOptions***REMOVED*** WatchOptions */
/** @typedef {import("./Compiler")***REMOVED*** Compiler */
/** @typedef {import("./util/fs").TimeInfoEntries***REMOVED*** TimeInfoEntries */
/** @typedef {import("./util/fs").WatchFileSystem***REMOVED*** WatchFileSystem */
/** @typedef {import("./util/fs").WatchMethod***REMOVED*** WatchMethod */
/** @typedef {import("./util/fs").Watcher***REMOVED*** Watcher */
const validate = createSchemaValidation(
	require("../schemas/plugins/WatchIgnorePlugin.check.js"),
	() => require("../schemas/plugins/WatchIgnorePlugin.json"),
	{
		name: "Watch Ignore Plugin",
		baseDataPath: "options"
	***REMOVED***
);

const IGNORE_TIME_ENTRY = "ignore";

class IgnoringWatchFileSystem {
	/**
	 * @param {WatchFileSystem***REMOVED*** wfs original file system
	 * @param {WatchIgnorePluginOptions["paths"]***REMOVED*** paths ignored paths
	 */
	constructor(wfs, paths) {
		this.wfs = wfs;
		this.paths = paths;
	***REMOVED***

	/** @type {WatchMethod***REMOVED*** */
	watch(files, dirs, missing, startTime, options, callback, callbackUndelayed) {
		files = Array.from(files);
		dirs = Array.from(dirs);
		/**
		 * @param {string***REMOVED*** path path to check
		 * @returns {boolean***REMOVED*** true, if path is ignored
		 */
		const ignored = path =>
			this.paths.some(p =>
				p instanceof RegExp ? p.test(path) : path.indexOf(p) === 0
			);

		const [ignoredFiles, notIgnoredFiles] = groupBy(
			/** @type {Array<string>***REMOVED*** */
			(files),
			ignored
		);
		const [ignoredDirs, notIgnoredDirs] = groupBy(
			/** @type {Array<string>***REMOVED*** */
			(dirs),
			ignored
		);

		const watcher = this.wfs.watch(
			notIgnoredFiles,
			notIgnoredDirs,
			missing,
			startTime,
			options,
			(err, fileTimestamps, dirTimestamps, changedFiles, removedFiles) => {
				if (err) return callback(err);
				for (const path of ignoredFiles) {
					/** @type {TimeInfoEntries***REMOVED*** */
					(fileTimestamps).set(path, IGNORE_TIME_ENTRY);
				***REMOVED***

				for (const path of ignoredDirs) {
					/** @type {TimeInfoEntries***REMOVED*** */
					(dirTimestamps).set(path, IGNORE_TIME_ENTRY);
				***REMOVED***

				callback(
					null,
					fileTimestamps,
					dirTimestamps,
					changedFiles,
					removedFiles
				);
			***REMOVED***,
			callbackUndelayed
		);

		return {
			close: () => watcher.close(),
			pause: () => watcher.pause(),
			getContextTimeInfoEntries: () => {
				const dirTimestamps = watcher.getContextTimeInfoEntries();
				for (const path of ignoredDirs) {
					dirTimestamps.set(path, IGNORE_TIME_ENTRY);
				***REMOVED***
				return dirTimestamps;
			***REMOVED***,
			getFileTimeInfoEntries: () => {
				const fileTimestamps = watcher.getFileTimeInfoEntries();
				for (const path of ignoredFiles) {
					fileTimestamps.set(path, IGNORE_TIME_ENTRY);
				***REMOVED***
				return fileTimestamps;
			***REMOVED***,
			getInfo:
				watcher.getInfo &&
				(() => {
					const info =
						/** @type {NonNullable<Watcher["getInfo"]>***REMOVED*** */
						(watcher.getInfo)();
					const { fileTimeInfoEntries, contextTimeInfoEntries ***REMOVED*** = info;
					for (const path of ignoredFiles) {
						fileTimeInfoEntries.set(path, IGNORE_TIME_ENTRY);
					***REMOVED***
					for (const path of ignoredDirs) {
						contextTimeInfoEntries.set(path, IGNORE_TIME_ENTRY);
					***REMOVED***
					return info;
				***REMOVED***)
		***REMOVED***;
	***REMOVED***
***REMOVED***

const PLUGIN_NAME = "WatchIgnorePlugin";

class WatchIgnorePlugin {
	/**
	 * @param {WatchIgnorePluginOptions***REMOVED*** options options
	 */
	constructor(options) {
		validate(options);
		this.paths = options.paths;
	***REMOVED***

	/**
	 * Apply the plugin
	 * @param {Compiler***REMOVED*** compiler the compiler instance
	 * @returns {void***REMOVED***
	 */
	apply(compiler) {
		compiler.hooks.afterEnvironment.tap(PLUGIN_NAME, () => {
			compiler.watchFileSystem = new IgnoringWatchFileSystem(
				/** @type {WatchFileSystem***REMOVED*** */
				(compiler.watchFileSystem),
				this.paths
			);
		***REMOVED***);
	***REMOVED***
***REMOVED***

module.exports = WatchIgnorePlugin;
