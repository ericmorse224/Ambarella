"use strict";

exports.__esModule = true;
exports.default = void 0;

var _declaration = _interopRequireDefault(require("./declaration"));

var _processor = _interopRequireDefault(require("./processor"));

var _stringify = _interopRequireDefault(require("./stringify"));

var _comment = _interopRequireDefault(require("./comment"));

var _atRule = _interopRequireDefault(require("./at-rule"));

var _vendor = _interopRequireDefault(require("./vendor"));

var _parse = _interopRequireDefault(require("./parse"));

var _list = _interopRequireDefault(require("./list"));

var _rule = _interopRequireDefault(require("./rule"));

var _root = _interopRequireDefault(require("./root"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj ***REMOVED***; ***REMOVED***

/**
 * Create a new {@link Processor***REMOVED*** instance that will apply `plugins`
 * as CSS processors.
 *
 * @param {Array.<Plugin|pluginFunction>|Processor***REMOVED*** plugins PostCSS plugins.
 *        See {@link Processor#use***REMOVED*** for plugin format.
 *
 * @return {Processor***REMOVED*** Processor to process multiple CSS.
 *
 * @example
 * import postcss from 'postcss'
 *
 * postcss(plugins).process(css, { from, to ***REMOVED***).then(result => {
 *   console.log(result.css)
 * ***REMOVED***)
 *
 * @namespace postcss
 */
function postcss() {
  for (var _len = arguments.length, plugins = new Array(_len), _key = 0; _key < _len; _key++) {
    plugins[_key] = arguments[_key];
  ***REMOVED***

  if (plugins.length === 1 && Array.isArray(plugins[0])) {
    plugins = plugins[0];
  ***REMOVED***

  return new _processor.default(plugins);
***REMOVED***
/**
 * Creates a PostCSS plugin with a standard API.
 *
 * The newly-wrapped function will provide both the name and PostCSS
 * version of the plugin.
 *
 * ```js
 * const processor = postcss([replace])
 * processor.plugins[0].postcssPlugin  //=> 'postcss-replace'
 * processor.plugins[0].postcssVersion //=> '6.0.0'
 * ```
 *
 * The plugin function receives 2 arguments: {@link Root***REMOVED***
 * and {@link Result***REMOVED*** instance. The function should mutate the provided
 * `Root` node. Alternatively, you can create a new `Root` node
 * and override the `result.root` property.
 *
 * ```js
 * const cleaner = postcss.plugin('postcss-cleaner', () => {
 *   return (root, result) => {
 *     result.root = postcss.root()
 *   ***REMOVED***
 * ***REMOVED***)
 * ```
 *
 * As a convenience, plugins also expose a `process` method so that you can use
 * them as standalone tools.
 *
 * ```js
 * cleaner.process(css, processOpts, pluginOpts)
 * // This is equivalent to:
 * postcss([ cleaner(pluginOpts) ]).process(css, processOpts)
 * ```
 *
 * Asynchronous plugins should return a `Promise` instance.
 *
 * ```js
 * postcss.plugin('postcss-import', () => {
 *   return (root, result) => {
 *     return new Promise( (resolve, reject) => {
 *       fs.readFile('base.css', (base) => {
 *         root.prepend(base)
 *         resolve()
 *       ***REMOVED***)
 *     ***REMOVED***)
 *   ***REMOVED***
 * ***REMOVED***)
 * ```
 *
 * Add warnings using the {@link Node#warn***REMOVED*** method.
 * Send data to other plugins using the {@link Result#messages***REMOVED*** array.
 *
 * ```js
 * postcss.plugin('postcss-caniuse-test', () => {
 *   return (root, result) => {
 *     root.walkDecls(decl => {
 *       if (!caniuse.support(decl.prop)) {
 *         decl.warn(result, 'Some browsers do not support ' + decl.prop)
 *       ***REMOVED***
 *     ***REMOVED***)
 *   ***REMOVED***
 * ***REMOVED***)
 * ```
 *
 * @param {string***REMOVED*** name          PostCSS plugin name. Same as in `name`
 *                               property in `package.json`. It will be saved
 *                               in `plugin.postcssPlugin` property.
 * @param {function***REMOVED*** initializer Will receive plugin options
 *                               and should return {@link pluginFunction***REMOVED***
 *
 * @return {Plugin***REMOVED*** PostCSS plugin.
 */


postcss.plugin = function plugin(name, initializer) {
  function creator() {
    var transformer = initializer.apply(void 0, arguments);
    transformer.postcssPlugin = name;
    transformer.postcssVersion = new _processor.default().version;
    return transformer;
  ***REMOVED***

  var cache;
  Object.defineProperty(creator, 'postcss', {
    get: function get() {
      if (!cache) cache = creator();
      return cache;
    ***REMOVED***
  ***REMOVED***);

  creator.process = function (css, processOpts, pluginOpts) {
    return postcss([creator(pluginOpts)]).process(css, processOpts);
  ***REMOVED***;

  return creator;
***REMOVED***;
/**
 * Default function to convert a node tree into a CSS string.
 *
 * @param {Node***REMOVED*** node       Start node for stringifing. Usually {@link Root***REMOVED***.
 * @param {builder***REMOVED*** builder Function to concatenate CSS from node’s parts
 *                          or generate string and source map.
 *
 * @return {void***REMOVED***
 *
 * @function
 */


postcss.stringify = _stringify.default;
/**
 * Parses source css and returns a new {@link Root***REMOVED*** node,
 * which contains the source CSS nodes.
 *
 * @param {string|toString***REMOVED*** css   String with input CSS or any object
 *                                with toString() method, like a Buffer
 * @param {processOptions***REMOVED*** [opts] Options with only `from` and `map` keys.
 *
 * @return {Root***REMOVED*** PostCSS AST.
 *
 * @example
 * // Simple CSS concatenation with source map support
 * const root1 = postcss.parse(css1, { from: file1 ***REMOVED***)
 * const root2 = postcss.parse(css2, { from: file2 ***REMOVED***)
 * root1.append(root2).toResult().css
 *
 * @function
 */

postcss.parse = _parse.default;
/**
 * Contains the {@link vendor***REMOVED*** module.
 *
 * @type {vendor***REMOVED***
 *
 * @example
 * postcss.vendor.unprefixed('-moz-tab') //=> ['tab']
 */

postcss.vendor = _vendor.default;
/**
 * Contains the {@link list***REMOVED*** module.
 *
 * @member {list***REMOVED***
 *
 * @example
 * postcss.list.space('5px calc(10% + 5px)') //=> ['5px', 'calc(10% + 5px)']
 */

postcss.list = _list.default;
/**
 * Creates a new {@link Comment***REMOVED*** node.
 *
 * @param {object***REMOVED*** [defaults] Properties for the new node.
 *
 * @return {Comment***REMOVED*** New comment node
 *
 * @example
 * postcss.comment({ text: 'test' ***REMOVED***)
 */

postcss.comment = function (defaults) {
  return new _comment.default(defaults);
***REMOVED***;
/**
 * Creates a new {@link AtRule***REMOVED*** node.
 *
 * @param {object***REMOVED*** [defaults] Properties for the new node.
 *
 * @return {AtRule***REMOVED*** new at-rule node
 *
 * @example
 * postcss.atRule({ name: 'charset' ***REMOVED***).toString() //=> "@charset"
 */


postcss.atRule = function (defaults) {
  return new _atRule.default(defaults);
***REMOVED***;
/**
 * Creates a new {@link Declaration***REMOVED*** node.
 *
 * @param {object***REMOVED*** [defaults] Properties for the new node.
 *
 * @return {Declaration***REMOVED*** new declaration node
 *
 * @example
 * postcss.decl({ prop: 'color', value: 'red' ***REMOVED***).toString() //=> "color: red"
 */


postcss.decl = function (defaults) {
  return new _declaration.default(defaults);
***REMOVED***;
/**
 * Creates a new {@link Rule***REMOVED*** node.
 *
 * @param {object***REMOVED*** [defaults] Properties for the new node.
 *
 * @return {Rule***REMOVED*** new rule node
 *
 * @example
 * postcss.rule({ selector: 'a' ***REMOVED***).toString() //=> "a {\n***REMOVED***"
 */


postcss.rule = function (defaults) {
  return new _rule.default(defaults);
***REMOVED***;
/**
 * Creates a new {@link Root***REMOVED*** node.
 *
 * @param {object***REMOVED*** [defaults] Properties for the new node.
 *
 * @return {Root***REMOVED*** new root node.
 *
 * @example
 * postcss.root({ after: '\n' ***REMOVED***).toString() //=> "\n"
 */


postcss.root = function (defaults) {
  return new _root.default(defaults);
***REMOVED***;

var _default = postcss;
exports.default = _default;
module.exports = exports.default;
//# sourceMappingURL=data:application/json;charset=utf8;base64,
