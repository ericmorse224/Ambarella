...
Importing multiple named exports from a single module will only count once (e.g. `import {x, y, z***REMOVED*** from './foo'` will only count as a single dependency).
...
Given a max value of `{"max": 2***REMOVED***`:
...
import {x, y, z***REMOVED*** from './foo'; // 2
```
...
Given `{"max": 2, "ignoreTypeImports": true***REMOVED***`:
