...
const instance = axios.create({
  ...
  timeout: 1000,
  headers: {'X-Custom-Header': 'foobar'***REMOVED***
***REMOVED***);
```
...
```js
{
  ...
  ***REMOVED***],
  ...
  headers: {'X-Requested-With': 'XMLHttpRequest'***REMOVED***,
  ...
  ***REMOVED***,
  ...
  paramsSerializer: function (params) {
    return Qs.stringify(params, {arrayFormat: 'brackets'***REMOVED***)
  ***REMOVED***,
  ...
  ***REMOVED***,
  ...
  httpAgent: new http.Agent({ keepAlive: true ***REMOVED***),
  httpsAgent: new https.Agent({ keepAlive: true ***REMOVED***),
...
```js
const myInterceptor = axios.interceptors.request.use(function () {/*...*/***REMOVED***);
...
```js
const instance = axios.create();
instance.interceptors.request.use(function () {/*...*/***REMOVED***);
```
...
axios.interceptors.request.use(function (config) {
  ...
  return config;
***REMOVED***, null, { synchronous: true ***REMOVED***);
```
...
axios.interceptors.request.use(function (config) {
  ...
  return config;
***REMOVED***, null, { runWhen: onGetCall ***REMOVED***);
```
...
```js
const qs = require('qs');
axios.post('/foo', qs.stringify({ 'bar': 123 ***REMOVED***));
```
...
```js
import qs from 'qs';
const data = { 'bar': 123 ***REMOVED***;
const options = {
  method: 'POST',
  headers: { 'content-type': 'application/x-www-form-urlencoded' ***REMOVED***,
...
axios.post('http://something.com/', querystring.stringify({ foo: 'bar' ***REMOVED***));
```
...
const params = new url.URLSearchParams({ foo: 'bar' ***REMOVED***);
...
axios.post('https://example.com', form, { headers: form.getHeaders() ***REMOVED***)
```
...
let user: User = null;
try {
  const { data ***REMOVED*** = await axios.get('/user?ID=12345');
