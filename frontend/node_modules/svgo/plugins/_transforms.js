 ...
 *
 * @param {String***REMOVED*** transformString input string
    ...
    transformString.split(regTransformSplit).forEach(function(item) {
        ...
        if (item) {
            ...
            if (regTransformTypes.test(item)) {
                ...
                transforms.push(current = { name: item ***REMOVED***);
 ...
 *
 * @param {Array***REMOVED*** input transforms array
 * @return {Array***REMOVED*** output matrix array
 */
 ...
 *
 * @param {Object***REMOVED*** data matrix transform object
 * @return {Object|Array***REMOVED*** transforms array or original transform object
 */
exports.matrixToTransform = function(transform, params) {
    ...
    if (data[4] || data[5]) {
        transforms.push({ name: 'translate', data: data.slice(4, data[5] ? 6 : 5) ***REMOVED***);
    ***REMOVED***

    // [sx, 0, tan(a)·sy, sy, 0, 0] → skewX(a)·scale(sx, sy)
    if (!data[1] && data[2]) {
        transforms.push({ name: 'skewX', data: [mth.atan(data[2] / sy, floatPrecision)] ***REMOVED***);

    // [sx, sx·tan(a), 0, sy, 0, 0] → skewY(a)·scale(sx, sy)
    ***REMOVED*** else if (data[1] && !data[2]) {
        transforms.push({ name: 'skewY', data: [mth.atan(data[1] / data[0], floatPrecision)] ***REMOVED***);
        sx = data[0];
        sy = data[3];

    // [sx·cos(a), sx·sin(a), sy·-sin(a), sy·cos(a), x, y] → rotate(a[, cx, cy])·(scale or skewX) or
    // [sx·cos(a), sy·sin(a), sx·-sin(a), sy·cos(a), x, y] → scale(sx, sy)·rotate(a[, cx, cy]) (if !scaleBefore)
    ***REMOVED*** else if (!colsSum || (sx == 1 && sy == 1) || !scaleBefore) {
        if (!scaleBefore) {
            sx = (data[0] < 0 ? -1 : 1) * Math.hypot(data[0], data[2]);
            sy = (data[3] < 0 ? -1 : 1) * Math.hypot(data[1], data[3]);
            transforms.push({ name: 'scale', data: [sx, sy] ***REMOVED***);
        ***REMOVED***
        var angle = Math.min(Math.max(-1, data[0] / sx), 1),
            rotate = [mth.acos(angle, floatPrecision) * ((scaleBefore ? 1 : sy) * data[1] < 0 ? -1 : 1)];

        if (rotate[0]) transforms.push({ name: 'rotate', data: rotate ***REMOVED***);
 ...
 *
 * @param {Object***REMOVED*** transform transform object
 * @return {Array***REMOVED*** matrix data
 */
 ...
 * SVD is being done with the formulae provided by Wolffram|Alpha (svd {{m0, m2***REMOVED***, {m1, m3***REMOVED******REMOVED***)
 *
 * @param {Array***REMOVED*** arc [a, b, rotation in deg]
 * @param {Array***REMOVED*** transform transformation matrix
 * @return {Array***REMOVED*** arc transformed input arc
 */
 ...
 *
 * @param {Array***REMOVED*** a matrix A data
 * @param {Array***REMOVED*** b matrix B data
 * @return {Array***REMOVED*** result
 */
