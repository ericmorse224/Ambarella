...
function commentAtPositionCallback(pos, end, kind, _nl, at) {
    return at >= pos && at < end ? { pos, end, kind ***REMOVED*** : undefined;
***REMOVED***
/**
            ...
            switch (expression.operator) {
                ...
                case ts.SyntaxKind.PlusToken:
                    return node_1.isNumericLiteral(expression.operand)
                        ? `${negate ? '-' : ''***REMOVED***${expression.operand.text***REMOVED***`
                        : _3_2_1.isBigIntLiteral(expression.operand)
                            ? `${negate ? '-' : ''***REMOVED***${expression.operand.text.slice(0, -1)***REMOVED***`
                            : undefined;
                default:
                    return;
            ***REMOVED***
...
***REMOVED***)(VariableDeclarationKind = exports.VariableDeclarationKind || (exports.VariableDeclarationKind = {***REMOVED***));
...
***REMOVED***)(ScopeBoundary = exports.ScopeBoundary || (exports.ScopeBoundary = {***REMOVED***));
...
***REMOVED***)(ScopeBoundarySelector = exports.ScopeBoundarySelector || (exports.ScopeBoundarySelector = {***REMOVED***));
    ...
    return forEachToken(node, (token) => {
        ...
        if (tokenStart !== token.pos) {
            ...
            while (pos < tokenStart) {
                ...
                cb(fullText, kind, { pos, end: textPos ***REMOVED***, token.parent);
            ...
            ***REMOVED***
        ***REMOVED***
        return cb(fullText, token.kind, { end: token.end, pos: tokenStart ***REMOVED***, token.parent);
    ***REMOVED***, sourceFile);
***REMOVED***
...
function forEachComment(node, cb, sourceFile = node.getSourceFile()) {
    ...
    ***REMOVED***, sourceFile);
    function commentCallback(pos, end, kind) {
        cb(fullText, { pos, end, kind ***REMOVED***);
    ***REMOVED***
***REMOVED***
...
***REMOVED***)(SideEffectOptions = exports.SideEffectOptions || (exports.SideEffectOptions = {***REMOVED***));
...
***REMOVED***)(AccessKind = exports.AccessKind || (exports.AccessKind = {***REMOVED***));
...
function parseJsDocWorker(node, nodeStart, sourceFile, considerTrailingComments) {
    ...
    (pos, _end, kind) => kind === ts.SyntaxKind.MultiLineCommentTrivia && sourceFile.text[pos + 2] === '*' ? { pos ***REMOVED*** : undefined);
    if (start === undefined)
        return [];
    const startPos = start.pos;
    const text = sourceFile.text.slice(startPos, nodeStart);
    const newSourceFile = ts.createSourceFile('jsdoc.ts', `${text***REMOVED***var a;`, sourceFile.languageVersion);
...
***REMOVED***)(ImportKind = exports.ImportKind || (exports.ImportKind = {***REMOVED***));
...
function getTsCheckDirective(source) {
    ...
    ts.forEachLeadingCommentRange(source, (ts.getShebang(source) || '').length, (pos, end, kind) => {
        if (kind === ts.SyntaxKind.SingleLineCommentTrivia) {
            ...
            const match = /^\/{2,3***REMOVED***\s*@ts-(no)?check(?:\s|$)/i.exec(text);
            if (match !== null)
                directive = { pos, end, enabled: match[1] === undefined ***REMOVED***;
        ***REMOVED***
    ***REMOVED***);
    return directive;
***REMOVED***
...
/** Returns true for `Object.defineProperty(o, 'prop', {value, writable: false***REMOVED***)` and  `Object.defineProperty(o, 'prop', {get: () => 1***REMOVED***)`*/
...
function getPropertyNameOfWellKnownSymbol(node) {
    return {
        displayName: `[Symbol.${node.name.text***REMOVED***]`,
...
function getLateBoundPropertyNamesOfPropertyName(node, checker) {
    ...
    return staticName !== undefined
        ? { known: true, names: [{ displayName: staticName, symbolName: ts.escapeLeadingUnderscores(staticName) ***REMOVED***] ***REMOVED***
        : node.kind === ts.SyntaxKind.PrivateIdentifier
            ? { known: true, names: [{ displayName: node.text, symbolName: checker.getSymbolAtLocation(node).escapedName ***REMOVED***] ***REMOVED***
...
function getSingleLateBoundPropertyNameOfPropertyName(node, checker) {
    ...
    if (staticName !== undefined)
        return { displayName: staticName, symbolName: ts.escapeLeadingUnderscores(staticName) ***REMOVED***;
    if (node.kind === ts.SyntaxKind.PrivateIdentifier)
        return { displayName: node.text, symbolName: checker.getSymbolAtLocation(node).escapedName ***REMOVED***;
    const { expression ***REMOVED*** = node;
...
function formatPseudoBigInt(v) {
    return `${v.negative ? '-' : ''***REMOVED***${v.base10Value***REMOVED***n`;
***REMOVED***
...
function getPrimitiveLiteralFromType(t) {
    ...
    if (exports.isTypeFlagSet(t, ts.TypeFlags.NumberLiteral))
        return `${exports.isTypeFlagSet(t, ts.TypeFlags.EnumLiteral) ? 'enum:' : ''***REMOVED***${t.value***REMOVED***`;
    if (exports.isTypeFlagSet(t, ts.TypeFlags.StringLiteral))
        return `${exports.isTypeFlagSet(t, ts.TypeFlags.EnumLiteral) ? 'enum:' : ''***REMOVED***string:${t.value***REMOVED***`;
