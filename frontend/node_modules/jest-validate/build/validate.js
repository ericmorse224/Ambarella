...
const _validate = (config, exampleConfig, options, path = []) => {
  ...
  ***REMOVED***
  ...
  for (const key in config) {
    ...
    ***REMOVED*** else if (allowsMultipleTypes(key)) {
      ...
      if (
        ...
        if (key === 'maxWorkers' && !isOfTypeStringOrNumber(value)) {
          throw new _utils.ValidationError(
            'Validation Error',
            `${key***REMOVED*** has to be of type string or number`,
            'maxWorkers=50% or\nmaxWorkers=3'
          );
        ***REMOVED***
      ***REMOVED***
    ***REMOVED*** else if (Object.hasOwnProperty.call(exampleConfig, key)) {
      if (
        typeof options.condition === 'function' &&
        typeof options.error === 'function' &&
...
const validate = (config, options) => {
  ...
  ***REMOVED***);
  ...
  const {hasDeprecationWarnings: hdw***REMOVED*** = _validate(
    config,
