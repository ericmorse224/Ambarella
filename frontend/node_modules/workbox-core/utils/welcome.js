...
if (process.env.NODE_ENV !== 'production') {
    ...
    logger.log(`You are currently using a development build. ` +
        ...
        `You can force this with workbox.setConfig({debug: true|false***REMOVED***).`);
    logger.log(`📖 Read the guides and documentation\n` +
        `${padding***REMOVED***https://developers.google.com/web/tools/workbox/`);
    logger.log(`❓ Use the [workbox] tag on Stack Overflow to ask questions\n` +
        `${padding***REMOVED***https://stackoverflow.com/questions/ask?tags=workbox`);
    logger.log(`🐛 Found a bug? Report it on GitHub\n` +
        `${padding***REMOVED***https://github.com/GoogleChrome/workbox/issues/new`);
    logger.groupEnd();
***REMOVED***
