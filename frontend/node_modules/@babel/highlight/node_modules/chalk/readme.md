...
```js
const ctx = new chalk.constructor({enabled: false***REMOVED***);
```
...
```js
const ctx = new chalk.constructor({level: 0***REMOVED***);
```
...
Blocks are delimited by an opening curly brace (`{`), a style, some content, and a closing curly brace (`***REMOVED***`).
