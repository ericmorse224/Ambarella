...
***REMOVED***[keyof EventData];

declare const listenerAdded: unique symbol;
declare const listenerRemoved: unique symbol;
type _OmnipresentEventData = {[listenerAdded]: Emittery.ListenerChangedData; [listenerRemoved]: Emittery.ListenerChangedData***REMOVED***;
...
interface DebugOptions<EventData> {
	...
	const emitter = new Emittery({debug: {name: 'myEmitter'***REMOVED******REMOVED***);
	...
	*/
	...
	const emitter1 = new Emittery({debug: {name: 'emitter1', enabled: true***REMOVED******REMOVED***);
	const emitter2 = new Emittery({debug: {name: 'emitter2'***REMOVED******REMOVED***);
	...
	***REMOVED***
	...
	const myLogger = (type, debugName, eventName, eventData) => console.log(`[${type***REMOVED***]: ${eventName***REMOVED***`);
...
`Emittery` has a generic `EventData` type that can be provided by users to strongly type the list of events and the data passed to the listeners for those events. Pass an interface of {[eventName]: undefined | <eventArg>***REMOVED***, with all the event names as the keys and the values as the type of the argument passed to listeners if there is one, or `undefined` if there isn't.
...
const emitter = new Emittery<
	// Pass `{[eventName: <string | symbol>]: undefined | <eventArg>***REMOVED***` as the first type argument for events that pass data to their listeners.
...
> {
	...
	const emitter1 = new Emittery({debug: {name: 'myEmitter1'***REMOVED******REMOVED***);
	const emitter2 = new Emittery({debug: {name: 'myEmitter2'***REMOVED******REMOVED***);
	...
	***REMOVED***);
	...
	emitter.on(Emittery.listenerRemoved, ({listener, eventName***REMOVED***) => {
		console.log(listener);
		//=> data => {***REMOVED***
	...
	*/
	...
	): <T extends {new (): any***REMOVED***>(klass: T) => T; // eslint-disable-line @typescript-eslint/prefer-function-type
