...
const PATH_NODE_MODULES = `${path.sep***REMOVED***node_modules${path.sep***REMOVED***`;
const PATH_JEST_PACKAGES = `${path.sep***REMOVED***jest${path.sep***REMOVED***packages${path.sep***REMOVED***`; // filter for noisy stack trace lines
...
function checkForCommonEnvironmentErrors(error) {
  ...
  ***REMOVED*** else if (error.includes('.unref is not a function')) {
...
function warnAboutWrongTestEnvironment(error, env) {
  return (
    _chalk.default.bold.red(
      ...
      )***REMOVED***.\nConsider using the "${env***REMOVED***" test environment.\n\n`
    ) + error
  );
***REMOVED*** // ExecError is an error thrown outside of the test suite (not inside an `it` or
// `before/after each` hooks). If it's thrown, none of the tests in the file
// are executed.

const formatExecError = (error, config, options, testPath, reuseMessage) => {
  if (!error || typeof error === 'number') {
    error = new Error(`Expected an Error, but "${String(error)***REMOVED***" was thrown`);
    error.stack = '';
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  stack =
    stack && !options.noStackTrace
      ? `\n${formatStackTrace(stack, config, options, testPath)***REMOVED***`
      : '';
  ...
  ***REMOVED***
  ...
  if (reuseMessage) {
    messageToUse = ` ${message.trim()***REMOVED***`;
  ***REMOVED*** else {
    messageToUse = `${EXEC_ERROR_MESSAGE***REMOVED***\n\n${message***REMOVED***`;
  ***REMOVED***

  return `${TITLE_INDENT + TITLE_BULLET + messageToUse + stack***REMOVED***\n`;
***REMOVED***;
...
const formatStackTrace = (stack, config, options, testPath) => {
  ...
  if (!options.noStackTrace && !options.noCodeFrame) {
    ...
    if (topFrame) {
      const {column, file: filename, line***REMOVED*** = topFrame;
...
const formatResultsErrors = (testResults, config, options, testPath) => {
  ...
  ***REMOVED***
  ...
  return failedResults
    .map(({result, content***REMOVED***) => {
      let {message, stack***REMOVED*** = separateMessageFromStack(content);
      stack = options.noStackTrace
        ? ''
        : `${STACK_TRACE_COLOR(
...
const separateMessageFromStack = content => {
  if (!content) {
    return {
      ...
      stack: ''
    ***REMOVED***;
  ***REMOVED*** // All lines up to what looks like a stack -- or if nothing looks like a stack
