...
2. Parse options (_Optional_, defaults to `{permissive: false, argv: process.argv.slice(2), stopAtPositional: false***REMOVED***`)
...
const args = arg(
	spec,
	(options = { permissive: false, argv: process.argv.slice(2) ***REMOVED***)
);
```
...
```javascript
const args = arg({ '--name': String ***REMOVED***);
