...
For example, with `{ 'ts-expect-error': 'allow-with-description' ***REMOVED***`:
...
For example, with `{ 'ts-expect-error': { descriptionFormat: '^: TS\\d+ because .+$' ***REMOVED*** ***REMOVED***`:
...
For example, with `{ 'ts-expect-error': 'allow-with-description', minimumDescriptionLength: 10 ***REMOVED***` the following pattern is:
