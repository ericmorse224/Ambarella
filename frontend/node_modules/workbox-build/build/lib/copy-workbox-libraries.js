 ...
 * {@link workbox-build.injectManifest***REMOVED*** who would
 * prefer not to use the CDN copies of Workbox. Developers using
 * {@link workbox-build.generateSW***REMOVED*** don't need to
 * explicitly call this method.
 *
 * @param {string***REMOVED*** destDirectory The path to the parent directory under which
 * the new directory of libraries will be created.
 * @return {Promise<string>***REMOVED*** The name of the newly created directory.
 *
...
async function copyWorkboxLibraries(destDirectory) {
    ...
    const workboxDirectoryName = `workbox-v${thisPkg.version ? thisPkg.version : ''***REMOVED***`;
    ...
    const copyPromises = [];
    const librariesToCopy = Object.keys(thisPkg.dependencies || {***REMOVED***).filter((dependency) => dependency.startsWith(WORKBOX_PREFIX));
    for (const library of librariesToCopy) {
        ...
        const libraryPath = upath_1.default.dirname(require.resolve(`${library***REMOVED***/package.json`));
    ...
    ***REMOVED***
        ...
        throw Error(`${errors_1.errors['unable-to-copy-workbox-libraries']***REMOVED*** ${error instanceof Error ? error.toString() : ''***REMOVED***`);
    ***REMOVED***
***REMOVED***
