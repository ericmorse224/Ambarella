...
const addBabelPlugin = (options, plugin) => {
  return _objectSpread2(_objectSpread2({***REMOVED***, options), {***REMOVED***, {
    plugins: options.plugins.concat(plugin)
  ***REMOVED***);
***REMOVED***;
const warned = {***REMOVED***;
function warnOnce(ctx, msg) {
  if (warned[msg]) return;
  warned[msg] = true;
  ctx.warn(msg);
***REMOVED***
const regExpCharactersRegExp = /[\\^$.*+?()[\]{***REMOVED***|]/g;
const escapeRegExpCharacters = str => str.replace(regExpCharactersRegExp, '\\$&');
function stripQuery(id) {
  // strip query params from import
  const [bareId, query] = id.split('?');
  const suffix = `${query ? `?${query***REMOVED***` : ''***REMOVED***`;
  return {
    bareId,
    query,
    suffix
...
const PREFLIGHT_INPUT = `export default "${PREFLIGHT_TEST_STRING***REMOVED***";`;
...
const mismatchError = (actual, expected, filename) => `You have declared using "${expected***REMOVED***" babelHelpers, but transforming ${filename***REMOVED*** resulted in "${actual***REMOVED***". Please check your configuration.`; // Revert to /\/helpers\/(esm\/)?inherits/ when Babel 8 gets released, this was fixed in https://github.com/babel/babel/issues/14185
...
async function preflightCheck(ctx, babelHelpers, transformOptions) {
  ...
  ***REMOVED*** // test unminifiable string content
  ...
  if (check.includes('Super expression must either be null or a function')) {
    ...
    ***REMOVED***
    ...
    if (babelHelpers === RUNTIME && !transformOptions.plugins.length) {
      ctx.error(`You must use the \`@babel/plugin-transform-runtime\` plugin when \`babelHelpers\` is "${RUNTIME***REMOVED***".\n`);
    ***REMOVED***
...
async function transformCode(inputCode, babelOptions, overrides, customOptions, ctx, finalizeOptions) {
  ...
  ***REMOVED***
  ...
  if (!overrides.result) {
    const {
      code,
      map
    ***REMOVED*** = await babel__namespace.transformAsync(inputCode, transformOptions);
    return {
      code,
      map
    ***REMOVED***;
  ***REMOVED***
  ...
  const {
    code,
    map
  ***REMOVED*** = await overrides.result.call(ctx, result, {
    code: inputCode,
    customOptions,
    config,
    transformOptions
...
const unpackOptions = (_ref = {***REMOVED***) => {
  let {
...
***REMOVED***) => {
  ...
  console.warn(`\`${deprecatedOption***REMOVED***\` has been removed in favor a \`babelHelpers\` option. Try changing your configuration to \`${suggestion***REMOVED***\`. ` + `Refer to the documentation to learn more: https://github.com/rollup/plugins/tree/master/packages/babel#babelhelpers`);
***REMOVED***;
...
const unpackInputPluginOptions = (_ref2, rollupVersion) => {
  ...
  ***REMOVED*** = _ref2,
  ...
  if ('runtimeHelpers' in rest) {
    ...
    ***REMOVED***);
  ***REMOVED*** else if ('externalHelpers' in rest) {
    warnAboutDeprecatedHelpersOption({
      deprecatedOption: 'externalHelpers',
      suggestion: `babelHelpers: 'external'`
    ***REMOVED***);
  ***REMOVED*** else if (!rest.babelHelpers) {
    // eslint-disable-next-line no-console
  ...
  ***REMOVED***
  ...
  return unpackOptions(_objectSpread2(_objectSpread2({***REMOVED***, rest), {***REMOVED***, {
    skipPreflightCheck,
...
const unpackOutputPluginOptions = (options, {
  format
***REMOVED***) => unpackOptions(_objectSpread2(_objectSpread2({
  configFile: false,
  sourceType: format === 'es' ? 'module' : 'script'
***REMOVED***, options), {***REMOVED***, {
  caller: _objectSpread2({
    supportsStaticESM: format === 'es'
  ***REMOVED***, options.caller)
***REMOVED***));

function getOptionsWithOverrides(pluginOptions = {***REMOVED***, overrides = {***REMOVED***) {
  ...
  ***REMOVED***;
  ...
  if (typeof overridden.then === 'function') {
    throw new Error(".options hook can't be asynchronous. It should return `{ customOptions, pluginsOptions ***REMOVED***` synchronously.");
  ***REMOVED***
...
function createBabelInputPluginFactory(customCallback = returnObject) {
  ...
  return pluginOptions => {
    ...
    ***REMOVED*** = getOptionsWithOverrides(pluginOptions, overrides);
    let babelHelpers;
    let babelOptions;
    let filter;
    ...
    return {
      ...
      options() {
        ...
        ***REMOVED*** = _unpackInputPluginOpt);
        ...
        const extensionRegExp = new RegExp(`(${extensions.map(escapeRegExpCharacters).join('|')***REMOVED***)$`);
      ...
      ***REMOVED***,
      ...
      transform(code, filename) {
        ...
        return transformCode(code, _objectSpread2(_objectSpread2({***REMOVED***, babelOptions), {***REMOVED***, {
          filename
        ***REMOVED***), overrides, customOptions, this, async transformOptions => {
...
function createBabelOutputPluginFactory(customCallback = returnObject) {
  ...
  return pluginOptions => {
    ...
    return {
      ...
      renderStart(outputOptions) {
        const {
          ...
          exclude,
          allowAllFormats
        ***REMOVED*** = pluginOptionsWithOverrides;
    ...
    ***REMOVED***;
  ***REMOVED***;
***REMOVED*** // export this for symmetry with output-related exports
