...
app.use('/api', createProxyMiddleware({ target: 'http://www.example.org', changeOrigin: true ***REMOVED***));
app.listen(3000);
...
import { createProxyMiddleware, Filter, Options, RequestHandler ***REMOVED*** from 'http-proxy-middleware';
...
const apiProxy = createProxyMiddleware('/api', { target: 'http://www.example.org' ***REMOVED***);
...
/** @type {import('http-proxy-middleware/dist/types').Options***REMOVED*** */
const options = {
...
- **path matching**
  ...
  - `createProxyMiddleware({...***REMOVED***)` - matches any path, all requests will be proxied.
  - `createProxyMiddleware('/', {...***REMOVED***)` - matches any path, all requests will be proxied.
  - `createProxyMiddleware('/api', {...***REMOVED***)` - matches paths starting with `/api`

- **multiple path matching**

  - `createProxyMiddleware(['/api', '/ajax', '/someotherpath'], {...***REMOVED***)`
...
- **wildcard path matching**
  ...
  - `createProxyMiddleware('**', {...***REMOVED***)` matches any path, all requests will be proxied.
  - `createProxyMiddleware('**/*.html', {...***REMOVED***)` matches any path which ends with `.html`
  - `createProxyMiddleware('/*.html', {...***REMOVED***)` matches paths directly under path-absolute
  - `createProxyMiddleware('/api/**/*.html', {...***REMOVED***)` matches requests ending with `.html` in the path of `/api`
  - `createProxyMiddleware(['/api/**', '/ajax/**'], {...***REMOVED***)` combine multiple patterns
  - `createProxyMiddleware(['/api/**', '!**/bad.json'], {...***REMOVED***)` exclusion
...
- **option.pathRewrite**: object/function, rewrite target's url path. Object-keys will be used as _RegExp_ to match paths.
  ...
  ```javascript
  // rewrite path
  pathRewrite: {'^/old/api' : '/new/api'***REMOVED***

  // remove path
  pathRewrite: {'^/remove/api' : ''***REMOVED***

  // add base path
  pathRewrite: {'^/' : '/basepath/'***REMOVED***

  // custom rewriting
  pathRewrite: function (path, req) { return path.replace('/api', '/base/api') ***REMOVED***
...
- **option.cookiePathRewrite**: rewrites path of `set-cookie` headers. Possible values:
  ...
  - Object: mapping of paths to new paths, use `"*"` to match all paths.
    ...
    ***REMOVED***
    ```
- **option.headers**: object, adds [request headers](https://en.wikipedia.org/wiki/List_of_HTTP_header_fields#Request_fields). (Example: `{host:'www.example.org'***REMOVED***`)
...
// createProxyMiddleware('/api', {target: 'http://www.example.org:8000'***REMOVED***);

createProxyMiddleware('http://www.example.org:8000/api/books/*/**.json');
// createProxyMiddleware('/api/books/*/**.json', {target: 'http://www.example.org:8000'***REMOVED***);

createProxyMiddleware('http://www.example.org:8000/api', { changeOrigin: true ***REMOVED***);
// createProxyMiddleware('/api', {target: 'http://www.example.org:8000', changeOrigin: true***REMOVED***);
```
...
```javascript
// verbose api
createProxyMiddleware('/', { target: 'http://echo.websocket.org', ws: true ***REMOVED***);

// shorthand
createProxyMiddleware('http://echo.websocket.org', { ws: true ***REMOVED***);
...
```javascript
const wsProxy = createProxyMiddleware('ws://echo.websocket.org', { changeOrigin: true ***REMOVED***);
...
```javascript
const { createProxyMiddleware, fixRequestBody ***REMOVED*** = require('http-proxy-middleware');
...
```javascript
const { createProxyMiddleware, responseInterceptor ***REMOVED*** = require('http-proxy-middleware');
