...
import {Queue***REMOVED*** from 'workbox-background-sync/Queue.js';
...
import {RouteMatchCallbackOptions***REMOVED*** from 'workbox-core/types.js';
...
import {
  ...
  GTM_JS_PATH,
  COLLECT_PATHS_REGEX,
***REMOVED*** from './utils/constants.js';
import './_version.js';

export interface GoogleAnalyticsInitializeOptions {
  cacheName?: string;
  parameterOverrides?: {[paramName: string]: string***REMOVED***;
...
const createOnSyncCallback = (config: GoogleAnalyticsInitializeOptions) => {
  return async ({queue***REMOVED***: {queue: Queue***REMOVED***) => {
    let entry;
      ...
      try {
        ...
        ***REMOVED***
        ...
        if (process.env.NODE_ENV !== 'production') {
          logger.log(
            `Request for '${getFriendlyURL(url.href)***REMOVED***' ` + `has been replayed`,
          );
        ***REMOVED***
      ***REMOVED*** catch (err) {
        ...
        if (process.env.NODE_ENV !== 'production') {
          logger.log(
            `Request for '${getFriendlyURL(url.href)***REMOVED***' ` +
...
const createCollectRoutes = (bgSyncPlugin: BackgroundSyncPlugin) => {
  const match = ({url***REMOVED***: RouteMatchCallbackOptions) =>
 ...
 */
const initialize = (options: GoogleAnalyticsInitializeOptions = {***REMOVED***): void => {
