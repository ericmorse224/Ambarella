...
- Extended `fork()` method to allow append syntax instead of overriding for `types`, `properties` and `atrules`, e.g. `csstree.fork({ types: { color: '| foo | bar' ***REMOVED*** ***REMOVED***)`
...
- Exposed version of the lib (i.e. `import { version ***REMOVED*** from 'css-tree'`)
...
- Lexer
    ...
    - Grammar
        ...
        - Changed `generate()` method:
            ...
            - When a second parameter is a function it treats as `decorate` option value, i.e. `generate(node, fn)` -> `generate(node, { decorate: fn ***REMOVED***)`
...
- Generator
    ...
    - Removed `translateMarkup(ast, enter, leave)` method, use `generate(ast, { decorator: (handlers) => { ... ***REMOVED******REMOVED***)` instead
    - Removed `translateWithSourceMap(ast)`, use `generate(ast, { sourceMap: true ***REMOVED***)` instead
    - Changed to support for children as an array
- Walker
    - Changed `walk()` to take an `options` argument instead of handler, with `enter`, `leave`, `visit` and `reverse` options (`walk(ast, fn)` is still works and equivalent to `walk(ast, { enter: fn ***REMOVED***)`)
    - Removed `walkUp(ast, fn)`, use `walk(ast, { leave: fn ***REMOVED***)`
    - Removed `walkRules(ast, fn)`, use `walk(ast, { visit: 'Rule', enter: fn ***REMOVED***)` instead
    - Removed `walkRulesRight(ast, fn)`, use `walk(ast, { visit: 'Rule', reverse: true, enter: fn ***REMOVED***)` instead
    - Removed `walkDeclarations(ast, fn)`, use `walk(ast, { visit: 'Declaration', enter: fn ***REMOVED***)` instead
...
- Lexer
    ...
    - Improved `grammar.walk()` to take a walk handler pair as an object, i.e. `walk(node, { enter: fn, leave: fn ***REMOVED***)`
...
- Parser
    - Fixed exception on parsing of unclosed `{***REMOVED***-block` in tolerant mode
