...
```js
rewritePattern('foo.bar', 'u', { unicodeFlag: "transform" ***REMOVED***);
// → 'foo(?:[\\0-\\t\\x0B\\f\\x0E-\\u2027\\u202A-\\uD7FF\\uDC00-\\uFFFF]|[\\uD800-\\uDBFF][\\uDC00-\\uDFFF]|[\\uD800-\\uDBFF])bar'

rewritePattern('[\\u{1D306***REMOVED***-\\u{1D308***REMOVED***a-z]', 'u', { unicodeFlag: "transform" ***REMOVED***);
// → '(?:[a-z]|\\uD834[\\uDF06-\\uDF08])'

rewritePattern('[\\u{1D306***REMOVED***-\\u{1D308***REMOVED***a-z]', 'ui', { unicodeFlag: "transform" ***REMOVED***);
// → '(?:[a-z\\u017F\\u212A]|\\uD834[\\uDF06-\\uDF08])'
```
...
rewritePattern('foo.bar', '', { unicodeFlag: "transform" ***REMOVED***);
// → 'foo(?:[\\0-\\t\\x0B\\f\\x0E-\\u2027\\u202A-\\uFFFF])bar'
...
- `unicodeFlag` - The `u` flag, enabling support for Unicode code point escapes in the form `\u{...***REMOVED***`.

  ```js
  rewritePattern('\\u{ab***REMOVED***', '', {
    unicodeFlag: 'transform'
  ***REMOVED***);
  // → '\\u{ab***REMOVED***'

  rewritePattern('\\u{ab***REMOVED***', 'u', {
    unicodeFlag: 'transform'
  ***REMOVED***);
  // → '\\xAB'
  ```
...
- `unicodePropertyEscapes` - [Unicode property escapes](property-escapes.md).
  ...
  By default they are compiled to Unicode code point escapes of the form `\u{...***REMOVED***`. If the `unicodeFlag` option is set to `'transform'` they often result in larger output, although there are cases (such as `\p{Lu***REMOVED***`) where it actually _decreases_ the output size.

  ```js
  rewritePattern('\\p{Script_Extensions=Anatolian_Hieroglyphs***REMOVED***', 'u', {
    unicodePropertyEscapes: 'transform'
  ***REMOVED***);
  // → '[\\u{14400***REMOVED***-\\u{14646***REMOVED***]'
...
- `unicodeSetsFlag` - [The `v` (`unicodeSets`) flag](https://github.com/tc39/proposal-regexp-set-notation)
  ...
  ```js
  rewritePattern('[\\p{Emoji***REMOVED***&&\\p{ASCII***REMOVED***]', 'v', {
