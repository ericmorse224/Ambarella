...
exports.default = util.createRule({
    ...
    meta: {
        ...
        ***REMOVED***,
        fixable: 'code',
        messages: {
            functionTypeOverCallableType: '{{ literalOrInterface ***REMOVED******REMOVED*** only has a call signature, you should use a function type instead.',
            unexpectedThisOnFunctionOnlyInterface: "`this` refers to the function type '{{ interfaceName ***REMOVED******REMOVED***', did you intend to use a generic `this` parameter like `<Self>(this: Self, ...) => Self` instead?",
        ***REMOVED***,
        schema: [],
        type: 'suggestion',
    ***REMOVED***,
                ...
                ***REMOVED***
                ...
                const fix = fixable
                    ...
                    : (fixer) => {
                        ...
                        let suggestion = `${text.slice(0, colonPos)***REMOVED*** =>${text.slice(colonPos + 1)***REMOVED***`;
                        const lastChar = suggestion.endsWith(';') ? ';' : '';
                        if (lastChar) {
                            suggestion = suggestion.slice(0, -1);
                        ***REMOVED***
                        if (shouldWrapSuggestion(node.parent)) {
                            suggestion = `(${suggestion***REMOVED***)`;
                        ***REMOVED***
                        if (node.type === utils_1.AST_NODE_TYPES.TSInterfaceDeclaration) {
                            if (node.typeParameters !== undefined) {
                                suggestion = `type ${sourceCode
                                    .getText()
                                    .slice(node.id.range[0], node.typeParameters.range[1])***REMOVED*** = ${suggestion***REMOVED***${lastChar***REMOVED***`;
                            ***REMOVED***
                            else {
                                suggestion = `type ${node.id.name***REMOVED*** = ${suggestion***REMOVED***${lastChar***REMOVED***`;
                            ***REMOVED***
                        ***REMOVED***
