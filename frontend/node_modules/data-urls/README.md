...
This package's main module's default export is a function that accepts a string and returns a `{ mimeType, body ***REMOVED***` object, or `null` if the result cannot be parsed as a `data:` URL.
...
```js
const parseDataURL = require("data-urls");
const { labelToName, decode ***REMOVED*** = require("whatwg-encoding");
