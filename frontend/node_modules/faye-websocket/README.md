...
var ws = new WebSocket.Client('ws://www.example.com/', [], {
  proxy: {
    ...
    headers: { 'User-Agent': 'node' ***REMOVED***,
    tls:     { cert: fs.readFileSync('client.crt') ***REMOVED***
  ***REMOVED***
***REMOVED***);
```
...
var ws = new WebSocket(request, socket, body, [], { extensions: [deflate] ***REMOVED***);
```
...
- `tls` - an object containing TLS settings for the origin server, this will be
  passed to
  [`tls.connect()`](https://nodejs.org/api/tls.html#tls_tls_connect_options_callback)
- `ca` - (legacy) a shorthand for passing `{ tls: { ca: value ***REMOVED*** ***REMOVED***`
...
- **`on('open', function(event) {***REMOVED***)`** fires when the socket connection is
  established. Event has no attributes.
- **`on('message', function(event) {***REMOVED***)`** fires when the socket receives a
  message. Event has one attribute, **`data`**, which is either a `String` (for
  text frames) or a `Buffer` (for binary frames).
- **`on('error', function(event) {***REMOVED***)`** fires when there is a protocol error due
  to bad data sent by the other peer. This event is purely informational, you do
  not need to implement error recover.
- **`on('close', function(event) {***REMOVED***)`** fires when either the client or the
...
- **`ping(message, function() {***REMOVED***)`** sends a ping frame with an optional message
...
server.on('request', function(request, response) {
  if (EventSource.isEventSource(request)) {
    ...
    var loop = setInterval(function() { es.send('Hello') ***REMOVED***, 1000);
...
```js
es.send('Breaking News!', { event: 'notification', id: '99' ***REMOVED***);
```
