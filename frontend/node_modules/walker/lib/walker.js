 ...
 *
 * @param root {String***REMOVED*** the directory to start with
 */
function Walker(root) {
  if (!(this instanceof Walker)) return new Walker(root)
  EventEmitter.call(this)
  this._pending = 0
  this._filterDir = function() { return true ***REMOVED***
  this.go(root)
***REMOVED***
 ...
 *
 * @param fn {Function***REMOVED*** a function that will be given a directory name, which
    ...
    ***REMOVED***
    ...
    if (stat.isDirectory()) {
      ...
      ***REMOVED*** else {
        ...
        ***REMOVED***)
      ***REMOVED***
    ***REMOVED*** else if (stat.isSymbolicLink()) {
      that.emit('entry', entry, stat)
      that.emit('symlink', entry, stat)
      that.doneOne()
    ***REMOVED*** else if (stat.isBlockDevice()) {
      that.emit('entry', entry, stat)
      that.emit('blockDevice', entry, stat)
      that.doneOne()
    ***REMOVED*** else if (stat.isCharacterDevice()) {
      that.emit('entry', entry, stat)
      that.emit('characterDevice', entry, stat)
      that.doneOne()
    ***REMOVED*** else if (stat.isFIFO()) {
      that.emit('entry', entry, stat)
      that.emit('fifo', entry, stat)
      that.doneOne()
    ***REMOVED*** else if (stat.isSocket()) {
      that.emit('entry', entry, stat)
      that.emit('socket', entry, stat)
      that.doneOne()
    ***REMOVED*** else if (stat.isFile()) {
      that.emit('entry', entry, stat)
