 ...
 * const {count, size, warnings***REMOVED*** = await injectManifest({
...
async function injectManifest(config) {
    ...
    ***REMOVED***
    ...
    const { count, size, manifestEntries, warnings ***REMOVED*** = await (0, get_file_manifest_entries_1.getFileManifestEntries)(options);
    let swFileContents;
    try {
        swFileContents = await fs_extra_1.default.readFile(options.swSrc, 'utf8');
    ***REMOVED***
    catch (error) {
        throw new Error(`${errors_1.errors['invalid-sw-src']***REMOVED*** ${error instanceof Error && error.message ? error.message : ''***REMOVED***`);
    ***REMOVED***
    ...
    if (!injectionResults) {
        if (upath_1.default.resolve(options.swSrc) === upath_1.default.resolve(options.swDest)) {
            throw new Error(`${errors_1.errors['same-src-and-dest']***REMOVED*** ${injectionPoint***REMOVED***`);
        ***REMOVED***
        throw new Error(`${errors_1.errors['injection-point-not-found']***REMOVED*** ${injectionPoint***REMOVED***`);
    ***REMOVED***
    (0, assert_1.default)(injectionResults.length === 1, `${errors_1.errors['multiple-injection-points']***REMOVED*** ${injectionPoint***REMOVED***`);
    ...
    const { destPath, srcPath, warning ***REMOVED*** = (0, translate_url_to_sourcemap_paths_1.translateURLToSourcemapPaths)(url, options.swSrc, options.swDest);
    if (warning) {
    ...
    ***REMOVED***
    ...
    if (srcPath && destPath) {
        const originalMap = (await fs_extra_1.default.readJSON(srcPath, {
            encoding: 'utf8',
        ***REMOVED***));
        const { map, source ***REMOVED*** = await (0, replace_and_update_source_map_1.replaceAndUpdateSourceMap)({
            originalMap,
