...
import {HTTPMethod, defaultMethod***REMOVED*** from './utils/constants.js';
...
class Router {
  ...
  addCacheListener(): void {
    ...
    self.addEventListener('message', ((event: ExtendableMessageEvent) => {
      ...
      if (event.data && event.data.type === 'CACHE_URLS') {
        ...
        const {payload***REMOVED***: CacheURLsMessageData = event.data;
          ...
          if (process.env.NODE_ENV !== 'production') {
            ...
            logger.groupCollapsed(
              `Error thrown when responding to: ` +
                ` ${getFriendlyURL(
                  url,
                )***REMOVED***. Falling back to route's Catch Handler.`,
            );
          ...
          if (process.env.NODE_ENV !== 'production') {
            ...
            logger.groupCollapsed(
              `Error thrown when responding to: ` +
                ` ${getFriendlyURL(
                  url,
                )***REMOVED***. Falling back to global Catch Handler.`,
            );
            logger.error(`Error thrown by:`, route);
            logger.error(err);
            logger.groupEnd();
  ...
  ***REMOVED***
  ...
  findMatchingRoute({
    ...
    request,
    event,
  ***REMOVED***: RouteMatchCallbackOptions): {
    route?: Route;
      ...
      if (matchResult) {
        if (process.env.NODE_ENV !== 'production') {
          ...
          if (matchResult instanceof Promise) {
            logger.warn(
              `While routing ${getFriendlyURL(url)***REMOVED***, an async ` +
        ...
        ***REMOVED*** else if (
          ...
          params = undefined;
        ***REMOVED*** else if (typeof matchResult === 'boolean') {
