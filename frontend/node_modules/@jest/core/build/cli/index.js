...
async function runCLI(argv, projects) {
  ...
  const {globalConfig, configs, hasDeprecationWarnings***REMOVED*** = await (0,
  ...
  ***REMOVED***
  ...
  if (argv.clearCache) {
    configs.forEach(config => {
      ...
      process.stdout.write(`Cleared ${config.cacheDirectory***REMOVED***\n`);
    ***REMOVED***);
    (0, _exit().default)(0);
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  const {openHandles***REMOVED*** = results;
  ...
  if (openHandles && openHandles.length) {
    ...
    const message =
      _chalk().default.red(
        `\nJest has detected the following ${openHandlesString***REMOVED*** potentially keeping Jest from exiting:\n\n`
      ) + formatted.join('\n\n');
...
const _run10000 = async (
  ...
  ***REMOVED***
  ...
  const {contexts, hasteMapInstances***REMOVED*** = await buildContextsAndHasteMaps(
    configs,
    globalConfig,
    outputStream
  );
