        ...
        ***REMOVED***);
        container.walkRules((r)=>{
            ...
            ***REMOVED***
            ...
            (0, _pluginUtils.updateAllClasses)(ast, (className)=>className === classCandidate ? `!${className***REMOVED***` : className);
...
function applyVariant(variant, matches, context) {
    if (matches.length === 0) {
        return matches;
    ***REMOVED***
    /** @type {{modifier: string | null, value: string | null***REMOVED******REMOVED*** */ let args = {
        modifier: null,
...
function isParsableCssValue(property, value) {
    ...
    ***REMOVED***
    try {
        _postcss.default.parse(`a{${property***REMOVED***:${value***REMOVED******REMOVED***`).toResult();
        return true;
    ***REMOVED*** catch (err) {
        return false;
    ***REMOVED***
...
function* resolveMatchedPlugins(classCandidate, context) {
    ...
    ***REMOVED***(extractArbitraryProperty(classCandidate, context));
    ...
    const hasMatchingPrefix = candidatePrefix.startsWith(twConfigPrefix) || candidatePrefix.startsWith(`-${twConfigPrefix***REMOVED***`);
    ...
    for (let [prefix, modifier] of candidatePermutations(candidatePrefix)){
        if (context.candidateRuleMap.has(prefix)) {
            yield [
                ...
                negative ? `-${modifier***REMOVED***` : modifier
            ];
        ***REMOVED***
    ***REMOVED***
***REMOVED***
        ...
        for (let [sort, plugin] of plugins){
            ...
            if (typeof plugin === "function") {
                ...
                ***REMOVED***))){
                    ...
                    ***REMOVED***
                ***REMOVED***
            ***REMOVED*** else if (modifier === "DEFAULT" || modifier === "-DEFAULT") {
                let ruleSet = plugin;
                let [rules, options] = parseRules(ruleSet, context.postCssNodeCache);
                for (let rule of rules){
                    matchesPerPlugin.push([
            ...
            ***REMOVED***
            if (matchesPerPlugin.length > 0) {
                ...
                let matchingTypes = Array.from((0, _pluginUtils.getMatchingTypes)((_sort_options_types = (_sort_options = sort.options) === null || _sort_options === void 0 ? void 0 : _sort_options.types) !== null && _sort_options_types !== void 0 ? _sort_options_types : [], modifier, (_sort_options1 = sort.options) !== null && _sort_options1 !== void 0 ? _sort_options1 : {***REMOVED***, context.tailwindConfig)).map(([_, type])=>type);
        ...
        if (isArbitraryValue(modifier)) {
            if (matches.length > 1) {
                ...
                let [withAny, withoutAny] = matches.reduce((group, plugin)=>{
                    let hasAnyType = plugin.some(([{ options  ***REMOVED***])=>options.types.some(({ type  ***REMOVED***)=>type === "any"));
                    if (hasAnyType) {
                        group[0].push(plugin);
                    ***REMOVED*** else {
                        group[1].push(plugin);
                    ...
                    return matches.find((rules)=>{
                        ...
                        return rules.some(([{ options  ***REMOVED***, rule])=>{
                            if (!isParsableNode(rule)) {
                                return false;
                            ***REMOVED***
                            return options.types.some(({ type , preferOnConflict  ***REMOVED***)=>matchingTypes.includes(type) && preferOnConflict);
                        ***REMOVED***);
                    ***REMOVED***);
                ***REMOVED***
                var _findFallback;
                    ...
                    for (let [idx, group] of typesPerPlugin.entries()){
                        for (let type of group){
                            let rules = matches[idx].map(([, rule])=>rule).flat().map((rule)=>rule.toString().split("\n").slice(1, -1) // Remove selector and closing '***REMOVED***'
                                .map((line)=>line.trim()).map((x)=>`      ${x***REMOVED***`) // Re-indent
                                .join("\n")).join("\n\n");
                            messages.push(`  Use \`${candidate.replace("[", `[${type***REMOVED***:`)***REMOVED***\` for \`${rules.trim()***REMOVED***\``);
                            break;
                        ***REMOVED***
                    ***REMOVED***
                    _log.default.warn([
                        ...
                        ...messages,
                        `If this is content and not a class, replace it with \`${candidate.replace("[", "&lsqb;").replace("]", "&rsqb;")***REMOVED***\` to silence this warning.`
                    ]);
                    continue;
                ***REMOVED***
            ***REMOVED***
