...
function generatePatchedLinterJsFileIfDoesNotExist(inputFilePath, outputFilePath) {
    ...
    ***REMOVED***
    ...
    outputFile += `
// --- BEGIN MONKEY PATCH ---
const bulkSuppressionsPatch = require(process.env.${constants_1.ESLINT_BULK_PATCH_PATH_ENV_VAR_NAME***REMOVED***);
...
`;
    ...
    outputFile += requireSection.replace(/require\s*\((?:'([^']+)'|"([^"]+)")\)/g, (match, p1, p2) => {
        ...
        if (importPath !== 'path') {
            if (p1) {
                return `requireFromPathToLinterJS('${p1***REMOVED***')`;
            ***REMOVED***
            if (p2) {
                return `requireFromPathToLinterJS("${p2***REMOVED***")`;
            ***REMOVED***
        ***REMOVED***
        // Keep as-is
        return match;
...
`;
    ...
    //    const problem = reportTranslator(...args);
    //    // --- BEGIN MONKEY PATCH ---
    //    if (bulkSuppressionsPatch.shouldBulkSuppress({ filename, currentNode, ruleId ***REMOVED***)) return;
    //    // --- END MONKEY PATCH ---
    //
    //    if (problem.fix && !(rule.meta && rule.meta.fixable)) {
    //        throw new Error("Fixable rules must set the `meta.fixable` property to \"code\" or \"whitespace\".");
    ...
    outputFile += `
                        // --- BEGIN MONKEY PATCH ---
                        if (bulkSuppressionsPatch.shouldBulkSuppress({ filename, currentNode, ruleId, problem ***REMOVED***)) return;
                        // --- END MONKEY PATCH ---
`;
    outputFile += scanUntilMarker('nodeQueue.forEach(traversalInfo => {');
    outputFile += scanUntilMarker('***REMOVED***);');
    outputFile += scanUntilNewline();
     ...
     */
    _conditionallyReinitialize({ cwd, configType ***REMOVED*** = {***REMOVED***) {
