...
module.exports = class ParsedError
  ...
  _getType: -> @error.type
  _getTrace: -> @_trace
  _fixPath: (path) -> path.replace(///[\\]{1,2***REMOVED***///g, '/')
  ...
  _rectifyPath: (path, nameForCurrentPackage) ->
    ...
    if typeof nameForCurrentPackage is 'string'
      parts.push "[#{nameForCurrentPackage***REMOVED***]"
      packages.push "[#{nameForCurrentPackage***REMOVED***]"
    else
      parts.push "[#{m[1].match(/([^\/]+)$/)[1]***REMOVED***]"
      packages.push m[1].match(/([^\/]+)$/)[1]

    rest = m[2]

    while m = rest.match /([^\/]+)\/node_modules\/(.+)$/
      parts.push "[#{m[1]***REMOVED***]"
      packages.push m[1]
      rest = m[2]
