...
const { UNICODE_SET, UNICODE_IV_SET ***REMOVED*** = require('./data/all-characters.js');
...
const getUnicodePropertyValueSet = (property, value) => {
	const path = value ?
		`${ property ***REMOVED***/${ value ***REMOVED***` :
		`Binary_Property/${ property ***REMOVED***`;
	try {
		return require(`regenerate-unicode-properties/${ path ***REMOVED***.js`);
	***REMOVED*** catch (exception) {
		throw new Error(
			`Failed to recognize value \`${ value ***REMOVED***\` for property ` +
			`\`${ property ***REMOVED***\`.`
		);
	***REMOVED***
***REMOVED***;
...
function configGetCaseEqFlags() {
	...
	if (config.modifiersData.i === true) {
		if (config.transform.modifiers) {
			...
			***REMOVED***
		***REMOVED***
	***REMOVED*** else if (config.modifiersData.i === undefined) {
 ...
 * @param {number***REMOVED*** codePoint input code point
 * @param {number***REMOVED*** flags bitwise flags composed of CASE_EQ_FLAG_*
	...
	***REMOVED***
	...
	if (flags & CASE_EQ_FLAG_BMP) {
		for (const cp of [codePoint].concat(result)) {
			...
			if (cp >= 0x41 && cp <= 0x5a) {
				result.push(cp + 0x20);
			***REMOVED*** else if (cp >= 0x61 && cp <= 0x7a) {
				result.push(cp - 0x20);
			***REMOVED*** else {
...
const buildHandler = (action) => {
	switch (action) {
		...
		***REMOVED***
		...
		default:
			throw new Error(`Unknown set action: ${ characterClassItem.kind ***REMOVED***`);
	***REMOVED***
***REMOVED***;
...
const computeCharacterClass = (characterClassItem, regenerateOptions, shouldApplySCF) => {
	...
	switch (characterClassItem.kind) {
		...
		/* node:coverage ignore next */
		default:
			throw new Error(`Unknown character class kind: ${ characterClassItem.kind ***REMOVED***`);
	***REMOVED***
	...
	for (const item of characterClassItem.body) {
		switch (item.type) {
			...
			/* node:coverage ignore next */
			default:
				throw new Error(`Unknown term type: ${ item.type ***REMOVED***`);
		***REMOVED***
...
const processCharacterClass = (
	...
	const { singleChars, transformed, longStrings ***REMOVED*** = computed;
	if (transformed) {
		...
		const setStr = singleChars.toString(Object.assign({***REMOVED***, regenerateOptions, { bmpOnly: bmpOnly ***REMOVED***));

		if (negative) {
			if (config.useUnicodeFlag) {
				update(characterClassItem, `[^${setStr[0] === '[' ? setStr.slice(1, -1) : setStr***REMOVED***]`)
			***REMOVED*** else {
				if (config.flags.unicode || config.flags.unicodeSets) {
					if (config.flags.ignoreCase) {
						...
						update(
							characterClassItem,
							`(?!${surrogateOrBMPSetStr***REMOVED***)[^]|${astralNegativeSetStr***REMOVED***`
						);
					***REMOVED*** else {
					...
					***REMOVED***
				***REMOVED*** else {
					update(characterClassItem, `(?!${setStr***REMOVED***)[^]`);
				***REMOVED***
			***REMOVED***
		***REMOVED*** else {
...
const assertNoUnmatchedReferences = (groups) => {
	...
	if (unmatchedReferencesNames.length > 0) {
		throw new Error(`Unknown group names: ${unmatchedReferencesNames***REMOVED***`);
	***REMOVED***
***REMOVED***;
...
const processModifiers = (item, regenerateOptions, groups) => {
	...
	const oldData = Object.assign({***REMOVED***, config.modifiersData);
...
const processTerm = (item, regenerateOptions, groups) => {
	switch (item.type) {
		case 'dot':
			...
			***REMOVED*** else if ((config.modifiersData.s != null ? config.modifiersData.s && config.transform.modifiers : config.transform.dotAllFlag)) {
		...
		case 'unicodePropertyEscape':
			...
			if (data.maybeIncludesStrings) {
				...
				***REMOVED***
			***REMOVED*** else if (config.transform.unicodePropertyEscapes || configGetCaseEqFlags()) {
				update(
					item,
			...
			***REMOVED***
			if (item.name) {
				...
				if (groups.namesConflicts[name]) {
					throw new Error(
						`Group '${ name ***REMOVED***' has already been defined in this context.`
					);
				***REMOVED***
		...
		case 'anchor':
			if (config.modifiersData.m && config.transform.modifiers) {
				if (item.kind == 'start') {
					update(item, `(?:^|(?<=${NEWLINE_SET.toString()***REMOVED***))`);
				***REMOVED*** else if (item.kind == 'end') {
					update(item, `(?:$|(?=${NEWLINE_SET.toString()***REMOVED***))`);
				***REMOVED***
			***REMOVED***
		case 'empty':
		...
		switch (key) {
			...
			case 'namedGroups':
				if (value != null && value !== false && value !== 'transform') {
					throw new Error(`.${key***REMOVED*** must be false (default) or 'transform'.`);
				***REMOVED***
				break;
			// todo: remove modifiers: 'parse' in regexpu-core v7
			case 'modifiers':
				if (value != null && value !== false && value !== 'parse' && value !== 'transform') {
					throw new Error(`.${key***REMOVED*** must be false (default), 'parse' or 'transform'.`);
				***REMOVED***
				break;
			case 'onNamedGroup':
			case 'onNewFlags':
				if (value != null && typeof value !== 'function') {
					throw new Error(`.${key***REMOVED*** must be a function.`);
				***REMOVED***
				break;
			default:
				throw new Error(`.${key***REMOVED*** is not a valid regexpu-core option.`);
		***REMOVED***
	***REMOVED***
***REMOVED***;
...
const rewritePattern = (pattern, flags, options) => {
	...
	***REMOVED***;
	...
	const groups = {
		...
		'lastIndex': 0,
		'names': Object.create(null), // { [name]: Array<index> ***REMOVED***
		'namesConflicts': Object.create(null), // { [name]: true ***REMOVED***
		'unmatchedReferences': Object.create(null) // { [name]: true ***REMOVED***
	***REMOVED***;
	...
	if (config.transform.modifiers) {
		if (/\(\?[a-z]*-[a-z]+:/.test(pattern)) {
			...
			while (node = itemStack.pop(), node != undefined) {
				...
				***REMOVED*** else if (typeof node == 'object' && node != null) {
