'use strict';var _slicedToArray = function () {function sliceIterator(arr, i) {var _arr = [];var _n = true;var _d = false;var _e = undefined;try {for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) {_arr.push(_s.value);if (i && _arr.length === i) break;***REMOVED******REMOVED*** catch (err) {_d = true;_e = err;***REMOVED*** finally {try {if (!_n && _i["return"]) _i["return"]();***REMOVED*** finally {if (_d) throw _e;***REMOVED******REMOVED***return _arr;***REMOVED***return function (arr, i) {if (Array.isArray(arr)) {return arr;***REMOVED*** else if (Symbol.iterator in Object(arr)) {return sliceIterator(arr, i);***REMOVED*** else {throw new TypeError("Invalid attempt to destructure non-iterable instance");***REMOVED******REMOVED***;***REMOVED***();var _contextCompat = require('eslint-module-utils/contextCompat');
...
var _docsUrl = require('../docsUrl');var _docsUrl2 = _interopRequireDefault(_docsUrl);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { 'default': obj ***REMOVED***;***REMOVED***function _toArray(arr) {return Array.isArray(arr) ? arr : Array.from(arr);***REMOVED***
...
function getDefaultImportName(node) {
  ...
  find(function (specifier) {return specifier.type === 'ImportDefaultSpecifier';***REMOVED***);
...
function hasNamespace(node) {
  ...
  filter(function (specifier) {return specifier.type === 'ImportNamespaceSpecifier';***REMOVED***);
...
function hasSpecifiers(node) {
  var specifiers = node.specifiers.
  filter(function (specifier) {return specifier.type === 'ImportSpecifier';***REMOVED***);
  return specifiers.length > 0;
***REMOVED***
...
function hasCommentBefore(node, sourceCode) {
  ...
  some(function (comment) {return comment.loc.end.line >= node.loc.start.line - 1;***REMOVED***);
***REMOVED***
...
function hasCommentAfter(node, sourceCode) {
  ...
  some(function (comment) {return comment.loc.start.line === node.loc.end.line;***REMOVED***);
***REMOVED***

// Checks whether `node` has any comments _inside,_ except inside the `{...***REMOVED***`
// part (if any).
function hasCommentInsideNonSpecifiers(node, sourceCode) {
  var tokens = sourceCode.getTokens(node);
  var openBraceIndex = tokens.findIndex(function (token) {return isPunctuator(token, '{');***REMOVED***);
  var closeBraceIndex = tokens.findIndex(function (token) {return isPunctuator(token, '***REMOVED***');***REMOVED***);
  // Slice away the first token, since we're no looking for comments _before_
  // `node` (only inside). If there's a `{...***REMOVED***` part, look for comments before
  // the `{`, but not before the `***REMOVED***` (hence the `+1`s).
  var someTokens = openBraceIndex >= 0 && closeBraceIndex >= 0 ?
  tokens.slice(1, openBraceIndex + 1).concat(tokens.slice(closeBraceIndex + 1)) :
  tokens.slice(1);
  return someTokens.some(function (token) {return sourceCode.getCommentsBefore(token).length > 0;***REMOVED***);
***REMOVED***
...
/** @type {(first: import('estree').ImportDeclaration, rest: import('estree').ImportDeclaration[], sourceCode: import('eslint').SourceCode.SourceCode, context: import('eslint').Rule.RuleContext) => import('eslint').Rule.ReportFixer | undefined***REMOVED*** */
function getFix(first, rest, sourceCode, context) {
  ...
  ***REMOVED***
  ...
  (0, _arrayPrototype2['default'])([].concat(first, rest || []), function (x) {return getDefaultImportName(x) || [];***REMOVED***));
  ...
  ***REMOVED***
  ...
  var restWithoutComments = rest.filter(function (node) {return !hasProblematicComments(node, sourceCode) && !hasNamespace(node);***REMOVED***);

  var specifiers = restWithoutComments.
  map(function (node) {
    var tokens = sourceCode.getTokens(node);
    var openBrace = tokens.find(function (token) {return isPunctuator(token, '{');***REMOVED***);
    var closeBrace = tokens.find(function (token) {return isPunctuator(token, '***REMOVED***');***REMOVED***);
    ...
    ***REMOVED***
    ...
    return {
      ...
      isEmpty: !hasSpecifiers(node) ***REMOVED***;
  ...
  ***REMOVED***).
  ...
  var unnecessaryImports = restWithoutComments.filter(function (node) {return !hasSpecifiers(node) &&
    !hasNamespace(node) &&
    !specifiers.some(function (specifier) {return specifier.importNode === node;***REMOVED***);***REMOVED***);
  ...
  ***REMOVED***
  ...
  /** @type {import('eslint').Rule.ReportFixer***REMOVED*** */
  return function (fixer) {
    ...
    split(',').
    map(function (x) {return x.trim();***REMOVED***));var _specifiers$reduce =
    ...
    function (_ref, specifier) {var _ref2 = _slicedToArray(_ref, 3),result = _ref2[0],needsComma = _ref2[1],existingIdentifiers = _ref2[2];
      ...
      ***REMOVED***, ['', existingIdentifiers]),_specifier$identifier2 = _slicedToArray(_specifier$identifier, 2),specifierText = _specifier$identifier2[0],updatedExistingIdentifiers = _specifier$identifier2[1];
    ...
    ***REMOVED***,
    ...
    /** @type {import('eslint').Rule.Fix[]***REMOVED*** */
    var fixes = [];

    if (shouldAddSpecifiers && preferInline && first.importKind === 'type') {
      // `import type {a***REMOVED*** from './foo'` → `import {type a***REMOVED*** from './foo'`
      var typeIdentifierToken = tokens.find(function (token) {return token.type === 'Identifier' && token.value === 'type';***REMOVED***);
      fixes.push(fixer.removeRange([typeIdentifierToken.range[0], typeIdentifierToken.range[1] + 1]));

      tokens.
      filter(function (token) {return firstExistingIdentifiers.has(token.value);***REMOVED***).
    ...
    ***REMOVED***
    ...
    if (shouldAddDefault && openBrace == null && shouldAddSpecifiers) {
      // `import './foo'` → `import def, {...***REMOVED*** from './foo'`
      fixes.push(
      fixer.insertTextAfter(firstToken, ' ' + String(defaultImportName) + ', {' + String(specifiersText) + '***REMOVED*** from'));

    ***REMOVED*** else if (shouldAddDefault && openBrace == null && !shouldAddSpecifiers) {
      // `import './foo'` → `import def from './foo'`
      fixes.push(fixer.insertTextAfter(firstToken, ' ' + String(defaultImportName) + ' from'));
    ***REMOVED*** else if (shouldAddDefault && openBrace != null && closeBrace != null) {
      // `import {...***REMOVED*** from './foo'` → `import def, {...***REMOVED*** from './foo'`
      fixes.push(fixer.insertTextAfter(firstToken, ' ' + String(defaultImportName) + ','));
      if (shouldAddSpecifiers) {
        // `import def, {...***REMOVED*** from './foo'` → `import def, {..., ...***REMOVED*** from './foo'`
        fixes.push(fixer.insertTextBefore(closeBrace, specifiersText));
      ***REMOVED***
    ***REMOVED*** else if (!shouldAddDefault && openBrace == null && shouldAddSpecifiers) {
      if (first.specifiers.length === 0) {
        // `import './foo'` → `import {...***REMOVED*** from './foo'`
        fixes.push(fixer.insertTextAfter(firstToken, ' {' + String(specifiersText) + '***REMOVED*** from'));
      ***REMOVED*** else {
        // `import def from './foo'` → `import def, {...***REMOVED*** from './foo'`
        fixes.push(fixer.insertTextAfter(first.specifiers[0], ', {' + String(specifiersText) + '***REMOVED***'));
      ***REMOVED***
    ***REMOVED*** else if (!shouldAddDefault && openBrace != null && closeBrace != null) {
      // `import {...***REMOVED*** './foo'` → `import {..., ...***REMOVED*** from './foo'`
      fixes.push(fixer.insertTextBefore(closeBrace, specifiersText));
    ***REMOVED***
...
/** @type {(imported: Map<string, import('estree').ImportDeclaration[]>, context: import('eslint').Rule.RuleContext) => void***REMOVED*** */
...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      url: (0, _docsUrl2['default'])('no-duplicates') ***REMOVED***,
  ...
  /** @param {import('eslint').Rule.RuleContext***REMOVED*** context */
  create: function () {function create(context) {
      ...
      /** @type {boolean***REMOVED*** */
      var preferInline = context.options[0] && context.options[0]['prefer-inline'];
      var defaultResolver = function () {function defaultResolver(sourcePath) {return (0, _resolve2['default'])(sourcePath, context) || sourcePath;***REMOVED***return defaultResolver;***REMOVED***();
      ...
      ***REMOVED*** : defaultResolver;

      /** @type {Map<unknown, { imported: Map<string, import('estree').ImportDeclaration[]>, nsImported: Map<string, import('estree').ImportDeclaration[]>, defaultTypesImported: Map<string, import('estree').ImportDeclaration[]>, namedTypesImported: Map<string, import('estree').ImportDeclaration[]>***REMOVED***>***REMOVED*** */
      var moduleMaps = new Map();

      /** @param {import('estree').ImportDeclaration***REMOVED*** n */
      /** @returns {typeof moduleMaps[keyof typeof moduleMaps]***REMOVED*** */
      function getImportMap(n) {
        if (!moduleMaps.has(n.parent)) {
          moduleMaps.set(n.parent, /** @type {typeof moduleMaps***REMOVED*** */{
            imported: new Map(),
            nsImported: new Map(),
            defaultTypesImported: new Map(),
            namedTypesImported: new Map() ***REMOVED***);
        ...
        ***REMOVED***
        if (!preferInline && n.specifiers.some(function (spec) {return spec.importKind === 'type';***REMOVED***)) {
      ...
      ***REMOVED***
      ...
      return {
        ...
        'Program:exit': function () {function ProgramExit() {var _iteratorNormalCompletion2 = true;var _didIteratorError2 = false;var _iteratorError2 = undefined;try {
              ...
              ***REMOVED******REMOVED*** catch (err) {_didIteratorError2 = true;_iteratorError2 = err;***REMOVED*** finally {try {if (!_iteratorNormalCompletion2 && _iterator2['return']) {_iterator2['return']();***REMOVED******REMOVED*** finally {if (_didIteratorError2) {throw _iteratorError2;***REMOVED******REMOVED******REMOVED***
