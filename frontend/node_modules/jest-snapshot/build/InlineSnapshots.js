...
***REMOVED***)).default; // prettier-ignore
...
***REMOVED***)).default; // @ts-expect-error requireOutside Babel transform

const {file, templateElement, templateLiteral***REMOVED*** = require(require.resolve(
  '@babel/types',
  {
    [(global['jest-symbol-do-not-touch'] || global.Symbol).for(
      'jest-resolve-outside-vm-option'
...
const {parseSync***REMOVED*** = require(require.resolve('@babel/core', {
  [(global['jest-symbol-do-not-touch'] || global.Symbol).for(
    'jest-resolve-outside-vm-option'
  )]: true
***REMOVED***));
...
const saveSnapshotsForFile = (snapshots, sourceFilePath, prettier) => {
  ...
  if (/\.tsx?$/.test(sourceFilePath)) {
    plugins.push([
      ...
      ***REMOVED***, // unique name to make sure Babel does not complain about a possible duplicate plugin.
      'TypeScript syntax plugin added by Jest snapshot'
    ]);
  ***REMOVED*** // Record the matcher names seen during traversal and pass them down one
  ...
  ***REMOVED***);
  ...
  if (!ast) {
    throw new Error(`jest-snapshot: Failed to parse ${sourceFilePath***REMOVED***`);
  ***REMOVED***
...
const groupSnapshotsBy = createKey => snapshots =>
  snapshots.reduce((object, inlineSnapshot) => {
    ...
    return {...object, [key]: (object[key] || []).concat(inlineSnapshot)***REMOVED***;
  ***REMOVED***, {***REMOVED***);

const groupSnapshotsByFrame = groupSnapshotsBy(({frame: {line, column***REMOVED******REMOVED***) =>
  typeof line === 'number' && typeof column === 'number'
    ? `${line***REMOVED***:${column - 1***REMOVED***`
    : ''
);
const groupSnapshotsByFile = groupSnapshotsBy(({frame: {file***REMOVED******REMOVED***) => file);
  ...
  return lines
    .map((line, index) => {
      if (index === 0) {
        ...
        return line;
      ***REMOVED*** else if (index !== lines.length - 1) {
        // Do not indent empty lines.
        if (line === '') {
          return line;
        ***REMOVED*** // Not last line, indent one level deeper than expect call.
...
const traverseAst = (snapshots, fileOrProgram, snapshotMatcherNames) => {
  ...
  const remainingSnapshots = new Set(snapshots.map(({snapshot***REMOVED***) => snapshot));
  babelTraverse(ast, {
    CallExpression({node***REMOVED***) {
      const {arguments: args, callee***REMOVED*** = node;
      ...
      ***REMOVED***
      ...
      const {line, column***REMOVED*** = callee.property.loc.start;
      const snapshotsForFrame = groupedSnapshots[`${line***REMOVED***:${column***REMOVED***`];
...
const runPrettier = (
  ...
  if (!inferredParser) {
    throw new Error(
      `Could not infer Prettier parser for file ${sourceFilePath***REMOVED***`
    );
  ***REMOVED*** // Snapshots have now been inserted. Run prettier to make sure that the code is
...
***REMOVED***; // This parser formats snapshots to the correct indentation.
...
const createFormattingParser =
  (snapshotMatcherNames, inferredParser) => (text, parsers, options) => {
    ...
    babelTraverse(ast, {
      CallExpression({node: {arguments: args, callee***REMOVED******REMOVED***) {
