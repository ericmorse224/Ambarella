...
function _extends() { _extends = Object.assign ? Object.assign.bind() : function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; ***REMOVED*** ***REMOVED*** ***REMOVED*** return target; ***REMOVED***; return _extends.apply(this, arguments); ***REMOVED***
const {
  types: t,
  template: template
...
const uniqueObjects = ["array", "string", "iterator", "async-iterator", "dom-collections"].map(v => new RegExp(`[a-z]*\\.${v***REMOVED***\\..*`));
const esnextFallback = (name, cb) => {
  if (cb(name)) return true;
  if (!name.startsWith("es.")) return false;
  const fallback = `esnext.${name.slice(3)***REMOVED***`;
...
***REMOVED***) {
  ...
  ***REMOVED***);
  ...
  function getCoreJSPureBase(useProposalBase) {
    return useBabelRuntime ? useProposalBase ? `${_utils.BABEL_RUNTIME***REMOVED***/core-js` : `${_utils.BABEL_RUNTIME***REMOVED***/core-js-stable` : useProposalBase ? "core-js-pure/features" : "core-js-pure/stable";
  ***REMOVED***
  ...
  ***REMOVED***
  function maybeInjectPure(desc, hint, utils, object) {
    if (desc.pure && !(object && desc.exclude && desc.exclude.includes(object)) && esnextFallback(desc.name, shouldInjectPolyfill)) {
      ...
      ***REMOVED*** else if (name.startsWith("es.") && !available.has(name)) {
        useProposalBase = true;
      ***REMOVED***
      ...
      ***REMOVED***
      ...
      return utils.injectDefaultImport(`${coreJSPureBase***REMOVED***/${desc.pure***REMOVED***${ext***REMOVED***`, hint);
    ***REMOVED***
  ***REMOVED***
  function isFeatureStable(name) {
    if (name.startsWith("esnext.")) {
      const esName = `es.${name.slice(7)***REMOVED***`;
    ...
    usagePure(meta, utils, path) {
      ...
      ***REMOVED***
      ...
      if (useBabelRuntime && resolved.desc.pure && resolved.desc.pure.slice(-6) === "/index") {
        ...
        resolved = _extends({***REMOVED***, resolved, {
          desc: _extends({***REMOVED***, resolved.desc, {
      ...
      ***REMOVED*** else if (resolved.kind === "static") {
        const id = maybeInjectPure(resolved.desc, resolved.name, utils,
        // @ts-expect-error
        meta.object);
        if (id) {
          ...
          if (parentPath.isOptionalMemberExpression() || parentPath.isOptionalCallExpression()) {
            do {
              ...
              ({
                parentPath
              ***REMOVED*** = parentPath);
            ***REMOVED*** while ((parentPath.isOptionalMemberExpression() || parentPath.isOptionalCallExpression()) && !parentPath.node.optional);
          ***REMOVED***
        ***REMOVED***
      ***REMOVED*** else if (resolved.kind === "instance") {
        const id = maybeInjectPure(resolved.desc, `${resolved.name***REMOVED***InstanceProperty`, utils,
        // @ts-expect-error
        meta.object);
        if (!id) return;
        const {
        ...
        if (t.isOptionalCallExpression(parent) && parent.callee === node) {
          ...
          if (!wasOptional) {
            ...
            path.replaceWith(check(template.expression.ast`
                  Function.call.bind(${id***REMOVED***(${thisArg***REMOVED***), ${thisArg2***REMOVED***)
                `));
          ***REMOVED*** else if (t.isOptionalMemberExpression(node)) {
            const check = (0, _utils.extractOptionalCheck)(path.scope, node);
            (0, _utils.callMethod)(path, id, true, check);
          ***REMOVED*** else {
            (0, _utils.callMethod)(path, id, true);
          ***REMOVED***
        ***REMOVED*** else if (t.isCallExpression(parent) && parent.callee === node) {
    ...
    ***REMOVED***,
    visitor: method === "usage-global" && {
      ...
      CallExpression(path) {
        ...
        ***REMOVED***
      ***REMOVED***,
      // (async function () { ***REMOVED***).finally(...)
      Function(path) {
        if (path.node.async) {
