...
Promise.reject('value').catch(() => {***REMOVED***);

Promise.reject('value').finally(() => {***REMOVED***);
```
...
Examples of **correct** code for this rule with `{ ignoreVoid: true ***REMOVED***`:
...
Examples of **correct** code for this rule with `{ ignoreIIFE: true ***REMOVED***`:
