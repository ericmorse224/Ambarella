'use strict';var _docsUrl = require('../docsUrl');var _docsUrl2 = _interopRequireDefault(_docsUrl);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { 'default': obj ***REMOVED***;***REMOVED***

var first = require('./first');

var newMeta = Object.assign({***REMOVED***,
first.meta, {
  deprecated: true,
  docs: {
    category: 'Style guide',
    description: 'Replaced by `import/first`.',
    url: (0, _docsUrl2['default'])('imports-first', '7b25c1cb95ee18acc1531002fd343e1e6031f9ed') ***REMOVED*** ***REMOVED***);



module.exports = Object.assign({***REMOVED***, first, { meta: newMeta ***REMOVED***);
//# sourceMappingURL=data:application/json;charset=utf-8;base64,