...
module.exports = {
	meta: {
		...
		defaultOptions: [{ allowElseIf: true ***REMOVED***],
	...
	***REMOVED***,
	...
	create(context) {
		const [{ allowElseIf ***REMOVED***] = context.options;
		...
		function isSafeToDeclare(names, scope) {
			...
			***REMOVED***
			...
			if (scope !== functionScope && scope.upper.type === "catch") {
				if (
					scope.upper.variables.some(({ name ***REMOVED***) =>
						names.includes(name),
					)
				) {
					return false;
			...
			***REMOVED***
			...
			const usedImplicitVariables = scope.variables.filter(
				({ defs, references ***REMOVED***) =>
					defs.length === 0 && references.length > 0,
			);

			if (
				usedImplicitVariables.some(({ name ***REMOVED***) => names.includes(name))
			) {
				return false;
			***REMOVED***
			...
			***REMOVED***
			...
			if (scope !== functionScope) {
				...
				const variablesToCheck = functionScope.variables.filter(
					({ name ***REMOVED***) => names.includes(name),
				);

				if (
					variablesToCheck.some(v =>
						v.defs.some(
							({ node: { range ***REMOVED*** ***REMOVED***) =>
				...
				fix(fixer) {
					...
					***REMOVED***
					...
					if (lastTokenOfElseBlock.value !== ";") {
						...
						if (
							nextTokenUnsafe ||
							(nextTokenOnSameLine && nextToken.value !== "***REMOVED***")
						) {
							return null;
						***REMOVED***
					***REMOVED***
