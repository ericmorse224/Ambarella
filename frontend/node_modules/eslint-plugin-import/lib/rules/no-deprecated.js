'use strict';var _declaredScope = require('eslint-module-utils/declaredScope');var _declaredScope2 = _interopRequireDefault(_declaredScope);
var _builder = require('../exportMap/builder');var _builder2 = _interopRequireDefault(_builder);
var _exportMap = require('../exportMap');var _exportMap2 = _interopRequireDefault(_exportMap);
var _docsUrl = require('../docsUrl');var _docsUrl2 = _interopRequireDefault(_docsUrl);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { 'default': obj ***REMOVED***;***REMOVED***

function message(deprecation) {
  return 'Deprecated' + (deprecation.description ? ': ' + String(deprecation.description) : '.');
***REMOVED***

function getDeprecation(metadata) {
  if (!metadata || !metadata.doc) {return;***REMOVED***

  return metadata.doc.tags.find(function (t) {return t.title === 'deprecated';***REMOVED***);
***REMOVED***

module.exports = {
  meta: {
    type: 'suggestion',
    docs: {
      category: 'Helpful warnings',
      description: 'Forbid imported names marked with `@deprecated` documentation tag.',
      url: (0, _docsUrl2['default'])('no-deprecated') ***REMOVED***,

    schema: [] ***REMOVED***,


  create: function () {function create(context) {
      var deprecated = new Map();
      var namespaces = new Map();

      function checkSpecifiers(node) {
        if (node.type !== 'ImportDeclaration') {return;***REMOVED***
        if (node.source == null) {return;***REMOVED*** // local export, ignore

        var imports = _builder2['default'].get(node.source.value, context);
        if (imports == null) {return;***REMOVED***

        var moduleDeprecation = imports.doc && imports.doc.tags.find(function (t) {return t.title === 'deprecated';***REMOVED***);
        if (moduleDeprecation) {
          context.report({ node: node, message: message(moduleDeprecation) ***REMOVED***);
        ***REMOVED***

        if (imports.errors.length) {
          imports.reportErrors(context, node);
          return;
        ***REMOVED***

        node.specifiers.forEach(function (im) {
          var imported = void 0;var local = void 0;
          switch (im.type) {

            case 'ImportNamespaceSpecifier':{
                if (!imports.size) {return;***REMOVED***
                namespaces.set(im.local.name, imports);
                return;
              ***REMOVED***

            case 'ImportDefaultSpecifier':
              imported = 'default';
              local = im.local.name;
              break;

            case 'ImportSpecifier':
              imported = im.imported.name;
              local = im.local.name;
              break;

            default:return; // can't handle this one
          ***REMOVED***

          // unknown thing can't be deprecated
          var exported = imports.get(imported);
          if (exported == null) {return;***REMOVED***

          // capture import of deep namespace
          if (exported.namespace) {namespaces.set(local, exported.namespace);***REMOVED***

          var deprecation = getDeprecation(imports.get(imported));
          if (!deprecation) {return;***REMOVED***

          context.report({ node: im, message: message(deprecation) ***REMOVED***);

          deprecated.set(local, deprecation);

        ***REMOVED***);
      ***REMOVED***

      return {
        Program: function () {function Program(_ref) {var body = _ref.body;return body.forEach(checkSpecifiers);***REMOVED***return Program;***REMOVED***(),

        Identifier: function () {function Identifier(node) {
            if (node.parent.type === 'MemberExpression' && node.parent.property === node) {
              return; // handled by MemberExpression
            ***REMOVED***

            // ignore specifier identifiers
            if (node.parent.type.slice(0, 6) === 'Import') {return;***REMOVED***

            if (!deprecated.has(node.name)) {return;***REMOVED***

            if ((0, _declaredScope2['default'])(context, node.name, node) !== 'module') {return;***REMOVED***
            context.report({
              node: node,
              message: message(deprecated.get(node.name)) ***REMOVED***);

          ***REMOVED***return Identifier;***REMOVED***(),

        MemberExpression: function () {function MemberExpression(dereference) {
            if (dereference.object.type !== 'Identifier') {return;***REMOVED***
            if (!namespaces.has(dereference.object.name)) {return;***REMOVED***

            if ((0, _declaredScope2['default'])(context, dereference.object.name, dereference) !== 'module') {return;***REMOVED***

            // go deep
            var namespace = namespaces.get(dereference.object.name);
            var namepath = [dereference.object.name];
            // while property is namespace and parent is member expression, keep validating
            while (namespace instanceof _exportMap2['default'] && dereference.type === 'MemberExpression') {
              // ignore computed parts for now
              if (dereference.computed) {return;***REMOVED***

              var metadata = namespace.get(dereference.property.name);

              if (!metadata) {break;***REMOVED***
              var deprecation = getDeprecation(metadata);

              if (deprecation) {
                context.report({ node: dereference.property, message: message(deprecation) ***REMOVED***);
              ***REMOVED***

              // stash and pop
              namepath.push(dereference.property.name);
              namespace = metadata.namespace;
              dereference = dereference.parent;
            ***REMOVED***
          ***REMOVED***return MemberExpression;***REMOVED***() ***REMOVED***;

    ***REMOVED***return create;***REMOVED***() ***REMOVED***;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,