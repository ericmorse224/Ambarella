...
With `{ "path": "never", "types": "never", "lib": "never" ***REMOVED***` options set, the following will all be **incorrect** usage:
...
Examples of **incorrect** code for the `{ "types": "prefer-import" ***REMOVED***` option. Note that these are only errors when **both** styles are used for the **same** module:
...
With `{ "path": "always", "types": "always", "lib": "always" ***REMOVED***` options set, the following will all be **correct** usage:
...
Examples of **correct** code for the `{ "types": "prefer-import" ***REMOVED***` option:
