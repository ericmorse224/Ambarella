...
```js
const { red, white, blue, bold ***REMOVED*** = require('kleur');
...
// nested methods
bold(`${ white().bgRed('[ERROR]') ***REMOVED*** ${ red().italic('Something happened')***REMOVED***`);
```
...
```js
const { yellow, red, cyan ***REMOVED*** = require('kleur');

console.log(yellow(`foo ${red().bold('red')***REMOVED*** bar ${cyan('cyan')***REMOVED*** baz`));
