...
(async () => {
  ...
  ***REMOVED***);
  ...
  console.log(response); // => { value: 24 ***REMOVED***
***REMOVED***)();
```
...
(async () => {
  ...
  // => response => { username, age, about ***REMOVED***
***REMOVED***)();
```
...
(async () => {
  ...
  const onSubmit = (prompt, answer) => console.log(`Thanks I got ${answer***REMOVED*** from ${prompt.name***REMOVED***`);
  const response = await prompts(questions, { onSubmit ***REMOVED***);
***REMOVED***)();
```
...
(async () => {
  ...
  const onCancel = prompt => {
    ...
    return true;
  ***REMOVED***
  const response = await prompts(questions, { onCancel ***REMOVED***);
***REMOVED***)();
```
    ...
    ***REMOVED***,
    {
      ...
      message: 'Pick colors',
      choices: [
        { title: 'Red', value: '#ff0000' ***REMOVED***,
        { title: 'Green', value: '#00ff00' ***REMOVED***,
        { title: 'Blue', value: '#0000ff' ***REMOVED***
      ],
    ***REMOVED***
  ]);
...
(async () => {
  ...
  // => { twitter: 'terkelg', color: [ '#ff0000', '#0000ff' ] ***REMOVED***
***REMOVED***)();
```
...
```js
{
  ...
  name: 'confirm',
  message: (prev, values) => `Please confirm that you eat ${values.dish***REMOVED*** times ${prev***REMOVED*** a day?`
***REMOVED***
```
...
```js
{
  ...
  name: 'price',
  message: 'Enter price',
  format: val => Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD' ***REMOVED***).format(val);
***REMOVED***
```
...
The state object has two properties `value` and `aborted`. E.g `{ value: 'This is ', aborted: false ***REMOVED***`
...
```js
{
  ...
  choices: [
    { title: 'Red', description: 'This option has a description', value: '#ff0000' ***REMOVED***,
    { title: 'Green', value: '#00ff00', disabled: true ***REMOVED***,
    { title: 'Blue', value: '#0000ff' ***REMOVED***
  ],
  initial: 1
***REMOVED***
...
| choices | `Array` | Array of strings or choices objects `[{ title, description, value, disabled ***REMOVED***, ...]`. The choice's index in the array will be used as its value if it is not specified. |
| onRender | `function` | On render callback. Keyword `this` refers to the current prompt |
| onState | `function` | On state change callback. Function signature is an `object` with two properties: `value` and `aborted` |
...
```js
{
  ...
  choices: [
    { title: 'Red', value: '#ff0000' ***REMOVED***,
    { title: 'Green', value: '#00ff00', disabled: true ***REMOVED***,
    { title: 'Blue', value: '#0000ff', selected: true ***REMOVED***
  ],
  max: 2,
...
| choices | `Array` | Array of strings or choices objects `[{ title, value, disabled ***REMOVED***, ...]`. The choice's index in the array will be used as its value if it is not specified. |
...
```js
{
  ...
  choices: [
    { title: 'Cage' ***REMOVED***,
    { title: 'Clooney', value: 'silver-fox' ***REMOVED***,
    { title: 'Gyllenhaal' ***REMOVED***,
    { title: 'Gibson' ***REMOVED***,
    { title: 'Grant' ***REMOVED***
  ]
***REMOVED***
```
...
| message | `string` | Prompt message to display |
| format | `function` | Receive user input. The returned value will be added to the response object |
| choices | `Array` | Array of auto-complete choices objects `[{ title, value ***REMOVED***, ...]` |
