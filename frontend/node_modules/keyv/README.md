...
```js
const users = new Keyv('redis://user:pass@localhost:6379', { namespace: 'users' ***REMOVED***);
const cache = new Keyv('redis://user:pass@localhost:6379', { namespace: 'cache' ***REMOVED***);
...
```js
const keyv = new Keyv({ serialize: JSON.stringify, deserialize: JSON.parse ***REMOVED***);
```
...
const keyv = new Keyv({ store: myAdapter ***REMOVED***);
```

Any store that follows the [`Map`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map) api will work.

```js
new Keyv({ store: new Map() ***REMOVED***);
```
...
const lru = new QuickLRU({ maxSize: 1000 ***REMOVED***);
const keyv = new Keyv({ store: lru ***REMOVED***);
```
...
const awesomeModule = new AwesomeModule({ cache: 'redis://localhost' ***REMOVED***);

// Some third-party module that implements the Map API
const awesomeModule = new AwesomeModule({ cache: some3rdPartyStore ***REMOVED***);
```
...
const keyv = new Keyv({ compression: KeyvGzip ***REMOVED***);
```
...
```js
const {keyvCompresstionTests***REMOVED*** = require('@keyv/test-suite');
const KeyvGzip = require('@keyv/compress-gzip');
