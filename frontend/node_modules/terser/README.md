...
```
```javascript
var x={o:3,t:1,i:function(){return this.t+this.o***REMOVED***,s:2***REMOVED***;console.log(x.i());
```
...
```
```javascript
var x={o:3,foo_:1,t:function(){return this.foo_+this.o***REMOVED***,bar_:2***REMOVED***;console.log(x.t());
```
...
```
```javascript
var x={o:3,t:1,calc:function(){return this.t+this.o***REMOVED***,i:2***REMOVED***;console.log(x.calc());
```
...
```
```javascript
var x={o:3,t:1,calc:function(){return this.t+this.o***REMOVED***,bar_:2***REMOVED***;console.log(x.calc());
```
...
```
```javascript
var o={foo:1,o:3***REMOVED***;o.foo+=o.o,console.log(o.foo);
```
...
```
```javascript
var o={_$foo$_:1,_$bar$_:3***REMOVED***;o._$foo$_+=o._$bar$_,console.log(o._$foo$_);
```
...
```javascript
const { minify ***REMOVED*** = require("terser");
```

Or,

```javascript
import { minify ***REMOVED*** from "terser";
```
...
```javascript
var code = "function add(first, second) { return first + second; ***REMOVED***";
var result = await minify(code, { sourceMap: true ***REMOVED***);
console.log(result.code);  // minified output: function add(n,d){return n+d***REMOVED***
...
```javascript
var code = {
    "file1.js": "function add(first, second) { return first + second; ***REMOVED***",
    "file2.js": "console.log(add(1 + 2, 3 + 4));"
***REMOVED***;
var result = await minify(code);
console.log(result.code);
// function add(d,n){return d+n***REMOVED***console.log(add(3,7));
```
...
var code = {
    ...
    "file2.js": "console.log(add(1 + 2, 3 + 4));"
***REMOVED***;
var options = { toplevel: true ***REMOVED***;
...
// function n(n,r){return n+r***REMOVED***
...
```javascript
try {
    const result = await minify({"foo.js" : "if (0) else console.log(1);"***REMOVED***);
    // Do something with result
***REMOVED*** catch (error) {
    const { message, filename, line, col, pos ***REMOVED*** = error;
...
- `parse` (default `{***REMOVED***`) — pass an object if you wish to specify some
...
- `nameCache` (default `null`) - pass an empty object `{***REMOVED***` or a previously
...
```javascript
var result = await minify({"file1.js": "var a = function() {***REMOVED***;"***REMOVED***, {
    sourceMap: {
        filename: "out.js",
        url: "out.js.map"
    ***REMOVED***
...
```javascript
var result = await minify({"compiled.js": "compiled code"***REMOVED***, {
    sourceMap: {
...
- `arrows` (default: `true`) -- Class and object literal methods are converted
  ...
  `m(){return x***REMOVED***` becomes `m:()=>x`. To do this to regular ES5 functions which
...
- `computed_props` (default: `true`) -- Transforms constant computed properties
  into regular ones: `{["computed"]: 1***REMOVED***` is converted to `{computed: 1***REMOVED***`.
...
- `global_defs` (default: `{***REMOVED***`) -- see [conditional compilation](#conditional-compilation)

- `hoist_funs` (default: `false`) -- hoist function declarations

- `hoist_props` (default: `true`) -- hoist properties from constant object and
  array literals into regular variables subject to a set of constraints. For example:
  `var o={p:1, q:2***REMOVED***; f(o.p, o.q);` is converted to `f(1, 2);`. Note: `hoist_props`
...
- `unsafe_methods` (default: false) -- Converts `{ m: function(){***REMOVED*** ***REMOVED***` to
  `{ m(){***REMOVED*** ***REMOVED***`. `ecma` must be set to `6` or greater to enable this transform.
...
// 'function funcName(a,n){***REMOVED***var globalVar;'

await minify(code, { mangle: { reserved: ['firstLongName'] ***REMOVED*** ***REMOVED***).code;
// 'function funcName(firstLongName,a){***REMOVED***var globalVar;'

await minify(code, { mangle: { toplevel: true ***REMOVED*** ***REMOVED***).code;
// 'function n(n,a){***REMOVED***var a;'
```
...
- `keep_quoted` (default: `false`) — How quoting properties (`{"prop": ...***REMOVED***` and `obj["prop"]`) controls what gets mangled.
...
- `ecma` (default `5`) -- set desired EcmaScript standard version for output.
  ...
  `{a***REMOVED***` instead of `{a: a***REMOVED***`.  The `ecma` option will only change the output in
...
function lookup(object, key) { return object[key]; ***REMOVED***
lookup({ i_will_be_mangled_too: "bar" ***REMOVED***, /*@__KEY__*/ "i_will_be_mangled_too");
```
...
```js
await minify(code, { compress: false, mangle: true ***REMOVED***);
```
