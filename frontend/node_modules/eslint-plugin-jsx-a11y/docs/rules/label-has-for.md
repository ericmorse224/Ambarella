...
const Label = props => {
  ...
  ***REMOVED*** = props;
  ...
  return (
    <label htmlFor={htmlFor***REMOVED*** {...otherProps***REMOVED*** />
  );
***REMOVED***
...
The `required` option (defaults to `"required": { "every": ["nesting", "id"] ***REMOVED***`) determines which checks are activated. You're allowed to pass in one of the following types:
...
The `allowChildren` option (defaults to `false`) determines whether `{children***REMOVED***` content is allowed to be passed into a `label` element. For example, the following pattern, by default, is not allowed:

```js
<label>{children***REMOVED***</label>
```

However, if `allowChildren` is set to `true`, no error will be raised. If you want to pass in `{children***REMOVED***` content without raising an error, because you cannot be sure what `{children***REMOVED***` will render, then set `allowChildren` to `true`.
...
```jsx
function Foo({ htmlFor, ...props***REMOVED***) {
    return <label htmlFor={htmlFor***REMOVED*** {...props***REMOVED*** />
***REMOVED***
...
function Foo(props) {
   ...
   return <label htmlFor={htmlFor***REMOVED*** {...otherProps***REMOVED*** />
***REMOVED***
```
