...
/** @typedef {import("../types").ESLint.FormatterFunction***REMOVED*** FormatterFunction */
...
***REMOVED***) {
	...
	const { fixed, messages, output ***REMOVED*** = linter.verifyAndFix(text, config, {
		allowInlineConfig,
...
function createConfigDataFromOptions(options) {
	const { ignorePattern, parser, parserOptions, plugins, rules ***REMOVED*** = options;
	...
	constructor(providedOptions, { preloadedPlugins ***REMOVED*** = {***REMOVED***) {
		...
		***REMOVED***);
		const lintResultCache = options.cache
			...
			: null;
		const linter = new Linter({ cwd: options.cwd, configType: "eslintrc" ***REMOVED***);
	...
	***REMOVED***
	...
	executeOnFiles(patterns) {
		...
		***REMOVED***
		...
		for (const { config, filePath, ignored ***REMOVED*** of fileEnumerator.iterateFiles(
			patterns,
		)) {
			if (ignored) {
