...
suite('fileTask', function () {
  ...
  ***REMOVED***);
  ...
  test('where a file-task prereq does not change with --always-make', function () {
    let out;
    out = exec(`${JAKE_CMD***REMOVED*** -q fileTest:foo/from-src1.txt`).toString().trim();
    assert.equal('fileTest:foo/src1.txt task\nfileTest:foo/from-src1.txt task',
      out);
    out = exec(`${JAKE_CMD***REMOVED*** -q -B fileTest:foo/from-src1.txt`).toString().trim();
    assert.equal('fileTest:foo/src1.txt task\nfileTest:foo/from-src1.txt task',
      out);
    cleanUpAndNext();
  ***REMOVED***);
  ...
  test('concating two files', function () {
    let out;
    out = exec(`${JAKE_CMD***REMOVED*** -q fileTest:foo/concat.txt`).toString().trim();
  ...
  ***REMOVED***);
  ...
  test('a preexisting file', function () {
    ...
    let out;
    out = exec(`${JAKE_CMD***REMOVED*** -q fileTest:foo/from-prereq.txt`).toString().trim();
  ...
  ***REMOVED***);
  ...
  test('a preexisting file with --always-make flag', function () {
    ...
    let data = fs.readFileSync(process.cwd() + '/foo/from-prereq.txt');
    assert.equal(prereqData, data.toString());
    out = exec(`${JAKE_CMD***REMOVED*** -q -B fileTest:foo/from-prereq.txt`).toString().trim();
    assert.equal('fileTest:foo/from-prereq.txt task', out);
    cleanUpAndNext();
  ***REMOVED***);

  test('nested directory-task', function () {
    exec(`${JAKE_CMD***REMOVED*** -q fileTest:foo/bar/baz/bamf.txt`);
  ...
  ***REMOVED***);
  ...
  test('partially existing prereqs', function () {
    ...
    */
    ...
    exec(`${JAKE_CMD***REMOVED*** -q fileTest:foo/output2a.txt`);
    // verify the final target gets built
    exec(`${JAKE_CMD***REMOVED*** -q fileTest:foo/output3.txt`);
