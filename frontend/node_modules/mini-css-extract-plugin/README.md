...
```ts
type attributes = Record<string, string>***REMOVED***;
```
...
module.exports = {
  plugins: [
    new MiniCssExtractPlugin({
      filename: ({ chunk ***REMOVED***) => `${chunk.name.replace("/js/", "/css/")***REMOVED***.css`,
    ***REMOVED***),
  ],
  module: {
    rules: [
...
async function loadTheme(newTheme) {
  ...
  ***REMOVED***
  ...
  if (themes[newTheme]) {
    // eslint-disable-next-line no-console
    console.log(`THEME ALREADY LOADED - ${newTheme***REMOVED***`);
...
MiniCssExtractPlugin.getCompilationHooks(compilation).beforeTagInsert.tap(
  ...
  (source, varNames) =>
    Template.asString([
      source,
      `${varNames.tag***REMOVED***.setAttribute("href", "https://github.com/webpack-contrib/mini-css-extract-plugin");`,
    ])
);
```
