...
var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; ***REMOVED*** ***REMOVED*** catch (err) { _d = true; _e = err; ***REMOVED*** finally { try { if (!_n && _i["return"]) _i["return"](); ***REMOVED*** finally { if (_d) throw _e; ***REMOVED*** ***REMOVED*** return _arr; ***REMOVED*** return function (arr, i) { if (Array.isArray(arr)) { return arr; ***REMOVED*** else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); ***REMOVED*** else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); ***REMOVED*** ***REMOVED***; ***REMOVED***();
...
function _objectWithoutProperties(obj, keys) { var target = {***REMOVED***; for (var i in obj) { if (keys.indexOf(i) >= 0) continue; if (!Object.prototype.hasOwnProperty.call(obj, i)) continue; target[i] = obj[i]; ***REMOVED*** return target; ***REMOVED***
...
function propertyToJSXAttribute(node) {
  ...
  return _extends({
    type: 'JSXAttribute',
    name: _extends({ type: 'JSXIdentifier', name: key.name ***REMOVED***, getBaseProps(key)),
    value: value.type === 'Literal' ? adjustRangeOfNode(value) : _extends({ type: 'JSXExpressionContainer', expression: adjustExpressionRange(value) ***REMOVED***, getBaseProps(value))
...
function adjustExpressionRange(_ref3) {
  ...
  return _extends({***REMOVED***, adjustRangeOfNode(expression), expressions ? { expressions: expressions.map(adjustRangeOfNode) ***REMOVED*** : {***REMOVED***, quasis ? { quasis: quasis.map(adjustRangeOfNode) ***REMOVED*** : {***REMOVED***);
***REMOVED***
...
function getBaseLocation(_ref5) {
  ...
  ***REMOVED***, source !== undefined ? { source: source ***REMOVED*** : {***REMOVED***, filename !== undefined ? { filename: filename ***REMOVED*** : {***REMOVED***);
***REMOVED***