...
import { version ***REMOVED*** from 'eslint/package.json';
...
const disableNewTS = semver.satisfies(tsParserVersion, '>= 4.1') // this rule is not useful on v4.1+ of the TS parser
  ? (x) => ({ ...x, features: [].concat(x.features, 'no-ts-new') ***REMOVED***)
  : (x) => x;
    ...
    return {
      ...
      babelOptions: {
        ...
        plugins: [
          ...
          ['@babel/plugin-syntax-decorators', { legacy: true ***REMOVED***],
        ],
        parserOpts: {
    ...
    const t = flatMap(tests, (test) => {
      ...
      ***REMOVED***
      ...
      function addComment(testObject, parser) {
        const extras = [].concat(
          `features: [${Array.from(features).join(',')***REMOVED***]`,
          `parser: ${parser***REMOVED***`,
          testObject.parserOptions ? `parserOptions: ${JSON.stringify(testObject.parserOptions)***REMOVED***` : [],
          testObject.options ? `options: ${JSON.stringify(testObject.options)***REMOVED***` : [],
          testObject.settings ? `settings: ${JSON.stringify(testObject.settings)***REMOVED***` : [],
        );

        const extraComment = `\n// ${extras.join(', ')***REMOVED***`;

        // Augment expected fix code output with extraComment
        const nextCode = { code: testObject.code + extraComment ***REMOVED***;
        const nextOutput = testObject.output && { output: testObject.output + extraComment ***REMOVED***;
            ...
            errors: testObject.errors.map(
              (errorObject) => {
                const nextSuggestions = errorObject.suggestions && {
                  suggestions: errorObject.suggestions.map((suggestion) => ({ ...suggestion, output: suggestion.output + extraComment ***REMOVED***)),
                ***REMOVED***;

                return { ...errorObject, ...nextSuggestions ***REMOVED***;
              ***REMOVED***,
            ),
          ***REMOVED***;
      ...
      ***REMOVED***
      ...
      return [].concat(
        skipBase ? [] : addComment(
          {
            ...test,
            ...typeof es === 'number' && {
              parserOptions: { ...test.parserOptions, ecmaVersion: es ***REMOVED***,
            ***REMOVED***,
          ***REMOVED***,
          'default',
        ),
        ...
        ***REMOVED***, '@babel/eslint-parser'),
        tsOld ? addComment({ ...test, parser: parsers.TYPESCRIPT_ESLINT ***REMOVED***, 'typescript-eslint') : [],
        tsNew ? addComment({ ...test, parser: parsers['@TYPESCRIPT_ESLINT'] ***REMOVED***, '@typescript-eslint/parser') : [],
      );
    ***REMOVED***);
    return t;
  ***REMOVED***,
