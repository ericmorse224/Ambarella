...
```js
import { colord ***REMOVED*** from "colord";
...
colord({ r: 255, g: 255, b: 255 ***REMOVED***);
colord({ r: 255, g: 255, b: 255, a: 1 ***REMOVED***);
colord({ h: 360, s: 100, l: 100 ***REMOVED***);
colord({ h: 360, s: 100, l: 100, a: 1 ***REMOVED***);
colord({ h: 360, s: 100, v: 100 ***REMOVED***);
colord({ h: 360, s: 100, v: 100, a: 1 ***REMOVED***);
```
...
```js
import { getFormat ***REMOVED*** from "colord";

getFormat("#aabbcc"); // "hex"
getFormat({ r: 13, g: 237, b: 162, a: 0.5 ***REMOVED***); // "rgb"
...
colord({ h: 300, s: 100, l: 50 ***REMOVED***).toHex(); // "#ff00ff"
colord({ r: 255, g: 255, b: 255, a: 0 ***REMOVED***).toHex(); // "#ffffff00"
```
...
```js
colord("#ff0000").toRgb(); // { r: 255, g: 0, b: 0, a: 1 ***REMOVED***
colord({ h: 180, s: 100, l: 50, a: 0.5 ***REMOVED***).toRgb(); // { r: 0, g: 255, b: 255, a: 0.5 ***REMOVED***
```
...
colord({ h: 180, s: 100, l: 50, a: 0.5 ***REMOVED***).toRgbString(); // "rgba(0, 255, 255, 0.5)"
```
...
```js
colord("#ffff00").toHsl(); // { h: 60, s: 100, l: 50, a: 1 ***REMOVED***
colord("rgba(0, 0, 255, 0.5) ").toHsl(); // { h: 240, s: 100, l: 50, a: 0.5 ***REMOVED***
```
...
```js
colord("#ffff00").toHsv(); // { h: 60, s: 100, v: 100, a: 1 ***REMOVED***
colord("rgba(0, 255, 255, 0.5) ").toHsv(); // { h: 180, s: 100, v: 100, a: 1 ***REMOVED***
```
...
```js
import { colord, extend ***REMOVED*** from "colord";
...
colord("#fe0000").toName({ closest: true ***REMOVED***); // "red" (closest color available)
```
...
colord("#ffffff").toCmyk(); // { c: 0, m: 0, y: 0, k: 0, a: 1 ***REMOVED***
colord("#555aaa").toCmyk(); // { c: 50, m: 47, y: 0, k: 33, a: 1 ***REMOVED***
```
...
colord("#ffffff").toHwb(); // { h: 0, w: 100, b: 0, a: 1 ***REMOVED***
colord("#555aaa").toHwb(); // { h: 236, w: 33, b: 33, a: 1 ***REMOVED***
```
...
colord("#ffffff").toLab(); // { l: 100, a: 0, b: 0, alpha: 1 ***REMOVED***
colord("#33221180").toLab(); // { l: 14.89, a: 5.77, b: 14.41, alpha: 0.5 ***REMOVED***
```
...
colord("#ffffff").toLch(); // { l: 100, c: 0, h: 0, a: 1 ***REMOVED***
colord("#213b0b").toLch(); // { l: 21.85, c: 31.95, h: 127.77, a: 1 ***REMOVED***
```
...
colord("#ffffff").toXyz(); // { x: 95.047, y: 100, z: 108.883, a: 1 ***REMOVED***
```
...
colord({ r: 0, g: 0, b: 0 ***REMOVED***).isValid(); // true
colord({ r: 0, g: 0, v: 0 ***REMOVED***).isValid(); // false
```
...
colord("#e60000").isReadable("#ffff47", { level: "AAA" ***REMOVED***); // false (normal red text on yellow bg does not conform to WCAG AAA)
colord("#e60000").isReadable("#ffff47", { level: "AAA", size: "large" ***REMOVED***); // true (large red text on yellow bg conforms to WCAG AAA)
```
...
```js
import { random ***REMOVED*** from "colord";

random().toHex(); // "#01c8ec"
random().alpha(0.5).toRgb(); // { r: 13, g: 237, b: 162, a: 0.5 ***REMOVED***
```
...
colord("rgba(170,170,170,0.4)").minify({ alphaHex: true ***REMOVED***); // "#aaa6"
```
...
colord({ c: 0, m: 0, y: 0, k: 100, a: 1 ***REMOVED***).toHex(); // "#000000"
...
colord("#999966").toHwb(); // { h: 60, w: 40, b: 40, a: 1 ***REMOVED***
colord("#003366").toHwbString(); // "hwb(210 0% 60%)"

colord({ h: 60, w: 40, b: 40 ***REMOVED***).toHex(); // "#999966"
...
colord({ l: 53.24, a: 80.09, b: 67.2 ***REMOVED***).toHex(); // "#ff0000"
colord("#ffffff").toLab(); // { l: 100, a: 0, b: 0, alpha: 1 ***REMOVED***
...
colord({ l: 100, c: 0, h: 0 ***REMOVED***).toHex(); // "#ffffff"
colord("lch(48.25% 30.07 196.38)").toHex(); // "#008080"

colord("#646464").toLch(); // { l: 42.37, c: 0, h: 0, a: 1 ***REMOVED***
...
colord("rgba(0, 0, 0, 0)").toName(); // "transparent"
colord("#fe0000").toName(); // undefined (the color is not specified in CSS specs)
colord("#fe0000").toName({ closest: true ***REMOVED***); // "red" (closest color)
```
...
colord("#ffffff").toXyz(); // { x: 95.047, y: 100, z: 108.883, a: 1 ***REMOVED***
colord({ x: 0, y: 0, z: 0 ***REMOVED***).toHex(); // "#000000"
```
...
```ts
import { RgbColor, RgbaColor, HslColor, HslaColor, HsvColor, HsvaColor ***REMOVED*** from "colord";

const foo: HslColor = { h: 0, s: 0, l: 0 ***REMOVED***;
const bar: RgbColor = { r: 0, g: 0, v: 0 ***REMOVED***; // ERROR
```
