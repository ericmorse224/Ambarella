...
```javascript
(options:{***REMOVED***, loader:{***REMOVED***) =>
  (item:{ uri:string, query: string, isAbsolute: boolean, bases:{***REMOVED*** ***REMOVED***) =>
    string | null
```

Where the `bases` are absolute directory paths `{ subString, value, property, selector ***REMOVED***` per the [algorithm](./how-it-works.md#algorithm). Note that returning `null` implies no substitution, the original relative `uri` is retained.
...
***REMOVED*** = require('resolve-url-loader');
...
```javascript
generator = function* (item: {***REMOVED***, options: {***REMOVED***, loader: {***REMOVED***): Iterator<[string,string]>
```

Or it can be simpler to write a function that returns `Array` and convert it to a generator using `asGenerator`.

```javascript
generator = asGenerator( function (item: {***REMOVED***, options: {***REMOVED***, loader: {***REMOVED***): Array<string> )
```
```javascript
generator = asGenerator( function (item: {***REMOVED***, options: {***REMOVED***, loader: {***REMOVED***): Array<[string,string]> )
```
...
* `item` consist of -
  ...
  * `bases: {***REMOVED***` are a hash where the keys are the sourcemap evaluation locations in the [algorithm](./how-it-works.md#algorithm) and the values are absolute paths that the sourcemap reports. These directories might not actually exist.
* `options` consist of -
...
* `loader` consists of the webpack loader API, useful items include -
   * `fs: {***REMOVED***` the virtual file-system from Webpack.
...
const myGenerator = asGenerator(
  ({ isAbsolute, bases: { substring, value, property, selector***REMOVED*** ***REMOVED***, { root ***REMOVED***) =>
...
const myGenerator = function* (
  { uri, isAbsolute, bases: { substring, value, property, selector ***REMOVED*** ***REMOVED***,
  { root, attempts = 1e3 ***REMOVED***,
  { fs ***REMOVED***,
) {
  if (isAbsolute) {
    yield [root, uri];
...
```javascript
implementation = function (item: {***REMOVED***, options: {***REMOVED***, loader: {***REMOVED***):
  Array<{
    base      : string,
    uri       : string,
    joined    : string,
