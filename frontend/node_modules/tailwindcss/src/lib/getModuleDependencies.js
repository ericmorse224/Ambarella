...
function resolveWithExtension(file, extensions) {
  ...
  for (let ext of extensions) {
    let full = `${file***REMOVED***${ext***REMOVED***`
  ...
  ***REMOVED***
  ...
  for (let ext of extensions) {
    let full = `${file***REMOVED***/index${ext***REMOVED***`
...
function* _getModuleDependencies(filename, base, seen, ext = path.extname(filename)) {
  ...
  for (let match of [
    ...contents.matchAll(/import[\s\S]*?['"](.{3,***REMOVED***?)['"]/gi),
    ...contents.matchAll(/import[\s\S]*from[\s\S]*?['"](.{3,***REMOVED***?)['"]/gi),
