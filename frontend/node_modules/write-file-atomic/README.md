...
```javascript
writeFileAtomic('message.txt', 'Hello Node', {chown:{uid:100,gid:50***REMOVED******REMOVED***, function (err) {
  if (err) throw err;
...
(async () => {
  try {
    await writeFileAtomic('message.txt', 'Hello Node', {chown:{uid:100,gid:50***REMOVED******REMOVED***);
    console.log('It\'s saved!');
  ***REMOVED*** catch (err) {
    console.error(err);
    process.exit(1);
