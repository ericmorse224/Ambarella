...
exports.removeStylesheet = (sheet, elementImpl) => {
  const { styleSheets ***REMOVED*** = elementImpl._ownerDocument;
...
function scanForImportRules(elementImpl, cssRules, baseURL) {
  ...
  ***REMOVED***
  ...
  for (let i = 0; i < cssRules.length; ++i) {
    ...
    ***REMOVED*** else if (cssRules[i].href) {
      // @import rule: fetch the resource and evaluate it.
      // See http://dev.w3.org/csswg/cssom/#css-import-rule
      //     If loading of the style sheet fails its cssRules list is simply
      //     empty. I.e. an @import rule always has an associated style sheet.
      const parsed = whatwgURL.parseURL(cssRules[i].href, { baseURL ***REMOVED***);
      if (parsed === null) {
        const window = elementImpl._ownerDocument._defaultView;
        if (window) {
          const error = new Error(`Could not parse CSS @import URL ${cssRules[i].href***REMOVED*** relative to base URL ` +
                                  `"${whatwgURL.serializeURL(baseURL)***REMOVED***"`);
