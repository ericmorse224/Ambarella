...
ext.createServerSession([
  { server_no_context_takeover: true, server_max_window_bits: 8 ***REMOVED***,
  { server_max_window_bits: 15 ***REMOVED***
]);
```
...
// e.g.  -> { server_max_window_bits: 8 ***REMOVED***
```
...
```js
session.processIncomingMessage(message, function(error, msg) { ... ***REMOVED***)
```
...
```js
session.processOutgoingMessage(message, function(error, msg) { ... ***REMOVED***)
```
