...
function createProjectProgram(parseSettings) {
    ...
    ***REMOVED***
    ...
    const describedPrograms = relativeProjects.length === 1
        ? relativeProjects[0]
        : `\n${relativeProjects.map(project => `- ${project***REMOVED***`).join('\n')***REMOVED***`;
    const errorLines = [
        `ESLint was configured to run on \`${describedFilePath***REMOVED***\` using \`parserOptions.project\`: ${describedPrograms***REMOVED***`,
    ];
    ...
    extraFileExtensions.forEach(extraExtension => {
        if (!extraExtension.startsWith('.')) {
            errorLines.push(`Found unexpected extension \`${extraExtension***REMOVED***\` specified with the \`parserOptions.extraFileExtensions\` option. Did you mean \`.${extraExtension***REMOVED***\`?`);
        ***REMOVED***
        if (DEFAULT_EXTRA_FILE_EXTENSIONS.includes(extraExtension)) {
            errorLines.push(`You unnecessarily included the extension \`${extraExtension***REMOVED***\` with the \`parserOptions.extraFileExtensions\` option. This extension is already handled by the parser by default.`);
        ***REMOVED***
    ***REMOVED***);
    const fileExtension = path_1.default.extname(parseSettings.filePath);
    if (!DEFAULT_EXTRA_FILE_EXTENSIONS.includes(fileExtension)) {
        const nonStandardExt = `The extension for the file (\`${fileExtension***REMOVED***\`) is non-standard`;
        if (extraFileExtensions.length > 0) {
            if (!extraFileExtensions.includes(fileExtension)) {
                errorLines.push(`${nonStandardExt***REMOVED***. It should be added to your existing \`parserOptions.extraFileExtensions\`.`);
                hasMatchedAnError = true;
            ***REMOVED***
        ***REMOVED***
        else {
            errorLines.push(`${nonStandardExt***REMOVED***. You should add \`parserOptions.extraFileExtensions\` to your config.`);
            hasMatchedAnError = true;
        ***REMOVED***
    ***REMOVED***
    if (!hasMatchedAnError) {
        ...
        errorLines.push(`However, ${describedInclusions***REMOVED*** include this file. Either:`, `- Change ESLint's list of included files to not include this file`, `- Change ${describedSpecifiers***REMOVED*** to include this file`, `- Create a new TSConfig that includes this file and include it in your parserOptions.project`, `See the typescript-eslint docs for more info: https://typescript-eslint.io/linting/troubleshooting#i-get-errors-telling-me-eslint-was-configured-to-run--however-that-tsconfig-does-not--none-of-those-tsconfigs-include-this-file`);
    ***REMOVED***
