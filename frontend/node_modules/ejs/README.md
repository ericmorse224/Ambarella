...
### Options
  ...
  - `_with`                 Whether or not to use `with() {***REMOVED***` constructs. If `false`
  ...
  - `includer`              Custom function to handle EJS includes, receives `(originalPath, parsedPath)`
    ...
    previously resolved path. Should return an object `{ filename, template ***REMOVED***`,
...
<ul>
  <% users.forEach(function(user){ %>
    <%- include('user/show', {user: user***REMOVED***) %>
  <% ***REMOVED***); %>
</ul>
```
...
// Just one template
ejs.render('<p>[?= users.join(" | "); ?]</p>', {users: users***REMOVED***, {delimiter: '?', openDelimiter: '[', closeDelimiter: ']'***REMOVED***);
// => '<p>geddy | neil | alex</p>'
...
ejs.openDelimiter = '[';
ejs.closeDelimiter = ']';
ejs.render('<p>[?= users.join(" | "); ?]</p>', {users: users***REMOVED***);
// => '<p>geddy | neil | alex</p>'
```
...
<script>
  let people = ['geddy', 'neil', 'alex'],
      html = ejs.render('<%= people.join(", "); %>', {people: people***REMOVED***);
  // With jQuery:
  $('#output').html(html);
  // Vanilla JS:
...
2. For the same reason, `include`s do not work unless you use an `include callback`. Here is an example:
  ```javascript
  let str = "Hello <%= include('file', {person: 'John'***REMOVED***); %>",
      fn = ejs.compile(str, {client: true***REMOVED***);

  fn(data, null, function(path, d){ // include callback
    // path -> 'file'
    // d -> {person: 'John'***REMOVED***
    // Put your code here
    // Return the contents of file as a string
  ***REMOVED***); // returns rendered string
  ```
