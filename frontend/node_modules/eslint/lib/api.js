/**
 * @fileoverview Expose out ESLint and CLI to require.
 * @author Ian Christian Myers
 */

"use strict";

//-----------------------------------------------------------------------------
// Requirements
//-----------------------------------------------------------------------------

const { ESLint, FlatESLint ***REMOVED*** = require("./eslint");
const { shouldUseFlatConfig ***REMOVED*** = require("./eslint/flat-eslint");
const { Linter ***REMOVED*** = require("./linter");
const { RuleTester ***REMOVED*** = require("./rule-tester");
const { SourceCode ***REMOVED*** = require("./source-code");

//-----------------------------------------------------------------------------
// Functions
//-----------------------------------------------------------------------------

/**
 * Loads the correct ESLint constructor given the options.
 * @param {Object***REMOVED*** [options] The options object
 * @param {boolean***REMOVED*** [options.useFlatConfig] Whether or not to use a flat config
 * @param {string***REMOVED*** [options.cwd] The current working directory
 * @returns {Promise<ESLint|LegacyESLint>***REMOVED*** The ESLint constructor
 */
async function loadESLint({ useFlatConfig, cwd = process.cwd() ***REMOVED*** = {***REMOVED***) {

    /*
     * Note: The v9.x version of this function doesn't have a cwd option
     * because it's not used. It's only used in the v8.x version of this
     * function.
     */

    const shouldESLintUseFlatConfig = typeof useFlatConfig === "boolean"
        ? useFlatConfig
        : await shouldUseFlatConfig({ cwd ***REMOVED***);

    return shouldESLintUseFlatConfig ? FlatESLint : ESLint;
***REMOVED***

//-----------------------------------------------------------------------------
// Exports
//-----------------------------------------------------------------------------

module.exports = {
    Linter,
    loadESLint,
    ESLint,
    RuleTester,
    SourceCode
***REMOVED***;
