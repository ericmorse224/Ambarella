...
const { spawnSync ***REMOVED*** = require("child_process");
const { URL ***REMOVED*** = require("whatwg-url");
...
const { documentBaseURLSerialized ***REMOVED*** = require("../helpers/document-base-url");
...
const { parseIntoDocument ***REMOVED*** = require("../../browser/parser");
const { fragmentSerialization ***REMOVED*** = require("../domparsing/serialization");
const { setupForSimpleEventAccessors ***REMOVED*** = require("../helpers/create-event-accessor");
const { parseJSONFromBytes ***REMOVED*** = require("../helpers/json");
const { fireAnEvent ***REMOVED*** = require("../helpers/events");
const { copyToArrayBufferInNewRealm ***REMOVED*** = require("../helpers/binary-data");

const { READY_STATES ***REMOVED*** = xhrUtils;
...
class XMLHttpRequestImpl extends XMLHttpRequestEventTargetImpl {
  constructor(window) {
    ...
    const { _ownerDocument ***REMOVED*** = this;
  ...
  ***REMOVED***
  get responseXML() {
    const { flag, properties ***REMOVED*** = this;
    ...
    ***REMOVED***
    const res = Document.createImpl(this._globalObject, [], {
      options: {
        ...
        cookieJar: { setCookieSync: () => undefined, getCookieStringSync: () => "" ***REMOVED***,
        encoding,
  ...
  ***REMOVED***
  ...
  abort() {
    ...
    const { client ***REMOVED*** = properties;
    if (client) {
      client.abort();
  ...
  ***REMOVED***
  getAllResponseHeaders() {
    const { properties, readyState ***REMOVED*** = this;
  ...
  ***REMOVED***
  ...
  open(method, uri, asynchronous, user, password) {
    const { flag, properties, _ownerDocument ***REMOVED*** = this;
    if (!_ownerDocument) {
      throw DOMException.create(this._globalObject, ["The object is in an invalid state.", "InvalidStateError"]);
    ***REMOVED***
  ...
  ***REMOVED***
  ...
  send(body) {
    const { flag, properties, upload, _ownerDocument ***REMOVED*** = this;
      ...
      ***REMOVED***
      ...
      if (body !== null) {
        ...
        if (Document.isImpl(body)) {
          ...
          flag.body = fragmentSerialization(body, { requireWellFormed: false ***REMOVED***);
        ***REMOVED*** else {
          if (typeof body === "string") {
            encoding = "UTF-8";
          ***REMOVED***
          const { buffer, formData, contentType ***REMOVED*** = extractBody(body);
        ...
        ***REMOVED*** else if (existingContentType !== null && encoding !== null) {
    ...
    ***REMOVED***
    ...
    if (flag.synchronous) {
      const flagStr = JSON.stringify(flag, function (k, v) {
        ...
        ***REMOVED***
        if (this === flag && k === "pool" && v) {
          return { maxSockets: v.maxSockets ***REMOVED***;
        ***REMOVED***
        return v;
      ***REMOVED***);
      const res = spawnSync(
        process.execPath,
        [syncWorkerFile],
        { input: flagStr, maxBuffer: Infinity ***REMOVED***
      );
      if (res.status !== 0) {
      ...
      ***REMOVED*** else {
        const { responseBuffer ***REMOVED*** = properties;
        ...
        const progressObj = { lengthComputable: false ***REMOVED***;
        if (bufferLength !== 0) {
...
function receiveResponse(xhr, response, currentURL) {
  const { flag, properties ***REMOVED*** = xhr;
  const { rawHeaders, statusCode ***REMOVED*** = response;

  let byteOffset = 0;

  const headers = {***REMOVED***;
  const filteredResponseHeaders = [];
  const headerMap = {***REMOVED***;
  ...
  for (let i = 0; i < n; i += 2) {
    ...
    if (uniqueResponseHeaders.has(kl)) {
      ...
      ***REMOVED***
      headers[k] = v;
    ***REMOVED*** else if (headerMap[kl] !== undefined) {
...
function setDispatchProgressEvents(xhr) {
  const { properties, upload ***REMOVED*** = xhr;
  const { client ***REMOVED*** = properties;
...
function extractBody(bodyInit) {
  ...
  if (Blob.isImpl(bodyInit)) {
    ...
    ***REMOVED***;
  ***REMOVED*** else if (isArrayBuffer(bodyInit)) {
    return {
      buffer: Buffer.from(bodyInit),
      contentType: null
    ***REMOVED***;
  ***REMOVED*** else if (ArrayBuffer.isView(bodyInit)) {
    return {
      buffer: Buffer.from(bodyInit.buffer, bodyInit.byteOffset, bodyInit.byteLength),
      contentType: null
    ***REMOVED***;
  ***REMOVED*** else if (FormData.isImpl(bodyInit)) {
    const formData = [];
