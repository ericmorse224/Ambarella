/**
 * @typedef {object***REMOVED*** ScreenValue
 * @property {number|undefined***REMOVED*** min
 * @property {number|undefined***REMOVED*** max
 * @property {string|undefined***REMOVED*** raw
 */

/**
 * @typedef {object***REMOVED*** Screen
 * @property {string***REMOVED*** name
 * @property {boolean***REMOVED*** not
 * @property {ScreenValue[]***REMOVED*** values
 */

/**
 * A function that normalizes the various forms that the screens object can be
 * provided in.
 *
 * Input(s):
 *   - ['100px', '200px'] // Raw strings
 *   - { sm: '100px', md: '200px' ***REMOVED*** // Object with string values
 *   - { sm: { min: '100px' ***REMOVED***, md: { max: '100px' ***REMOVED*** ***REMOVED*** // Object with object values
 *   - { sm: [{ min: '100px' ***REMOVED***, { max: '200px' ***REMOVED***] ***REMOVED*** // Object with object array (multiple values)
 *
 * Output(s):
 *   - [{ name: 'sm', values: [{ min: '100px', max: '200px' ***REMOVED***] ***REMOVED***] // List of objects, that contains multiple values
 *
 * @returns {Screen[]***REMOVED***
 */
export function normalizeScreens(screens, root = true) {
  if (Array.isArray(screens)) {
    return screens.map((screen) => {
      if (root && Array.isArray(screen)) {
        throw new Error('The tuple syntax is not supported for `screens`.')
      ***REMOVED***

      if (typeof screen === 'string') {
        return { name: screen.toString(), not: false, values: [{ min: screen, max: undefined ***REMOVED***] ***REMOVED***
      ***REMOVED***

      let [name, options] = screen
      name = name.toString()

      if (typeof options === 'string') {
        return { name, not: false, values: [{ min: options, max: undefined ***REMOVED***] ***REMOVED***
      ***REMOVED***

      if (Array.isArray(options)) {
        return { name, not: false, values: options.map((option) => resolveValue(option)) ***REMOVED***
      ***REMOVED***

      return { name, not: false, values: [resolveValue(options)] ***REMOVED***
    ***REMOVED***)
  ***REMOVED***

  return normalizeScreens(Object.entries(screens ?? {***REMOVED***), false)
***REMOVED***

/**
 * @param {Screen***REMOVED*** screen
 * @returns {{result: false, reason: string***REMOVED*** | {result: true, reason: null***REMOVED******REMOVED***
 */
export function isScreenSortable(screen) {
  if (screen.values.length !== 1) {
    return { result: false, reason: 'multiple-values' ***REMOVED***
  ***REMOVED*** else if (screen.values[0].raw !== undefined) {
    return { result: false, reason: 'raw-values' ***REMOVED***
  ***REMOVED*** else if (screen.values[0].min !== undefined && screen.values[0].max !== undefined) {
    return { result: false, reason: 'min-and-max' ***REMOVED***
  ***REMOVED***

  return { result: true, reason: null ***REMOVED***
***REMOVED***

/**
 * @param {'min' | 'max'***REMOVED*** type
 * @param {Screen | 'string'***REMOVED*** a
 * @param {Screen | 'string'***REMOVED*** z
 * @returns {number***REMOVED***
 */
export function compareScreens(type, a, z) {
  let aScreen = toScreen(a, type)
  let zScreen = toScreen(z, type)

  let aSorting = isScreenSortable(aScreen)
  let bSorting = isScreenSortable(zScreen)

  // These cases should never happen and indicate a bug in Tailwind CSS itself
  if (aSorting.reason === 'multiple-values' || bSorting.reason === 'multiple-values') {
    throw new Error(
      'Attempted to sort a screen with multiple values. This should never happen. Please open a bug report.'
    )
  ***REMOVED*** else if (aSorting.reason === 'raw-values' || bSorting.reason === 'raw-values') {
    throw new Error(
      'Attempted to sort a screen with raw values. This should never happen. Please open a bug report.'
    )
  ***REMOVED*** else if (aSorting.reason === 'min-and-max' || bSorting.reason === 'min-and-max') {
    throw new Error(
      'Attempted to sort a screen with both min and max values. This should never happen. Please open a bug report.'
    )
  ***REMOVED***

  // Let the sorting begin
  let { min: aMin, max: aMax ***REMOVED*** = aScreen.values[0]
  let { min: zMin, max: zMax ***REMOVED*** = zScreen.values[0]

  // Negating screens flip their behavior. Basically `not min-width` is `max-width`
  if (a.not) [aMin, aMax] = [aMax, aMin]
  if (z.not) [zMin, zMax] = [zMax, zMin]

  aMin = aMin === undefined ? aMin : parseFloat(aMin)
  aMax = aMax === undefined ? aMax : parseFloat(aMax)
  zMin = zMin === undefined ? zMin : parseFloat(zMin)
  zMax = zMax === undefined ? zMax : parseFloat(zMax)

  let [aValue, zValue] = type === 'min' ? [aMin, zMin] : [zMax, aMax]

  return aValue - zValue
***REMOVED***

/**
 *
 * @param {PartialScreen> | string***REMOVED*** value
 * @param {'min' | 'max'***REMOVED*** type
 * @returns {Screen***REMOVED***
 */
export function toScreen(value, type) {
  if (typeof value === 'object') {
    return value
  ***REMOVED***

  return {
    name: 'arbitrary-screen',
    values: [{ [type]: value ***REMOVED***],
  ***REMOVED***
***REMOVED***

function resolveValue({ 'min-width': _minWidth, min = _minWidth, max, raw ***REMOVED*** = {***REMOVED***) {
  return { min, max, raw ***REMOVED***
***REMOVED***
