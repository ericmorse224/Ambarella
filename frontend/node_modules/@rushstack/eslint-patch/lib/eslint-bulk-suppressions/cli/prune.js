...
async function pruneAsync() {
    ...
    ***REMOVED***
    if (args.length > 0) {
        throw new Error(`@rushstack/eslint-bulk: Unknown arguments: ${args.join(' ')***REMOVED***`);
    ***REMOVED***
    ...
    if (allFiles.length > 0) {
        ...
        console.log(`Pruning suppressions for ${allFiles.length***REMOVED*** files...`);
...
async function getAllFilesWithExistingSuppressionsForCwdAsync(normalizedCwd) {
    const { jsonObject: bulkSuppressionsConfigJson ***REMOVED*** = (0, bulk_suppressions_file_1.getSuppressionsConfigForEslintrcFolderPath)(normalizedCwd);
    const allFiles = new Set();
    for (const { file: filePath ***REMOVED*** of bulkSuppressionsConfigJson.suppressions) {
    ...
    ***REMOVED***
    ...
    await Promise.all(allFilesArray.map(async (filePath) => {
        ...
        ***REMOVED***
    ***REMOVED***));
    console.log(`Found ${allExistingFiles.length***REMOVED*** files with existing suppressions.`);
    const deletedCount = allFilesArray.length - allExistingFiles.length;
    if (deletedCount > 0) {
        console.log(`${deletedCount***REMOVED*** files with suppressions were deleted.`);
    ***REMOVED***
