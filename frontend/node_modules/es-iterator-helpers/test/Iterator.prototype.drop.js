'use strict';

var defineProperties = require('define-properties');
var test = require('tape');
var callBind = require('call-bind');
var functionsHaveNames = require('functions-have-names')();
var hasStrictMode = require('has-strict-mode')();
var forEach = require('for-each');
var debug = require('object-inspect');
var v = require('es-value-fixtures');
var hasSymbols = require('has-symbols/shams')();
var hasPropertyDescriptors = require('has-property-descriptors')();
var iterate = require('iterate-iterator');

var index = require('../Iterator.prototype.drop');
var impl = require('../Iterator.prototype.drop/implementation');

var fnName = 'drop';

var isEnumerable = Object.prototype.propertyIsEnumerable;

var testIterator = require('./helpers/testIterator');

module.exports = {
	tests: function (drop, name, t) {
		t['throws'](
			function () { return new drop(); ***REMOVED***, // eslint-disable-line new-cap
			TypeError,
			'`' + name + '` itself is not a constructor'
		);

		forEach(v.primitives.concat(v.objects), function (nonIterator) {
			t['throws'](
				function () { iterate(drop(nonIterator, 0)); ***REMOVED***,
				TypeError,
				debug(nonIterator) + ' is not an Object with a callable `next` method'
			);

			var badNext = { next: nonIterator ***REMOVED***;
			t['throws'](
				function () { iterate(drop(badNext, 0)); ***REMOVED***,
				TypeError,
				debug(badNext) + ' is not an Object with a callable `next` method'
			);
		***REMOVED***);

		t.test('observable lookups', { skip: !hasPropertyDescriptors ***REMOVED***, function (st) {
			var effects = [];

			var obj = {***REMOVED***;
			Object.defineProperty(obj, 'next', {
				configurable: true,
				enumerable: true,
				get: function next() {
					effects.push('get next');
					return function () {
						return { done: true, value: undefined ***REMOVED***;
					***REMOVED***;
				***REMOVED***
			***REMOVED***);
			drop(obj, {
				valueOf: function valueOf() {
					effects.push('ToNumber limit');
					return 0;
				***REMOVED***
			***REMOVED***);

			st.deepEqual(effects, [
				'ToNumber limit',
				'get next'
			]);
			st.end();
		***REMOVED***);

		var arr = [1, 2, 3];

		t.test('actual iteration', { skip: !hasSymbols ***REMOVED***, function (st) {
			var iterator = callBind(arr[Symbol.iterator], arr);

			st['throws'](
				function () { drop(iterator(), -3); ***REMOVED***,
				RangeError,
				'-3 is not >= 0'
			);

			st['throws'](
				function () { return new drop(iterator()); ***REMOVED***, // eslint-disable-line new-cap
				TypeError,
				'`' + name + '` iterator is not a constructor'
			);
			st['throws'](
				function () { return new drop(iterator(), 0); ***REMOVED***, // eslint-disable-line new-cap
				TypeError,
				'`' + name + '` iterator is not a constructor'
			);

			testIterator(iterator(), [1, 2, 3], st, 'original');
			testIterator(drop(iterator(), 0), [1, 2, 3], st, 'drop 0');
			testIterator(drop(iterator(), 1), [2, 3], st, 'drop 1');
			testIterator(drop(iterator(), 2), [3], st, 'drop 2');
			testIterator(drop(iterator(), 3), [], st, 'drop 3');
			testIterator(drop(iterator(), Infinity), [], st, 'drop ∞');

			st.end();
		***REMOVED***);

		t.test('262: test/built-ins/Iterator/prototype/drop/get-return-method-throws', { skip: !hasPropertyDescriptors ***REMOVED***, function (st) {
			var badIterator = {
				next: function next() {
					return {
						done: false,
						value: 1
					***REMOVED***;
				***REMOVED***
			***REMOVED***;

			Object.defineProperty(badIterator, 'return', {
				configurable: true,
				enumerable: true,
				get: function () { throw new SyntaxError(); ***REMOVED***
			***REMOVED***);

			var iter = drop(badIterator, 1);
			iter.next();

			st['throws'](
				function () { iter['return'](); ***REMOVED***,
				SyntaxError,
				'gets the `return` method, whose getter throws'
			);

			st.end();
		***REMOVED***);

		t.test('262: test/built-ins/Iterator/prototype/drop/return-is-forwarded', function (st) {
			var returnCount = 0;

			var makeBadIterator = function makeBadIterator() {
				return {
					next: function next() {
						return {
							done: false,
							value: 1
						***REMOVED***;
					***REMOVED***,
					'return': function () {
						returnCount += 1;
						return {***REMOVED***;
					***REMOVED***
				***REMOVED***;
			***REMOVED***;

			var iter1 = drop(makeBadIterator(), 0);
			st.equal(returnCount, 0, 'iter1, before return()');
			iter1['return']();
			st.equal(returnCount, 1, 'iter1, after return()');

			var iter2 = drop(makeBadIterator(), 1);
			st.equal(returnCount, 1, 'iter2, before return()');
			iter2['return']();
			st.equal(returnCount, 2, 'iter2, after return()');

			// 5 drops (i wish i had pipeline)
			var iter3 = drop(
				drop(
					drop(
						drop(
							drop(
								makeBadIterator(),
								1
							),
							1
						),
						1
					),
					1
				),
				1
			);
			st.equal(returnCount, 2, 'iter3, before return()');
			iter3['return']();
			st.equal(returnCount, 3, 'iter3, after return()');

			st.end();
		***REMOVED***);

		t.test('262: test/built-ins/Iterator/prototype/drop/return-is-not-forwarded-after-exhaustion', { skip: !hasPropertyDescriptors ***REMOVED***, function (st) {
			var makeBadIterator = function makeBadIterator() {
				return {
					next: function next() {
						return {
							done: true,
							value: undefined
						***REMOVED***;
					***REMOVED***,
					'return': function () {
						throw new SyntaxError();
					***REMOVED***
				***REMOVED***;
			***REMOVED***;

			var iter1 = drop(makeBadIterator(), 0);
			st['throws'](
				function () { iter1['return'](); ***REMOVED***,
				SyntaxError,
				'iter1, return() throws'
			);
			iter1.next();
			iter1['return']();

			var iter2 = drop(makeBadIterator(), 1);
			st['throws'](
				function () { iter2['return'](); ***REMOVED***,
				SyntaxError,
				'iter2, return() throws'
			);
			iter2.next();
			iter2['return']();

			// 5 drops (i wish i had pipeline)
			var iter3 = drop(
				drop(
					drop(
						drop(
							drop(
								makeBadIterator(),
								1
							),
							1
						),
						1
					),
					1
				),
				1
			);
			st['throws'](
				function () { iter3['return'](); ***REMOVED***,
				SyntaxError,
				'iter3, return() throws'
			);
			iter3.next();
			iter3['return']();

			var iter4 = drop(makeBadIterator(), 10);
			st['throws'](
				function () { iter4['return'](); ***REMOVED***,
				SyntaxError,
				'iter4, return() throws'
			);
			iter4.next();
			iter4['return']();

			st.end();
		***REMOVED***);
	***REMOVED***,
	index: function () {
		test('Iterator.prototype.' + fnName + ': index', function (t) {
			module.exports.tests(index, 'Iterator.prototype.' + fnName, t);

			t.end();
		***REMOVED***);
	***REMOVED***,
	implementation: function () {
		test('Iterator.prototype.' + fnName + ': implementation', function (t) {
			module.exports.tests(callBind(impl), 'Iterator.prototype.' + fnName, t);

			t.end();
		***REMOVED***);
	***REMOVED***,
	shimmed: function () {
		test('Iterator.prototype.' + fnName + ': shimmed', function (t) {
			t.test('Function name', { skip: !functionsHaveNames ***REMOVED***, function (st) {
				st.equal(Iterator.prototype[fnName].name, fnName, 'Iterator#' + fnName + ' has name "' + fnName + '"');
				st.end();
			***REMOVED***);

			t.test('enumerability', { skip: !defineProperties.supportsDescriptors ***REMOVED***, function (et) {
				et.equal(false, isEnumerable.call(Iterator.prototype, fnName), 'Iterator#' + fnName + ' is not enumerable');
				et.end();
			***REMOVED***);

			t.test('bad string/this value', { skip: !hasStrictMode ***REMOVED***, function (st) {
				st['throws'](function () { return Iterator.prototype[fnName].call(undefined, 'a'); ***REMOVED***, TypeError, 'undefined is not an object');
				st['throws'](function () { return Iterator.prototype[fnName].call(null, 'a'); ***REMOVED***, TypeError, 'null is not an object');
				st.end();
			***REMOVED***);

			module.exports.tests(callBind(Iterator.prototype[fnName]), 'Iterator.prototype.' + fnName, t);

			t.end();
		***REMOVED***);
	***REMOVED***
***REMOVED***;
