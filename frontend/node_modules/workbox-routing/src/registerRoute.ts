...
import {RouteHandler, RouteMatchCallback***REMOVED*** from 'workbox-core/types.js';

import {Route***REMOVED*** from './Route.js';
import {RegExpRoute***REMOVED*** from './RegExpRoute.js';
import {HTTPMethod***REMOVED*** from './utils/constants.js';
...
function registerRoute(
  ...
  if (typeof capture === 'string') {
    ...
    ***REMOVED***
    ...
    const matchCallback: RouteMatchCallback = ({url***REMOVED***) => {
      if (process.env.NODE_ENV !== 'production') {
        if (
          ...
          logger.debug(
            `${capture***REMOVED*** only partially matches the cross-origin URL ` +
  ...
  ***REMOVED*** else if (capture instanceof RegExp) {
    // If `capture` is a `RegExp` then `handler` and `method` must be present.
    route = new RegExpRoute(capture, handler!, method);
  ***REMOVED*** else if (typeof capture === 'function') {
    // If `capture` is a function then `handler` and `method` must be present.
    route = new Route(capture, handler!, method);
  ***REMOVED*** else if (capture instanceof Route) {
    route = capture;
  ***REMOVED*** else {
