...
function _util() {
  ...
  ***REMOVED***;
  return data;
***REMOVED***
function _getRequireWildcardCache(e) { if ("function" != typeof WeakMap) return null; var r = new WeakMap(), t = new WeakMap(); return (_getRequireWildcardCache = function (e) { return e ? t : r; ***REMOVED***)(e); ***REMOVED***
function _interopRequireWildcard(e, r) { if (!r && e && e.__esModule) return e; if (null === e || "object" != typeof e && "function" != typeof e) return { default: e ***REMOVED***; var t = _getRequireWildcardCache(r); if (t && t.has(e)) return t.get(e); var n = { __proto__: null ***REMOVED***, a = Object.defineProperty && Object.getOwnPropertyDescriptor; for (var u in e) if ("default" !== u && {***REMOVED***.hasOwnProperty.call(e, u)) { var i = a ? Object.getOwnPropertyDescriptor(e, u) : null; i && (i.get || i.set) ? Object.defineProperty(n, u, i) : n[u] = e[u]; ***REMOVED*** return n.default = e, t && t.set(e, n), n; ***REMOVED***
const own$1 = {***REMOVED***.hasOwnProperty;
const classRegExp = /^([A-Z][a-z\d]*)+$/;
const kTypes = new Set(['string', 'function', 'number', 'object', 'Function', 'Object', 'boolean', 'bigint', 'symbol']);
const codes = {***REMOVED***;
function formatList(array, type = 'and') {
  return array.length < 3 ? array.join(` ${type***REMOVED*** `) : `${array.slice(0, -1).join(', ')***REMOVED***, ${type***REMOVED*** ${array[array.length - 1]***REMOVED***`;
***REMOVED***
...
codes.ERR_INVALID_ARG_TYPE = createError('ERR_INVALID_ARG_TYPE', (name, expected, actual) => {
  ...
  ***REMOVED*** else {
    ...
    message += `"${name***REMOVED***" ${type***REMOVED*** `;
  ***REMOVED***
  message += 'must be ';
  const types = [];
  const instances = [];
  ...
  for (const value of expected) {
    ...
    ***REMOVED*** else if (classRegExp.exec(value) === null) {
  ...
  ***REMOVED***
  if (types.length > 0) {
    message += `${types.length > 1 ? 'one of type' : 'of type'***REMOVED*** ${formatList(types, 'or')***REMOVED***`;
    if (instances.length > 0 || other.length > 0) message += ' or ';
  ***REMOVED***
  if (instances.length > 0) {
    message += `an instance of ${formatList(instances, 'or')***REMOVED***`;
    if (other.length > 0) message += ' or ';
  ***REMOVED***
  if (other.length > 0) {
    if (other.length > 1) {
      message += `one of ${formatList(other, 'or')***REMOVED***`;
    ***REMOVED*** else {
      if (other[0].toLowerCase() !== other[0]) message += 'an ';
      message += `${other[0]***REMOVED***`;
    ***REMOVED***
  ***REMOVED***
  message += `. Received ${determineSpecificType(actual)***REMOVED***`;
  return message;
***REMOVED***, TypeError);
codes.ERR_INVALID_MODULE_SPECIFIER = createError('ERR_INVALID_MODULE_SPECIFIER', (request, reason, base = undefined) => {
  return `Invalid module "${request***REMOVED***" ${reason***REMOVED***${base ? ` imported from ${base***REMOVED***` : ''***REMOVED***`;
***REMOVED***, TypeError);
codes.ERR_INVALID_PACKAGE_CONFIG = createError('ERR_INVALID_PACKAGE_CONFIG', (path, base, message) => {
  return `Invalid package config ${path***REMOVED***${base ? ` while importing ${base***REMOVED***` : ''***REMOVED***${message ? `. ${message***REMOVED***` : ''***REMOVED***`;
***REMOVED***, Error);
codes.ERR_INVALID_PACKAGE_TARGET = createError('ERR_INVALID_PACKAGE_TARGET', (packagePath, key, target, isImport = false, base = undefined) => {
  ...
  if (key === '.') {
    ...
    return `Invalid "exports" main target ${JSON.stringify(target)***REMOVED*** defined ` + `in the package config ${packagePath***REMOVED***package.json${base ? ` imported from ${base***REMOVED***` : ''***REMOVED***${relatedError ? '; targets must start with "./"' : ''***REMOVED***`;
  ***REMOVED***
  return `Invalid "${isImport ? 'imports' : 'exports'***REMOVED***" target ${JSON.stringify(target)***REMOVED*** defined for '${key***REMOVED***' in the package config ${packagePath***REMOVED***package.json${base ? ` imported from ${base***REMOVED***` : ''***REMOVED***${relatedError ? '; targets must start with "./"' : ''***REMOVED***`;
***REMOVED***, Error);
codes.ERR_MODULE_NOT_FOUND = createError('ERR_MODULE_NOT_FOUND', (path, base, exactUrl = false) => {
  return `Cannot find ${exactUrl ? 'module' : 'package'***REMOVED*** '${path***REMOVED***' imported from ${base***REMOVED***`;
***REMOVED***, Error);
...
codes.ERR_PACKAGE_IMPORT_NOT_DEFINED = createError('ERR_PACKAGE_IMPORT_NOT_DEFINED', (specifier, packagePath, base) => {
  return `Package import specifier "${specifier***REMOVED***" is not defined${packagePath ? ` in package ${packagePath***REMOVED***package.json` : ''***REMOVED*** imported from ${base***REMOVED***`;
***REMOVED***, TypeError);
codes.ERR_PACKAGE_PATH_NOT_EXPORTED = createError('ERR_PACKAGE_PATH_NOT_EXPORTED', (packagePath, subpath, base = undefined) => {
  if (subpath === '.') return `No "exports" main defined in ${packagePath***REMOVED***package.json${base ? ` imported from ${base***REMOVED***` : ''***REMOVED***`;
  return `Package subpath '${subpath***REMOVED***' is not defined by "exports" in ${packagePath***REMOVED***package.json${base ? ` imported from ${base***REMOVED***` : ''***REMOVED***`;
***REMOVED***, Error);
...
codes.ERR_UNKNOWN_FILE_EXTENSION = createError('ERR_UNKNOWN_FILE_EXTENSION', (extension, path) => {
  return `Unknown file extension "${extension***REMOVED***" for ${path***REMOVED***`;
***REMOVED***, TypeError);
codes.ERR_INVALID_ARG_VALUE = createError('ERR_INVALID_ARG_VALUE', (name, value, reason = 'is invalid') => {
  ...
  if (inspected.length > 128) {
    inspected = `${inspected.slice(0, 128)***REMOVED***...`;
  ***REMOVED***
  const type = name.includes('.') ? 'property' : 'argument';
  return `The ${type***REMOVED*** '${name***REMOVED***' ${reason***REMOVED***. Received ${inspected***REMOVED***`;
***REMOVED***, TypeError);
  ...
  function NodeError(...parameters) {
    ...
    Object.defineProperties(error, {
      ...
      ***REMOVED***,
      toString: {
        value() {
          return `${this.name***REMOVED*** [${key***REMOVED***]: ${this.message***REMOVED***`;
        ***REMOVED***,
        enumerable: false,
        writable: true,
        configurable: true
...
function getMessage(key, parameters, self) {
  ...
  if (typeof message === 'function') {
    _assert()(message.length <= parameters.length, `Code: ${key***REMOVED***; The provided arguments length (${parameters.length***REMOVED***) does not ` + `match the required ones (${message.length***REMOVED***).`);
    return Reflect.apply(message, self, parameters);
  ***REMOVED***
  const regex = /%[dfijoOs]/g;
  let expectedLength = 0;
  while (regex.exec(message) !== null) expectedLength++;
  _assert()(expectedLength === parameters.length, `Code: ${key***REMOVED***; The provided arguments length (${parameters.length***REMOVED***) does not ` + `match the required ones (${expectedLength***REMOVED***).`);
...
function determineSpecificType(value) {
  ...
  ***REMOVED***
  if (typeof value === 'function' && value.name) {
    return `function ${value.name***REMOVED***`;
  ***REMOVED***
  if (typeof value === 'object') {
    if (value.constructor && value.constructor.name) {
      return `an instance of ${value.constructor.name***REMOVED***`;
    ***REMOVED***
  ...
  ***REMOVED***);
  if (inspected.length > 28) {
    inspected = `${inspected.slice(0, 25)***REMOVED***...`;
  ***REMOVED***
  return `type ${typeof value***REMOVED*** (${inspected***REMOVED***)`;
***REMOVED***
const hasOwnProperty$1 = {***REMOVED***.hasOwnProperty;
const {
...
***REMOVED***) {
  ...
  ***REMOVED***;
  if (string !== undefined) {
    ...
    ***REMOVED*** catch (error_) {
      const cause = error_;
      const error = new ERR_INVALID_PACKAGE_CONFIG$1(jsonPath, (base ? `"${specifier***REMOVED***" from ` : '') + (0, _url().fileURLToPath)(base || specifier), cause.message);
      error.cause = cause;
      throw error;
    ***REMOVED***
    result.exists = true;
...
***REMOVED*** = codes;
const hasOwnProperty = {***REMOVED***.hasOwnProperty;
...
function getDataProtocolModuleFormat(parsed) {
  const {
    1: mime
  ***REMOVED*** = /^([^/]+\/[^;,]+)[^,]*?(;base64)?,/.exec(parsed.pathname) || [null, null, null];
...
***REMOVED***
function getHttpProtocolModuleFormat() {***REMOVED***
...
***REMOVED*** = codes;
const own = {***REMOVED***.hasOwnProperty;
...
const doubleSlashRegEx = /[/\\]{2***REMOVED***/;
function emitInvalidSegmentDeprecation(target, request, match, packageJsonUrl, internal, base, isTarget) {
  ...
  ***REMOVED***
  ...
  _process().emitWarning(`Use of deprecated ${double ? 'double slash' : 'leading or trailing slash matching'***REMOVED*** resolving "${target***REMOVED***" for module ` + `request "${request***REMOVED***" ${request === match ? '' : `matched to "${match***REMOVED***" `***REMOVED***in the "${internal ? 'imports' : 'exports'***REMOVED***" field module resolution of the package at ${pjsonPath***REMOVED***${base ? ` imported from ${(0, _url().fileURLToPath)(base)***REMOVED***` : ''***REMOVED***.`, 'DeprecationWarning', 'DEP0166');
***REMOVED***
function emitLegacyIndexDeprecation(url, packageJsonUrl, base, main) {
  ...
  ***REMOVED***);
  ...
  if (!main) {
    _process().emitWarning(`No "main" or "exports" field defined in the package.json for ${packagePath***REMOVED*** resolving the main entry point "${urlPath.slice(packagePath.length)***REMOVED***", imported from ${basePath***REMOVED***.\nDefault "index" lookups for the main are deprecated for ES modules.`, 'DeprecationWarning', 'DEP0151');
  ***REMOVED*** else if (_path().resolve(packagePath, main) !== urlPath) {
    _process().emitWarning(`Package ${packagePath***REMOVED*** has a "main" field set to "${main***REMOVED***", ` + `excluding the full filename and extension to the resolved file at "${urlPath.slice(packagePath.length)***REMOVED***", imported from ${basePath***REMOVED***.\n Automatic extension resolution of the "main" field is ` + 'deprecated for ES modules.', 'DeprecationWarning', 'DEP0151');
  ***REMOVED***
***REMOVED***
function tryStatSync(path) {
  try {
    return (0, _fs().statSync)(path);
  ***REMOVED*** catch (_unused2) {***REMOVED***
***REMOVED***
...
function legacyMainResolve(packageJsonUrl, packageConfig, base) {
  ...
  if (packageConfig.main !== undefined) {
    ...
    const tries = [`./${packageConfig.main***REMOVED***.js`, `./${packageConfig.main***REMOVED***.json`, `./${packageConfig.main***REMOVED***.node`, `./${packageConfig.main***REMOVED***/index.js`, `./${packageConfig.main***REMOVED***/index.json`, `./${packageConfig.main***REMOVED***/index.node`];
    let i = -1;
...
function throwInvalidSubpath(request, match, packageJsonUrl, internal, base) {
  const reason = `request is not a valid match in pattern "${match***REMOVED***" for the "${internal ? 'imports' : 'exports'***REMOVED***" resolution of ${(0, _url().fileURLToPath)(packageJsonUrl)***REMOVED***`;
  throw new ERR_INVALID_MODULE_SPECIFIER(request, reason, base && (0, _url().fileURLToPath)(base));
***REMOVED***
function invalidPackageTarget(subpath, target, packageJsonUrl, internal, base) {
  target = typeof target === 'object' && target !== null ? JSON.stringify(target, null, '') : `${target***REMOVED***`;
  ...
  if (!target.startsWith('./')) {
    if (internal && !target.startsWith('../') && !target.startsWith('/')) {
      ...
      try {
        new (_url().URL)(target);
        isURL = true;
      ***REMOVED*** catch (_unused3) {***REMOVED***
      if (!isURL) {
...
function isArrayIndex(key) {
  ...
  if (`${keyNumber***REMOVED***` !== key) return false;
...
function isConditionalExportsMainSugar(exports, packageJsonUrl, base) {
  ...
  while (++keyIndex < keys.length) {
    ...
    ***REMOVED*** else if (isConditionalSugar !== currentIsConditionalSugar) {
...
function emitTrailingSlashPatternDeprecation(match, pjsonUrl, base) {
  ...
  ***REMOVED***
  ...
  _process().emitWarning(`Use of deprecated trailing slash pattern mapping "${match***REMOVED***" in the ` + `"exports" field module resolution of the package at ${pjsonPath***REMOVED***${base ? ` imported from ${(0, _url().fileURLToPath)(base)***REMOVED***` : ''***REMOVED***. Mapping specifiers ending in "/" is no longer supported.`, 'DeprecationWarning', 'DEP0155');
***REMOVED***
...
function packageResolve(specifier, base, conditions) {
  ...
  ***REMOVED***
  const {
    ...
    packageSubpath,
    isScoped
  ***REMOVED*** = parsePackageName(specifier, base);
  const packageConfig = getPackageScopeConfig(base);
  if (packageConfig.exists) {
  ...
  ***REMOVED*** while (packageJsonPath.length !== lastPath.length);
...
function moduleResolve(specifier, base, conditions, preserveSymlinks) {
  ...
  if (shouldBeTreatedAsRelativeOrAbsolutePath(specifier)) {
    ...
    ***REMOVED*** catch (error_) {
      ...
      throw error;
    ***REMOVED***
  ***REMOVED*** else if (protocol === 'file:' && specifier[0] === '#') {
...
function throwIfInvalidParentURL(parentURL) {
  ...
  ***REMOVED***
***REMOVED***
function defaultResolve(specifier, context = {***REMOVED***) {
  const {
    parentURL
  ***REMOVED*** = context;
  ...
  if (parentURL) {
    ...
    ***REMOVED*** catch (_unused4) {***REMOVED***
  ***REMOVED***
  let parsed;
  let protocol;
  try {
    ...
    if (protocol === 'data:') {
      ...
      ***REMOVED***;
    ***REMOVED***
  ***REMOVED*** catch (_unused5) {***REMOVED***
