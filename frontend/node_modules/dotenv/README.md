...
```js
require('dotenv').config({ path: '/custom/path/to/.env' ***REMOVED***)
```
...
```js
require('dotenv').config({ encoding: 'latin1' ***REMOVED***)
```
...
```js
require('dotenv').config({ debug: process.env.DEBUG ***REMOVED***)
```
...
console.log(typeof config, config) // object { BASIC : 'basic' ***REMOVED***
```
...
- `BASIC=basic` becomes `{BASIC: 'basic'***REMOVED***`
- empty lines are skipped
- lines beginning with `#` are treated as comments
- empty values become empty strings (`EMPTY=` becomes `{EMPTY: ''***REMOVED***`)
- inner quotes are maintained (think JSON) (`JSON={"foo": "bar"***REMOVED***` becomes `{JSON:"{\"foo\": \"bar\"***REMOVED***"`)
- whitespace is removed from both ends of unquoted values (see more on [`trim`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/Trim)) (`FOO=  some value  ` becomes `{FOO: 'some value'***REMOVED***`)
- single and double quoted values are escaped (`SINGLE_QUOTE='quoted'` becomes `{SINGLE_QUOTE: "quoted"***REMOVED***`)
- single and double quoted values maintain whitespace from both ends (`FOO="  some value  "` becomes `{FOO: '  some value  '***REMOVED***`)
...
```js
import { Client ***REMOVED*** from 'best-error-reporting-service'
