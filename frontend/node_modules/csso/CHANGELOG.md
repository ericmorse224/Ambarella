...
- `minify()` and `minifyBlock()` is always return an object as result now (i.e. `{ css: String, map: SourceMapGenerator or null ***REMOVED***`)
- `parse()`
...
- `compress()`
    ...
    - Returns an object as result instead of AST (i.e. `{ ast: Object ***REMOVED***`)
