...
import {WorkboxPackageJSON***REMOVED*** from '../types';
...
export async function copyWorkboxLibraries(
  ...
  ***REMOVED***`;
  ...
  const librariesToCopy = Object.keys(thisPkg.dependencies || {***REMOVED***).filter(
  ...
  for (const library of librariesToCopy) {
    ...
    const libraryPath = upath.dirname(
      require.resolve(`${library***REMOVED***/package.json`),
    );
  ...
  ***REMOVED*** catch (error) {
    throw Error(
      `${errors['unable-to-copy-workbox-libraries']***REMOVED*** ${
