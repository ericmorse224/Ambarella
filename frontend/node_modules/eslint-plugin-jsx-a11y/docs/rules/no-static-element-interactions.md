...
```jsx
<div
  onClick={onClickHandler***REMOVED***
...
```jsx
{/* The <div> element has a child <button> element that allows keyboard interaction */***REMOVED***
{/* eslint-disable-next-line jsx-a11y/no-static-element-interactions */***REMOVED***
<div onClick={this.handleButtonClick***REMOVED***>
  <button>Save</button>
...
```jsx
<div role={ROLE_BUTTON***REMOVED*** onClick={() => {***REMOVED******REMOVED*** />;
// In case of a conditional expression, there should be literals on both sides of ternary operator
<div role={isButton ? "button" : "link"***REMOVED*** onClick={() => {***REMOVED******REMOVED*** />;
```

### Succeed

```jsx
<button onClick={() => {***REMOVED******REMOVED*** className="foo" />
<div className="foo" onClick={() => {***REMOVED******REMOVED*** role="button" />
<input type="text" onClick={() => {***REMOVED******REMOVED*** />
```
