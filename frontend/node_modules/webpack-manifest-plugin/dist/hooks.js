...
const beforeRunHook = ({ emitCountMap, manifestFileName ***REMOVED***, _, callback) => {
...
const emitHook = function emit({ compiler, emitCountMap, manifestAssetId, manifestFileName, moduleAssets, options ***REMOVED***, compilation) {
    ...
    ***REMOVED***);
    ...
    const { basePath, removeKeyHash ***REMOVED*** = options;
    emitCountMap.set(manifestFileName, emitCount);
    const auxiliaryFiles = {***REMOVED***;
    ...
    files = files.filter(({ name, path ***REMOVED***) => {
        var _a;
    ...
    ***REMOVED***);
    ...
    if (isLastEmit) {
        ...
        if (options.writeToFileEmit) {
            fs_1.mkdirSync(path_1.dirname(manifestFileName), { recursive: true ***REMOVED***);
...
const normalModuleLoaderHook = ({ moduleAssets ***REMOVED***, loaderContext, module) => {
    const { emitFile ***REMOVED*** = loaderContext;
    loaderContext.emitFile = (file, content, sourceMap) => {
        if (module.userRequest && !moduleAssets[file]) {
            Object.assign(moduleAssets, { [file]: path_1.join(path_1.dirname(file), path_1.basename(module.userRequest)) ***REMOVED***);
        ***REMOVED***
